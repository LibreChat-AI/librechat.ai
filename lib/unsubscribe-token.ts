...
export function createUnsubscribeToken(
  ...
  const signature = createHmac('sha256', secret)
    .update(`${email}:${issuedAt}:${generation}:${tokenId}`)
    .digest('base64url')
  return `${issuedAt}.${generation}.${tokenId}.${signature}`
}
...
export async function verifyUnsubscribeToken(email: string, token: string): Promise<boolean> {
  ...
  const [issuedAtValue, generationValue, tokenId, signature, extra] = token.split('.')
  if (!issuedAtValue || !generationValue || !tokenId || !signature || extra) return false
