...
/**
 * Every blog and changelog URL, read from the content directory.
 *
 * next-sitemap derives its URL set from Next's prerender manifest, so a post
 * that renders dynamically never reaches the sitemap. `/blog/*` opts into ISR
 * via `generateStaticParams` + `revalidate`, but a post embedding live server
 * data calls `connection()`, which forces dynamic rendering for that page alone
 * and silently drops it from the manifest —
 * /blog/2026-07-26_clickhouse-analytics was missing for exactly this reason.
 *
 * Enumerating the collection instead means a published post is listed whether
 * or not it happens to prerender. next-sitemap merges these by `loc`, so a URL
 * the manifest already produced is updated rather than duplicated.
 */
function contentRoutes() {
  return ['blog', 'changelog'].flatMap((collection) => {
    const dir = path.join(ROOT, 'content', collection)
    if (!fs.existsSync(dir)) return []
    return fs
      .readdirSync(dir)
      .filter((name) => /\.mdx?$/.test(name))
      .map((name) => `/${collection}/${name.replace(/\.mdx?$/, '')}`)
  })
}
