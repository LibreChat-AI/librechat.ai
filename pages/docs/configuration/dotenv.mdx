...
<OptionTable
  options={[
    ['IMAGE_GEN_OAI_MODEL', 'string', 'Default model for OpenAI image generation requests. Default: `gpt-image-1`. Example values: `gpt-image-1`, `gpt-image-1.5`','# IMAGE_GEN_OAI_MODEL='],
  ]}
/>
...
- Passing username to models:

By default, LibreChat will send the User Id as the "user" field when making API calls.

<OptionTable
  options={[
    [
      'SEND_USERNAME_TO_MODEL ',
      'boolean',
      'Set to true to send username to model API calls instead of user id.',
      'SEND_USERNAME_TO_MODEL=true',
    ],
  ]}
/>
