...
#### [Forwarded Authentication](/docs/configuration/authentication/forwarded)

For more information: **[Forwarded Authentication](/docs/configuration/authentication/forwarded)**

<OptionTable
  options={[
    ['FORWARD_AUTH_ENABLED', 'boolean', 'Enable forwarded authentication via HTTP headers from a reverse proxy.', 'FORWARD_AUTH_ENABLED=true'],
    ['FORWARD_AUTH_USERNAME_HEADER', 'string', 'Required header containing the username (case-insensitive).', 'FORWARD_AUTH_USERNAME_HEADER=X-Forwarded-User'],
    ['FORWARD_AUTH_EMAIL_HEADER', 'string', 'Optional header containing the user\'s email (case-insensitive).', 'FORWARD_AUTH_EMAIL_HEADER=X-Forwarded-Email'],
  ]}
/>
