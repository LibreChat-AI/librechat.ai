...
<OptionTable
  options={[
    ...
    ['MCP_SKIP_CODE_CHALLENGE_CHECK', 'boolean', 'Skip code challenge method validation (e.g., for AWS Cognito that supports S256 but doesn\'t advertise it\). When set to true, forces S256 code challenge even if not advertised in .well-known/openid-configuration', '# MCP_SKIP_CODE_CHALLENGE_CHECK=false'],
  ]}
/>
