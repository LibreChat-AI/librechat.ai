...
2. Click on `Identity Provider Metadata: Download`.
3. Copy the Metadata file to your project folder

![auth0-4](/images/docs/configuration/authentication/SAML/auth0/4.png)

## Step 4: Configure LibreChat with SAML

1. Open the `.env` file in your project folder and add the following variables:

  ```bash filename=".env"
  SAML_METADATA=[MetadataFilePathFromProjectRootFolder]
