...
Controls whether file citations are displayed when using file search functionality. When enabled, sources and references from searched files will be shown as citations in the response. This works in conjunction with the file search feature and requires appropriate permissions.

**Note:** 
- This setting only affects the display of citations in the UI. The actual citation processing is controlled by the `FILE_CITATIONS` permission.
...
- If a user lacks the `FILE_CITATIONS` permission, citations will not be displayed regardless of this setting.

**Key:**
<OptionTable
  options={[
    ['fileCitations', 'Boolean', 'Enables or disables the display of file citations when using file search functionality.'],
  ]}
/>
