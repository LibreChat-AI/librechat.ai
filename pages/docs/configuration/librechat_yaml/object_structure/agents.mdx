...
<OptionTable
  options={[
    ['defaultAgentId', 'String', 'Sets the default agent ID for new users on first chat interface open.', 'Only applies to first-time users (new signups) who haven\'t selected an agent yet. Does not override user\'s last-used agent preference after initial interaction.'],
  ]}
/>

**Default:** Not set (users select their own agent)

**Example:**
```yaml filename="endpoints / agents / defaultAgentId"
endpoints:
  agents:
...
For more information, see [Default Agent Configuration](/docs/features/agents#default-agent-configuration).

## Example Configuration

Here is an example of configuring the `agents` endpoint with custom capabilities, file citation settings, and default agent:
...
endpoints:
  agents:
    ...
    defaultAgentId: "agent_welcome_assistant"
    # Custom capabilities
    capabilities:
      - "execute_code"
      - "file_search"
...
- New users will automatically start with the "agent_welcome_assistant" agent
