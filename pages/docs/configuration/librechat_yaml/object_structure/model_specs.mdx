...
There are 5 main fields under `modelSpecs`:

  - `enforce` (optional; default: false)
  - `prioritize` (optional; default: true)
  - `hideBaseModels` (optional; default: false)
  - `list` (required)
...
<OptionTable
  options={[
    ['hideBaseModels', 'Boolean', 'When enabled, hides base/raw models from the model selector for endpoints that have model specs defined.', 'Only affects endpoints with at 
least one model spec. Endpoints without specs will still show their models.'],
  ]}
/>
...
**Description:**
When set to `true`, base models are hidden from the model selector for any endpoint that has model specs defined. This allows administrators to present only curated model options
  to users, avoiding duplicate entries when a model spec wraps an underlying base model.

**Example:**
```yaml filename="modelSpecs / hideBaseModels"
modelSpecs:
  hideBaseModels: true
  list:
    - name: "gpt-5-codex"
