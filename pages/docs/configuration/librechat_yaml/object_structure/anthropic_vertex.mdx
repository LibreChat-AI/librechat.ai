...
<OptionTable
  options={[
    ['enabled', 'Boolean', 'Enables Anthropic via Vertex AI. When set to `true`, LibreChat will use Google Cloud Vertex AI instead of the direct Anthropic API.', 'Required to enable Vertex AI mode.'],
  ]}
/>
...
**Example:**
```yaml filename="endpoints / anthropic / vertex / enabled"
enabled: true
```
...
- Service key: `api/data/auth.json`
...
- Vertex AI and direct Anthropic API are mutually exclusive. When `vertex.enabled` is `true`, the `ANTHROPIC_API_KEY` environment variable is ignored.
