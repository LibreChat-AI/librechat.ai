...
<OptionTable
  options={[
    ...
    ['clientSideImageResize', 'Object', 'Configures client-side image resizing to optimize file uploads and prevent upload errors due to large image sizes.', ''],
  ]}
/>
...
The `clientSideImageResize` configuration enables automatic client-side image resizing before upload. This feature helps:
...
**Example:**
```yaml filename="clientSideImageResize"
fileConfig:
  clientSideImageResize:
    enabled: true
    maxWidth: 1920
...
- HEIC/HEIF images are automatically converted to JPEG format before resizing
