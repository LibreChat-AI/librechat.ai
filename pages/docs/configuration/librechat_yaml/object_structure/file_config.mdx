...
**Note:** Text parsing is the default method used by the "Upload as Text" feature (controlled by the `context` capability). It works first by attempting to use the same text parsing done by the RAG API, and if it fails or RAG is not connected, it falls back to text parsing without requiring any external services. See [Upload as Text](/docs/features/upload_as_text) for more information.
