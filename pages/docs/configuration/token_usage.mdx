...
You may receive an error trying to run this command line from shell on Digital Ocean. To execute the above commands to modify token allocation, after logging into your Digital Ocean droplet you need to then run an interactive shell within your docker container.

To find the appropriate Docker Container:
```
docker ps
```
this will list the Docker Containers available. You need to use the ContainerID relating to "ghcr.io/danny-avila/librechat-dev:latest "

Then execute:
```
docker exec -it <ContainerID> sh
```
This will launch an interactive shell in the Docker container, then run the commands as above. 
