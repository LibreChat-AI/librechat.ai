...
Currently, you can limit user token usage by enabling user balances. To enable token credit limiting for the OpenAI/Plugins endpoints, set the following .env variables:

<OptionTable
  options={[
    ['CHECK_BALANCE', 'boolean', 'Enable token credit balances for the OpenAI/Plugins endpoints.','CHECK_BALANCE=false'],
    ['REGISTRATION_BALANCE', 'integer', 'The default balance for newly registered users.','REGISTRATION_BALANCE=0'],
  ]}
/>
