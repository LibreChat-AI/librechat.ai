...
> **Important**: OAuth authentication is user-specific, not app-level. Each user will be prompted to authenticate with their own OAuth login when they first use an OAuth-enabled MCP server. This ensures that connection and authentication details are unique to each user, maintaining security and privacy in multi-user environments. The tokens shown during app startup are for app-level initialization only and are not used for individual user connections.
