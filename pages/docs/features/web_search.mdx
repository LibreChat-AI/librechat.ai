...
1. **Environment Variables** (Recommended for admins):
   ...
   # Optional: Custom Cohere API endpoint (for LiteLLM proxy, etc.)
...
**Custom Cohere Endpoints:**

- For LiteLLM proxy deployments: Set `COHERE_API_URL=http://litellm:8000/v1`
- For custom Cohere-compatible endpoints: Set `COHERE_API_URL=https://your-custom-endpoint.com/v1`
- If not specified, defaults to the official Cohere API (`https://api.cohere.ai/v1`)
