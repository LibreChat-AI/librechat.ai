...
- **Per-User Credentials for MCP Tools**:
  - LibreChat allows users to provide their own credentials for MCP servers that require authentication. Administrators can define `customUserVars` for an MCP server, enabling users to input their specific values for these variables. For more details on configuring `customUserVars`, see the [MCP Servers documentation](/docs/configuration/librechat_yaml/object_structure/mcp_servers#customuservars).
...
mcpServers:
  ...
  user-context-server:
    ...
    env:
      ...
      API_KEY: "${MCP_API_KEY}"
      CUSTOM_USER_SETTING: "{{USER_SPECIFIC_VALUE}}" # Example of using a custom user variable in env
    customUserVars:
      USER_SPECIFIC_VALUE:
        title: "User Specific Setting"
        description: "A custom setting that each user can define for this server."
```
