...
The starring system allows users to mark their favorite agents for quick access. Starred agents are easily accessible and help users organize their most frequently used agents.

### User Starring

Users can star and unstar agents through the agent interface:

- **Star an Agent**: Click the star icon to add an agent to your starred list
- **Unstar an Agent**: Click the starred icon to remove an agent from your starred list
- **Quick Access**: Starred agents appear in your favorites list for easy access

Starred agents are stored in the user's `starredAgents` field, which contains an array of agent IDs.

### Default Starred Agents (Admin Feature)

Administrators can mark agents as "default starred" to automatically star them for all new users. This provides a curated onboarding experience where new users immediately have access to recommended agents.

#### Setting Default Starred Status

To mark an agent as default starred:

1. Open the Agent Builder for the agent you want to set as default starred
2. In the Advanced Settings or Sharing panel, find the "Default Starred" toggle
3. Enable the toggle (this option is only available to administrators)
4. Save the agent

**Note:** Only administrators can set or change the `is_default_starred` field. Regular users cannot modify this setting.

#### How Default Starring Works

When a new user is created:

1. LibreChat automatically queries for all agents with `is_default_starred: true`
2. These agent IDs are automatically added to the new user's `starredAgents` array
3. New users immediately see these agents in their starred/favorites list
4. Users can still unstar these agents if they don't want them

This feature enables administrators to:

- Highlight recommended agents for new users
- Provide a curated onboarding experience
- Ensure important agents are easily discoverable

**Note:** Default starred agents are only automatically added when users are first created. Existing users are not affected unless they are recreated.

## Default Agent Configuration

Administrators can configure a default agent that is automatically selected when new users first open the chat interface.

### Configuration

Set the default agent ID in your `librechat.yaml` configuration file:

```yaml filename="librechat.yaml"
endpoints:
  agents:
    # (optional) Default agent ID for new users on first chat interface open
    # Only applies to first-time users (new signups) who haven't selected an agent yet
    # Does not override user's last-used agent preference after initial interaction
    defaultAgentId: 'agent-id-here'
```

### How It Works

- **First-Time Users**: When a new user (who has never used agents before) opens the chat interface, the default agent is automatically selected
- **Existing Users**: Users who have previously selected an agent will continue using their last-selected agent preference
- **User Override**: After the first interaction, users can change their agent selection, and that preference will be remembered

### Use Cases

- Provide a helpful default agent for new users
- Highlight a specific agent that represents your platform's capabilities
- Guide new users to a recommended starting point

**Note:** The default agent ID must match an existing agent's ID in your system. If the agent doesn't exist or the user doesn't have access to it, LibreChat will fall back to the first available agent or another endpoint.
