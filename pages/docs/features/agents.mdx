...
- [More info about the Code Interpreter API](/docs/features/code_interpreter)
  - **Requires an API key from [code.librechat.ai](https://code.librechat.ai/pricing)**
...
Agents can also be enhanced with various built-in tools (with more planned):
...
- The ability to customize a titling agent, for use across the start of chat threads
- The ability to customize a memory agent, for managing user context and history
- The ability to customize a prompt enhancement or predictions agent, for assisting with user input
