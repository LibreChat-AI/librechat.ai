...
- When you run the app for the first time, you need to create a new account by clicking on "Sign up" on the login page. The first account you make will be the admin account. Ath the moment the only privilege of the admin user is to configure [agents](docs/features/agents#administrator-controls), and it might be useful for the admin dashboard to manage other users later.
