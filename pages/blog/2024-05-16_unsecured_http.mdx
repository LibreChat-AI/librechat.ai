---
title: Fix the need to log in every time you refresh when using http
date: 2024/05/16
...
## Conclusion:
The "login required on every refresh" issue in LibreChat can be a hindrance to a smooth user experience, especially when accessing the instance on a local network. By understanding the cause and implementing the solution provided in this blog post, you can easily overcome this problem. Remember to create the `docker-compose.override.yml` file with the specified content and restart your LibreChat instance. With this fix in place, users can enjoy a seamless browsing experience without the need to log in repeatedly.