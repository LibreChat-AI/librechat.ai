...
- Enhanced SSRF protection for `actions.allowedDomains` and `mcpSettings.allowedDomains`
  - If not configured, SSRF targets are blocked by default (localhost, private IPs, `.internal`/`.local` TLDs)
  ...
  - See [Actions Configuration](/docs/configuration/librechat_yaml/object_structure/actions#alloweddomains) for details
