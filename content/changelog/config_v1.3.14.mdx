...
- Added Code Interpreter runtime settings
  ...
  - `endpoints.agents.statefulCodeSessions.allowedEnvironments` restricts selectable stateful workspace scopes; omitting it allows `user`, `agent-user`, and `conversation`
...
- Added trusted Agent trigger delivery configuration
  - `AGENT_TRIGGERS_SELF_URL` overrides the bound listener used for internal fire, continue, and steer admission when it must traverse another trusted HTTP origin
  - `ENABLE_SUBAGENT_COMPLETION_WAKEUPS=true` opts saved parent Agents into an idempotent new turn after detached child completion; enable only after every API replica is upgraded
  - Mongo-backed delivery provides source-qualified idempotency, ordering lanes, replica-safe leases, bounded retries, 90-day successful receipts, and retained dead letters
  - The delivery layer remains a trusted source-adapter foundation and does not expose a public webhook API
