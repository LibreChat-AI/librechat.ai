...
- Added `interface.sharedLinks`
  ...
  - Added `snapshotFiles` to control whether shared links include file snapshots for preview/download
  - Added `SHARED_LINKS_SNAPSHOT_FILES` as an environment override and global kill switch for shared-link file snapshots

- Added `interface.defaultPinnedTools`
  - Seeds the initial prompt-bar pinned tool state for users who have not customized their pins
  - Supports built-in tool keys such as `artifacts`, `execute_code`, `web_search`, `file_search`, `skills`, plus `mcp` or a specific MCP server name
  - Omitted values leave tools unpinned while preserving the MCP dropdown's default pinned state

- Added bundled Admin Panel Docker settings
  - `ADMIN_PANEL_SESSION_SECRET` provides the session encryption secret for the bundled admin-panel service
  - The bundled admin panel now requires operators to provide `ADMIN_PANEL_SESSION_SECRET`; the compose stacks no longer ship a development default
  - `ADMIN_PANEL_PORT` controls the host port for the admin panel in the default `docker-compose.yml`
  - `deploy-compose.yml` serves the bundled panel at `http://admin.localhost` through nginx and sets `ADMIN_PANEL_URL` for the API service

- Added `FILE_PREVIEW_MAX_EXTRACT_BYTES`
  - Controls the maximum source file size for code-execution artifact inline preview extraction
  - Files larger than the limit remain available for download; rendered preview HTML remains separately capped

- Added Google `url_context` model parameter support
  - Enables Google's URL Context tool for supported Gemini text models, including Gemini 2.5+ and Gemini 3.x
  - YouTube links are routed through Gemini's native video understanding when `url_context` is enabled
...
- Added `endpoints.custom[].customParams.includeReasoningContent`
  - Replays provider `reasoning_content` within tool-call turns for OpenAI-compatible custom endpoints that require it

- Added `endpoints.custom[].customParams.includeReasoningHistory`
  - Reconstructs `reasoning_content` from persisted conversation history across turns
  - Implies `includeReasoningContent`
...
- Added `promptCacheTtl` model parameter support
  - Supports `5m` and `1h` cache lifetimes for Anthropic, OpenRouter custom endpoints, and supported Bedrock models
  - Can be set in presets, model specs, and URL query parameters where prompt caching is enabled
