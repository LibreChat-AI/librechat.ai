...
> Note: This was scoped in 2025 but deprioritized in favor of revenue-critical features. After the ClickHouse acquisition, priorities shifted, but the feature first required a proper RBAC system capable of governing app-level configuration--something that didn't exist yet.
...
2. **Configuration Profiles** -- apply configuration profiles to roles initially, with group-level profiles as a stretch goal
...
These are ranked roughly by GitHub reactions and community discussions. The best way to move an item up is to react on its GitHub issue and contribute via pull requests.
