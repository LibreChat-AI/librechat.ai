...
icon: Bookmark
description: >-
  Memori pengguna terstruktur dengan kontrol manual, ekstraksi otomatis
  opsional, dan partisi yang dikelola agen
---

## Ikhtisar [#overview]

User Memory di LibreChat adalah **key/value store** yang menyimpan informasi spesifik pengguna secara persisten di seluruh percakapan. Pengguna dapat mengelola entri secara langsung, agen memori opsional dapat mengekstrak pembaruan dari obrolan, dan Agen dapat menggunakan alat memori saat diminta secara eksplisit untuk mengingat atau melupakan sesuatu.
...
- **Ekstraksi Otomatis Opsional**: Atur `memory.agent.enabled: true` untuk menjalankan agen memori yang dikonfigurasi dengan permintaan obrolan
...
- **Agent Tools**: Agen dapat menyimpan atau menghapus memori saat pengguna memintanya secara eksplisit
- **Agent Partitions**: Menjaga memori agen tetap terpisah dari kumpulan memori pribadi pengguna yang dibagikan
...
Ketika `memory.agent.enabled: true`, agen memori yang dikonfigurasi akan berjalan bersamaan dengan permintaan obrolan. Agen ini dieksekusi secara bersamaan dengan respons obrolan utama — agen dimulai sebelum respons utama dimulai dan dibatasi hingga durasi permintaan utama ditambah hingga 3 detik setelah permintaan tersebut selesai.
...
## Memori Agen [#agent-memory]

Endpoint Agents menyertakan kapabilitas `memory`. Ketika memory dikonfigurasi, pengguna memiliki izin tulis memory, dan personalisasi diaktifkan, menambahkan **Memory** ke agen akan memberikan alat `set_memory` dan `delete_memory`. Agen diinstruksikan untuk menggunakan alat-alat ini hanya ketika pengguna secara eksplisit memintanya untuk mengingat, memperbarui, atau melupakan sesuatu.

Agen menggunakan kumpulan memori pribadi yang dibagikan pengguna secara default. Di pengaturan Memory pada Agent Builder, aktifkan **Keep memories separate for this agent** untuk mengisolasi penyimpanan berdasarkan pengguna dan agen. Agen yang terisolasi tidak dapat melihat memori pribadi yang sudah ada atau memori milik agen terisolasi lainnya. Entri yang dicakup oleh agen akan muncul dengan nama agen di panel Memory dan dapat difilter berdasarkan partisi.

Partisi Agen ditambatkan pada akses Agen. Membaca, membuat, atau memperbarui partisi memerlukan pengguna untuk memiliki akses Agen dan akses tampilan saat ini ke Agen yang dirujuk; manajer Agen mempertahankan akses mereka yang lebih luas seperti biasa. LibreChat menolak ID Agen yang hilang atau tidak dapat diakses alih-alih memperlakukannya sebagai ruang nama memori arbitrer. Penghapusan masih dapat menargetkan partisi yang Agennya telah dihapus sehingga memori yatim piatu dapat dibersihkan.

Agen memori otomatis dan alat memori Agen inline bersifat independen. `memory.agent.enabled: true` mengontrol ekstraksi otomatis, sementara kapabilitas `memory` pada endpoint Agents mengontrol apakah Agen yang disimpan dapat mengelola memori selama percakapan.
