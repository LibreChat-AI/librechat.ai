...
- Se `skills_enabled` è impostato su true e non è definita alcuna allowlist di skill, l'agente può utilizzare l'intero catalogo attivo visibile all'utente, incluse le skill aggiunte in seguito.
...
Nell'Agent Builder, attiva **Use all skills** per utilizzare l'intero catalogo. Disattivandolo, verrà ripristinata la selezione di skill che era attiva prima dell'abilitazione dell'opzione.
...
Il backend memorizza quei file insieme alla skill. L'agente può risolvere i file della skill quando la skill è attiva e nell'ambito di riferimento. I file nidificati mantengono i loro percorsi relativi e vengono indirizzati attraverso lo spazio dei nomi `skills/{skillName}/...` rivolto al modello. Nel visualizzatore di Skill, i link Markdown relativi vengono risolti a partire dal file corrente e aprono il file di riferimento all'interno della stessa Skill. Quando Code Interpreter è abilitato, gli stessi file vengono montati in `/mnt/data/skills/{skillName}/...` in modo che le operazioni di shell e di codice possano utilizzarli direttamente.
