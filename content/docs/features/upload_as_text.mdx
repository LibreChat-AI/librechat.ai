...
icon: Upload
description: Attach supported files and let LibreChat route or extract their content automatically.
---

# Upload Files as Text

Ever wanted to hand a PDF, a code file, or a spreadsheet to the AI and just say _"read this"_? LibreChat's unified uploader routes the file to the provider or extracts its text automatically based on its type and the selected endpoint.
...
<Cards>
  <Card title="Model context" href="#what-happens-under-the-hood">
...
Operators can restore the earlier destination chooser with [`fileConfig.legacyFileUploadUX`](/docs/configuration/librechat_yaml/object_structure/file_config#legacyfileuploadux), or control automatic routing with [`defaultLLMDeliveryPath`](/docs/configuration/librechat_yaml/object_structure/file_config#defaultllmdeliverypath).
