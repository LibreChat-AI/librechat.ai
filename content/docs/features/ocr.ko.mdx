...
OCR 연결은 기본적으로 사설, 루프백, 링크 로컬 및 클라우드 메타데이터 대상을 차단합니다. 신뢰할 수 있는 사설 OCR 서비스를 사용하려면 해당 서비스의 정확한 사설 호스트와 포트를 [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses)에 추가하십시오. 이는 SSRF 보호에 대한 예외 사항이므로, 귀하가 직접 제어하는 인프라만 나열하십시오.
