...
- **`ADMIN`**: atribuído à primeira conta registrada em uma implantação de locatário único sem escopo. Implantações com escopo de locatário devem provisionar administradores por meio de um fluxo administrativo confiável. Administradores podem visualizar todos os recursos, modificar qualquer configuração, acessar o painel administrativo e configurar o comportamento de toda a plataforma.
...
- **Skills** (`skill`), veja [Skills](/docs/features/skills)
...
| `read:configs` / `manage:configs`    | Visualizar / modificar toda a configuração do sistema |
| `read:configs:<section>` / `manage:configs:<section>` | Visualizar / modificar uma seção de configuração de nível superior |
| `assign:configs:{user\|group\|role}` | Atribuir perfis de substituição de configuração a principais |
| `read:usage`                         | Visualizar o uso da plataforma e telemetria |
...
As capacidades de gerenciamento implicam suas capacidades de leitura correspondentes (por exemplo, possuir `manage:users` concede automaticamente `read:users`). Isso também se aplica às seções de configuração: `manage:configs:endpoints` satisfaz `read:configs:endpoints`. Usuários com acesso de configuração apenas em nível de seção recebem respostas de configuração de administrador filtradas para as seções que podem ler; usuários sem permissão de leitura de configuração recebem `403`.

Um usuário `SystemRoles.ADMIN` possui implicitamente todas as capacidades; as permissões (grants) permitem que você **delegue** um subconjunto de poderes administrativos a principais não administradores sem torná-los administradores completos. O acesso à interface administrativa (admin UI) ainda requer `access:admin`.
...
### Ajuste de Cache do Principal de ACL [#acl-principal-cache-tuning]

O LibreChat armazena em cache os IDs de grupo usados para resolver os principais de ACL de um usuário por cinco minutos, por padrão. O cache é delimitado por locatário (tenant) e identidade do membro, e alterações na associação de grupo invalidam as entradas afetadas. Ele não armazena em cache a função do usuário ou a busca de `idOnTheSource`, portanto, esses valores continuam sendo resolvidos a partir do usuário da solicitação atual ou do banco de dados.
...
Quando o cache principal é suportado pelo Redis, as configurações de bloqueio impedem que várias réplicas reconstruam a mesma entrada fria ao mesmo tempo. Defina `USER_PRINCIPALS_LOCK_TTL_MS=0` para desativar apenas o bloqueio de construção; o armazenamento em cache compartilhado e a invalidação entre processos permanecem ativos. Falhas de cache e de bloqueio recorrem ao banco de dados e não bloqueiam as verificações de permissão.
