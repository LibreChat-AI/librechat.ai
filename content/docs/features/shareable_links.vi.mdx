...
Hai biến môi trường trong tệp `.env` của bạn kiểm soát tính năng này:
...
`sharedLinks.public` kiểm soát việc người dùng có thể bật/tắt tùy chọn "chia sẻ với mọi người" hay không. `ALLOW_SHARED_LINKS_PUBLIC` vẫn quyết định liệu các liên kết công khai đó có thể xem được mà không cần đăng nhập hay không.
