...
Google endpointの場合、`url_context=true` を設定することで、サポートされているGeminiテキストモデルがユーザーメッセージに含まれるURLを読み取れるようになります。URL Contextが有効な場合、YouTubeのURLはネイティブの動画理解機能によって処理されます。

**Anthropic, Bedrock (Anthropic models), OpenRouter カスタム endpoint:**

`promptCache` を `true` または `false` に設定して、プロンプトキャッシュの有効/無効を切り替えます。プロンプトキャッシュが有効な場合のキャッシュの有効期間を選択するには、`promptCacheTtl` を `5m` や `1h` に設定します。
