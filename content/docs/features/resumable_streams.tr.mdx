...
Yüksek token oranlı dağıtımlar için `STREAM_DELTA_COALESCE_MS=25`, Redis delta yayınlarını gruplandırabilir ve 25 ms'ye kadar teslimat gecikmesi pahasına Redis üzerindeki iş yükünü azaltabilir. Gruplandırmayı devre dışı bırakmak için ayarlanmamış bırakın veya `0` değerini atayın; yalnızca her kopya (replica) grup çerçevelerini destekledikten sonra etkinleştirin. Bkz. [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
