...
Child agents draaien met een geïsoleerde context. De tool-search-status van de parent en de verzamelde context worden niet gekopieerd naar de child-run. Het modelgebruik van de child-run wordt gefactureerd bij de parent-transactie en opgenomen in de gebruikstotalen van de parent. De UI toont de child-run als een uitklapbaar agent-activiteitsonderdeel met status, ticker-updates en het volledige eindresultaat, inclusief resultaattekst die incrementeel werd geleverd terwijl de child streamde.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      'Maximaal aantal expliciete subagents per bovenliggende agent. Configureerbaar via `endpoints.agents.maxSubagents`, tot 50.',
      '10',
    ],
...
De geconfigureerde waarde is van toepassing op verzoeken voor het aanmaken, bijwerken en dupliceren van agents, op de `subagents.agent_ids` allowlists van Model Specs, en op de subagent-kiezer in de Agent Builder. Zie [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) voor de volledige referentie. De bovenstaande limieten voor diepte, graafknooppunten en run-configuratie zijn vast.
