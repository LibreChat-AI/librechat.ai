...
Für Bereitstellungen mit hoher Token-Rate kann `STREAM_DELTA_COALESCE_MS=25` Redis-Delta-Publikationen bündeln und die Redis-Auslastung reduzieren, auf Kosten von bis zu 25 ms Übermittlungslatenz. Lassen Sie es ungesetzt oder setzen Sie es auf `0`, um die Bündelung zu deaktivieren, und aktivieren Sie es erst, nachdem jedes Replikat Batch-Frames unterstützt. Siehe [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
...
- **Memory-first Wiederherstellung.** Die erneute Verbindung mit derselben Instanz liest aus dem lokalen Cache, wodurch ein Redis-Roundtrip vermieden wird.
- **Bereinigung beim Zugriff.** Veraltete Job-Einträge werden während Abfragen entfernt und abgeschlossene Streams laufen automatisch ab.
- **Garbage-collected storage.** Der In-Memory-Modus speichert Stream-Graphen mit `WeakRef`, sodass sie gelöscht werden, sobald eine Konversation endet.
...
- **Trennen.** Unterbrechen Sie kurz die Netzwerkverbindung und stellen Sie sie dann wieder her.
- **Navigation.** Navigieren Sie während eines laufenden Streams weg und kehren Sie dann zurück.
...
**Inhalt scheint dupliziert zu sein.** Dies deutet normalerweise auf eine Diskrepanz der Client-Version hin. Aktualisieren Sie auf die neueste Version von LibreChat.

**Hohe Speicherauslastung im Single-Instance-Modus.** Abgeschlossene Streams werden durch die Garbage Collection bereinigt. Wenn die Speicherauslastung hoch bleibt, suchen Sie nach sehr lange laufenden Streams, die nie abgeschlossen werden, oder nach Streams, die mit einem Fehler abgebrochen wurden, ohne bereinigt zu werden.

## Zugehörige Dokumentation [#related-documentation]

- [Redis-Konfiguration](/docs/configuration/redis): Einrichtung von Redis für Caching und horizontales Skalieren
