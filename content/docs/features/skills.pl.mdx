...
- Jeśli `skills_enabled` ma wartość true i nie ustawiono listy dozwolonych umiejętności (skill allowlist), agent może korzystać z pełnego aktywnego katalogu widocznego dla użytkownika, w tym z umiejętności dodanych później.
...
W kreatorze Agent Builder włącz opcję **Use all skills**, aby korzystać z pełnego katalogu. Wyłączenie tej opcji przywraca wybór umiejętności, który był aktywny przed włączeniem tego przełącznika.
...
Backend przechowuje te pliki wraz z umiejętnością (skill). Agent może rozpoznawać pliki umiejętności, gdy umiejętność jest aktywna i znajduje się w zakresie. Zagnieżdżone pliki zachowują swoje ścieżki względne i są adresowane poprzez przestrzeń nazw `skills/{skillName}/...` widoczną dla modelu. W przeglądarce umiejętności (Skill viewer) względne linki Markdown są rozpoznawane względem bieżącego pliku i otwierają przywołany plik w ramach tej samej umiejętności. Gdy włączony jest Code Interpreter, te same pliki są montowane w `/mnt/data/skills/{skillName}/...`, dzięki czemu operacje powłoki i kodu mogą korzystać z nich bezpośrednio.
