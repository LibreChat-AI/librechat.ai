...
Code API uploads recover from `429` rate limits by honoring `Retry-After`, reopening the source stream for each attempt, and sharing a bounded wait budget. Administrators can tune per-user, per-route recovery with [`codeApiUploadConcurrency` and `codeApiMaxRetryWaitMs`](/docs/configuration/librechat_yaml/object_structure/agents#code-api-upload-recovery).
...
With [Share files with subagents](/docs/features/subagents#share-files-with-subagents) enabled, children can use current-message attachments in separate managed Code Interpreter environments. Outputs stay private across code calls until the producing child uses `publish_artifact` to return a downloadable file. These runs use foreground execution; background code and attached workstation environments are not supported.
...
With [`ordinaryToolCancellation: true`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks), owners can request cooperative cancellation of ordinary background Code tools, including attached Bash. The task remains active and consumes capacity until the invocation settles; a tool that ignores cancellation may still complete.
...
Attached Bash uses a 30-second timeout by default. An environment can expose a larger model-requestable budget with [`configSchema.limits.maxCommandTimeoutMs`](/docs/configuration/librechat_yaml/object_structure/agents#statefulcodesessions), up to the five-minute protocol ceiling. The requested timeout never exceeds the deployment limit. Administrators can separately bound capacity retries with [`userConfig.bash.maxQueueWaitMs`](/docs/configuration/librechat_yaml/object_structure/agents#statefulcodesessions); this queue wait does not cancel an admitted or running command.
