...
This feature is experimental and opt-in. Enable `execute_code` and `stateful_code_sessions` in the [Agents endpoint capabilities](/docs/configuration/librechat_yaml/object_structure/agents#capabilities), enable Code Interpreter on the agent, then turn on **Stateful code sessions** in its Advanced settings. The workspace may occasionally reset, so important outputs should be saved under `/mnt/data`.
...
4. **[Stateful Code Sessions](/docs/features/agents#stateful-code-sessions)**: Reuse a persistent per-conversation workspace across an agent's code executions.
