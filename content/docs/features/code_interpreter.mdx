...
This feature is opt-in. Add `run_in_background` alongside `execute_code` in the [Agents endpoint capabilities](/docs/configuration/librechat_yaml/object_structure/agents#capabilities), enable Code Interpreter on the agent, then open its settings in the Tools marketplace and turn on **Background execution**. This makes code calls eligible for background dispatch; the model still decides per call whether to use it.
