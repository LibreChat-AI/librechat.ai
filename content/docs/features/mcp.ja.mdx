...
`notifications/tools/list_changed` を送信する MCP サーバーは、LibreChat を再起動することなくツールカタログを更新できます。LibreChat はライブ接続から現在のリストを取得し、追加されたツールを公開し、サーバーが削除したツールの広告を停止します。再接続時にも、接続が準備完了と見なされる前にカタログが同期されます。
