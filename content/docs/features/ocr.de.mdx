...
OCR-Verbindungen blockieren standardmäßig Ziele wie private Adressen, Loopback, Link-Local und Cloud-Metadaten. Um einen vertrauenswürdigen privaten OCR-Dienst zu nutzen, fügen Sie dessen exakten privaten Host und Port zu [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses) hinzu. Dies stellt eine Ausnahme vom SSRF-Schutz dar; listen Sie daher nur Infrastruktur auf, die Sie selbst kontrollieren.
...
Für Organisationen, die Azure AI Foundry nutzen, können Mistral OCR-Modelle in der eigenen Azure-Infrastruktur bereitgestellt werden. Derzeit ist das Modell **Mistral OCR 2503** für die Azure-Bereitstellung verfügbar.
...
4. Legen Sie die Umgebungsvariable `GOOGLE_SERVICE_KEY_FILE` unter Verwendung einer der unterstützten Methoden fest

### 4. Benutzerdefiniertes OCR (Geplant) [#4-custom-ocr-planned]
...
Wenn Sie 'Upload to Provider' für PDFs im Dropdown-Menü für Anhänge Ihres Chats mit Azure OpenAI nicht sehen, stellen Sie sicher, dass der API-Parameter für Antworten (Responses) im Parameter-Bereich aktiviert ist.
...
- **Spezialisierte Agenten**: Erstellen Sie Agenten mit fachspezifischem Wissen aus Dokumenten
- **Dokumentenbasierte Assistenten**: Erstellen Sie Agenten, die immer auf spezifische Handbücher oder Anleitungen verweisen
- **Code-Dateien**: Fügen Sie Codebeispiele oder Bibliotheken in die Agenten-Anweisungen ein
...
- Eine `user_provided` Strategieoption, die es Benutzern ermöglicht, ihren bevorzugten OCR-Dienst zu wählen
...
- LibreChat enthält derzeit nicht den geparsten Bildinhalt aus dem OCR-Prozess in seinen Antworten, obwohl Dienste wie [Mistral's OCR API](https://docs.mistral.ai/api/endpoint/ocr) diese im Ergebnis bereitstellen können. Diese Funktion wird möglicherweise in zukünftigen Updates unterstützt.
