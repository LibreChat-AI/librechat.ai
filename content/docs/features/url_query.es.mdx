...
Esto configura el endpoint a `assistants` automáticamente.

## Parámetros admitidos [#supported-parameters]

### Ajustes de LibreChat [#librechat-settings]
...
Para los endpoints de Google, establece `url_context=true` para permitir que los modelos de texto de Gemini compatibles lean las URLs incluidas en el mensaje del usuario. Las URLs de YouTube se gestionan con comprensión de video nativa cuando URL Context está habilitado.

**Anthropic, Bedrock (modelos de Anthropic), endpoints personalizados de OpenRouter:**

Establezca `promptCache` en `true` o `false` para activar o desactivar el almacenamiento en caché de prompts. Establezca `promptCacheTtl` en `5m` o `1h` para elegir el tiempo de vida de la caché cuando el almacenamiento en caché de prompts esté habilitado:
...
3. Mantenga los valores dentro de los límites del proveedor.
