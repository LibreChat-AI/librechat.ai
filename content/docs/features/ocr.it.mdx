...
Le connessioni OCR bloccano per impostazione predefinita le destinazioni private, loopback, link-local e i metadati cloud. Per un servizio OCR privato attendibile, aggiungi il suo host privato e la sua porta esatti a [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses). Questa è un'esenzione dalla protezione SSRF, quindi elenca solo le infrastrutture che controlli.
