...
只有 `enabled` 和 `allowSelf` 会在启动配置中发送给客户端。`agent_ids` 白名单保留在服务器端，并根据有效的子智能体（subagents）限制进行验证，该限制默认为 10，可通过 [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) 进行配置。客户端请求负载无法提供或覆盖 Model Specs 子智能体配置。
...
子智能体在隔离的上下文中运行。父级工具搜索状态和累积的上下文不会被复制到子运行中。子运行的模型使用量会计入父级交易，并包含在父级的使用总量中。UI 将子运行显示为一个可展开的智能体活动部分，包含状态、实时更新以及完整的最终结果，其中包括在子运行流式传输时增量交付的结果文本。
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      '每个父智能体允许的最大显式子智能体数量。可通过 `endpoints.agents.maxSubagents` 配置，上限为 50。',
      '10',
    ],
...
配置的值适用于智能体创建、更新和复制请求，适用于 Model Specs `subagents.agent_ids` 允许列表，以及智能体构建器（Agent Builder）中的子智能体选择器。有关完整参考，请参阅 [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents)。上述深度、图节点和运行配置限制是固定的。
