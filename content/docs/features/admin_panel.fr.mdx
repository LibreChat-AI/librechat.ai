...
Définissez le secret de session du panneau dans le fichier `.env` de LibreChat ; les fichiers compose le transmettent en tant que `SESSION_SECRET` du panneau :
...
| ------------------------------- | --------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SESSION_SECRET` | **Oui** en production | Repli de développement codé en dur lors de l'exécution de `bun dev` ; **aucun défaut** dans l'image Docker | Clé de chiffrement de session. Doit comporter au moins 32 caractères. |
...
| `API_SERVER_URL` | Non | Se replie sur `VITE_API_BASE_URL` | URL côté serveur pour les appels API LibreChat. Utile lorsque le serveur du panneau d'administration accède à LibreChat via une URL différente de celle du navigateur (par ex. service Kubernetes interne vs nom d'hôte public). |
| `PORT` | Non | `3000` | Port sur lequel le panneau d'administration écoute. |
| `ADMIN_SSO_ONLY` | Non | `false` | Masque le formulaire e-mail/mot de passe, forçant la connexion SSO uniquement. |
...
| `ADMIN_PANEL_METRICS_SECRET` | Non | _non défini_ | Jeton Bearer requis pour scraper le point de terminaison Prometheus `/metrics`. Le point de terminaison renvoie `401` lorsqu'il n'est pas défini ou ne correspond pas. |
