...
Nếu bạn chạy LibreChat với [`docker-compose.yml`](https://github.com/LibreChat-AI/LibreChat/blob/main/docker-compose.yml) hoặc [`deploy-compose.yml`](https://github.com/LibreChat-AI/LibreChat/blob/main/deploy-compose.yml) chính thức, bảng điều khiển quản trị (admin panel) sẽ được cung cấp dưới dạng một dịch vụ và tự động khởi chạy cùng với LibreChat -- không cần triển khai riêng biệt.
...
| `ADMIN_PANEL_METRICS_SECRET`    | Không                    | _chưa thiết lập_                                                                           | Bearer token bắt buộc để thu thập dữ liệu từ endpoint Prometheus `/metrics`. Endpoint sẽ trả về `401` khi chưa được thiết lập hoặc không khớp. |
...
Đây là bề mặt đằng sau [hệ thống ghi đè cấu hình theo từng chủ thể dựa trên DB](https://github.com/LibreChat-AI/LibreChat/pull/12354) của LibreChat. Các trường hợp sử dụng điển hình:
