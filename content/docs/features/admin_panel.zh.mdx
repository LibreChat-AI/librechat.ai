...
### 使用 Docker 运行（推荐） [#run-with-docker-recommended]
...
| `ADMIN_SSO_ONLY`                | 否 | `false` | 隐藏电子邮件/密码登录表单，强制仅使用 SSO 登录。 |
...
这是 LibreChat [基于数据库的每个主体配置覆盖系统](https://github.com/danny-avila/LibreChat/pull/12354)背后的实现。典型用例包括：
...
- [v0.8.5 更新日志](/changelog/v0.8.5)：admin API 基础
