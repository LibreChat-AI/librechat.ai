...
Para comenzar con la búsqueda web, configure un proveedor de búsqueda y un scraper. La mayoría de los proveedores requieren claves de API; Keenable funciona sin una tanto para la búsqueda como para la obtención de páginas, con una clave opcional para aumentar sus límites de tasa públicos. La re-clasificación (reranking) puede utilizar Jina o Cohere, o puede desactivarse con `rerankerType: "none"`. Puede configurar la pila de dos maneras:

<Callout type="warning" title="Endpoints privados autohospedados">
  Las conexiones de búsqueda web, scraping y rerank bloquean de forma predeterminada los destinos privados, loopback, link-local y de metadatos en la nube. Si el endpoint de SearXNG, Firecrawl, Jina u otro proveedor configurado es privado, añade su host y puerto exactos a [`webSearch.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/web_search#ssrf-protection-and-private-providers).
</Callout>
...
1. **Variables de entorno** (Recomendado para administradores):
   ...
   Usar Keenable sin una clave de entorno aún requiere la selección del proveedor en `librechat.yaml`:
   ...
   Consulte la [configuración completa de Keenable](/docs/configuration/librechat_yaml/object_structure/web_search#keenablescraperoptions) para ver los ajustes opcionales de búsqueda, obtención y límite de velocidad.
...
2. **Interfaz de usuario** (Si las variables de entorno no están configuradas):
   ...
   - Pueden elegir qué proveedor de búsqueda (Serper, SearXNG, Tavily o Keenable), scraper y servicio de reranker (Jina, Cohere o ninguno) utilizar.
...
4. Agregue el `host:port` exacto de su instancia a [`allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/web_search#ssrf-protection-and-private-providers) si se encuentra en una dirección privada o de loopback; de lo contrario, LibreChat bloqueará la conexión.
5. Opcionalmente, ajusta qué motores consulta tu instancia con [`searxngSearchOptions`](/docs/configuration/librechat_yaml/object_structure/web_search#searxngsearchoptions). El conjunto predeterminado es de solo tres motores e incluye DuckDuckGo, el cual presenta CAPTCHAs a la mayoría de las instancias autohospedadas, por lo que ampliarlo ayuda si las búsquedas devuelven resultados vacíos.
...
1. Seleccione Keenable como el proveedor de búsqueda, scraper, o ambos
2. No se requiere una API key para los endpoints públicos de búsqueda y obtención (fetch).
3. Opcionalmente, configura `KEENABLE_API_KEY` para aumentar los límites de tasa públicos.
4. Use `KEENABLE_API_URL` solo para anular el endpoint de búsqueda y `KEENABLE_FETCH_URL` solo para anular el endpoint de obtención de páginas.
...
- **SearXNG**: Motor de búsqueda meta de código abierto y autohospedado
  ...
  - Motores configurables, idioma de los resultados, intervalo de tiempo y tiempo de espera de la solicitud
...
- **Keenable**: Búsqueda web sin clave con autenticación opcional de mayor tasa
  - Admite búsqueda restringida por dominio, límites de resultados, títulos de atribución y tiempos de espera de solicitud.
  - También puede servir como proveedor de scraper
...
- **Keenable**: Obtención de páginas sin clave
  - Utiliza la misma clave de API opcional que la búsqueda de Keenable
  - Admite títulos de atribución y tiempos de espera de solicitud
...
5. Configure los reemplazos de endpoint de Tavily o Keenable si es necesario (opcional)
...
- La búsqueda habilitable y la obtención de páginas son sin clave de forma predeterminada; una clave opcional aumenta los límites de tasa públicos.
- `KEENABLE_API_URL` solo sobrescribe la búsqueda, mientras que `KEENABLE_FETCH_URL` solo sobrescribe la obtención de páginas
- Empareja `searchProvider: "keenable"`, `scraperProvider: "keenable"` y `rerankerType: "none"` para una pila completamente sin claves.
