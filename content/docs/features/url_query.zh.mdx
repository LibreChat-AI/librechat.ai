...
| `spec` | 通过精确名称选择一个 [Model Specs](/docs/configuration/librechat_yaml/object_structure/model_specs)。设置此项后，其他模型参数将被忽略，优先使用该 spec。如果 spec 配置了 `enforce: true`，则 URL 查询参数要生效可能需要此参数。 |
...
请参阅 [Anthropic prompt caching docs](https://www.anthropic.com/news/prompt-caching) 和 [Bedrock prompt caching docs](https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-caching.html#prompt-caching-get-started) 以获取详细信息。
