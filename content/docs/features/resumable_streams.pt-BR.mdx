...
Para implantações com alta taxa de tokens, `STREAM_DELTA_COALESCE_MS=25` pode agrupar publicações de delta do Redis e reduzir o trabalho do Redis ao custo de até 25 ms de latência de entrega. Deixe sem definir ou defina como `0` para desativar o agrupamento, e ative-o apenas após cada réplica suportar quadros de agrupamento. Veja [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
