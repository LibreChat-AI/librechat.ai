...
Les connexions OCR bloquent par défaut les destinations privées, de bouclage (loopback), locales au lien (link-local) et les métadonnées cloud. Pour un service OCR privé de confiance, ajoutez son hôte et son port privés exacts à [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses). Il s'agit d'une exemption à la protection SSRF, ne listez donc que l'infrastructure que vous contrôlez.
...
- **Support multilingue** : Traite du texte dans plusieurs langues et écritures
- **Gestion de mise en page complexe** : Gère le texte en plusieurs colonnes et le contenu mixte
- **Reconnaissance d'expressions mathématiques** : Traite avec précision les équations et les formules
- **Traitement à haute vitesse** : Traite jusqu'à 2000 pages par minute

**Considérations :**
- **Coût** : L'utilisation de Mistral OCR peut entraîner des frais car il s'agit d'un service d'API payant (bien que des essais gratuits puissent être disponibles).
...
**AWS Bedrock** prend en charge les téléchargements de documents via l'API Converse pour les formats suivants :
**PDF, CSV, DOC, DOCX, XLS, XLSX, HTML, TXT et Markdown (.md)**

Contraintes:
- La taille maximale par défaut des fichiers est de **4,5 Mo**
- Lorsque `fileConfig` ne définit pas de limite inférieure, les fichiers PDF Claude 4+ et les fichiers PDF ou DOCX Amazon Nova peuvent atteindre **32 Mo**.
...
Pour les questions temporaires sur des documents dans le chat, consultez [Upload as Text](/docs/features/upload_as_text).
...
- Les documents très volumineux peuvent être tronqués en raison des limitations de jetons des modèles d'IA sous-jacents.
...
LibreChat prévoit d'étendre les capacités d'OCR dans les futures versions :
...
- Options de traitement de documents améliorées
