...
Las conexiones OCR bloquean de forma predeterminada los destinos privados, de bucle local (loopback), de enlace local (link-local) y de metadatos en la nube. Para un servicio OCR privado de confianza, añada su host y puerto privados exactos a [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses). Esto es una exención de la protección SSRF, por lo que debe incluir únicamente la infraestructura que usted controle.
...
- **Manejo de diseños complejos**: Gestiona texto en varias columnas y contenido mixto
...
**Consideraciones:**
- **Coste**: El uso de Mistral OCR puede conllevar costes, ya que es un servicio de API de pago (aunque es posible que haya pruebas gratuitas disponibles).
...
Para organizaciones que utilizan Azure AI Foundry, pueden implementar modelos Mistral OCR en su infraestructura de Azure. Actualmente, el modelo **Mistral OCR 2503** está disponible para su implementación en Azure.
...
Actualmente, los cinco proveedores mencionados anteriormente ofrecen soporte para imágenes y archivos PDF, y Google también incluye soporte para archivos de audio y video cuando se utilizan junto con modelos multimodales compatibles. AWS Bedrock admite además documentos CSV, DOC, DOCX, XLS, XLSX, HTML, TXT y Markdown.
...
Si no ves 'Upload to Provider' como una opción para archivos PDF en el menú desplegable de archivos adjuntos de tu chat con Azure OpenAI, asegúrate de que el parámetro Responses API esté habilitado en el panel de Parameters.
...
Restricciones:
- El tamaño máximo de archivo predeterminado es **4.5 MB**
...
Para más detalles sobre la configuración de Bedrock, consulta la [guía de configuración de AWS Bedrock](/docs/configuration/pre_configured_ai/bedrock).
</Callout>

## Configuración detallada [#detailed-configuration]

Para obtener opciones de configuración adicionales y detalladas, consulte la [Estructura del objeto de configuración OCR](/docs/configuration/librechat_yaml/object_structure/ocr).
...
Los archivos que coincidan con estos patrones usarán OCR cuando:
- Subido al contexto de archivo del agente (siempre, si OCR está configurado)
- Cargado como texto en el chat (si OCR está configurado; de lo contrario, recurre al análisis de texto)
...
Para preguntas sobre documentos temporales en el chat, consulte [Upload as Text](/docs/features/upload_as_text).
...
- Control más granular sobre los ajustes de OCR
...
- LibreChat actualmente no incluye el contenido de imagen analizado del proceso de OCR en sus respuestas, a pesar de que servicios como [Mistral's OCR API pueden proporcionar](https://docs.mistral.ai/api/endpoint/ocr) estos resultados. Es posible que esta función sea compatible en futuras actualizaciones.
