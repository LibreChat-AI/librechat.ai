...
Agentes filhos são executados com contexto isolado. O estado de busca de ferramentas do pai e o contexto acumulado não são copiados para a execução do filho. O uso do modelo na execução do filho é cobrado na transação do pai e incluído nos totais de uso do pai. A interface do usuário exibe a execução do filho como uma parte expansível da atividade do agente com status, atualizações em tempo real e o resultado final completo, incluindo o texto do resultado entregue incrementalmente enquanto o filho realizava o streaming.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      'Máximo de subagentes explícitos por agente pai. Configurável via `endpoints.agents.maxSubagents`, até 50.',
      '10',
    ],
...
O valor configurado aplica-se a solicitações de criação, atualização e duplicação de agentes, a listas de permissão `subagents.agent_ids` do Model Specs e ao seletor de subagentes no Agent Builder. Consulte [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) para a referência completa. Os limites de profundidade, nó de grafo e configuração de execução acima são fixos.
