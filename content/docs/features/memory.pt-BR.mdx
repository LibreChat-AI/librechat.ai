...
icon: Bookmark
description: >-
  Memória de usuário estruturada com controles manuais, extração automática
  opcional e partições gerenciadas por agente
---

## Visão geral [#overview]

A User Memory no LibreChat é um **armazenamento de chave/valor** que persiste informações específicas do usuário entre conversas. Os usuários podem gerenciar entradas diretamente, um agente de memória opcional pode extrair atualizações de chats, e os Agents podem usar ferramentas de memória quando solicitados explicitamente a lembrar ou esquecer algo.
...
- **Extração Automática Opcional**: Defina `memory.agent.enabled: true` para executar o agente de memória configurado com as solicitações de chat
...
- **Agent Tools**: Agentes podem salvar ou excluir memórias quando o usuário solicita explicitamente
- **Partições de Agentes**: Mantenha as memórias de um agente separadas do pool pessoal compartilhado do usuário
...
Quando `memory.agent.enabled: true`, o agente de memória configurado é executado com as solicitações de chat. Ele é executado simultaneamente com a resposta principal do chat — ele começa antes que a resposta principal inicie e é limitado à duração da solicitação principal mais até 3 segundos após o seu término.
...
## Memória do Agente [#agent-memory]

O endpoint Agents inclui uma funcionalidade de `memory`. Quando a memória está configurada, o usuário possui permissões de escrita na memória e a personalização está ativada, adicionar **Memory** a um agente fornece a ele as ferramentas `set_memory` e `delete_memory`. O agente é instruído a usar essas ferramentas apenas quando o usuário solicitar explicitamente que ele memorize, atualize ou esqueça algo.

Agentes usam o pool de memória pessoal compartilhado do usuário por padrão. Nas configurações de Memória do Agent Builder, ative **Keep memories separate for this agent** para isolar o armazenamento por usuário e agente. Um agente isolado não vê memórias pessoais existentes ou memórias pertencentes a outros agentes isolados. Entradas com escopo de agente aparecem com o nome do agente no painel de Memória e podem ser filtradas por partição.

As partições de Agent são vinculadas ao acesso de Agent. Ler, criar ou atualizar uma requer que o usuário tenha acesso a Agents e acesso de visualização atual ao Agent referenciado; gerentes de Agent mantêm seu acesso mais amplo normal. O LibreChat rejeita IDs de Agent ausentes ou inacessíveis em vez de tratá-los como namespaces de memória arbitrários. A exclusão ainda pode ter como alvo uma partição cujo Agent foi removido, para que memórias órfãs possam ser limpas.

O agente de memória automática e as ferramentas de memória de Agente integradas são independentes. `memory.agent.enabled: true` controla a extração automática, enquanto a capacidade `memory` do endpoint Agents controla se os Agentes salvos podem gerenciar memórias durante uma conversa.
