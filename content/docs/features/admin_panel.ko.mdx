...
LibreChat의 `.env` 파일에 패널의 session secret을 설정하세요. compose 파일은 이를 패널의 `SESSION_SECRET`으로 전달합니다:
...
| 변수 | 필수 여부 | 기본값 | 설명 |
| ------------------------------- | --------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SESSION_SECRET`                | 프로덕션 환경에서 **필수** | `bun dev` 실행 시 하드코딩된 개발용 폴백; Docker 이미지에는 **기본값 없음** | 세션 암호화 키. 최소 32자 이상이어야 합니다. |
...
| `PORT`                          | 아니요 | `3000` | 관리자 패널이 수신 대기할 포트. |
...
| `ADMIN_PANEL_METRICS_SECRET`    | 아니요 | _설정되지 않음_ | `/metrics` Prometheus 엔드포인트를 스크랩하는 데 필요한 Bearer 토큰. 설정되지 않았거나 일치하지 않으면 엔드포인트가 `401`을 반환합니다. |
