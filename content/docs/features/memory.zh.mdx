...
title: 用户记忆
icon: Bookmark
description: 具有手动控制、可选自动提取和智能体管理分区的结构化用户记忆
---

## 概述 [#overview]

LibreChat 中的 User Memory 是一个**键/值存储**，用于在不同对话之间持久化存储用户特定的信息。用户可以直接管理这些条目，可选的内存代理（memory agent）可以从聊天中提取更新，而 Agents 在被明确要求记住或忘记某事时，可以使用内存工具。
...
- **可选自动提取**：设置 `memory.agent.enabled: true` 以在聊天请求中运行已配置的 memory agent。
...
- **Agent Tools**: 智能体可以在用户明确要求时保存或删除记忆
- **Agent Partitions**: 将智能体的记忆与用户的共享个人池分离开来
...
当 `memory.agent.enabled: true` 时，配置的 memory agent 会随聊天请求一起运行。它与主聊天响应并发执行——它在主响应开始前启动，并限制在主请求的持续时间内，外加结束后最多 3 秒的运行时间。
...
Agents endpoint 包含 `memory` 功能。当配置了 memory、用户拥有 memory 写入权限且启用了个性化设置时，为 agent 添加 **Memory** 将使其获得 `set_memory` 和 `delete_memory` 工具。系统会指示 agent 仅在用户明确要求记住、更新或忘记某事时才使用这些工具。

Agent 默认使用用户共享的个人记忆池。在 Agent Builder 的 Memory 设置中，启用 **Keep memories separate for this agent** 即可按用户和 Agent 隔离存储。隔离后的 Agent 无法查看现有的个人记忆或其他隔离 Agent 的记忆。Agent 作用域内的条目会在 Memory 面板中显示该 Agent 的名称，并可通过分区进行筛选。

Agent 分区锚定于 Agent 访问权限。读取、创建或更新分区需要用户具备 Agent 访问权限以及对所引用 Agent 的当前查看权限；Agent 管理员保留其常规的更广泛访问权限。LibreChat 会拒绝缺失或无法访问的 Agent ID，而不是将其视为任意的内存命名空间。删除操作仍可针对其 Agent 已被移除的分区进行，以便清理孤立的内存。

自动记忆 Agent 和内联 Agent 记忆工具是相互独立的。`memory.agent.enabled: true` 控制自动提取，而 Agents endpoint 的 `memory` 功能则控制已保存的 Agents 是否可以在对话期间管理记忆。
