...
Examples include `read:configs:interface`, `manage:configs:endpoints`, and `manage:configs:langfuse`. Delegated users still need `access:admin` to enter the admin surface. Operations that affect the base profile as a whole can continue to require broad configuration management even when field-level changes are section-scoped.
