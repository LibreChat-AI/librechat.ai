...
Solo `enabled` e `allowSelf` vengono inviati ai client nella configurazione di avvio. La whitelist `agent_ids` rimane lato server e viene convalidata rispetto al limite effettivo di subagent, che è impostato su 10 per impostazione predefinita ed è configurabile con [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents). I payload delle richieste del client non possono fornire o sovrascrivere la configurazione dei Subagent di Model Specs.
...
Gli agenti secondari (child agents) vengono eseguiti con un contesto isolato. Lo stato della ricerca di strumenti (tool-search) del genitore e il contesto accumulato non vengono copiati nell'esecuzione secondaria. L'utilizzo del modello dell'esecuzione secondaria viene fatturato nella transazione del genitore e incluso nei totali di utilizzo del genitore. L'interfaccia utente mostra l'esecuzione secondaria come una parte dell'attività dell'agente espandibile con stato, aggiornamenti in tempo reale e il risultato finale completo, incluso il testo del risultato fornito in modo incrementale durante lo streaming dell'agente secondario.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      'Numero massimo di subagent espliciti per agente principale. Configurabile tramite `endpoints.agents.maxSubagents`, fino a 50.',
      '10',
    ],
...
Il valore configurato si applica alle richieste di creazione, aggiornamento e duplicazione degli agenti, alle allowlist `subagents.agent_ids` dei Model Specs e al selettore di subagent nell'Agent Builder. Vedi [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) per il riferimento completo. I limiti di profondità, nodo del grafo e configurazione di esecuzione sopra indicati sono fissi.
