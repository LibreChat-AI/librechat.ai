...
The backend stores those files with the skill. The agent can resolve skill files when the skill is active and in scope. Nested files keep their relative paths and are addressed through the model-facing `skills/{skillName}/...` namespace. When Code Interpreter is enabled, the same files are mounted at `/mnt/data/skills/{skillName}/...` so shell and code operations can use them directly.
