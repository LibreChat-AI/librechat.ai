...
Başlangıç yapılandırmasında istemcilere yalnızca `enabled` ve `allowSelf` gönderilir. `agent_ids` izin listesi sunucu tarafında kalır ve varsayılan olarak 10 olan ve [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) ile yapılandırılabilen etkin alt aracı (subagent) sınırı üzerinden doğrulanır. İstemci istek yükleri, Model Specs Alt Aracı yapılandırmasını sağlayamaz veya geçersiz kılamaz.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      'Ebeveyn ajan başına maksimum açık alt ajan sayısı. `endpoints.agents.maxSubagents` aracılığıyla 50\'ye kadar yapılandırılabilir.',
      '10',
    ],
...
Yapılandırılan değer; temsilci oluşturma, güncelleme ve kopyalama istekleri, `subagents.agent_ids` izin listeleri için Model Specs ve Temsilci Oluşturucu'daki (Agent Builder) alt temsilci seçici için geçerlidir. Tam referans için [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) bölümüne bakın. Yukarıdaki derinlik, grafik düğümü ve çalıştırma yapılandırması sınırları sabittir.
