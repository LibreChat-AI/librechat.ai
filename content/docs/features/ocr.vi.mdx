...
Các kết nối OCR chặn các đích đến private, loopback, link-local và cloud-metadata theo mặc định. Đối với một dịch vụ OCR private đáng tin cậy, hãy thêm chính xác host và port private của nó vào [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses). Đây là một ngoại lệ đối với bảo vệ SSRF, vì vậy chỉ liệt kê cơ sở hạ tầng mà bạn kiểm soát.
...
- Khi `fileConfig` không đặt giới hạn nhỏ hơn, các tệp PDF Claude 4+ và các tệp PDF hoặc DOCX Amazon Nova có thể có dung lượng lên đến **32 MB**
