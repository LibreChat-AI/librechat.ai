...
Before consuming generation capacity, LibreChat checks every selected MCP tool required by the scheduled Agent and its runnable Subagent graph. A missing tool, unavailable server, revoked permission, incomplete catalog, or authentication flow requiring interaction prevents the run and identifies the Agent that needs repair. The schedule UI provides an **Open agent** recovery action. Disabled schedules can still be edited while an MCP server is unavailable.
...
Use [`admissionConcurrency`, `mcpPreflightConcurrency`, and `mcpPreflightTimeoutMs`](/docs/configuration/librechat_yaml/object_structure/interface#schedules) to bound schedule admission and MCP readiness work independently from generation concurrency.
