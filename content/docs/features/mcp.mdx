...
Passive catalog loads coalesce OAuth discovery attempts and progressively back off after failures, so a disconnected integration does not retry discovery on every request. Credential changes are generation-fenced across replicas, preventing an older discovery result from overriding newer authentication. Operators can tune the bounded recovery policy under [`mcpSettings.catalogRecovery`](/docs/configuration/librechat_yaml/object_structure/mcp_settings#catalogrecovery); the defaults are appropriate for most deployments.
...
Trusted operators can use `Authorization: 'Bearer {{LIBRECHAT_OPENID_ACCESS_TOKEN}}'` on a YAML-defined remote server to pass through the user's live OpenID access token. Explicit MCP OAuth, on-behalf-of exchange, and administrator API-key authentication take precedence. After a `401` or `403`, LibreChat can refresh the connection once, but it never automatically replays the rejected tool call. See the [MCP server placeholder reference](/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers) for setup and security guidance.
...
#### Restricting a Variable to Predefined Values

Not every user-provided variable is a free-form secret. When a variable only accepts a known set of inputs — a region, an environment, a set of scopes — declare them under `values`. The field then renders as a select instead of a text input, and only the listed values are accepted: anything else is rejected before it is stored, so the restriction holds even for a request that bypasses the UI.

```yaml
mcpServers:
  my-api-server:
    type: streamable-http
    url: 'https://{{MY_API_ENV}}.example.com/mcp'
    headers:
      X-Auth-Token: '{{MY_API_KEY}}'
      X-Scopes: '{{MY_API_SCOPES}}'
    customUserVars:
      MY_API_KEY:
        title: 'API Key'
      ...
      MY_API_ENV:
        title: 'Environment'
        description: 'Which deployment to connect to'
        sensitive: false
        values:
          - value: api
            label: 'Production'
...
- A plain string entry is used as both the stored value and the label; use the `value` / `label` object form when the label should read differently from the technical value.
- `multiple: true` lets the user pick several values, stored as a single comma-joined string (`read,write`). It requires `values`, and no declared value may itself contain a comma.
- Marking a select `sensitive: false` also preselects the current choice when the user reopens the dialog. Values of secret variables are never sent back to the browser, so a select left `sensitive` (the default) always opens empty.

Because the restriction is enforced server-side, `values` also bounds what a user can point a server at — the directory handed to a `stdio` server, or the host of a remote one, as in the `url` above.
