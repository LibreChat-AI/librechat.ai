...
Untuk penyebaran dengan tingkat token tinggi, `STREAM_DELTA_COALESCE_MS=25` dapat melakukan batch pada publikasi delta Redis dan mengurangi beban kerja Redis dengan konsekuensi latensi pengiriman hingga 25 ms. Biarkan tidak disetel atau setel ke `0` untuk menonaktifkan batching, dan aktifkan hanya setelah setiap replika mendukung batch frames. Lihat [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
