...
Các MCP server phát ra `notifications/tools/list_changed` có thể cập nhật danh mục công cụ của chúng mà không cần khởi động lại LibreChat. LibreChat sẽ tìm nạp danh sách hiện tại từ kết nối trực tiếp, xuất bản các công cụ mới được thêm vào và ngừng quảng bá các công cụ mà server đã xóa. Việc kết nối lại cũng đồng bộ hóa danh mục trước khi kết nối được coi là sẵn sàng.
