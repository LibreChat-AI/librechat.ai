...
Wenn Sie LibreChat mit der offiziellen [`docker-compose.yml`](https://github.com/LibreChat-AI/LibreChat/blob/main/docker-compose.yml) oder [`deploy-compose.yml`](https://github.com/LibreChat-AI/LibreChat/blob/main/deploy-compose.yml) ausführen, wird das Admin-Panel als Dienst mitgeliefert und startet automatisch zusammen mit LibreChat -- es ist keine separate Bereitstellung erforderlich.
...
| ------------------------------- | --------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SESSION_SECRET` | **Ja** in Produktion | Hardcodierter Dev-Fallback bei Ausführung von `bun dev`; **kein Standardwert** im Docker-Image | Sitzungsverschlüsselungsschlüssel. Muss mindestens 32 Zeichen lang sein. |
...
| `ADMIN_PANEL_SESSION_SECRET` | **Ja** in Produktion | Greift auf `CREDS_KEY` in gebündelten LibreChat Docker-Stacks zurück | LibreChat-seitige Variable, die dem `SESSION_SECRET` des Admin-Panels für den gebündelten Admin-Panel-Dienst zugeordnet ist. Generieren Sie für die Produktion einen eindeutigen Wert mit mindestens 32 Zeichen. |
| `ADMIN_PANEL_PORT` | Nein | `3000` | Host-Port, der vom gebündelten Admin-Panel-Dienst in der Standard-`docker-compose.yml` freigegeben wird. |
| `ADMIN_SSO_ONLY` | Nein | `false` | Blendet das E-Mail/Passwort-Formular aus und erzwingt den SSO-only-Login. |
...
In den gebündelten Docker-Stacks von LibreChat läuft das Admin-Panel als `admin-panel`-Dienst. Die standardmäßige `docker-compose.yml` macht es über `ADMIN_PANEL_PORT` verfügbar; `deploy-compose.yml` leitet es über nginx unter `http://admin.localhost` weiter und setzt `ADMIN_PANEL_URL` für den API-Dienst.
...
Dies ist die Oberfläche hinter dem [DB-gestützten Konfigurations-Override-System pro Principal](https://github.com/LibreChat-AI/LibreChat/pull/12354) von LibreChat. Typische Anwendungsfälle:
