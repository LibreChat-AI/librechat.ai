...
- **`ADMIN`**: kapsamı belirlenmemiş tek kiracılı bir dağıtımda kaydedilen ilk hesaba atanır. Kiracı kapsamlı dağıtımlar, yöneticileri güvenilir bir idari akış aracılığıyla sağlamalıdır. Yöneticiler her kaynağı görebilir, herhangi bir ayarı değiştirebilir, yönetici paneline erişebilir ve platform genelindeki davranışları yapılandırabilir.
...
- **Beceriler** (`skill`), bkz. [Beceriler](/docs/features/skills)
...
| `read:configs` / `manage:configs`    | Tüm sistem yapılandırmasını görüntüleme / değiştirme |
| `read:configs:<section>` / `manage:configs:<section>` | Üst düzey bir yapılandırma bölümünü görüntüleme / değiştirme |
...
Yönetme (manage) yetenekleri, karşılık gelen okuma (read) yeteneklerini de kapsar (örneğin, `manage:users` yetkisine sahip olmak otomatik olarak `read:users` yetkisini de verir). Bu durum yapılandırma bölümleri için de geçerlidir: `manage:configs:endpoints`, `read:configs:endpoints` yetkisini karşılar. Yalnızca bölüm kapsamlı yapılandırma erişimine sahip kullanıcılar, okuyabildikleri bölümlerle filtrelenmiş yönetici yapılandırma yanıtlarını alırlar; hiçbir yapılandırma okuma yetkisi olmayan kullanıcılar ise `403` hatası alırlar.

Bir `SystemRoles.ADMIN` kullanıcısı örtük olarak tüm yetkilere sahiptir; izinler (grants), yönetici olmayan asıllara (principals), onları tam yönetici yapmadan yönetici yetkilerinin bir alt kümesini **devretmenize** olanak tanır. Yönetici arayüzünün kendisine erişim hala `access:admin` gerektirir.
...
### ACL Principal Önbellek Ayarları [#acl-principal-cache-tuning]

LibreChat, bir kullanıcının ACL sorumlularını çözümlemek için kullanılan grup kimliklerini varsayılan olarak beş dakika boyunca önbelleğe alır. Önbellek, kiracı ve üye kimliğine göre kapsamlandırılır ve grup üyeliğindeki değişiklikler etkilenen girdileri geçersiz kılar. Kullanıcının rolünü veya `idOnTheSource` aramasını önbelleğe almaz, bu nedenle bu değerler mevcut istek kullanıcısından veya veritabanından çözümlenmeye devam eder.
...
Ana önbellek Redis destekli olduğunda, kilit ayarları birden fazla replikanın aynı soğuk girdiyi aynı anda yeniden oluşturmasını engeller. Yalnızca oluşturma kilitlemesini devre dışı bırakmak için `USER_PRINCIPALS_LOCK_TTL_MS=0` olarak ayarlayın; paylaşılan önbellek depolaması ve süreçler arası geçersiz kılma etkin kalmaya devam eder. Önbellek ve kilit hataları veritabanına geri döner ve izin denetimlerini engellemez.
