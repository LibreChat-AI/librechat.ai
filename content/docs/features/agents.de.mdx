...
description: >-
  ...
  leistungsstarkes Framework zum Aufbau benutzerdefinierter KI-Assistenten mit
  jedem beliebigen Modellanbieter.
---
...
- [Weitere Informationen zur Code Interpreter API](/docs/features/code_interpreter)
  - **Erfordert ein API-Abonnement von [code.librechat.ai](https://code.librechat.ai/pricing)**
...
- Semantische Suche über hochgeladene Dokumente
...
MCP ist ein offenes Protokoll, das standardisiert, wie Anwendungen Large Language Models (LLMs) Kontext bereitstellen, und fungiert dabei wie ein universeller Adapter für KI-Tools und Datenquellen.
...
6. Speichere deine Änderungen am Agenten
...
Deferred Tools ermöglichen es Agenten, auf viele MCP Tools zuzugreifen, ohne diese alle vorab in den LLM-Kontext zu laden. Stattdessen sind Deferred Tools zur Laufzeit über einen **Tool Search**-Mechanismus auffindbar.
...
**Konfiguration verzögerter Tools:**
...
Skills ermöglichen es Agenten, wiederverwendbare Anweisungen aus `SKILL.md`-Definitionen zu laden. Sie können manuell im Chat über `$` ausgewählt, vom Modell automatisch über den Skill-Katalog entdeckt oder bei jedem Turn immer angewendet werden.

Für Details zu Erstellung, Aufruf und Zugriffskontrolle siehe [Skills](/docs/features/skills).
...
Wenn aktiviert, werden standardmäßig zusätzliche Anweisungen speziell für die Verwendung von Artefakten hinzugefügt. Zu den Optionen gehören:

- **Anweisungen für shadcn/ui aktivieren**: Fügt Anweisungen für die Verwendung von shadcn/ui-Komponenten hinzu (eine Sammlung wiederverwendbarer Komponenten, die mit Radix UI und Tailwind CSS erstellt wurden)
...
- Tools können über die [`librechat.yaml`](/docs/configuration/librechat_yaml) Konfigurationsdatei deaktiviert werden:
...
- Actions können über die [`librechat.yaml`](/docs/configuration/librechat_yaml) Konfigurationsdatei deaktiviert werden:
  - [Weitere Informationen](/docs/configuration/librechat_yaml/object_structure/agents#capabilities)
...
- Beachten Sie, dass Sie auf Operationsebene in der OpenAPI-Spezifikation für Actions das Flag 'x-strict': true hinzufügen können.
  ...
  - Der Strict-Modus unterstützt nur eine Teilmenge von json. Lesen Sie https://platform.openai.com/docs/guides/structured-outputs/some-type-specific-keywords-are-not-yet-supported für Details.
...
- **Hinweis:** Greifen Sie über das Advanced Settings-Panel im Agent Builder auf diese Funktion zu.
...
Subagents ermöglichen es einem Agenten, eine fokussierte Aufgabe an einen isolierten untergeordneten Prozess (Child Run) zu delegieren. Das untergeordnete Element erhält sein eigenes Kontextfenster sowie einen eigenen Tool-Ausführungsablauf und gibt anschließend ein kompaktes Ergebnis an das übergeordnete Element zurück, anstatt den Kontext des übergeordneten Elements mit jedem Zwischenschritt zu füllen.
...
Erweiterte Einstellungen für Ihren Agenten (zu finden in der Ansicht „Erweitert“ des Agenten-Formulars) außerhalb der „Fähigkeiten“ (capabilities).

#### Maximale Agenten-Schritte [#max-agent-steps]

Diese Einstellung ermöglicht es Ihnen, die Anzahl der Schritte zu begrenzen, die ein Agent in einem „Run“ ausführen kann, was sich auf die Agenten-Schleife bezieht, bevor eine endgültige Antwort gegeben wird.
...
1. API-Anfrage -> 2. Tool-Nutzung (1 oder mehrere Tools) -> 3. Nachfolge-API-Anfrage
...
2. **Als Text hochladen** (erfordert `context`-Funktionalität)
...
3. **Upload für die Dateisuche** (erfordert die `file_search`-Funktion, eingeschaltet)
   - Verwendet semantische Suche (RAG) mit Vektorspeichern
   - Gibt relevante Chunks über Tool-Nutzung zurück
   - Optimal für große Dokumente/mehrere Dateien
   - Nicht optimal für strukturierte Daten (CSV, Excel, JSON usw.)
...
1. Öffne den Agenten im Agent Builder
2. Klicken Sie auf die Schaltfläche **Teilen** in der Fußzeile (sichtbar, wenn Sie der Ersteller oder ein Administrator sind oder wenn Ihnen die `SHARE`-Berechtigung für diesen spezifischen Agenten erteilt wurde).
3. Suchen Sie im Personenauswahl-Feld nach Benutzern, Gruppen oder Rollen und weisen Sie jedem eine Rolle zu
...
- Agent-Sharing für alle Benutzer aktivieren/deaktivieren
- Berechtigungen für die Agentennutzung steuern
...
- Nur ursprüngliche Autoren, Administratoren und Berechtigte mit Eigentümer-Berechtigung können geteilte Agents löschen.
- Agents sind für Autoren privat, sofern sie nicht geteilt werden

## Optionale Konfiguration [#optional-configuration]

LibreChat ermöglicht es Administratoren, die Nutzung von Agenten über die [`librechat.yaml`](/docs/configuration/librechat_yaml) Datei zu konfigurieren:
...
Wenn du dem [Ubuntu Docker Guide](/docs/remote/docker_linux) gefolgt bist:
...
- Fähigkeit, Agenten für verschiedene Funktionen anzupassen: Titulierung (Benennung von Chat-Threads), Speicherverwaltung (Benutzerkontext/-verlauf) und Prompt-Verbesserung (Eingabeunterstützung/-vorhersagen)
- Mehr Tools, konfigurierbare Tool-Parameter, dynamische Tool-Erstellung.

Darüber hinaus führt das Update ein neues Paradigma für LibreChat ein, da die zugrunde liegende Architektur eine dringend benötigte Auffrischung für die App bietet, die sowohl die Benutzererfahrung als auch die allgemeine App-Leistung optimiert.
