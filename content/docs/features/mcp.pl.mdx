---
title: MCP
icon: Blocks
description: >-
  Dowiedz się, jak LibreChat wykorzystuje Model Context Protocol (MCP), aby
  zapewnić płynną integrację z zewnętrznymi narzędziami, źródłami danych i
  wyspecjalizowanymi usługami.
---

Model Context Protocol (MCP) to otwarty protokół, który standaryzuje sposób, w jaki aplikacje dostarczają kontekst dla dużych modeli językowych (LLM). Pomyśl o MCP jak o **"USB-C dla AI"** – tak jak USB-C zapewnia uniwersalny standard połączeń dla urządzeń elektronicznych, tak MCP oferuje ustandaryzowany sposób łączenia modeli AI z różnorodnymi narzędziami, źródłami danych i usługami.

LibreChat wykorzystuje MCP, aby znacząco rozszerzyć możliwości Twoich agentów AI, umożliwiając integrację wszystkiego, od dostępu do systemu plików, przeglądarek internetowych i specjalistycznych API, po niestandardowe narzędzia biznesowe.

## Dlaczego MCP ma znaczenie [#why-mcp-matters]

LLMs są ograniczone do swoich wbudowanych możliwości. Dzięki MCP, LibreChat przełamuje te bariery poprzez:

- **Łączenie z dowolnym narzędziem lub usługą**, która udostępnia serwer MCP
- **Standaryzacja integracji**, dzięki której nie musisz edytować kodu LibreChat dla każdego narzędzia
- **Obsługa środowisk wieloużytkownikowych** z odpowiednim uwierzytelnianiem i izolacją
- **Dodawanie i aktualizowanie serwerów w czasie wykonywania** z poziomu panelu ustawień MCP, bez konieczności edytowania pliku konfiguracyjnego czy restartowania

## Jak działa MCP w LibreChat [#how-mcp-works-in-librechat]

LibreChat zapewnia dwa sposoby korzystania z serwerów MCP: w obszarze czatu lub z agentami.

Możesz skonfigurować serwery MCP ręcznie w pliku `librechat.yaml` lub korzystając z [smithery.ai](https://smithery.ai), aby znaleźć i zainstalować serwery MCP w `librechat.yaml` ([zobacz przykład poniżej](#basic-configuration)). Za każdym razem, gdy dodajesz lub edytujesz serwer w `librechat.yaml`, musisz zrestartować LibreChat, aby zainicjować połączenia. Serwery dodane za pośrednictwem [panelu ustawień MCP](#adding-mcp-servers-in-the-ui) zaczynają działać bez konieczności restartu. Serwer utworzony w panelu, który korzysta z OAuth, jest rejestrowany przy zapisie, ale początkowo pozostaje rozłączony: [uwierzytelnij go](#step-3-check-connection-status-and-authenticate), aby nawiązać połączenie.

### Adres URL wywołania zwrotnego OAuth [#oauth-callback-url]

Dla serwerów MCP z włączonym OAuth, adresem URL wywołania zwrotnego (callback URL) LibreChat jest:

```text
${DOMAIN_SERVER}/api/mcp/<server-name>/oauth/callback
```

`<server-name>` to klucz używany w sekcji `mcpServers` w pliku `librechat.yaml` lub nazwa serwera utworzona w interfejsie ustawień MCP. Na przykład serwer o nazwie `salesforce` z parametrem `DOMAIN_SERVER=https://chat.example.com` używa adresu `https://chat.example.com/api/mcp/salesforce/oauth/callback`.

Zarejestruj ten dokładny adres URL wywołania zwrotnego (callback URL) u dostawcy OAuth. Lokalne instalacje Docker zazwyczaj używają `http://localhost:3080` jako podstawowego adresu URL.

### W obszarze czatu [#in-chat-area]

![Narzędzia MCP w obszarze czatu](/images/agents/mcp_chat.png)

LibreChat wyświetla skonfigurowane serwery MCP bezpośrednio w obszarze czatu podczas korzystania z tradycyjnych endpoint (OpenAI, Anthropic, Google, Bedrock itp.):

- Najpierw wybierz dowolny endpoint inny niż agent oraz model kompatybilny z narzędziami
- Serwery MCP pojawiają się na liście rozwijanej w interfejsie czatu poniżej pola wprowadzania tekstu
- Po wybraniu, wszystkie narzędzia z tego serwera stają się dostępne dla Twojego bieżącego modelu
- Szybki dostęp do narzędzi MCP bez tworzenia agenta, umożliwiający korzystanie z wielu serwerów jednocześnie

Aby wykluczyć serwer MCP z regularnego wyboru czatu, ustaw [`chatMenu: false`](/docs/configuration/librechat_yaml/object_structure/mcp_servers#chatmenu) w swojej konfiguracji:

```yaml
mcpServers:
  internal-tools:
    command: npx
    args: ['-y', 'internal-mcp-server']
    chatMenu: false # Not selectable in regular chat
```

To ustawienie jest wymuszane zarówno w żądaniach czatu, jak i w selektorze, dzięki czemu nieaktualny zapisany wybór nie może nadal korzystać z ukrytego serwera. Nie usuwa ono serwera z zapisanych Agentów ani ze specyfikacji modelu (Model Specs), która przypisuje go w sposób jawny. LibreChat pomija również serwery, do których użytkownik może dotrzeć tylko za pośrednictwem autoryzowanego Agenta, w standardowym selektorze czatu i ścieżce żądania.

### Z Agentami [#with-agents]

Serwery MCP integrują się płynnie z agentami LibreChat:

1. Utwórz lub edytuj agenta
2. Otwórz **Add tools** w Agent Builder i wybierz **MCP**
3. Wybierz serwer MCP; każdy serwer pojawia się jako pojedynczy wpis w katalogu
4. Rozwiń wybrany serwer, aby włączyć lub wyłączyć poszczególne narzędzia
5. Zapisz swojego agenta

![Narzędzia MCP w Kreatorze Agentów](/images/mcp/mcp_ui_agent.png)

Ta organizacja wyższego poziomu sprawia, że interfejs pozostaje przejrzysty – nawet serwery z ponad 20 narzędziami (takie jak Spotify) są wyświetlane jako pojedyncze wpisy, które można rozwinąć w celu uzyskania szczegółowej kontroli.

### Podstawowa konfiguracja [#basic-configuration]

Dodaj serwery MCP do swojego pliku `librechat.yaml` ręcznie:

```yaml
mcpServers:
  # ClickHouse Cloud
  clickhouse-cloud:
    type: streamable-http
    url: https://mcp.clickhouse.cloud/mcp

  # File system access
  filesystem:
    command: npx
    args:
      - -y
      - '@modelcontextprotocol/server-filesystem'
      - /path/to/your/documents

  # Web browser automation
  puppeteer:
    command: npx
    args:
      - -y
      - '@modelcontextprotocol/server-puppeteer'

  # Production-ready cloud service
  business-api:
    type: streamable-http
    url: https://api.yourbusiness.com/mcp
    headers:
      X-User-ID: '{{LIBRECHAT_USER_ID}}'
      Authorization: 'Bearer ${API_TOKEN}'
    timeout: 30000
    serverInstructions: true
```

### Dodawanie serwerów MCP w interfejsie użytkownika [#adding-mcp-servers-in-the-ui]

Możesz również dodawać i konfigurować serwery MCP bezpośrednio z interfejsu LibreChat bez edytowania jakichkolwiek plików konfiguracyjnych czy restartowania serwera.

#### Krok 1: Otwórz panel ustawień MCP [#step-1-open-the-mcp-settings-panel]

Przejdź do panelu **MCP Settings** z prawego paska bocznego. Zobaczysz tutaj listę istniejących serwerów MCP oraz przycisk **+**, służący do dodawania nowych.

![Panel ustawień MCP](/images/mcp/mcp_ui_settings_panel.png)

#### Krok 2: Wypełnij szczegóły serwera [#step-2-fill-out-server-details]

Naciśnij przycisk **+** i wypełnij nazwę serwera MCP, opis, URL, typ transportu oraz metodę uwierzytelniania, a następnie kliknij **Create**.

![Okno dialogowe dodawania serwera MCP](/images/mcp/mcp_ui_configure.png)

Twój nowy serwer pojawi się w panelu MCP Settings wraz z powiadomieniem potwierdzającym.

![MCP Server Created Successfully](/images/mcp/mcp_ui_success_registered.png)

#### Krok 3: Sprawdź status połączenia i uwierzytelnij się [#step-3-check-connection-status-and-authenticate]

Sprawdź [wskaźnik stanu połączenia](#connection-status-indicators) dla swojego nowego serwera. Jeśli serwer wymaga uwierzytelnienia OAuth, stan będzie wyświetlany jako rozłączony. Kliknij przycisk uwierzytelniania/połączenia serwera (możesz to zrobić, klikając sam serwer MCP w menu rozwijanym czatu lub klikając najpierw ikonę połączenia, aby przejść do okna dialogowego z większą ilością informacji o stanie połączenia), aby rozpocząć proces uwierzytelniania.

![Stan połączenia - Rozłączono](/images/mcp/mcp_ui_unconnected.png)

Po zainicjowaniu wskaźnik stanu zaktualizuje się, aby pokazać, że uwierzytelnianie jest w toku.

![Stan połączenia - Uwierzytelnianie](/images/mcp/mcp_ui_authenticating.png)

#### Krok 4: Kontynuuj w karcie OAuth [#step-4-continue-in-the-oauth-tab]

Otworzy się nowa karta przeglądarki dla dostawcy OAuth. Zweryfikuj adres URL wywołania zwrotnego (callback URL) i kliknij **Continue**, aby autoryzować LibreChat.

![OAuth Continue Prompt](/images/mcp/mcp_ui_callback_confirm.png)

#### Krok 5: Uwierzytelnianie zakończone sukcesem [#step-5-authentication-successful]

Po uwierzytelnieniu zobaczysz potwierdzenie sukcesu. To okno zamknie się automatycznie i przekieruje Cię z powrotem do LibreChat.

![Uwierzytelnianie zakończone sukcesem](/images/mcp/mcp_ui_success_redirect.png)

#### Krok 6: Serwer gotowy do użycia [#step-6-server-ready-for-use]

LibreChat potwierdza pomyślne uwierzytelnienie i automatycznie wybiera serwer MCP do użycia w ramach Twojej konwersacji. Serwer wyświetla teraz wskaźnik stanu połączenia i jest zaznaczony na liście rozwijanej MCP Servers.

![MCP Server Authenticated and Auto-Selected](/images/mcp/mcp_ui_done.png)

Twój nowy serwer MCP jest również dostępny w Agent Builder, gdzie możesz dodać jego narzędzia do dowolnego agenta i dostosować, który podzbiór narzędzi jest dozwolony.

![MCP Server Available in Agent Builder](/images/mcp/mcp_ui_agent.png)

#### Zmienne poświadczeń dla serwerów utworzonych w UI [#credential-variables-for-ui-created-servers]

Podczas dodawania serwera MCP przez interfejs użytkownika, możesz wymagać od użytkowników podania własnych kluczy API. W sekcji Authentication okna dialogowego MCP Server Builder wybierz "API Key" i zaznacz **"User provides key"**. Wybierz format nagłówka (Bearer, Basic lub Custom) i zapisz serwer.

W tle LibreChat automatycznie tworzy wpis `customUserVars` o nazwie `MCP_API_KEY` i konfiguruje odpowiedni szablon nagłówka (np. `Authorization: Bearer {{MCP_API_KEY}}`). Każdy użytkownik podaje własny klucz za pośrednictwem okna dialogowego wyboru narzędzi MCP (MCP Tool Select Dialog) podczas konfigurowania agenta — tego samego interfejsu, który jest używany w przypadku [zdefiniowanych w YAML `customUserVars`](#user-provided-credentials).

Gdy skonfigurowany jest klucz API administratora lub użytkownika, LibreChat traktuje serwer jako uwierzytelniany za pomocą klucza API i pomija automatyczne wykrywanie OAuth. Jawna konfiguracja OAuth nadal ma pierwszeństwo. Jeśli serwer z kluczem API utworzonym w interfejsie użytkownika został zapisany przez starszą wersję LibreChat i pozostaje w stanie **OAuth Required**, należy edytować i zapisać serwer ponownie, aby odświeżyć przechowywane metadane połączenia.

<Callout type="warning">
Ze względów bezpieczeństwa serwery MCP utworzone przez interfejs użytkownika (pochodzące z bazy danych) mogą **tylko** rozpoznawać symbole zastępcze `customUserVars` (`{{VAR_NAME}}`). Zmienne środowiskowe po stronie serwera (`${ENV_VAR}`), pola profilu użytkownika (`{{LIBRECHAT_USER_*}}`) oraz tokeny OIDC (`{{LIBRECHAT_OPENID_*}}`) są celowo blokowane, aby zapobiec nieautoryzowanemu dostępowi do sekretów serwera lub danych innych użytkowników. Aby uzyskać pełną obsługę symboli zastępczych, skonfiguruj serwer w pliku `librechat.yaml`.
</Callout>

### Dodawanie serwerów MCP za pomocą Smithery [#adding-mcp-servers-with-smithery]

Smithery.ai zapewnia usprawniony sposób odkrywania i instalowania serwerów MCP dla LibreChat. Wykonaj poniższe kroki, aby rozpocząć:

#### Krok 1: Wyszukaj serwery MCP [#step-1-search-for-mcp-servers]

Odwiedź [smithery.ai](https://smithery.ai) i wyszukaj serwer MCP, który chcesz dodać do swojej instancji LibreChat.

![Interfejs wyszukiwania Smithery](/images/mcp/mcp_smithery_search.png)

#### Krok 2: Wybierz swój serwer MCP [#step-2-select-your-mcp-server]

Kliknij serwer MCP w wynikach wyszukiwania, aby wyświetlić szczegóły i dostępne narzędzia.

![Strona szczegółów serwera MCP](/images/mcp/mcp_smithery_mcp_server.png)

#### Krok 3: Konfiguracja dla LibreChat [#step-3-configure-for-librechat]

Przejdź do karty **Auto** w sekcji **Connect** i wybierz **LibreChat** jako żądanego klienta.

![Konfiguracja integracji LibreChat](/images/mcp/mcp_smithery_librechat.png)

#### Krok 4: Zainstaluj serwer MCP [#step-4-install-the-mcp-server]

Skopiuj i uruchom wygenerowane polecenie w swoim terminalu, aby zainstalować serwer MCP.

![Komenda instalacji](/images/mcp/mcp_smithery_copy.png)

#### Krok 5: Restart i weryfikacja [#step-5-restart-and-verify]

Twój serwer MCP jest teraz zainstalowany i można go skonfigurować w `librechat.yaml`. Zrestartuj LibreChat, aby zainicjować połączenia i zacząć korzystać z nowego serwera MCP.

![MCP Server Successfully Installed](/images/mcp/mcp_show_installed.png)
_Serwer MCP zainstalowany przez smithery.ai i gotowy do użycia w LibreChat_

Szczegółowe opcje konfiguracji i przykłady znajdują się w:

- [Przewodniki po serwerach MCP](/docs/mcp_servers)
- [Konfiguracja serwerów MCP](/docs/configuration/librechat_yaml/object_structure/mcp_servers)
- [Konfiguracja agenta](/docs/configuration/librechat_yaml/object_structure/agents)
- [Zaawansowane funkcje agentów](/docs/features/agents#model-context-protocol-mcp)
- [Agent Plugins](/docs/features/agent_plugins) do eksperymentalnego łączenia serwerów MCP wdrożenia ze Skills

## Zarządzanie serwerem MCP [#mcp-server-management]

LibreChat zapewnia kompleksowe narzędzia do zarządzania połączeniami z serwerami MCP, oferując śledzenie statusu połączenia oraz wsparcie dla uwierzytelniania i inicjalizacji OAuth w interfejsie użytkownika.

### Wskaźniki stanu połączenia [#connection-status-indicators]

LibreChat wyświetla dynamiczne ikony statusu pokazujące bieżący stan każdego serwera MCP na liście rozwijanej czatu oraz w panelu ustawień:

![Ikony statusu serwera MCP](/images/mcp/mcp_server_status_icons.png)

**Typy statusów:**

- **Połączono** (zielona ikona koła zębatego): Serwer jest połączony i posiada konfigurowalne customUserVars
- **Wymagane OAuth** (bursztynowy klucz): Serwer wymaga uwierzytelniania OAuth
- **Rozłączono** (pomarańczowa wtyczka): Połączenie z serwerem nie powiodło się lub zostało zerwane
- **Inicjalizacja** (niebieski pasek ładowania): Serwer uruchamia się lub łączy ponownie
- **Błąd** (czerwony trójkąt): Serwer napotkał błąd
- **Anulowanie** (czerwony x): proces OAuth jest anulowany

### Inicjalizacja serwera [#server-initialization]

Możesz zainicjować lub ponownie zainicjować serwery MCP bezpośrednio z poziomu interfejsu:

**Jedno kliknięcie:**

- Inicjalizacja jednym kliknięciem z menu rozwijanego wyboru serwera MCP

  <Video src="/videos/mcp_one_click_init.mp4" title="Inicjalizacja MCP jednym kliknięciem" />

**Z MCPConfigDialog:**

- Kliknij ikonę statusu obok serwera MCP w menu rozwijanym czatu, aby otworzyć MCPConfigDialog
- Skonfiguruj niestandardowe zmienne użytkownika i kliknij przycisk Authenticate/Initialize w zależności od typu uwierzytelniania serwera

       <Video src="/videos/mcp_config_dialog_auth.mp4" title="Uwierzytelnianie w oknie konfiguracji MCP" />

  **Z panelu ustawień MCP:**

- Kliknij dowolny serwer na liście serwerów w panelu ustawień MCP, aby uzyskać dostęp do kontroli konfiguracji i inicjalizacji.
- Skonfiguruj niestandardowe zmienne użytkownika i kliknij przycisk Authenticate/Initialize w zależności od typu uwierzytelniania serwera

  <Video src="/videos/mcp_settings_init.mp4" title="Inicjalizacja panelu ustawień MCP" />

Błędy inicjalizacji rozróżniają teraz nieosiągalny serwer, brakujące `customUserVars`, połączenie OAuth wymagające ponownego uwierzytelnienia oraz ogólny błąd inicjalizacji. Przed ponowieniem próby należy użyć nazwanych brakujących zmiennych lub akcji ponownego uwierzytelnienia wskazanych w komunikacie; w tych przypadkach serwer nie musi już być usuwany i dodawany ponownie.

Pasywne ładowanie katalogu scala próby wykrywania OAuth i stopniowo zwiększa czas oczekiwania po wystąpieniu błędów, dzięki czemu rozłączona integracja nie ponosi prób wykrywania przy każdym żądaniu. Zmiany poświadczeń są zabezpieczone generacyjnie między replikami, co zapobiega nadpisywaniu nowszych uwierzytelnień przez starsze wyniki wykrywania. Operatorzy mogą dostosować ograniczoną politykę odzyskiwania w sekcji [`mcpSettings.catalogRecovery`](/docs/configuration/librechat_yaml/object_structure/mcp_settings#catalogrecovery); wartości domyślne są odpowiednie dla większości wdrożeń.

Jeśli Agent wyraźnie oczekuje narzędzi MCP, ale żadne z nich nie może zostać rozpoznane na początku uruchomienia, LibreChat przerywa działanie, wyświetlając wskazówki dotyczące połączenia i dostępu, zamiast uruchamiać się po cichu bez tych narzędzi.

Serwery MCP, które wysyłają `notifications/tools/list_changed`, mogą aktualizować swój katalog narzędzi bez konieczności restartu LibreChat. LibreChat pobiera aktualną listę z aktywnego połączenia, publikuje dodane narzędzia i przestaje reklamować narzędzia usunięte przez serwer. Ponowne połączenia również synchronizują katalog, zanim połączenie zostanie uznane za gotowe. Zarządzane odświeżanie migawek i wykrywanie zimnych serwerów współdzielą jedną bramkę dla trzech operacji w skali całego procesu. Po dopuszczeniu, każda operacja pozostaje ograniczona limitem czasu listy narzędzi lub wykrywania dla każdego serwera, w tym wszelkimi krótszymi `initTimeout`, dzięki czemu powolna praca nad katalogiem nie może blokować slotu w nieskończoność.

W przypadku serwerów HTTP obsługujących strumieniowanie, ponowne połączenie może na krótko zwrócić kod HTTP `409`, gdy serwer nadal utrzymuje nieaktualny, samodzielny strumień SSE dla starej sesji. LibreChat traktuje to jako stan przejściowy, zamyka nieaktualną sesję i automatycznie odtwarza połączenie; jeśli odzyskiwanie zakończy się powodzeniem, interwencja operatora nie jest wymagana. Jeśli odtworzone połączenie również zakończy się niepowodzeniem, LibreChat będzie nadal wyświetlać podstawowy błąd połączenia.

LibreChat klasyfikuje odpowiedzi HTTP typu Streamable na podstawie parsowanego typu mediów `Content-Type`, a nie dopasowania podciągu. Parametry nie mogą sprawić, by odpowiedź niebędąca SSE wyglądała jak `text/event-stream`, więc błędy rozmiaru odpowiedzi są nadal obsługiwane przez właściwą ścieżkę obsługi SSE lub inną niż SSE.

### Widoczność panelu ustawień MCP [#mcp-settings-panel-visibility]

Panel ustawień MCP pojawia się w prawym pasku bocznym, gdy LibreChat wykryje serwery MCP, które mogą wymagać interwencji użytkownika podczas ich inicjalizacji. Panel będzie widoczny, gdy którykolwiek ze skonfigurowanych serwerów spełnia jedno z poniższych kryteriów:

- **Niestandardowe zmienne użytkownika**: Serwer posiada zdefiniowane `customUserVars`, które mogą zawierać dane uwierzytelniające dostarczone przez użytkownika
- **Uwierzytelnianie OAuth**: Podczas uruchamiania wykryto, że serwer wymaga uwierzytelniania OAuth
- **Ręczna inicjalizacja**: Serwer ma skonfigurowane `startup: false`, co wymaga ręcznej inicjalizacji

## Funkcje specyficzne dla LibreChat [#librechat-specific-features]

Implementacja MCP w LibreChat została zaprojektowana z myślą o wysoce konfigurowalnych, rzeczywistych środowiskach wieloużytkownikowych.

### Połączenia specyficzne dla użytkownika [#user-specific-connections]

- Każdy użytkownik otrzymuje własne, odizolowane połączenie z serwerami MCP
- Uwierzytelnianie użytkowników i uprawnienia są respektowane
- Dane osobowe i kontekst pozostają prywatne

### Udostępnianie serwerów MCP [#sharing-mcp-servers]

Serwery MCP uczestniczą w systemie [szczegółowej kontroli dostępu](/docs/features/access_control) LibreChat. Oprócz serwerów zdefiniowanych w `librechat.yaml` (które są zarządzane przez administratorów i podlegają uprawnieniom funkcji [`interface.mcpServers`](/docs/configuration/librechat_yaml/object_structure/interface#mcpservers)), utworzone przez użytkowników serwery MCP posiadają własne listy ACL i mogą być udostępniane konkretnym **użytkownikom**, **grupom**, **rolom** lub **publicznie**, na poziomie przeglądającego (Viewer), edytora (Editor) lub właściciela (Owner).

Flagi funkcji `USE`, `CREATE`, `SHARE` oraz `SHARE_PUBLIC` w sekcji `interface.mcpServers` kontrolują, kto w ogóle ma uprawnienia do tworzenia i udostępniania serwerów MCP. Zobacz [Access Control](/docs/features/access_control), aby dowiedzieć się, jak tworzone są warstwy uprawnień.

### Dynamiczny kontekst użytkownika [#dynamic-user-context]

Serwery MCP mogą uzyskiwać dostęp do informacji o użytkowniku za pomocą symboli zastępczych w **adresach URL i nagłówkach** (dla transportów SSE i Streamable HTTP):

```yaml
mcpServers:
  user-api:
    type: streamable-http
    url: https://api.example.com/users/{{LIBRECHAT_USER_USERNAME}}/mcp
    headers:
      X-User-ID: '{{LIBRECHAT_USER_ID}}'
      X-User-Email: '{{LIBRECHAT_USER_EMAIL}}'
      X-User-Role: '{{LIBRECHAT_USER_ROLE}}'
      Authorization: 'Bearer ${API_TOKEN}'
```

Dostępne symbole zastępcze to:

- `{{LIBRECHAT_USER_ID}}` - Unikalny identyfikator użytkownika
- `{{LIBRECHAT_USER_EMAIL}}` - Adres e-mail użytkownika
- `{{LIBRECHAT_USER_ROLE}}` - Rola użytkownika (admin, user itp.)
- `{{LIBRECHAT_USER_USERNAME}}` - Nazwa użytkownika
- I wiele innych (zobacz [Konfiguracja serwerów MCP](/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers), aby uzyskać pełną listę)

Serwery MCP zdefiniowane w YAML mogą również używać symboli zastępczych `{{LIBRECHAT_OPENID_*}}`, `{{LIBRECHAT_GRAPH_*}}` oraz `{{LIBRECHAT_BODY_*}}`. Wartości `{{LIBRECHAT_BODY_*}}` są ograniczone do zakresu żądania (request-scoped), więc LibreChat tworzy połączenia dla aktywnego uruchomienia, ponownie wykorzystuje je podczas wywołań narzędzi w ramach tego uruchomienia i czyści je po zakończeniu żądania. Serwery ograniczone do zakresu żądania są wykluczone z trwałej pamięci podręcznej narzędzi, dzięki czemu nagłówki i adresy URL specyficzne dla żądania nie są ponownie wykorzystywane poza aktywnym uruchomieniem. Ponieważ ich lista narzędzi nie może zostać rozwiązana przed zaistnieniem żądania czatu, Agent Builder udostępnia jeden wybór narzędzi uruchomieniowych dla całego serwera i rozwiązuje jego narzędzia w czasie wykonywania. Ukończenie OAuth przechowuje autoryzację, ale odracza połączenie do momentu, aż pierwsze uruchomienie dostarczy kontekst żądania. Symbole zastępcze User, OpenID i Graph są ograniczone do zakresu użytkownika (user-scoped); transporty HTTP odświeżają swoje rozwiązane nagłówki przed każdym wywołaniem narzędzia bez konieczności samodzielnego ponownego łączenia.

Zaufani operatorzy mogą użyć `Authorization: 'Bearer {{LIBRECHAT_OPENID_ACCESS_TOKEN}}'` na zdalnym serwerze zdefiniowanym w YAML, aby przekazać aktywny token dostępu OpenID użytkownika. Jawne uwierzytelnianie MCP OAuth, wymiana typu on-behalf-of oraz klucze API administratora mają pierwszeństwo. Po otrzymaniu błędu `401` lub `403`, LibreChat może jednorazowo odświeżyć połączenie, ale nigdy nie ponawia automatycznie odrzuconego wywołania narzędzia. Zobacz [dokumentację dotyczącą symboli zastępczych serwera MCP](/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers), aby uzyskać wskazówki dotyczące konfiguracji i bezpieczeństwa.

`{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` wymienia token dostępu OpenID użytkownika wielokrotnego użytku poprzez przepływ on-behalf-of firmy Microsoft. [`GRAPH_API_SCOPES`](/docs/configuration/dotenv#microsoft-graph-api--entra-id-integration) ustawia żądane zakresy rozdzielone spacjami, a jego domyślna wartość to `https://graph.microsoft.com/.default`. Jest to niezależne od `OPENID_GRAPH_SCOPES`, które konfiguruje wyszukiwanie osób i grup.

Symbol zastępczy `{{LIBRECHAT_BODY_PARENTMESSAGEID}}` jest wypełniany dla natywnych czatów Agentów, wywołań API Agentów zgodnych z OpenAI oraz starszych uruchomień Assistants. Open Responses nie posiada odpowiednika identyfikatora wiadomości nadrzędnej i kończy się niepowodzeniem, gdy serwer MCP wymaga tej wartości. Zobacz [dokumentację symboli zastępczych serwera MCP](/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers).

### Instrukcje serwera [#server-instructions]

`serverInstructions` to funkcja LibreChat, która dynamicznie dodaje skonfigurowane instrukcje, gdy wybrane zostanie dowolne narzędzie z tego serwera MCP:

```yaml
mcpServers:
  filesystem:
    command: npx
    args: ['-y', '@modelcontextprotocol/server-filesystem', '/docs']
    serverInstructions: |
      When accessing files:
      - Always check file permissions first
      - Use absolute paths for reliability
      - Handle errors gracefully
```

Opcje:

- `true`: Użyj instrukcji udostępnionych przez serwer, gdy są dostępne
- `false`: Wyłącz instrukcje
- `string`: Użyj instrukcji niestandardowych dosłownie (pokazanych powyżej)

LibreChat przechowuje tę skonfigurowaną deklarację oddzielnie od instrukcji pobieranych podczas inspekcji serwera. Deklaracja `true` pozostaje zatem aktywna podczas inspekcji i przeładowań pamięci podręcznej rejestru, podczas gdy niestandardowy ciąg znaków pozostaje nadrzędny.

### Konfiguracja limitu czasu (Timeout) [#timeout-configuration]

W przypadku długotrwałych operacji MCP należy skonfigurować odpowiednie limity czasu (timeouts) zarówno dla inicjalizacji, jak i operacji narzędzi.

```yaml
mcpServers:
  data-processor:
    type: streamable-http
    url: https://api.example.com/mcp
    initTimeout: 15000 # 15 seconds for server initialization
    timeout: 60000 # 60 seconds for tool operations
```

**Uwaga**: Jeśli operacje są nadal przerywane, sprawdź konfigurację swojego proxy (np. nginx, traefik itp.), które może przedwcześnie zrywać połączenia z powodu domyślnych limitów czasu (timeouts).

Gdy użytkownik naciśnie **Stop** podczas wywołania narzędzia Agent na pierwszym planie, LibreChat przekazuje sygnał anulowania uruchomienia do MCP. W przypadku serwerów HTTP obsługujących strumieniowanie, zestaw SDK MCP wysyła `notifications/cancelled`; serwer musi uwzględnić to powiadomienie, aby zatrzymać własną pracę. Odłączone wywołania w tle celowo zachowują oddzielny cykl życia i nie są anulowane wraz z turą na pierwszym planie.

### Dane uwierzytelniające dostarczone przez użytkownika [#user-provided-credentials]

Możesz zezwolić użytkownikom na podawanie własnych danych uwierzytelniających do serwerów MCP za pomocą `customUserVars`. Umożliwia to bezpieczne, spersonalizowane uwierzytelnianie bez konieczności przechowywania danych logowania w plikach konfiguracyjnych.

```yaml
mcpServers:
  my-api-server:
    type: streamable-http
    url: 'https://api.example.com/mcp'
    headers:
      X-Auth-Token: '{{MY_API_KEY}}' # Uses the user-provided value
    customUserVars:
      MY_API_KEY:
        title: 'API Key'
        description: "Enter your personal API key from <a href='https://example.com/keys' target='_blank'>your account settings</a>"
```

Użytkownicy mogą skonfigurować te dane uwierzytelniające:

- **Z obszaru czatu**: Kliknij ikonę ustawień obok konfigurowalnych serwerów MCP na liście rozwijanej wyboru narzędzi
- **Z panelu ustawień MCP**: Uzyskaj dostęp do „MCP Settings” w prawym panelu, aby zarządzać poświadczeniami dla wszystkich skonfigurowanych serwerów

#### Ponowna inicjalizacja serwerów MCP z poświadczeniami użytkownika [#reinitializing-mcp-servers-with-user-credentials]

W przypadku serwerów MCP, które wymagają poświadczeń specyficznych dla użytkownika, zanim będzie można z nich korzystać (np. `PAT_TOKEN` w [oficjalnym serwerze MCP GitHub](https://github.com/github/github-mcp-server)), LibreChat umożliwia użytkownikom podanie tych poświadczeń, a następnie ponowne zainicjowanie serwera MCP z poziomu interfejsu użytkownika bez konieczności restartowania całej aplikacji:

1. Gdy wybierzesz MCP, który korzysta z `customUserVars`, będziesz mógł **Zapisać** lub **Cofnąć** wartość `customUserVar` dla wybranego serwera MCP z poziomu Panelu MCP.
2. Po zapisaniu wartości dla `customUserVar`, kliknij przycisk ponownej inicjalizacji (ikonę z kolistymi strzałkami obok każdej nazwy serwera w panelu MCP).
3. LibreChat spróbuje połączyć się z serwerem przy użyciu podanych przez Ciebie danych uwierzytelniających i powiadomi Cię za pomocą komunikatu typu toast, czy proces ponownej inicjalizacji zakończył się sukcesem, czy niepowodzeniem.

> _Wskazówka: Jeśli wiesz, że serwer będzie wymagał poświadczeń niedostępnych przy pierwszym uruchomieniu, możesz dodać `startup: false` do jego konfiguracji. Informuje to LibreChat, aby nie próbował łączyć się z tym serwerem, dopóki nie zostanie on ręcznie zainicjowany ponownie w interfejsie użytkownika._

**Przykład:**

```yaml
mcpServers:
  github-mcp:
    type: streamable-http
    url: 'https://api.githubcopilot.com/mcp/'
    headers:
      Authorization: '{{PAT_TOKEN}}'
    customUserVars:
      PAT_TOKEN:
        title: 'GitHub PAT Token'
        description: 'GitHub Personal Access Token'
    startup: false
```

### Uwierzytelnianie OAuth [#oauth-authentication]

LibreChat obsługuje uwierzytelnianie OAuth dla serwerów MCP, zgodnie z zaleceniami Anthropic dotyczącymi bezpiecznych połączeń MCP. OAuth zapewnia ustandaryzowany, bezpieczny sposób uwierzytelniania bez konieczności przechowywania długoterminowych danych uwierzytelniających.

#### Obsługiwane przepływy OAuth [#supported-oauth-flows]

Serwery MCP LibreChat obsługują OAuth 2.0 z:

- **Authorization Code Flow with PKCE**: Zalecany dla zapewnienia maksymalnego bezpieczeństwa
- **Wykrywanie klienta**: Automatyczna rejestracja klienta, gdy jest obsługiwana przez dostawcę OAuth
- **Refresh Tokens**: Automatyczne odnawianie tokenów, gdy jest dostępne
- **Wykrywanie endpointu tokenów**: Wstępnie skonfigurowani klienci mogą wykryć, czy dostawca oczekuje danych uwierzytelniających klienta w treści POST, czy w nagłówku autoryzacji HTTP Basic

W kreatorze MCP, **Token Exchange Method** oferuje opcje **Auto**, **Default (POST request)** oraz **Basic authorization header**. Opcja Auto pozostawia metodę nieustawioną, dzięki czemu LibreChat może korzystać z zaufanych metadanych dostawcy, przełączając się na uwierzytelnianie Basic, gdy dostawca nie deklaruje żadnej metody.

#### Przykłady konfiguracji [#configuration-examples]

```yaml
mcpServers:
  # Public remote MCP server for PayPal, uses OAuth Client Discovery
  # ❌ Refresh Tokens: you may need to re-authenticate periodically
  # More info: https://developer.paypal.com/tools/mcp-server/
  paypal:
    type: 'sse'
    initTimeout: 150000 # higher timeout to allow for initial authentication
    url: 'https://mcp.paypal.com/sse'

  # Example self-hosted remote MCP server for Spotify, uses OAuth Client Discovery
  # ✅ Refresh Tokens: refreshes token for authentication automatically
  # Hosted on Cloudflare Workers, more info: https://github.com/LibreChat-AI/spotify-mcp
  spotify:
    type: 'streamable-http'
    initTimeout: 150000
    url: 'https://mcp-spotify-oauth-example.account.workers.dev/mcp'
```

#### Przepływ uwierzytelniania OAuth [#oauth-authentication-flow]

Gdy po raz pierwszy konfigurujesz serwer MCP z włączonym OAuth:

1. **Początkowe połączenie**: LibreChat próbuje połączyć się z serwerem MCP
2. **Wymagane uwierzytelnienie**: Jeśli nie istnieje żaden ważny token, w menu rozwijanym czatu dla danego serwera zobaczysz wskaźnik uwierzytelnienia OAuth.
3. **Interfejs przycisku**: Kliknij wskaźnik uwierzytelniania, aby otworzyć okno dialogowe konfiguracji MCP i rozpocząć proces OAuth
4. **Kontynuuj lub użyj innego urządzenia**: Kontynuuj w bieżącej przeglądarce, skopiuj link autoryzacyjny lub wyświetl kod QR, aby otworzyć go na innym urządzeniu
5. **Przekierowanie przeglądarki**: Dokończ uwierzytelnianie u dostawcy OAuth
6. **Obsługa powrotu**: LibreChat automatycznie przetwarza callback OAuth po uwierzytelnieniu się.
7. **Przechowywanie tokenów**: LibreChat bezpiecznie przechowuje tokeny do późniejszego wykorzystania
8. **Połączenie nawiązane**: Po uwierzytelnieniu serwer MCP zostanie połączony i będzie można z niego korzystać w czacie

#### Adres URL wywołania zwrotnego OAuth [#oauth-callback-url-1]

Gdy serwer MCP korzysta z OAuth, LibreChat udostępnia endpoint wywołania zwrotnego (callback), na który dostawca OAuth przekierowuje użytkownika po pomyślnej autoryzacji.

Adres URL wywołania zwrotnego (callback URL) musi być zgodny z tym formatem:

`${baseUrl}/api/mcp/${serverName}/oauth/callback`

Gdzie `${serverName}` to klucz serwera MCP zdefiniowany w Twojej konfiguracji `librechat.yaml`. LibreChat obsługuje przekierowanie pod tym endpoint, finalizuje wymianę tokenów i przypisuje poświadczenia do odpowiedniego serwera MCP.

<Callout type="info" title="Przykład adresu URL wywołania zwrotnego OAuth">
Biorąc pod uwagę poniższą konfigurację serwera MCP:

```yaml
mcpServers:
  # Example self-hosted remote MCP server for Spotify, uses OAuth Client Discovery
  # ✅ Refresh Tokens: refreshes token for authentication automatically
  # Hosted on Cloudflare Workers, more info: https://github.com/LibreChat-AI/spotify-mcp
  spotify:
    type: 'streamable-http'
    initTimeout: 150000
    url: 'https://mcp-spotify-oauth-example.account.workers.dev/mcp'
```

Adres URL wywołania zwrotnego (callback URL) to `${baseUrl}/api/mcp/spotify/oauth/callback`.

</Callout>

Uwaga:

- Adres URL wywołania zwrotnego (callback URL) musi być dokładnie zarejestrowany u dostawcy OAuth, aby przepływ działał poprawnie.
- Inne ścieżki, takie jak `/api/oauth/callback` lub `/api/oauth/openid/callback`, nie są poprawne dla przepływów OAuth MCP.

#### Zarządzanie tokenami [#token-management]

LibreChat inteligentnie zarządza tokenami OAuth:

- **Bezpieczne przechowywanie**: Tokeny są szyfrowane i przechowywane w bezpieczny sposób
- **Automatyczne odświeżanie**: Gdy dostępne są tokeny odświeżania (refresh tokens), LibreChat automatycznie odnawia wygasłe tokeny dostępu.
- **Cicha obsługa błędu 401**: Jeśli połączenie OAuth MCP otrzyma w trakcie sesji odmowę uwierzytelnienia, LibreChat koordynuje jedną ograniczoną próbę odświeżenia i ponownego połączenia na użytkownika i serwer. Współbieżni wywołujący współdzielą wynik tej próby odzyskiwania, zanim LibreChat wyświetli nowy monit o uwierzytelnienie.
- **Zarządzanie sesjami**: Każdy użytkownik utrzymuje własne sesje OAuth w środowiskach wieloużytkownikowych

Jeśli zapisany strumień Agenta zostanie przeładowany lub wznowiony, podczas gdy monit o autoryzację MCP jest nadal oczekujący, LibreChat odtworzy ten monit ze stanu trwałej generacji. Zakończone, anulowane, nieudane lub wygasłe monity nie są przywracane.

Zapisani klienci i tokeny OAuth są powiązani z adresem URL serwera MCP oraz ustawieniami klienta/endpointu OAuth. Zmiana tych ustawień wymaga od użytkowników ponownego uwierzytelnienia. Podczas edycji serwera utworzonego w interfejsie użytkownika, który przechowuje zapisany `client_secret`, należy ponownie wprowadzić sekret w przypadku zmiany pola powiązania, takiego jak adres URL serwera, identyfikator klienta (client ID), adres URL autoryzacji, adres URL tokena lub metoda uwierzytelniania tokena.

W wdrożeniach skalowanych horyzontalnie status przepływu OAuth oraz gotowość powiązanych tokenów mogą być uzgadniane pomiędzy podami LibreChat. Interfejs użytkownika zgłasza serwer jako gotowy dopiero po pomyślnej inicjalizacji po zakończeniu OAuth, nawet jeśli wywołanie zwrotne (callback) i odpytywanie o status trafią do różnych podów. Wywołania zwrotne z zastąpionych prób są odrzucane.

Gdy Redis jest włączony, generowanie katalogów MCP na poziomie aplikacji oraz poprawki publikacji są koordynowane między replikami. Wdrożenia Redis Cluster wykorzystują klucze katalogów i aktualizacje bezpieczne dla slotów, dzięki czemu synchronizacja przy uruchomieniu oraz odświeżanie `tools/list_changed` nie wymagają układu Redis z pojedynczym węzłem.

Każdy użytkownik zostanie poproszony o uwierzytelnienie za pomocą własnego loginu OAuth przy pierwszym użyciu serwera MCP z włączoną obsługą OAuth. Zapewnia to, że szczegóły połączenia i uwierzytelniania są unikalne dla każdego użytkownika, co pozwala zachować bezpieczeństwo i prywatność w środowiskach wieloużytkownikowych.

#### Czas OAuth [#oauth-timing]

Ukończenie MCP OAuth w LibreChat wykorzystuje własny limit czasu skonfigurowany na serwerze, zamiast używać `initTimeout` serwera MCP. Domyślnie LibreChat czeka do 10 minut na ukończenie procesu MCP OAuth przez użytkownika i przechowuje stan przepływu przez 15 minut.

Użyj tych zmiennych środowiskowych, gdy dostawca OAuth lub przepływ pracy użytkownika wymaga więcej czasu:

```bash filename=".env"
MCP_OAUTH_HANDLING_TIMEOUT=600000
MCP_OAUTH_FLOW_TTL=900000
```

`MCP_OAUTH_FLOW_TTL` jest ograniczony tak, aby trwał dłużej niż `MCP_OAUTH_HANDLING_TIMEOUT`, dzięki czemu wywołania zwrotne (callbacks) bliskie upływu terminu nadal mogą odnaleźć swój stan przepływu. Okno odpytywania karty serwera MCP jest zgodne ze skonfigurowanym limitem czasu obsługi (handling timeout).

![Przepływ uwierzytelniania OAuth specyficzny dla użytkownika](/images/agents/mcp_oauth_flow.png)

> Uwaga: Tokeny wyświetlane podczas uruchamiania aplikacji służą wyłącznie do inicjalizacji na poziomie aplikacji i nie są używane do indywidualnych połączeń użytkowników.

Przykład automatycznego odświeżania tokenów:

```bash
[MCP][spotify] Access token missing
[MCP][spotify] Attempting to refresh token
[MCP][spotify] Successfully refreshed and stored OAuth tokens
[MCP][spotify] ✓ Initialized
```

#### Najlepsze praktyki [#best-practices]

1. **Używaj OAuth, gdy jest dostępny**: Preferuj OAuth zamiast kluczy API dla lepszego bezpieczeństwa
2. **Skonfiguruj odpowiednie limity czasu**: Użyj `MCP_OAUTH_HANDLING_TIMEOUT` oraz `MCP_OAUTH_FLOW_TTL` dla okien zakończenia OAuth; użyj `initTimeout` dla inicjalizacji serwera
3. **Monitorowanie wygaśnięcia tokenów**: Sprawdź logi pod kątem problemów z uwierzytelnianiem
4. **Plan ponownego uwierzytelniania**: Niektórzy dostawcy nie obsługują tokenów odświeżania

## Server Transports [#server-transports]

Serwery MCP można skonfigurować tak, aby korzystały z różnych mechanizmów transportowych:

**Serwery STDIO**

- Dobrze sprawdzają się w lokalnych środowiskach dla jednego użytkownika
- Nie skalowalne w przypadku wdrożeń zdalnych lub w chmurze
- Dołączony pakiet MCP SDK ogranicza każdą pojedynczą wiadomość STDIO do 10 MB. Większy pojedynczy wynik powoduje zamknięcie transportu z błędem; limit ten jest niezależny od całkowitego rozmiaru sesji lub strumienia.

**Serwery Server-Sent Events (SSE)**

- Mechanizm zdalnego transportu, ale niezalecany w środowisku produkcyjnym

**Strumieniowalne serwery HTTP**

- Używa HTTP POST do wysyłania wiadomości i obsługuje strumieniowanie odpowiedzi
- Działa jako niezależny proces, który może obsługiwać wiele połączeń klienckich
- Obsługuje zarówno podstawowe żądania, jak i przesyłanie strumieniowe za pomocą Server-Sent Events (SSE)
- Wydajniejsza alternatywa dla starszego transportu HTTP+SSE
- Obsługuje odpowiednie konfiguracje serwerów wieloużytkownikowych

**W środowiskach produkcyjnych** zalecane są wyłącznie serwery MCP z [transportem "Streamable HTTP"](https://modelcontextprotocol.io/specification/draft/basic/transports/streamable-http). W przeciwieństwie do SSE, które utrzymuje długotrwałe połączenia, Streamable HTTP oferuje opcje bezstanowe, które lepiej sprawdzają się w skalowalnych wdrożeniach wieloużytkownikowych.

LibreChat znajduje się w czołówce wdrażania elastycznych i skalowalnych integracji serwerów MCP, aby wspierać różnorodne scenariusze użycia i pomagać w budowaniu przepływów pracy AI jutra.

---

**Gotowy na rozszerzenie swoich możliwości AI?** Zacznij od skonfigurowania swojego pierwszego serwera MCP i odkryj, jak LibreChat może połączyć się z praktycznie każdym narzędziem lub usługą, której potrzebuje Twoja organizacja.
