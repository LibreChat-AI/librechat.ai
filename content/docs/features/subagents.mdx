...
- **Share files with subagents**: shares current-message attachments with authorized child runs and lets children publish generated files. This is disabled by default and appears only when an administrator has enabled [file sharing](#share-files-with-subagents).
...
## Share Files with Subagents

File sharing lets children read attachments from the current message and return generated files to the parent. It is disabled by default. An administrator enables it in `librechat.yaml`:
...
Restart LibreChat and refresh the browser, then turn on **Share files with subagents** in the parent Agent's **Subagents** settings and save. The toggle appears only when the deployment setting and the parent's Subagents toggle are enabled. With Docker, edit the configuration loaded by the container, normally mounted at `/app/librechat.yaml`.

Attach a file to a message to the parent and ask it to delegate the work. Give the child the tools needed for the task, such as Code Interpreter for generating a CSV. For example:

> Ask the analyst subagent to read the attached report, check the quarterly totals, and publish a CSV of the results before returning. Then summarize the findings for me.

Participating Agents receive `list_run_files` to discover authorized files and private output versions, and `publish_artifact` to publish a selected version. A child can inspect and refine files across multiple code calls before publishing. Its outputs stay private until it publishes them; the parent cannot publish a child's private output for it.

Published files appear in the conversation and Files panel for download or attachment to another message. Later sandbox changes do not alter the published version. Sharing with siblings requires the administrator to enable `endpoints.agents.fileSharing.allowSiblingSharing` and the producing Agent to name authorized recipients when publishing.

- Only current-message attachments are shared automatically. Earlier conversation attachments and another Agent's setup files are excluded.
- Shared-file runs use foreground chat delegation and managed Code Interpreter environments when code tools are needed. Detached child threads, background code execution, and attached workstation environments are not supported.
- Saved teams require matching provider, endpoint, model, Responses API, and image-detail settings across members.
- Unpublished outputs are cleared on completion, cancellation, expiry, or a pause for approval and must be regenerated after resume. Published files follow the normal retention and deletion rules.
...
Child agents run with isolated context. Parent tool-search state and accumulated context are not copied into the child run. Foreground child model usage is included in the parent transaction and usage totals; detached child calls that finish after the parent request closes record usage independently. The compact child card opens a unified activity side panel for foreground, graph, legacy, and detached runs, showing bounded lifecycle, reasoning, tool, approval, and result details appropriate to that transport. A named Agent's configured name and avatar remain on its card and panel after streaming completes and after reload; older records without saved identity use the generic fallback.
