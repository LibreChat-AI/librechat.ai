...
When using an agent, you can pass arbitrary key-value pairs to dynamically customize its instructions at runtime. Any query parameter prefixed with `custom_` is treated as a custom variable:

```bash
https://your-domain.com/c/new?agent_id=your-agent-id&custom_region=your-region&custom_department=your-department
```

These variables are injected into the agent's instructions using `{{variable_name}}` placeholders (without the `custom_` prefix). For example, if your agent's instructions contain:

```
You are assisting {{department}} in the {{region}} region.
```

The values passed via the URL will be substituted at runtime:

```
You are assisting your-department in the your-region region.
```

**Notes:**
- Variables are persisted on the conversation, so they remain available throughout multi-turn exchanges without needing to re-pass them in the URL.
