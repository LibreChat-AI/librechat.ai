...
对于高 Token 速率的部署，`STREAM_DELTA_COALESCE_MS=25` 可以批量处理 Redis 增量发布并减少 Redis 的工作负载，代价是最多增加 25 毫秒的交付延迟。若要禁用批量处理，请将其留空或设置为 `0`；仅在所有副本都支持批量帧（batch frames）后才启用此功能。请参阅 [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing)。
