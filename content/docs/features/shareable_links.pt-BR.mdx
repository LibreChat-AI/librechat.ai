...
Duas variáveis de ambiente no seu arquivo `.env` controlam o recurso:
...
`sharedLinks.public` controla se um usuário pode alternar a opção "compartilhar com todos". `ALLOW_SHARED_LINKS_PUBLIC` ainda decide se esses links públicos são visualizáveis sem fazer login.
