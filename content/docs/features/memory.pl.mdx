...
icon: Bookmark
description: >-
  Ustrukturyzowana pamięć użytkownika z ręcznym sterowaniem, opcjonalną
  automatyczną ekstrakcją i partycjami zarządzanymi przez agenta
---

## Przegląd [#overview]

User Memory w LibreChat to **magazyn klucz-wartość**, który przechowuje informacje specyficzne dla użytkownika pomiędzy różnymi konwersacjami. Użytkownicy mogą zarządzać wpisami bezpośrednio, opcjonalny agent pamięci może wyodrębniać aktualizacje z czatów, a Agenci mogą korzystać z narzędzi pamięci, gdy zostaną wyraźnie poproszeni o zapamiętanie lub zapomnienie czegoś.
...
- **Opcjonalna automatyczna ekstrakcja**: Ustaw `memory.agent.enabled: true`, aby uruchomić skonfigurowanego agenta pamięci wraz z żądaniami czatu
...
- **Narzędzia agenta (Agent Tools)**: Agenci mogą zapisywać lub usuwać wspomnienia, gdy użytkownik wyraźnie o to poprosi
- **Partycje agentów**: Utrzymuj pamięć agenta oddzielnie od współdzielonej puli osobistej użytkownika
...
Gdy `memory.agent.enabled: true`, skonfigurowany agent pamięci działa wraz z żądaniami czatu. Wykonuje się on współbieżnie z główną odpowiedzią czatu — rozpoczyna się przed rozpoczęciem głównej odpowiedzi i jest ograniczony do czasu trwania głównego żądania oraz maksymalnie 3 sekund po jego zakończeniu.
...
Punkt końcowy Agents zawiera funkcję `memory`. Gdy pamięć jest skonfigurowana, użytkownik posiada uprawnienia do zapisu w pamięci, a personalizacja jest włączona, dodanie **Memory** do agenta zapewnia mu narzędzia `set_memory` oraz `delete_memory`. Agent jest instruowany, aby używał tych narzędzi tylko wtedy, gdy użytkownik wyraźnie poprosi go o zapamiętanie, zaktualizowanie lub zapomnienie czegoś.

Agenci domyślnie korzystają ze współdzielonej puli pamięci osobistej użytkownika. W ustawieniach pamięci w kreatorze Agent Builder włącz opcję **Keep memories separate for this agent**, aby odizolować przechowywanie danych według użytkownika i agenta. Odizolowany agent nie widzi istniejących pamięci osobistych ani pamięci należących do innych odizolowanych agentów. Wpisy przypisane do agenta pojawiają się z nazwą agenta w panelu Memory i mogą być filtrowane według partycji.

Partycje agentów są powiązane z dostępem do agentów. Odczyt, tworzenie lub aktualizacja partycji wymaga od użytkownika posiadania uprawnień dostępu do agentów oraz bieżącego dostępu do podglądu wskazanego agenta; menedżerowie agentów zachowują swój standardowy, szerszy zakres dostępu. LibreChat odrzuca brakujące lub niedostępne identyfikatory agentów, zamiast traktować je jako dowolne przestrzenie nazw pamięci. Usuwanie może nadal dotyczyć partycji, której agent został usunięty, dzięki czemu można czyścić osierocone pamięci.

Automatyczny agent pamięci oraz wbudowane narzędzia pamięci Agenta są od siebie niezależne. `memory.agent.enabled: true` steruje automatyczną ekstrakcją, podczas gdy funkcja `memory` punktu końcowego Agents kontroluje, czy zapisani Agenci mogą zarządzać wspomnieniami podczas konwersacji.
