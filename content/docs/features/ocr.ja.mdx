...
title: ドキュメント用OCR
icon: ScanText
description: LibreChatのファイルアップロード機能におけるテキスト抽出を強化するための、光学文字認識（OCR）の設定方法を学びます。
---
...
OCR接続は、デフォルトでプライベート、ループバック、リンクローカル、およびクラウドメタデータの宛先をブロックします。信頼できるプライベートOCRサービスを使用する場合は、その正確なプライベートホストとポートを [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses) に追加してください。これはSSRF保護の例外となるため、自身が管理するインフラストラクチャのみをリストに含めてください。
...
LibreChatは、さまざまなデプロイメントのニーズや要件を満たすために、複数のOCR戦略をサポートしています。インフラストラクチャとコンプライアンスの要件に最も適した戦略を選択してください。
...
サポートされているLLMプロバイダー（**OpenAI, AzureOpenAI, Anthropic, Google, および AWS Bedrock**）とその各モデルにおいて、ファイルをメッセージの添付ファイルとしてプロバイダーのAPIに直接送信できるようになりました。これにより、ファイル添付ドロップダウンメニューの `Upload to Provider` オプションを使用して、プロバイダー独自のネイティブOCR実装によるファイルの解析が可能になります。

現在、前述の5つのプロバイダーすべてが画像とPDFのサポートを提供しており、Googleは互換性のあるマルチモーダルモデルと併用する場合に限り、音声および動画ファイルもサポートしています。AWS Bedrockはさらに、CSV、DOC、DOCX、XLS、XLSX、HTML、TXT、およびMarkdownドキュメントをサポートしています。
...
注: 標準の OpenAI endpoint は、Chat Completions API と Responses API の両方で PDF アップロードをサポートしています。
</Callout>
...
Bedrockの設定に関する詳細は、[AWS Bedrock setup guide](/docs/configuration/pre_configured_ai/bedrock) を参照してください。
</Callout>
...
- エージェントのファイルコンテキストにアップロードされました（OCRが設定されている場合は常に適用されます）
...
- テキスト抽出の精度は、ファイルの種類、画質、ドキュメントの複雑さ、およびテキストの鮮明度によって異なる場合があります。
...
- LibreChatは現在、OCRプロセスで解析された画像コンテンツをレスポンスに含めていません。これは、[Mistral's OCR API](https://docs.mistral.ai/api/endpoint/ocr)のようなサービスが結果としてそれらを提供する場合であっても同様です。この機能は将来のアップデートでサポートされる可能性があります。
