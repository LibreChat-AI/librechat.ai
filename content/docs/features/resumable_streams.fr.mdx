...
Pour les déploiements à haut débit de jetons, `STREAM_DELTA_COALESCE_MS=25` peut regrouper les publications delta Redis et réduire la charge de travail de Redis au prix d'une latence de livraison pouvant atteindre 25 ms. Laissez cette valeur non définie ou réglez-la sur `0` pour désactiver le regroupement, et ne l'activez qu'une fois que chaque réplica prend en charge les trames de regroupement. Voir [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
