...
OCR 连接默认会阻止私有地址、回环地址、链路本地地址以及云元数据目的地。对于受信任的私有 OCR 服务，请将其确切的私有主机和端口添加到 [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses) 中。这是对 SSRF 保护的豁免，因此请仅列出您所控制的基础设施。
