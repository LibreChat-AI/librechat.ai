...
目前上述所有五个提供商均支持图像和 PDF，当与兼容的多模态模型结合使用时，Google 还支持音频和视频文件。AWS Bedrock 额外支持 CSV、DOC、DOCX、XLS、XLSX、HTML、TXT 和 Markdown 文档。
...
约束：
- 每个文档的最大文件大小：**4.5 MB**
- 文件名会经过清理以符合 Bedrock 的命名要求（仅限字母数字、空格、连字符、圆括号、方括号；最大 200 个字符）
...
有关聊天中临时文档的问题，请参阅 [Upload as Text](/docs/features/upload_as_text)。
...
- LibreChat 目前不会在响应中包含来自 OCR 处理过程的解析图像内容，尽管诸如 [Mistral's OCR API may provide](https://docs.mistral.ai/api/#tag/ocr) 之类的服务可能会在结果中提供这些内容。此功能可能会在未来的更新中得到支持。
