...
Per distribuzioni con un elevato tasso di token, `STREAM_DELTA_COALESCE_MS=25` può raggruppare le pubblicazioni delta di Redis e ridurre il carico di lavoro di Redis a costo di un massimo di 25 ms di latenza nella consegna. Lascialo non impostato o impostalo su `0` per disabilitare il raggruppamento e abilitalo solo dopo che ogni replica supporta i frame di raggruppamento. Vedi [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
