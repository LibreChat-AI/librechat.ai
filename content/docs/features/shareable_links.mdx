...
Shareable links let you publish a read-only snapshot of a conversation that others can open through a generated URL or QR code. Recipients see the messages, branches, and artifacts without changing the shared original. Signed-in recipients can continue from the visible branch as a personal copy.
