...
icon: Bot
description: >-
  Apprenez à créer, personnaliser et exploiter les agents IA de LibreChat - un
...
- [Plus d'informations sur l'API Code Interpreter](/docs/features/code_interpreter)
  - **Nécessite un abonnement API depuis [code.librechat.ai](https://code.librechat.ai/pricing)**
...
**Remarque :** Le contexte de fichier inclut le texte extrait dans les instructions système de l'agent. Pour des questions sur des documents temporaires dans des conversations individuelles, utilisez [Upload as Text](/docs/features/upload_as_text) depuis le chat.
...
**Note :** La fonctionnalité `deferred_tools` est activée par défaut. Elle peut être activée ou désactivée via la configuration des agents dans [`librechat.yaml`](/docs/configuration/librechat_yaml/object_structure/agents#capabilities).
...
- Notez que vous pouvez ajouter l'indicateur 'x-strict': true au niveau de l'opération dans la spécification OpenAPI pour les actions.
  ...
  - Le mode strict ne prend en charge qu'un sous-ensemble partiel de json. Lisez https://platform.openai.com/docs/guides/structured-outputs/some-type-specific-keywords-are-not-yet-supported pour plus de détails.
...
Les Agents utilisent le système de [contrôle d'accès](/docs/features/access_control) granulaire de LibreChat. Chaque agent possède sa propre liste de contrôle d'accès (ACL) et peut être partagé avec des **utilisateurs**, **groupes**, **rôles** spécifiques, ou **publiquement**, chacun avec un niveau d'autorisation choisi.
