...
W przypadku wdrożeń o wysokim wskaźniku tokenów, `STREAM_DELTA_COALESCE_MS=25` może grupować publikacje delta Redis i zmniejszyć obciążenie Redis kosztem opóźnienia dostarczania do 25 ms. Pozostaw tę wartość nieustawioną lub ustaw na `0`, aby wyłączyć grupowanie, i włącz ją tylko wtedy, gdy każda replika obsługuje ramki grupowe. Zobacz [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
