...
Stel het sessiegeheim van het paneel in in het `.env`-bestand van LibreChat; de compose-bestanden geven dit door als de `SESSION_SECRET` van het paneel:
...
| `API_SERVER_URL` | Nee | Valt terug op `VITE_API_BASE_URL` | Server-side URL voor LibreChat API-aanroepen. Handig wanneer de admin-panel server LibreChat via een andere URL bereikt dan de browser (bijv. interne Kubernetes-service vs. publieke hostnaam). |
| `PORT` | Nee | `3000` | Poort waarop het admin-panel luistert. |
| `ADMIN_SSO_ONLY` | Nee | `false` | Verbergt het e-mail/wachtwoord-formulier, waardoor alleen SSO-login mogelijk is. |
...
| `ADMIN_PANEL_METRICS_SECRET` | Nee | _niet ingesteld_ | Bearer-token vereist om het `/metrics` Prometheus-endpoint uit te lezen. Het endpoint geeft `401` terug wanneer dit niet is ingesteld of niet overeenkomt. |
