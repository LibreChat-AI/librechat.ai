...
높은 토큰 속도의 배포 환경에서는 `STREAM_DELTA_COALESCE_MS=25`를 설정하여 Redis 델타 게시를 일괄 처리하고 Redis 작업 부하를 줄일 수 있습니다. 단, 이 경우 최대 25ms의 전송 지연이 발생할 수 있습니다. 일괄 처리를 비활성화하려면 이 값을 설정하지 않거나 `0`으로 설정하십시오. 모든 복제본이 배치 프레임을 지원하는 경우에만 이 기능을 활성화하십시오. [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing)을 참조하십시오.
