...
Machine tokens must be unexpired, match the configured issuer and audience, identify one allowlisted client through `azp` or `client_id`, and satisfy the configured subject rule. Missing configuration or invalid credentials return `401`; a bound account undergoing deletion returns `409`. See [`managementApi.auth.clients`](/docs/configuration/librechat_yaml/object_structure/agents#managementapiauthclients) for the complete claim and binding rules.
