...
2. **User Interface** (If environment variables are not set):
   ...
   - They can choose which search provider (Serper, SearXNG, or Tavily) and which reranker service to use (Jina or Cohere)
...
- **Tavily**: Batch URL extraction via Tavily Extract API
  ...
  - Supports configurable extract depth, image extraction, and chunking
