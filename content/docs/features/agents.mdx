...
The **Ask User** tool lets an agent pause a run to request missing information, present choices, or confirm how to proceed. After the user answers, the same run resumes with the response. Add **Ask User** from the Tools marketplace; administrators can remove the default `ask_user_question` capability to hide it.
