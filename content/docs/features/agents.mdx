...
**Interrupt & steer** stops generation at the next provider-safe boundary, keeps the partial response, inserts the message, and resumes the same assistant response. It does not interrupt active model reasoning; the steer applies after reasoning reaches a safe boundary. Use **Stop** when you need to end reasoning itself. Use the dedicated composer control, choose it from the send-button menu, or press `Command/Ctrl + Shift + .`. The action falls back to ordinary steering when the deployment cannot interrupt the active provider stream.
