...
- **Code Interpreter:** Enable Code Interpreter on the agent, open its settings in the Tools marketplace, and turn on **Background execution**. This makes code execution and shell commands eligible for background dispatch.
...
One Ask User call accepts a question up to 2,000 characters, optional supporting context up to 4,000 characters, and up to 12 choices. Choice labels are limited to 120 characters, choice values to 500 characters, and the submitted answer to 16,000 characters.
...
Agent cards and landing views show the configured support contact. When no support contact is set, LibreChat falls back to the first resolvable owner so users still know whom to contact.
...
Under **Settings → Chat**, **While generating, Enter will** chooses the default action. The send-button menu can override that choice for an individual message.
...
Queued follow-ups can be sent immediately, converted into a steer while the run is still active, edited, or removed. Removing a queued message restores it to an empty composer when possible. One active run accepts up to 10 pending steers; each steer can include up to 10 files and is limited by [`STEER_MAX_LENGTH`](/docs/configuration/dotenv#agent-conversation-controls).
