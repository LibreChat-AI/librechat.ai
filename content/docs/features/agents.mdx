...
The **Ask User** tool lets an agent pause a run to request missing information, present choices, or confirm how to proceed. While the model prepares the call, LibreChat streams the question into a progress card; the interactive prompt replaces it when ready. After the user answers, the same run resumes with the response. Add **Ask User** from the Tools marketplace; administrators can remove the default `ask_user_question` capability to hide it.

One Ask User call accepts a question up to 2,000 characters, optional supporting context up to 4,000 characters, and up to 12 choices. Choice labels are limited to 280 characters, choice values to 500 characters, and the submitted answer to 16,000 characters.
