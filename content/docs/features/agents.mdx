...
Within an attached workspace, Agents can list directories, read files, search text, author files, and run Bash through the selected worker. A per-Agent Git name and email can identify commits made there without supplying repository credentials. Deployments can limit new personal workers with [`statefulCodeSessions.principalWorkers`](/docs/configuration/librechat_yaml/object_structure/agents#statefulcodesessions).

When an attached worker advertises multiple workspace roots, the chat composer requires one selection for each attached environment used by the Agent or its Subagents. The conversation retains those selections across turns and approval resumes; LibreChat validates them against the live worker and blocks sending if a required workspace is missing or unavailable. A worker that advertises exactly one workspace is selected automatically for a new conversation. See [Attached environments and pairing](/docs/features/code_interpreter#attached-environments-and-pairing) for details and current limitations.
...
For attached environments, the composer can expose **Ask before changes**, **Accept edits**, and **Full access** as a conversation-level Code approval mode. Availability is limited by the environment's administrator-defined permission schema; choosing a more permissive mode never overrides role grants, explicit tool rules, hooks, or sandbox boundaries. See [Attached-environment tool permissions](/docs/features/code_interpreter#attached-environment-tool-permissions).
...
`check_background_task` reports `started_at` and the current `elapsed_ms` while process-local work is running. After settlement, it also reports `settled_at` and freezes `elapsed_ms` at the completed duration. A terminal result recovered from durable storage may include `settled_at` without a start time or elapsed duration when the original process-local dispatch timestamp is unavailable.
...
Administrators can also opt in to owner-scoped cancellation with [`endpoints.agents.backgroundTasks.ordinaryToolCancellation: true`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks). The task reports that cancellation was requested until the underlying invocation settles, and its capacity remains reserved during that interval. Cancellation is cooperative: a tool that ignores the abort signal may still complete successfully. The control applies to ordinary process-local background tools, including attached Bash, but not independently owned detached Subagents.
...
Trusted services can also manage Agents, attached files, and Skills through the beta [Agent Management API](/docs/features/agents_api#agent-management-api). Administrators bind each OIDC machine client to one existing LibreChat user and tenant; every request inherits that user's current role capabilities and resource ACLs rather than receiving separate administrative access.
