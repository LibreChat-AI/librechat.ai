...
- Eğer `skills_enabled` değeri true ise ve herhangi bir yetenek izin listesi (allowlist) ayarlanmamışsa, ajan, daha sonra eklenen yetenekler de dahil olmak üzere kullanıcı tarafından görülebilen tüm aktif kataloğu kullanabilir.
...
Agent Builder içinde, tüm kataloğu kullanmak için **Use all skills** seçeneğini açın. Bu seçeneği kapatmak, özellik etkinleştirilmeden önce aktif olan yetenek seçimini geri yükler.
...
Backend, bu dosyaları skill ile birlikte saklar. Ajan, skill aktif olduğunda ve kapsam dahilinde olduğunda skill dosyalarını çözümleyebilir. İç içe geçmiş dosyalar göreceli yollarını korur ve modele yönelik `skills/{skillName}/...` ad alanı üzerinden adreslenir. Skill görüntüleyicide, göreceli Markdown bağlantıları mevcut dosyadan çözümlenir ve referans verilen dosyayı aynı Skill içinde açar. Code Interpreter etkinleştirildiğinde, aynı dosyalar `/mnt/data/skills/{skillName}/...` dizinine bağlanır, böylece kabuk ve kod işlemleri bunları doğrudan kullanabilir.
