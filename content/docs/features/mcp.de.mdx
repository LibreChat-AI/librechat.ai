...
MCP-Server, die `notifications/tools/list_changed` aussenden, können ihren Tool-Katalog aktualisieren, ohne dass ein Neustart von LibreChat erforderlich ist. LibreChat ruft die aktuelle Liste von der aktiven Verbindung ab, veröffentlicht hinzugefügte Tools und beendet die Bereitstellung von Tools, die der Server entfernt hat. Wiederverbindungen synchronisieren ebenfalls den Katalog, bevor die Verbindung als bereit eingestuft wird.
