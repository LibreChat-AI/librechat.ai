...
As conexões de OCR bloqueiam destinos privados, loopback, link-local e de metadados de nuvem por padrão. Para um serviço de OCR privado confiável, adicione seu host e porta privados exatos em [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses). Esta é uma exceção à proteção contra SSRF, portanto, liste apenas a infraestrutura que você controla.
