...
Aby rozpocząć korzystanie z wyszukiwania w sieci, skonfiguruj dostawcę wyszukiwania oraz scraper. Większość dostawców wymaga kluczy API; Keenable działa bez nich zarówno w przypadku wyszukiwania, jak i pobierania stron, z opcjonalnym kluczem w celu zwiększenia publicznych limitów szybkości. Reranking może korzystać z Jina lub Cohere, albo zostać wyłączony za pomocą `rerankerType: "none"`. Możesz skonfigurować stos na dwa sposoby:

<Callout type="warning" title="Prywatne, samodzielnie hostowane endpointy">
  Połączenia służące do przeszukiwania sieci, scrapowania i rerankingu domyślnie blokują miejsca docelowe typu private, loopback, link-local oraz cloud-metadata. Jeśli punkt końcowy (endpoint) SearXNG, Firecrawl, Jina lub innego skonfigurowanego dostawcy jest prywatny, dodaj jego dokładny host i port do [`webSearch.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/web_search#ssrf-protection-and-private-providers).
</Callout>
...
1. **Zmienne środowiskowe** (Zalecane dla administratorów):
   ...
   Używanie Keenable bez klucza środowiskowego nadal wymaga wyboru dostawcy w `librechat.yaml`:
   ...
   Zobacz [pełną konfigurację Keenable](/docs/configuration/librechat_yaml/object_structure/web_search#keenablescraperoptions), aby uzyskać informacje o opcjonalnych ustawieniach wyszukiwania, pobierania i limitów szybkości.
...
2. **Interfejs użytkownika** (jeśli zmienne środowiskowe nie są ustawione):
   ...
   - Mogą oni wybrać, którego dostawcę wyszukiwania (Serper, SearXNG, Tavily lub Keenable), narzędzie do scrapingu oraz usługę rerankingu (Jina, Cohere lub brak) chcą używać.
...
4. Dodaj dokładny `host:port` swojej instancji do [`allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/web_search#ssrf-protection-and-private-providers), jeśli znajduje się ona pod adresem prywatnym lub pętli zwrotnej (loopback), w przeciwnym razie LibreChat zablokuje połączenie.
5. Opcjonalnie dostosuj, z których silników korzysta Twoja instancja, używając [`searxngSearchOptions`](/docs/configuration/librechat_yaml/object_structure/web_search#searxngsearchoptions). Domyślny zestaw obejmuje tylko trzy silniki i zawiera DuckDuckGo, który wyświetla CAPTCHA większości instancji hostowanych samodzielnie, więc jego rozszerzenie pomaga, jeśli wyszukiwania zwracają puste wyniki.
...
1. Wybierz Keenable jako dostawcę wyszukiwania, scrapera lub obie te funkcje
2. Klucz API nie jest wymagany dla publicznych endpointów wyszukiwania i pobierania.
3. Opcjonalnie ustaw `KEENABLE_API_KEY`, aby zwiększyć publiczne limity zapytań (rate limits)
4. Używaj `KEENABLE_API_URL` tylko do nadpisywania endpointu wyszukiwania, a `KEENABLE_FETCH_URL` tylko do nadpisywania endpointu pobierania stron.
...
- **SearXNG**: Otwartoźródłowa, hostowana samodzielnie metawyszukiwarka
  ...
  - Konfigurowalne silniki, język wyników, zakres czasu oraz limit czasu żądania
...
- **Keenable**: Wyszukiwanie internetowe bez klucza z opcjonalnym uwierzytelnianiem o wyższej stawce
  - Obsługuje wyszukiwanie z ograniczeniem domen, limity wyników, tytuły atrybucji oraz limity czasu żądań
  - Może również służyć jako dostawca scrapingu
...
- **Keenable**: Pobieranie stron bez klucza
  - Używa tego samego opcjonalnego klucza API co wyszukiwanie Keenable
  - Obsługuje tytuły atrybucji i limity czasu żądań
...
5. Skonfiguruj nadpisania endpointów Tavily lub Keenable, jeśli to konieczne (opcjonalne)
...
- Wyszukiwanie Keenable i pobieranie stron są domyślnie bezkluczowe; opcjonalny klucz zwiększa publiczne limity zapytań.
- `KEENABLE_API_URL` nadpisuje tylko wyszukiwanie, podczas gdy `KEENABLE_FETCH_URL` nadpisuje tylko pobieranie stron
- Połącz `searchProvider: "keenable"`, `scraperProvider: "keenable"` oraz `rerankerType: "none"`, aby uzyskać w pełni bezkluczowy stos (keyless stack).
