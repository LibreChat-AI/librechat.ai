...
Untergeordnete Agents (Child Agents) werden mit isoliertem Kontext ausgeführt. Der Tool-Suchstatus und der angesammelte Kontext des übergeordneten Agents (Parent) werden nicht in den untergeordneten Lauf kopiert. Die Modellnutzung des untergeordneten Laufs wird der übergeordneten Transaktion in Rechnung gestellt und in die Nutzungssummen des übergeordneten Agents einbezogen. Die Benutzeroberfläche zeigt den untergeordneten Lauf als erweiterbaren Agent-Aktivitätsteil mit Status, Ticker-Updates und dem vollständigen Endergebnis an, einschließlich des Ergebnistextes, der inkrementell während des Streamings des untergeordneten Agents geliefert wurde.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      'Maximale explizite Sub-Agents pro übergeordnetem Agent. Konfigurierbar über `endpoints.agents.maxSubagents`, bis zu 50.',
      '10',
    ],
...
Der konfigurierte Wert gilt für Anfragen zum Erstellen, Aktualisieren und Duplizieren von Agents, für die `subagents.agent_ids`-Allowlists in den Model Specs sowie für die Subagent-Auswahl im Agent Builder. Siehe [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) für die vollständige Referenz. Die oben genannten Grenzwerte für Tiefe, Graph-Knoten und Laufzeitkonfiguration sind fest vorgegeben.
