...
Establezca el secreto de sesión del panel en el archivo `.env` de LibreChat; los archivos compose lo pasan como `SESSION_SECRET` del panel:
...
| ------------------------------- | --------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SESSION_SECRET` | **Sí** en producción | Fallback de desarrollo hardcodeado al ejecutar `bun dev`; **sin valor predeterminado** en la imagen de Docker | Clave de cifrado de sesión. Debe tener al menos 32 caracteres. |
...
| `ADMIN_PANEL_METRICS_SECRET` | No | _sin definir_ | Token Bearer requerido para extraer datos del endpoint de Prometheus `/metrics`. El endpoint devuelve `401` cuando no está definido o no coincide. |
