...
OCR bağlantıları varsayılan olarak özel, loopback, link-local ve bulut meta veri hedeflerini engeller. Güvenilir bir özel OCR hizmeti için, tam özel ana bilgisayarını ve bağlantı noktasını [`ocr.allowedAddresses`](/docs/configuration/librechat_yaml/object_structure/ocr#allowedaddresses) kısmına ekleyin. Bu, SSRF korumasından bir muafiyettir, bu nedenle yalnızca kontrol ettiğiniz altyapıyı listeleyin.
