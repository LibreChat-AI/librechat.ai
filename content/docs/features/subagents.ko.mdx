...
`enabled` 및 `allowSelf`만 시작 구성(startup config)에서 클라이언트로 전송됩니다. `agent_ids` 허용 목록은 서버 측에 유지되며, 기본값이 10이고 [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents)를 통해 구성 가능한 유효 서브에이전트 제한에 따라 검증됩니다. 클라이언트 요청 페이로드는 Model Specs 서브에이전트 구성을 제공하거나 재정의할 수 없습니다.
...
하위 에이전트는 격리된 컨텍스트에서 실행됩니다. 상위 도구 검색 상태와 누적된 컨텍스트는 하위 실행으로 복사되지 않습니다. 하위 실행 모델 사용량은 상위 트랜잭션으로 청구되며 상위의 총 사용량에 포함됩니다. UI는 하위 실행을 상태, 티커 업데이트, 그리고 하위 실행이 스트리밍되는 동안 점진적으로 전달된 결과 텍스트를 포함한 전체 최종 결과가 표시되는 확장 가능한 에이전트 활동 부분으로 보여줍니다.
...
<OptionTable
  options={[
    [
      'MAX_SUBAGENTS',
      'Number',
      '부모 에이전트당 최대 명시적 하위 에이전트 수입니다. `endpoints.agents.maxSubagents`를 통해 최대 50개까지 설정할 수 있습니다.',
      '10',
    ],
...
구성된 값은 에이전트 생성, 업데이트 및 복제 요청, Model Specs의 `subagents.agent_ids` 허용 목록, 그리고 에이전트 빌더(Agent Builder)의 서브에이전트 선택기에 적용됩니다. 전체 참조는 [maxSubagents](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents)를 확인하세요. 위의 깊이(depth), 그래프 노드 및 실행 구성 제한은 고정되어 있습니다.
