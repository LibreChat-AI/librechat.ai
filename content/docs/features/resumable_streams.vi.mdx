...
Đối với các triển khai có tốc độ token cao, `STREAM_DELTA_COALESCE_MS=25` có thể gộp các bản tin delta của Redis và giảm tải công việc cho Redis với mức đánh đổi là độ trễ phân phối lên tới 25 ms. Hãy để trống hoặc đặt thành `0` để tắt tính năng gộp, và chỉ bật tính năng này sau khi mọi bản sao (replica) đều hỗ trợ các khung gộp (batch frames). Xem [Stream Delta Coalescing](/docs/configuration/redis#stream-delta-coalescing).
