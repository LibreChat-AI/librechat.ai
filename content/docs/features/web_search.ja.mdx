...
Web検索を開始するには、検索プロバイダーとスクレイパーのAPIキーを設定する必要があります。リランキングにはJinaまたはCohereを使用できます。また、`rerankerType: "none"`と設定することで無効化することも可能です。これを行うには、以下の2つの方法があります。
...
2. **ユーザーインターフェース** (環境変数が設定されていない場合):
   ...
   - ユーザーは、どの検索プロバイダー（Serper、SearXNG、またはTavily）と、どのリランカーサービス（Jina、Cohere、またはなし）を使用するかを選択できます。
