...
For screenshots that should adapt to the active theme, use the `ThemeImage` component. Provide a `light` and `dark` source and it renders the variant matching the reader's theme. It also supports click-to-zoom, like every other image.
