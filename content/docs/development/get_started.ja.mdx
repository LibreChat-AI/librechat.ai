...
- LibreChat リポジトリをフォークします: [https://github.com/LibreChat-AI/LibreChat/fork](https://github.com/LibreChat-AI/LibreChat/fork)
...
<Callout type="warning" title="警告">
  `.env.example` のデフォルト値は通常そのままで問題ありませんが、`MONGO_URI` は例外です。ご自身の値を指定してください。
...
- **バックエンドの起動:**
  - 通常運用には `npm run backend` を使用してください。
...
### プルリクエスト (PR) を作成する [#create-a-pull-request-pr]

**Pull Request**は、機能ブランチからメインブランチへ変更をマージします。

1. mainブランチから最新の変更をプルし、競合を解決してください。
...
<Callout type="warning" title="警告">
  ESLintエラーが原因で `git commit` が失敗した場合は、その問題を理解して修正してください。
</Callout>

## コミットを安全にリバートする [#revert-commits-safely]

機能ブランチでの変更を元に戻すには、以下の手順を慎重に行ってください：
