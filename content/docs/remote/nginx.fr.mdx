...
4. Attendez que les modifications DNS se propagent à l'échelle mondiale (vous pouvez vérifier en effectuant un ping sur votre domaine : `ping your_domain.com`).
...
## Configurer Nginx en tant que proxy inverse [#set-up-nginx-as-a-reverse-proxy]
...
1. Mettez à jour `client/nginx.conf` avec votre domaine et les chemins d'accès à vos certificats.
