...
Le fichier d'exemple suffit pour démarrer un déploiement Docker Compose unique et fournir les identifiants de modèle depuis l'application web.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` et `JWT_REFRESH_SECRET` sont vides par défaut. Au premier démarrage, LibreChat génère des valeurs temporaires uniques et les stocke dans `/app/data/.env.temp` ; le volume `librechat-data` fourni conserve ce fichier lors de la recréation du conteneur.

Pour la production, générez plutôt des valeurs permanentes :

[Générateur d'identifiants](/docs/toolkit/credentials-generator)
...
Gardez ces valeurs et le volume `librechat-data` privés. Chaque réplique doit utiliser les mêmes valeurs permanentes. La perte du fichier temporaire ou la modification des identifiants établis peut invalider les sessions et rendre les enregistrements chiffrés illisibles ; LibreChat avertit lorsque les empreintes des identifiants actifs ne correspondent plus au marqueur de la base de données.
...
Suivez [Votre premier chat](/docs/quick_start/first_chat) pour créer un compte, ajouter une clé API de modèle et envoyer votre premier message.
...
Après avoir extrait les images actuelles, le script de mise à jour exécute `sudo docker image prune -f` pour supprimer les images inutilisées laissées par les versions précédentes. Cela peut libérer un espace disque considérable, y compris pour des images non liées à LibreChat ; les images étiquetées, les conteneurs et les volumes ne sont pas supprimés.
