...
証明書の取得を進める前に、ドメインをクラウドサーバーのIPアドレスに向けてください。この手順は基本となるものであり、DNSレコードが世界中に反映されるまでに時間がかかる可能性があるため、SSL証明書の設定よりも前に行う必要があります。
...
2. 証明書を取得する:
    ...
    - 画面の指示に従ってください。Certbotが情報を要求し、検証プロセスを完了させます。
...
1. `client/nginx.conf` を、お使いのドメインと証明書のパスで更新してください。
...
### Option B: Dockerを使用しないホストベースのデプロイ [#option-b-host-based-deployment-without-docker]
...
Nginxは、OWASP Core Rule Set (CRS) を活用することで、Webアプリケーションファイアウォール (WAF) として機能するように構成できます。OWASP CRSは、一般的なWebアプリケーションの脆弱性や攻撃から保護するための堅牢なルールセットを提供します。NginxでOWASP CRSを使用することで、保護レイヤーが追加され、LibreChatデプロイメントのセキュリティを強化できます。
...
Nginx設定でOWASP CRSを有効にすることで、プロジェクトが提供する包括的なルールセットを活用し、SQLインジェクション、クロスサイトスクリプティング（XSS）、リモートファイルインクルードなど、さまざまなWebアプリケーションの脆弱性や攻撃を検知・緩和することができます。
...
これにより、LibreChatが静的ファイルを圧縮することを防ぎ、NGINXがより効率的に圧縮を処理できるようになります。
