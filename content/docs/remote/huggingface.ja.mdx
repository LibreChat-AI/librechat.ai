...
title: HuggingFace
icon: HuggingFace
description: Hugging Face SpacesでLibreChatを簡単にデプロイ
---
...
**3.** スペースに名前を付け、`Secrets` と `Variables` を入力します
...
**1.** `secrets` と `variables` の入力が完了したら、ウィンドウ下部にある `Duplicate Space` をクリックします。
...
**3.** 準備が完了すると、`Building` が `Running` に変わります。
...
## Meilisearch セットアップ (オプション) [#meilisearch-setup-optional]
...
**2. Meilisearchスペースの設定:**
   ...
   *   **MEILI_MASTER_KEY:** 安全な16文字のマスターキーを生成します。[https://randomkeygen.com/](https://randomkeygen.com/) のようなツールを使用してランダムなキーを生成できます。このキーをMeilisearchスペースの `MEILI_MASTER_KEY` 環境変数の値として設定してください。*重要: このキーは安全に保管してください！*
...
**5. `MEILI_MASTER_KEY` シークレットを LibreChat に追加する:**
   ...
   *   **値:** Meilisearchスペースをセットアップした際に使用した*同じ*マスターキーを入力してください。
