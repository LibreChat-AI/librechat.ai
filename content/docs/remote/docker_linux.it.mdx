...
Il file di esempio è sufficiente per avviare una distribuzione Docker Compose singola e fornire le credenziali del modello dall'applicazione web.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` e `JWT_REFRESH_SECRET` sono vuoti per impostazione predefinita. Al primo avvio, LibreChat genera valori temporanei univoci e li memorizza in `/app/data/.env.temp`; il volume `librechat-data` incluso mantiene tale file anche in caso di ricreazione del container.

Per la produzione, genera invece valori permanenti:

[Generatore di credenziali](/docs/toolkit/credentials-generator)
...
Mantieni questi valori e il volume `librechat-data` privati. Ogni replica deve utilizzare gli stessi valori permanenti. La perdita del file temporaneo o la modifica delle credenziali stabilite può invalidare le sessioni e rendere illeggibili i record crittografati; LibreChat avvisa quando le impronte digitali delle credenziali attive non corrispondono più al marcatore del database.
...
Segui [Your First Chat](/docs/quick_start/first_chat) per registrare un account, aggiungere una API key di un modello e inviare il tuo primo messaggio.
...
Dopo aver eseguito il pull delle immagini correnti, lo script di aggiornamento esegue `sudo docker image prune -f` per rimuovere le immagini inutilizzate rimaste dalle versioni precedenti. Questa operazione può recuperare una quantità considerevole di spazio su disco, incluse immagini non correlate a LibreChat; le immagini con tag, i container e i volumi non vengono rimossi.
