...
Przykładowy plik jest wystarczający, aby rozpocząć wdrożenie Docker Compose i podać dane uwierzytelniające modelu z poziomu aplikacji internetowej.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` oraz `JWT_REFRESH_SECRET` są domyślnie puste. Przy pierwszym uruchomieniu LibreChat generuje unikalne tymczasowe wartości i zapisuje je w `/app/data/.env.temp`; dołączony wolumen `librechat-data` zachowuje ten plik nawet po ponownym utworzeniu kontenera.

W przypadku środowiska produkcyjnego należy wygenerować stałe wartości:

[Generator poświadczeń](/docs/toolkit/credentials-generator)
...
Zachowaj te wartości oraz wolumen `librechat-data` jako prywatne. Każda replika musi używać tych samych stałych wartości. Utrata pliku tymczasowego lub zmiana ustalonych danych uwierzytelniających może unieważnić sesje i sprawić, że zaszyfrowane rekordy staną się nieczytelne; LibreChat wyświetla ostrzeżenie, gdy odciski palców aktywnych danych uwierzytelniających przestają być zgodne z oznaczeniem w bazie danych.
...
Postępuj zgodnie z [Your First Chat](/docs/quick_start/first_chat), aby zarejestrować konto, dodać klucz API modelu i wysłać swoją pierwszą wiadomość.
...
Po pobraniu aktualnych obrazów, skrypt aktualizujący uruchamia `sudo docker image prune -f`, aby usunąć nieużywane, osierocone obrazy pozostawione przez poprzednie wersje. Może to odzyskać znaczną ilość miejsca na dysku, w tym osierocone obrazy niezwiązane z LibreChat; otagowane obrazy, kontenery i wolumeny nie są usuwane.
