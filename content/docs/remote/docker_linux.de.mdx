...
Die Beispieldatei reicht aus, um eine einzelne Docker Compose-Bereitstellung zu starten und Modell-Anmeldedaten über die Web-App bereitzustellen.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` und `JWT_REFRESH_SECRET` sind standardmäßig leer. Beim ersten Start generiert LibreChat eindeutige temporäre Werte und speichert diese unter `/app/data/.env.temp`; das gebündelte `librechat-data`-Volume behält diese Datei auch bei einer Neuerstellung des Containers bei.

Für die Produktion generieren Sie stattdessen permanente Werte:
...
Bewahren Sie diese Werte und das `librechat-data` Volume privat auf. Jedes Replikat muss dieselben permanenten Werte verwenden. Der Verlust der temporären Datei oder das Ändern etablierter Anmeldedaten kann Sitzungen ungültig machen und verschlüsselte Datensätze unlesbar machen; LibreChat warnt, wenn aktive Fingerabdrücke von Anmeldedaten nicht mehr mit der Datenbankmarkierung übereinstimmen.
...
Folgen Sie [Your First Chat](/docs/quick_start/first_chat), um ein Konto zu registrieren, einen Modell-API-Schlüssel hinzuzufügen und Ihre erste Nachricht zu senden.
...
Nach dem Abrufen der aktuellen Images führt das Update-Skript `sudo docker image prune -f` aus, um ungenutzte, verwaiste Images zu entfernen, die von früheren Versionen zurückgelassen wurden. Dies kann erheblichen Speicherplatz freigeben, einschließlich verwaister Images, die nicht mit LibreChat in Verbindung stehen; getaggte Images, Container und Volumes werden nicht entfernt.
