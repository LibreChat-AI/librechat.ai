...
Tệp ví dụ là đủ để bắt đầu một triển khai Docker Compose đơn lẻ và cung cấp thông tin xác thực mô hình từ ứng dụng web.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` và `JWT_REFRESH_SECRET` mặc định để trống. Trong lần khởi động đầu tiên, LibreChat sẽ tạo các giá trị tạm thời duy nhất và lưu trữ chúng tại `/app/data/.env.temp`; volume `librechat-data` đi kèm sẽ giữ lại tệp đó ngay cả khi container được tạo lại.

Đối với môi trường production, hãy tạo các giá trị vĩnh viễn thay thế:

[Trình tạo thông tin xác thực](/docs/toolkit/credentials-generator)
...
Giữ các giá trị này và volume `librechat-data` ở chế độ riêng tư. Mọi bản sao (replica) phải sử dụng cùng các giá trị cố định. Việc làm mất tệp tạm thời hoặc thay đổi thông tin xác thực đã thiết lập có thể làm mất hiệu lực các phiên làm việc và khiến các bản ghi được mã hóa không thể đọc được; LibreChat sẽ cảnh báo khi dấu vân tay của thông tin xác thực đang hoạt động không còn khớp với điểm đánh dấu trong cơ sở dữ liệu.
...
Hãy làm theo [Your First Chat](/docs/quick_start/first_chat) để đăng ký tài khoản, thêm API key của model và gửi tin nhắn đầu tiên của bạn.
...
Sau khi tải xuống các image hiện tại, tập lệnh cập nhật sẽ chạy `sudo docker image prune -f` để xóa các image không sử dụng bị bỏ lại từ các phiên bản trước. Việc này có thể giải phóng đáng kể dung lượng ổ đĩa, bao gồm cả các image không liên quan đến LibreChat; các image đã được gắn thẻ (tagged), container và volume sẽ không bị xóa.
