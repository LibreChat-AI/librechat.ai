...
Ardından, uygulamanın ayarlarının yanı sıra [özel endpoint'lerin](/docs/configuration/librechat_yaml/ai_endpoints) özelleştirilmesine olanak tanıyan ve `librechat.yaml` olarak da bilinen [LibreChat yapılandırma dosyasını](/docs/configuration/librechat_yaml) oluşturuyoruz.
...
Örnek dosya, tek bir Docker Compose dağıtımını başlatmak ve web uygulamasından model kimlik bilgilerini sağlamak için yeterlidir.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` ve `JWT_REFRESH_SECRET` varsayılan olarak boştur. İlk başlatmada LibreChat benzersiz geçici değerler oluşturur ve bunları `/app/data/.env.temp` konumunda saklar; paketlenmiş `librechat-data` birimi, konteyner yeniden oluşturulsa bile bu dosyayı korur.

Üretim ortamı için bunun yerine kalıcı değerler oluşturun:
...
Bu değerleri ve `librechat-data` birimini gizli tutun. Her kopya aynı kalıcı değerleri kullanmalıdır. Geçici dosyayı kaybetmek veya oluşturulmuş kimlik bilgilerini değiştirmek, oturumları geçersiz kılabilir ve şifrelenmiş kayıtları okunamaz hale getirebilir; LibreChat, aktif kimlik bilgisi parmak izleri artık veritabanı işaretiyle eşleşmediğinde uyarı verir.
...
Bir hesap oluşturmak, bir model API anahtarı eklemek ve ilk mesajınızı göndermek için [İlk Sohbetiniz](/docs/quick_start/first_chat) bölümünü takip edin.
...
Mevcut imajları çektikten sonra, güncelleme betiği önceki sürümlerden kalan kullanılmayan askıda kalmış imajları kaldırmak için `sudo docker image prune -f` komutunu çalıştırır. Bu işlem, LibreChat ile ilgisi olmayan askıda kalmış imajlar da dahil olmak üzere önemli miktarda disk alanı açabilir; etiketli imajlar, konteynerler ve birimler (volumes) kaldırılmaz.
