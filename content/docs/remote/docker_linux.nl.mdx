...
# Windows (PowerShell)
docker images -a --filter "reference=registry.librechat.ai/librechat-ai/librechat*" --format "{{.ID}}" | ForEach-Object { docker rmi $_ }
docker images -a --filter "reference=ghcr.io/librechat-ai/librechat*" --format "{{.ID}}" | ForEach-Object { docker rmi $_ }
```
...
Standaard haalt deze configuratie de laatste updates van de main branch van LibreChat op. Als je liever de laatste "stabiele" release gebruikt, die wordt bepaald door de [latest tags](https://github.com/LibreChat-AI/LibreChat/releases), dan moet je deploy-compose.yml bewerken en je wijzigingen precies zoals hierboven in Deel V doorvoeren. Houd er rekening mee dat je op die manier niet direct profiteert van de nieuwste functies zodra ze beschikbaar zijn.
...
```yaml
image: registry.librechat.ai/librechat-ai/librechat-api:latest
```
