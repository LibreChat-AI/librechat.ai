...
O arquivo de exemplo é suficiente para iniciar uma implantação única do Docker Compose e fornecer credenciais de modelo a partir do aplicativo web.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` e `JWT_REFRESH_SECRET` estão vazios por padrão. Na primeira inicialização, o LibreChat gera valores temporários únicos e os armazena em `/app/data/.env.temp`; o volume `librechat-data` incluído mantém esse arquivo durante a recriação do container.

Para produção, gere valores permanentes em vez disso:

[Gerador de Credenciais](/docs/toolkit/credentials-generator)
...
Mantenha estes valores e o volume `librechat-data` privados. Cada réplica deve usar os mesmos valores permanentes. Perder o arquivo temporário ou alterar credenciais estabelecidas pode invalidar sessões e tornar registros criptografados ilegíveis; o LibreChat avisa quando as impressões digitais das credenciais ativas não correspondem mais ao marcador do banco de dados.
...
Siga [Seu Primeiro Chat](/docs/quick_start/first_chat) para registrar uma conta, adicionar uma chave de API de modelo e enviar sua primeira mensagem.
...
Após baixar as imagens atuais, o script de atualização executa `sudo docker image prune -f` para remover imagens órfãs não utilizadas deixadas por versões anteriores. Isso pode recuperar um espaço em disco substancial, incluindo imagens órfãs não relacionadas ao LibreChat; imagens com tags, containers e volumes não são removidos.
