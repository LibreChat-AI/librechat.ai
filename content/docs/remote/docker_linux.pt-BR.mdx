...
Por padrão, esta configuração buscará as atualizações mais recentes da branch principal do LibreChat. Se você preferir a versão "estável" mais recente, que é definida pelas [latest tags](https://github.com/LibreChat-AI/LibreChat/releases), você precisará editar o deploy-compose.yml e confirmar (commit) suas alterações exatamente como acima na Parte V. Esteja ciente de que você não se beneficiará dos recursos mais recentes assim que eles forem lançados se optar por isso.
