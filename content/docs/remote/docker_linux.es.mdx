...
El archivo de ejemplo es suficiente para iniciar un despliegue de Docker Compose y proporcionar credenciales de modelo desde la aplicación web.
...
`CREDS_KEY`, `CREDS_IV`, `JWT_SECRET` y `JWT_REFRESH_SECRET` están vacíos de forma predeterminada. En el primer inicio, LibreChat genera valores temporales únicos y los almacena en `/app/data/.env.temp`; el volumen `librechat-data` incluido conserva ese archivo durante la recreación del contenedor.

Para producción, genere valores permanentes en su lugar:

[Generador de credenciales](/docs/toolkit/credentials-generator)
...
Mantenga estos valores y el volumen `librechat-data` privados. Cada réplica debe utilizar los mismos valores permanentes. Perder el archivo temporal o cambiar las credenciales establecidas puede invalidar las sesiones y hacer que los registros cifrados sean ilegibles; LibreChat advierte cuando las huellas digitales de las credenciales activas ya no coinciden con el marcador de la base de datos.
...
Sigue [Tu primer chat](/docs/quick_start/first_chat) para registrar una cuenta, añadir una API key de un modelo y enviar tu primer mensaje.
...
Después de extraer las imágenes actuales, el script de actualización ejecuta `sudo docker image prune -f` para eliminar las imágenes huérfanas no utilizadas que dejaron las versiones anteriores. Esto puede recuperar una cantidad considerable de espacio en disco, incluyendo imágenes huérfanas no relacionadas con LibreChat; las imágenes etiquetadas, los contenedores y los volúmenes no se eliminan.
