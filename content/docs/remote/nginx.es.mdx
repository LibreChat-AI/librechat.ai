...
3. Acceso de terminal a su servidor en la nube.
...
4. Espera a que los cambios de DNS se propaguen globalmente (puedes comprobarlo haciendo ping a tu dominio: `ping your_domain.com`).
...
2. Obtenga el certificado:
    - Ejecuta `sudo certbot --nginx` para obtener e instalar el certificado automáticamente para Nginx.
...
El archivo `deploy-compose.yml` incluye un contenedor Nginx y utiliza el archivo `client/nginx.conf` para la configuración de Nginx. Sin embargo, dado que `sudo certbot --nginx` extrae el certificado a la configuración del host, necesitas duplicar el certificado en los contenedores Docker.
...
2. Iniciar Nginx: `sudo systemctl start nginx`
...
Si configuró nginx utilizando la Opción A recomendada anteriormente, siga estos pasos para renovar los certificados:
...
3. Configurar OWASP CRS:
    - El paquete OWASP CRS normalmente incluye un archivo de configuración (por ejemplo, `/etc/nginx/modsecurity.d/nginx-modsecurity.conf`) donde puede ajustar varias configuraciones y conjuntos de reglas según sus requisitos.
...
Esto evitará que LibreChat comprima archivos estáticos, permitiendo que NGINX maneje la compresión de manera más eficiente.
