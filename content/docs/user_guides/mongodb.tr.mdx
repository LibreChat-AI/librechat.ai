...
## Amazon DocumentDB Uyumluluğu [#amazon-documentdb-compatibility]

LibreChat, dağıtım hedefi olarak **Amazon DocumentDB 5.0 veya sonraki sürüm örnek tabanlı kümeleri** destekler. DocumentDB yeniden denenebilir yazma işlemlerini (retryable writes) desteklemediği için `MONGO_URI` değerini `retryWrites=false` olacak şekilde yapılandırın:
...
Mevcut AWS CA paketini ve VPC'niz için gereken ağ yolunu kullanın. AWS'nin [MongoDB'den işlevsel farklılıklar](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) ve [programatik bağlantı kılavuzu](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) sayfalarına bakın.

DocumentDB 4.0, LibreChat'i azaltılmış veritabanı zorlamasıyla çalıştırabilir: LibreChat'in OAuth kimlikleri, oluşturulan kod dosyaları ve harici grup kimlikleri için kullandığı kısmi benzersiz dizinleri (partial unique indexes) desteklemez. LibreChat, başlangıçta dizin oluşturma hatalarını günlüğe kaydeder, ancak bu kısıtlamalar veritabanı tarafından zorlanmaz. DocumentDB 5.0+ örnek tabanlı kümeler (instance-based clusters), gerekli kısmi dizin biçimlerini destekler.

<Callout type="warning" title="Elastic kümeler desteklenmemektedir">
  Amazon DocumentDB elastic clusters, LibreChat'in benzersiz kullanıcı kimliği dizini de dahil olmak üzere benzersiz dizinleri desteklemez ve desteklenen bir LibreChat veritabanı hedefi değildir. AWS [elastic-cluster limitations](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html) bölümüne bakın.
</Callout>

`Model.syncIndexes()` komutunu operasyonel bir geçiş adımı olarak DocumentDB üzerinde çalıştırmayın; DocumentDB'nin `collMod` desteği sınırlıdır. LibreChat'in düzenli başlangıç dizin oluşturma işlemi, amaçlanan yoldur.
