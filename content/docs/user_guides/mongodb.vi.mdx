...
## Khả năng tương thích với Amazon DocumentDB [#amazon-documentdb-compatibility]

LibreChat hỗ trợ **các cụm dựa trên phiên bản Amazon DocumentDB 5.0 trở lên** làm mục tiêu triển khai. Hãy cấu hình `MONGO_URI` với `retryWrites=false`, vì DocumentDB không hỗ trợ ghi có thể thử lại (retryable writes):
...
Sử dụng gói CA AWS hiện tại và đường dẫn mạng cần thiết cho VPC của bạn. Xem [các khác biệt về chức năng so với MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) và [hướng dẫn kết nối](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) của AWS.

DocumentDB 4.0 có thể chạy LibreChat với các ràng buộc cơ sở dữ liệu hạn chế: nó không hỗ trợ các chỉ mục duy nhất một phần (partial unique indexes) mà LibreChat sử dụng cho danh tính OAuth, tệp mã được tạo và ID nhóm bên ngoài. LibreChat sẽ ghi lại các lỗi tạo chỉ mục khi khởi động, nhưng các ràng buộc đó không được cơ sở dữ liệu thực thi. Các cụm dựa trên phiên bản DocumentDB 5.0+ hỗ trợ các dạng chỉ mục một phần bắt buộc.

<Callout type="warning" title="Các cụm Elastic không được hỗ trợ">
  Các cụm elastic clusters của Amazon DocumentDB không hỗ trợ chỉ mục duy nhất (unique indexes), bao gồm cả chỉ mục định danh người dùng duy nhất của LibreChat, và không phải là mục tiêu cơ sở dữ liệu được LibreChat hỗ trợ. Xem [các hạn chế của elastic-cluster](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html) trên AWS.
</Callout>

Không chạy `Model.syncIndexes()` đối với DocumentDB như một bước di chuyển vận hành; hỗ trợ `collMod` của DocumentDB bị hạn chế. Việc tạo chỉ mục khi khởi động thông thường của LibreChat là quy trình được thiết kế cho việc này.
