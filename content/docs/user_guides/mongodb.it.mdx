...
## Compatibilità con Amazon DocumentDB [#amazon-documentdb-compatibility]

LibreChat supporta **cluster basati su istanze Amazon DocumentDB 5.0 o versioni successive** come destinazione di distribuzione. Configura `MONGO_URI` con `retryWrites=false`, poiché DocumentDB non supporta le scritture riprovabili (retryable writes):
...
Utilizza l'attuale bundle CA di AWS e il percorso di rete richiesto per il tuo VPC. Consulta le [differenze funzionali rispetto a MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) e la [guida alla connessione](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) di AWS.

DocumentDB 4.0 può eseguire LibreChat con un'applicazione delle regole del database ridotta: non supporta gli indici univoci parziali che LibreChat utilizza per le identità OAuth, i file di codice generati e gli ID di gruppo esterni. LibreChat registra gli errori di creazione degli indici all'avvio, ma tali vincoli non vengono applicati dal database. I cluster basati su istanze DocumentDB 5.0+ supportano le forme di indice parziale richieste.

<Callout type="warning" title="I cluster Elastic non sono supportati">
  I cluster elastici di Amazon DocumentDB non supportano gli indici univoci, incluso l'indice di identità utente univoco di LibreChat, e non sono un database target supportato da LibreChat. Consulta le [limitazioni dei cluster elastici](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html) di AWS.
</Callout>

Non eseguire `Model.syncIndexes()` su DocumentDB come passaggio di migrazione operativa; il supporto di DocumentDB per `collMod` è limitato. La creazione degli indici all'avvio regolare di LibreChat è il percorso previsto.
