...
## Compatibilidade com Amazon DocumentDB [#amazon-documentdb-compatibility]

O LibreChat oferece suporte a **clusters baseados em instância do Amazon DocumentDB 5.0 ou superior** como destino de implantação. Configure a `MONGO_URI` com `retryWrites=false`, pois o DocumentDB não oferece suporte a gravações repetíveis (retryable writes):
...
Use o pacote de CA da AWS atual e o caminho de rede necessário para sua VPC. Consulte as [diferenças funcionais em relação ao MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) e as [orientações de conexão](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) da AWS.

O DocumentDB 4.0 pode executar o LibreChat com aplicação reduzida de regras de banco de dados: ele não oferece suporte aos índices únicos parciais que o LibreChat utiliza para identidades OAuth, arquivos de código gerados e IDs de grupos externos. O LibreChat registra falhas na criação de índices durante a inicialização, mas essas restrições não são aplicadas pelo banco de dados. Clusters baseados em instâncias do DocumentDB 5.0+ oferecem suporte às formas de índice parcial necessárias.

<Callout type="warning" title="Clusters elásticos não são suportados">
  Os clusters elásticos do Amazon DocumentDB não suportam índices únicos, incluindo o índice de identidade de usuário único do LibreChat, e não são um destino de banco de dados suportado pelo LibreChat. Veja as [limitações de elastic-cluster](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html) da AWS.
</Callout>

Não execute `Model.syncIndexes()` contra o DocumentDB como uma etapa de migração operacional; o suporte do DocumentDB ao `collMod` é limitado. A criação de índices durante a inicialização regular do LibreChat é o caminho pretendido.
