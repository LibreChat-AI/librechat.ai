...
## Amazon DocumentDB-Kompatibilität [#amazon-documentdb-compatibility]

LibreChat unterstützt **Amazon DocumentDB 5.0 oder neuere instanzbasierte Cluster** als Bereitstellungsziel. Konfigurieren Sie `MONGO_URI` mit `retryWrites=false`, da DocumentDB keine wiederholbaren Schreibvorgänge (retryable writes) unterstützt:
...
Verwenden Sie das aktuelle AWS CA-Bundle und den für Ihre VPC erforderlichen Netzwerkpfad. Siehe die [funktionalen Unterschiede zu MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) sowie die [Verbindungsanleitung](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) von AWS.

DocumentDB 4.0 kann LibreChat mit eingeschränkter Datenbank-Durchsetzung ausführen: Es unterstützt nicht die partiellen eindeutigen Indizes, die LibreChat für OAuth-Identitäten, generierte Codedateien und externe Gruppen-IDs verwendet. LibreChat protokolliert beim Start Fehler beim Erstellen von Indizes, aber diese Einschränkungen werden von der Datenbank nicht erzwungen. Instanzbasierte Cluster von DocumentDB 5.0+ unterstützen die erforderlichen Formen partieller Indizes.

<Callout type="warning" title="Elastic-Cluster werden nicht unterstützt">
  Amazon DocumentDB Elastic Clusters unterstützen keine eindeutigen Indizes (unique indexes), einschließlich des eindeutigen Benutzeridentitätsindex von LibreChat, und sind kein unterstütztes Datenbankziel für LibreChat. Siehe die AWS [elastic-cluster limitations](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html).
</Callout>

Führen Sie `Model.syncIndexes()` nicht als operativen Migrationsschritt gegen DocumentDB aus; die `collMod`-Unterstützung von DocumentDB ist begrenzt. Die reguläre Indexerstellung beim Start von LibreChat ist der vorgesehene Weg.
