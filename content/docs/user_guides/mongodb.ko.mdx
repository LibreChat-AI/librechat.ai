...
## Amazon DocumentDB 호환성 [#amazon-documentdb-compatibility]

LibreChat은 배포 대상으로 **Amazon DocumentDB 5.0 이상 인스턴스 기반 클러스터**를 지원합니다. DocumentDB는 재시도 가능한 쓰기(retryable writes)를 지원하지 않으므로 `MONGO_URI`를 `retryWrites=false`로 설정하십시오:
...
현재 AWS CA 번들과 VPC에 필요한 네트워크 경로를 사용하세요. AWS의 [MongoDB와의 기능적 차이점](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) 및 [연결 지침](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html)을 참조하세요.

DocumentDB 4.0은 데이터베이스 제약 조건이 완화된 상태로 LibreChat을 실행할 수 있습니다. 하지만 OAuth ID, 생성된 코드 파일, 외부 그룹 ID에 대해 LibreChat이 사용하는 부분 고유 인덱스(partial unique indexes)를 지원하지 않습니다. LibreChat은 시작 시 인덱스 빌드 실패를 로그로 기록하지만, 해당 제약 조건은 데이터베이스에 의해 강제되지 않습니다. DocumentDB 5.0 이상의 인스턴스 기반 클러스터는 필요한 부분 인덱스 형식을 지원합니다.

<Callout type="warning" title="Elastic 클러스터는 지원되지 않습니다">
  Amazon DocumentDB elastic clusters는 LibreChat의 고유 사용자 식별 인덱스를 포함한 고유 인덱스를 지원하지 않으며, LibreChat에서 지원하는 데이터베이스 대상이 아닙니다. AWS [elastic-cluster limitations](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html)를 참조하세요.
</Callout>

운영 마이그레이션 단계의 일환으로 DocumentDB에서 `Model.syncIndexes()`를 실행하지 마십시오. DocumentDB의 `collMod` 지원은 제한적입니다. LibreChat의 일반적인 시작 시 인덱스 생성 방식이 의도된 경로입니다.
