...
## Amazon DocumentDB-compatibiliteit [#amazon-documentdb-compatibility]

LibreChat ondersteunt **Amazon DocumentDB 5.0 of latere instance-based clusters** als implementatiedoel. Configureer `MONGO_URI` met `retryWrites=false`, omdat DocumentDB geen retryable writes ondersteunt:
...
Gebruik de huidige AWS CA-bundel en het netwerkpad dat vereist is voor uw VPC. Zie de [functionele verschillen met MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) en de [verbindingsrichtlijnen](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) van AWS.

DocumentDB 4.0 kan LibreChat draaien met beperkte database-handhaving: het ondersteunt niet de gedeeltelijke unieke indexen die LibreChat gebruikt voor OAuth-identiteiten, gegenereerde codebestanden en externe groeps-ID's. LibreChat logt fouten bij het opbouwen van indexen tijdens het opstarten, maar deze beperkingen worden niet afgedwongen door de database. DocumentDB 5.0+ instance-based clusters ondersteunen de vereiste vormen van gedeeltelijke indexen.

<Callout type="warning" title="Elastic clusters worden niet ondersteund">
  Amazon DocumentDB elastic clusters ondersteunen geen unieke indexen, inclusief de unieke gebruikersidentiteitsindex van LibreChat, en zijn geen ondersteund LibreChat-doel voor databases. Zie de AWS [elastic-cluster limitations](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html).
</Callout>

Voer `Model.syncIndexes()` niet uit tegen DocumentDB als een operationele migratiestap; de `collMod`-ondersteuning van DocumentDB is beperkt. Het aanmaken van indexen bij het regulier opstarten van LibreChat is de bedoelde werkwijze.
