...
## Zgodność z Amazon DocumentDB [#amazon-documentdb-compatibility]

LibreChat obsługuje **klastry oparte na instancjach Amazon DocumentDB w wersji 5.0 lub nowszej** jako cel wdrożenia. Skonfiguruj `MONGO_URI` z parametrem `retryWrites=false`, ponieważ DocumentDB nie obsługuje ponawialnych zapisów (retryable writes):
...
Użyj bieżącego pakietu CA AWS oraz ścieżki sieciowej wymaganej dla Twojego VPC. Zobacz [różnice funkcjonalne względem MongoDB](https://docs.aws.amazon.com/documentdb/latest/developerguide/functional-differences.html) oraz [wskazówki dotyczące połączenia](https://docs.aws.amazon.com/documentdb/latest/developerguide/connect_programmatically.html) w dokumentacji AWS.

DocumentDB 4.0 może uruchomić LibreChat z ograniczonym wymuszaniem reguł bazy danych: nie obsługuje on częściowych unikalnych indeksów, których LibreChat używa dla tożsamości OAuth, wygenerowanych plików kodu oraz zewnętrznych identyfikatorów grup. LibreChat rejestruje błędy budowania indeksów podczas uruchamiania, ale te ograniczenia nie są wymuszane przez bazę danych. Klastry oparte na instancjach DocumentDB 5.0+ obsługują wymagane formy indeksów częściowych.

<Callout type="warning" title="Klastry Elastic nie są obsługiwane">
  Elastyczne klastry Amazon DocumentDB nie obsługują unikalnych indeksów, w tym unikalnego indeksu tożsamości użytkownika LibreChat, i nie są wspieranym celem bazy danych dla LibreChat. Zobacz [ograniczenia elastycznych klastrów](https://docs.aws.amazon.com/documentdb/latest/developerguide/docdb-using-elastic-clusters.html) AWS.
</Callout>

Nie uruchamiaj `Model.syncIndexes()` w odniesieniu do DocumentDB jako operacyjnego kroku migracji; obsługa `collMod` w DocumentDB jest ograniczona. Regularne tworzenie indeksów podczas uruchamiania LibreChat jest zamierzoną ścieżką.
