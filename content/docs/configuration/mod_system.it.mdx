...
<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', 'Indica se abilitare o meno il ban degli utenti per violazioni (saranno comunque registrati).','BAN_VIOLATIONS=true'],
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_IP', 'boolean', 'Se limitare o meno la quantità di messaggi che un IP può inviare per `MESSAGE_IP_WINDOW`.','LIMIT_MESSAGE_IP=true'],
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_USER', 'boolean', 'Se limitare o meno la quantità di messaggi che un utente può inviare per `MESSAGE_USER_WINDOW`.','LIMIT_MESSAGE_USER=false'],
...
<OptionTable
  options={[    
    ['LIMIT_IMPORT_USER', 'boolean', 'Indica se limitare il numero di importazioni di conversazioni che un utente può eseguire per `IMPORT_USER_WINDOW`.','LIMIT_IMPORT_USER=false'],
    ...
    ['IMPORT_USER_WINDOW', 'integer', 'In minuti, determina la finestra temporale per le importazioni di `IMPORT_USER_MAX`.','IMPORT_USER_WINDOW=1'],
  ]}
/>
...
<OptionTable
  options={[    
    ['LIMIT_FORK_IP', 'boolean', 'Indica se limitare il numero di fork di conversazione che un IP può creare per `FORK_IP_WINDOW`.','LIMIT_FORK_IP=true'],
...
<OptionTable
  options={[    
    ['LIMIT_FORK_USER', 'boolean', 'Se limitare il numero di fork di conversazione che un utente può creare per `FORK_USER_WINDOW`.','LIMIT_FORK_USER=false'],
...
<OptionTable
  options={[    
    ['OPENAI_MODERATION', 'boolean', 'Se abilitare o meno la moderazione OpenAI sugli endpoint **OpenAI** e **Plugins**.','OPENAI_MODERATION=false'],
