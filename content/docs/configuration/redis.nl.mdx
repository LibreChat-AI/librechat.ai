...
- [Generation Protocol Rollout](#generation-protocol-rollout)
...
### Uitrol van het Generation Protocol [#generation-protocol-rollout]

Het v2-generatieprotocol voegt de duurzame wachtrij, onderbreking, pauzeren/hervatten en herstelgaranties toe die worden gebruikt door huidige Agent-runs. Implementaties die gebruikmaken van Redis blijven opzettelijk op protocol v1 wanneer `GENERATION_PROTOCOL_VERSION` niet is ingesteld, zodat replica's die verschillende LibreChat-versies draaien naast elkaar kunnen bestaan tijdens een rolling upgrade.

Gebruik een uitrol in twee fasen:

1. Implementeer de v2-geschikte LibreChat-image naar **elke** replica waarbij `GENERATION_PROTOCOL_VERSION` niet is ingesteld of is ingesteld op `1`.
2. Wacht tot actieve generaties van de vorige afbeelding zijn voltooid of zijn afgerond.
3. Stel `GENERATION_PROTOCOL_VERSION=2` in en rol dezelfde image opnieuw uit.
...
<Callout type="warning" title="Veiligheid bij rolling deployment">
  Schakel protocol v2 pas in wanneer elke replica dit ondersteunt. Nadat v2-generaties zijn begonnen, mag je niet terugkeren naar een LibreChat-image van vóór v2 totdat die generaties zijn afgehandeld; oudere replica's kunnen niet veilig werken met hun Redis-status.
</Callout>

In-memory generatiestreams delen geen status tussen replica's en gebruiken standaard protocol v2, dus ze vereisen deze overgang niet.
