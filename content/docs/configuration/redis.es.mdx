...
- [Protocolo de Generación Rollout](#generation-protocol-rollout)
...
### Despliegue del Protocolo de Generación [#generation-protocol-rollout]

El protocolo de generación v2 añade la cola duradera, la interrupción, la pausa/reanudación y las garantías de recuperación utilizadas por las ejecuciones actuales de Agentes. Las implementaciones respaldadas por Redis permanecen intencionalmente en el protocolo v1 cuando `GENERATION_PROTOCOL_VERSION` no está configurado, de modo que las réplicas que ejecutan diferentes versiones de LibreChat puedan coexistir durante una actualización gradual.

Utilice un despliegue en dos etapas:

1. Despliegue la imagen de LibreChat compatible con v2 en **cada** réplica con `GENERATION_PROTOCOL_VERSION` sin establecer o configurado en `1`.
2. Espere a que las generaciones activas propiedad de la imagen anterior finalicen o se agoten.
3. Establezca `GENERATION_PROTOCOL_VERSION=2` y despliegue la misma imagen nuevamente.
...
<Callout type="warning" title="Seguridad de despliegue continuo">
  No habilite el protocolo v2 hasta que todas las réplicas lo admitan. Una vez que comiencen las generaciones v2, no vuelva a una imagen de LibreChat anterior a la v2 hasta que esas generaciones se hayan vaciado; las réplicas más antiguas no pueden operar de forma segura en su estado de Redis.
</Callout>

Los flujos de generación en memoria no comparten estado entre réplicas y utilizan el protocolo v2 de forma predeterminada, por lo que no requieren esta transición.
