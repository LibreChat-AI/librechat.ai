...
**Nota:** actualmente, la mayoría de estos valores se configuran a través del archivo .env, pero es posible que pronto migren para configurarse exclusivamente desde el [archivo de configuración `librechat.yaml`](/docs/configuration/librechat_yaml/object_structure/config#ratelimits).

### Violación, Intervalo, Duración [#violation-interval-duration]
...
<OptionTable
  options={[
    ...
    ['VIOLATION_SCORE_TTL', 'integer', 'Cuánto tiempo permanece una puntuación de infracción sin nuevas infracciones (en milisegundos). Cada nueva infracción reinicia la cuenta regresiva, por lo que las puntuaciones decaen tras un periodo de inactividad en lugar de acumularse indefinidamente. Establezca en 0 para que las puntuaciones nunca caduquen (comportamiento heredado).','VIOLATION_SCORE_TTL=1000 * 60 * 60'],
  ]}
/>
