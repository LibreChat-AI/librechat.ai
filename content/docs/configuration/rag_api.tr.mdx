...
Varsayılan Docker kurulumuyla, `.env` dosyası LibreChat ve RAG API arasında paylaşılır. RAG değişkenlerini aynı dosya içerisinde tanımlayın. Listenin tamamı [RAG API README](https://github.com/LibreChat-AI/rag-api/blob/main/README.md) dosyasında yer almaktadır.
</Callout>
...
Docker, varsayılan olarak yalnızca OpenAI'dan veya yapılandırdığınız uzak bir HuggingFace/Ollama hizmetinden gelen uzak yerleştirmeleri (embeddings) destekleyen RAG API'nin "lite" imajını (`registry.librechat.ai/librechat-ai/librechat-rag-api-dev-lite:latest`) kullanır.

Yerel embedding'ler için, compose dosyasındaki imajı tam yapılandırma olan `registry.librechat.ai/librechat-ai/librechat-rag-api-dev:latest` ile değiştirin. Bu değişikliği [Docker Compose Geçersiz Kılma Dosyanızda](/docs/configuration/docker_override) yapın. Bir örnek için projenin kök dizinindeki `docker-compose.override.yml.example` dosyasına bakın.
...
Konteyner dışı bir kurulum daha fazla manuel işlem gerektirir. [RAG API repo](https://github.com/LibreChat-AI/rag-api/) içindeki talimatları izleyin.
...
Değişkenlerin ve açıklamalarının tam listesi için [RAG API repo](https://github.com/LibreChat-AI/rag-api/) sayfasına bakın.
