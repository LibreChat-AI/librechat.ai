...
- [Peluncuran Protokol Generasi](#generation-protocol-rollout)
...
Protokol generasi v2 menambahkan antrean persisten, interupsi, jeda/lanjutkan, dan jaminan pemulihan yang digunakan oleh proses Agent saat ini. Deployment yang didukung Redis sengaja tetap menggunakan protokol v1 saat `GENERATION_PROTOCOL_VERSION` tidak diatur agar replika yang menjalankan versi LibreChat berbeda dapat hidup berdampingan selama peningkatan bertahap (rolling upgrade).

Gunakan peluncuran dua tahap:

1. Deploy image LibreChat yang mendukung v2 ke **setiap** replika dengan `GENERATION_PROTOCOL_VERSION` yang tidak disetel atau disetel ke `1`.
2. Tunggu hingga generasi aktif yang dimiliki oleh gambar sebelumnya selesai atau dikosongkan.
3. Atur `GENERATION_PROTOCOL_VERSION=2` dan luncurkan kembali image yang sama.
...
<Callout type="warning" title="Keamanan deployment bergulir">
  Jangan aktifkan protokol v2 sampai setiap replika mendukungnya. Setelah generasi v2 dimulai, jangan lakukan *rollback* ke *image* LibreChat pra-v2 sampai generasi tersebut habis; replika yang lebih lama tidak dapat beroperasi dengan aman pada *state* Redis mereka.
</Callout>

Aliran pembuatan dalam memori (in-memory generation streams) tidak membagikan status di seluruh replika dan menggunakan protokol v2 secara default, sehingga tidak memerlukan peralihan ini.
