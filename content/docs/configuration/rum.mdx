...
In proxy mode, the browser SDK sends telemetry to `/api/rum`. LibreChat requires the active user JWT on that request, validates the session, strips LibreChat auth before forwarding, and sends the payload to `RUM_PROXY_TARGET_URL`. The proxy accepts OTLP traces and logs at `/api/rum/v1/traces` and `/api/rum/v1/logs`.
...
<OptionTable
  options={[
    ...
    [
      'RUM_URL',
      'string',
      'OTLP HTTP ingest URL for browser telemetry in `publicToken` mode.',
      'RUM_URL=http://localhost:4318',
    ],
    [
      'RUM_SERVICE_NAME',
