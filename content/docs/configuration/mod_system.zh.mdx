...
在生产环境中，您应该部署 Cloudflare 或其他 DDoS 防护措施，以真正保护服务器免受过度请求的影响，但这些更改将在很大程度上保护您免受针对您部署的实例进行代理的个别或少数恶意行为者的攻击。
...
<OptionTable
  options={[    
    ...
    ['IMPORT_IP_MAX', 'integer', '每个 IP 在 `IMPORT_IP_WINDOW` 时间内可以执行的最大对话导入次数。','IMPORT_IP_MAX=100'],
...
<OptionTable
  options={[    
    ['LIMIT_FORK_USER', 'boolean', '是否限制用户在 `FORK_USER_WINDOW` 内可以创建的对话分支数量。','LIMIT_FORK_USER=false'],
