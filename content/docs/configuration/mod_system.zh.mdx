...
自动审核系统使用评分机制来跟踪用户违规行为。当用户进行过度登录、注册或发送消息等操作时，他们会积累违规分数。一旦达到设定的阈值，该用户及其 IP 地址将被暂时封禁。该系统通过监控并惩罚快速或可疑的活动，确保了平台的安全性。
...
<OptionTable
  options={[    
    ...
    ['IMPORT_IP_MAX', 'integer', '每个 IP 在 `IMPORT_IP_WINDOW` 时间窗口内可以执行的最大对话导入次数。','IMPORT_IP_MAX=100'],
