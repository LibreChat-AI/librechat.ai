...
> 注意：你可以同时使用这两种限制器，但默认情况下仅按 IP 进行限制。
...
<OptionTable
  options={[    
    ['LIMIT_IMPORT_IP', 'boolean', '是否限制一个 IP 在 `IMPORT_IP_WINDOW` 时间窗口内可以执行的对话导入次数。','LIMIT_IMPORT_IP=true'],
...
<OptionTable
  options={[    
    ['LIMIT_FORK_USER', 'boolean', '是否限制用户在每个 `FORK_USER_WINDOW` 内可以创建的对话分支数量。','LIMIT_FORK_USER=false'],
