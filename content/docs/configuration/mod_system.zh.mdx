...
description: >-
  ...
  将被暂时封禁。此系统通过监控并惩罚快速或可疑的活动来确保平台安全。
---
...
**有关更多详细信息，请参阅此处提供的用户指南：[自动审核](/docs/features/mod_system)**
...
**注意：** 目前，这些值大多通过 .env 文件进行配置，但它们很快可能会迁移到仅通过 [`librechat.yaml` 配置文件](/docs/configuration/librechat_yaml/object_structure/config#ratelimits)进行配置。
...
<OptionTable
  options={[    
    ...
    ['LOGIN_WINDOW', 'number', '以分钟为单位，确定 LOGIN_MAX 次登录的时间窗口。默认为 `5`。'],
