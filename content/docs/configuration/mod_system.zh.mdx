...
自动化审核系统使用评分机制来跟踪用户违规行为。当用户进行诸如频繁登录、注册或发送消息等操作时，他们会积累违规分数。一旦达到设定的阈值，该用户及其 IP 地址将被暂时封禁。该系统通过监控并惩罚快速或可疑的活动来确保平台安全。
...
<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', '是否启用因违规而封禁用户的功能（用户仍会被记录在案）。','BAN_VIOLATIONS=true'],
...
<OptionTable
  options={[    
    ...
    ['REGISTRATION_VIOLATION_SCORE', 'integer', '注册违规得分。','REGISTRATION_VIOLATION_SCORE=1'],
    ...
    ['VERIFY_EMAIL_VIOLATION_SCORE', 'integer', '电子邮件验证违规的分数。','VERIFY_EMAIL_VIOLATION_SCORE=0'],
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_IP', 'boolean', '是否限制每个 IP 在 `MESSAGE_IP_WINDOW` 时间内可以发送的消息数量。','LIMIT_MESSAGE_IP=true'],
    ['MESSAGE_IP_MAX', 'integer', '一个 IP 在 `MESSAGE_IP_WINDOW` 时间窗口内可以发送的最大消息数量。','MESSAGE_IP_MAX=40'],
...
<OptionTable
  options={[    
    ['OPENAI_MODERATION', 'boolean', '是否在 **OpenAI** 和 **Plugins** endpoint 上启用 OpenAI 审核。','OPENAI_MODERATION=false'],
