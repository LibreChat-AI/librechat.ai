...
自动审核系统使用评分机制来跟踪用户违规行为。当用户进行诸如频繁登录、注册或发送消息等操作时，他们会积累违规分数。一旦达到设定的阈值，该用户及其 IP 地址将被暂时封禁。该系统通过监控并惩罚快速或可疑的活动来确保平台安全。
...
<OptionTable
  options={[    
    ...
    ['MESSAGE_IP_MAX', 'integer', '一个 IP 在 `MESSAGE_IP_WINDOW` 内可以发送的最大消息数。','MESSAGE_IP_MAX=40'],
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_USER', 'boolean', '是否限制用户在 `MESSAGE_USER_WINDOW` 内可以发送的消息数量。','LIMIT_MESSAGE_USER=false'],
    ['MESSAGE_USER_MAX', 'integer', '用户在 `MESSAGE_USER_WINDOW` 时间窗口内可发送的最大消息数量。','MESSAGE_USER_MAX=40'],
