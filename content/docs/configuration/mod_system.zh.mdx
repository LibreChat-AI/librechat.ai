...
<OptionTable
  options={[    
    ...
    ['MESSAGE_IP_MAX', 'integer', '一个 IP 在 `MESSAGE_IP_WINDOW` 内可发送的最大消息数量。','MESSAGE_IP_MAX=40'],
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_USER', 'boolean', '是否限制用户在 `MESSAGE_USER_WINDOW` 时间窗口内可以发送的消息数量。','LIMIT_MESSAGE_USER=false'],
