...
自动化审核系统使用评分机制来跟踪用户违规行为。当用户进行诸如频繁登录、注册或发送消息等操作时，他们会积累违规分数。一旦达到设定的阈值，该用户及其 IP 地址将被暂时封禁。该系统通过监控并惩罚快速或可疑的活动，确保了平台的安全性。
...
<OptionTable
  options={[
    ...
    ['BAN_INTERVAL', 'integer', '每当用户的分数达到或超过区间阈值时，该用户将被封禁。','BAN_INTERVAL=20'],
  ]}
/>
...
<OptionTable
  options={[    
    ...
    ['NON_BROWSER_VIOLATION_SCORE', 'integer', '非浏览器违规的分数。','NON_BROWSER_VIOLATION_SCORE=20'],
    ...
    ['RESET_PASSWORD_VIOLATION_SCORE', 'integer', '密码重置违规的评分。','RESET_PASSWORD_VIOLATION_SCORE=0'],
...
<OptionTable
  options={[    
    ['LIMIT_IMPORT_USER', 'boolean', '是否限制用户在 `IMPORT_USER_WINDOW` 时间窗口内可以执行的对话导入次数。','LIMIT_IMPORT_USER=false'],
