...
- Şu anda bağlam penceresini kullanan token kategorilerini ayıran renk kodlu bir sayaç
- Diyalog, tutulan araç sonuçları, Ajan talimatları, önbelleğe alınmış girdi, tamamlamalar, dal toplamları ve konuşma toplamları için tıklanabilir bir döküm
- Uyumlu anlık görüntüler (snapshots) kalan bağlam baskısını öngörebildiğinde, araç bazlı açıklamalar ve bir çalışma süresi tahmini
- Yetkili yöneticiler için, konuşmanın aktif uygulama içi bağlantıda örneklenmiş bir izi (trace) olduğunda doğrudan Langfuse oturum bağlantısı

Ayrıntılı döküm varsayılan olarak daraltılmıştır, böylece kategorize edilmiş sayaç kompakt kalır. Kullanıcılar toplam tutarlara ihtiyaç duyduklarında bunu genişletebilirler ve LibreChat bu tercihi sonraki konuşmalar için hatırlar. Ertelenmiş araç tanımları, taralı bir işlemle aynı kategori rengini kullanırken, bilinen bir kategori bileşimi olmayan tahminler tek bir dolgu olarak kalır.

Araç trafiği, önbellek ve Agent talimatları, bulundukları kategorilerin alt kümeleridir ve toplamı ikinci kez artırmazlar. Çalışma süresi tahmini yalnızca ardışık anlık görüntüler aynı modeli, sağlayıcıyı, Agent'ı, bağlam bütçesini ve sabit genel giderleri paylaştığında görünür. Kullanım dökümleri, son birincil çağrının önbellek bölünmesi ve tamamlanan çıktı dahil olmak üzere mesajlar ve konuşmalarla birlikte kalıcı hale getirilir. Yeniden açılan sohbetler, yalnızca aktif akış oturumuna güvenmek yerine dal ve toplam kullanım/maliyet ayrıntılarını korur; Subagent toplamları, tüm dalları kapsayacak şekilde açıkça etiketlenmiştir.
