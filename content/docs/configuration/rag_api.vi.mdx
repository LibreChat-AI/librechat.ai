...
Với thiết lập Docker mặc định, tệp `.env` được chia sẻ giữa LibreChat và RAG API. Hãy định nghĩa các biến RAG trong cùng tệp đó. Danh sách đầy đủ nằm trong [RAG API README](https://github.com/LibreChat-AI/rag_api/blob/main/README.md).
</Callout>
...
Docker mặc định sử dụng image "lite" của RAG API (`registry.librechat.ai/librechat-ai/librechat-rag-api-dev-lite:latest`), chỉ hỗ trợ các embedding từ xa từ OpenAI hoặc dịch vụ HuggingFace/Ollama từ xa mà bạn đã cấu hình.

Đối với local embeddings, hãy chuyển đổi image trong file compose sang bản build đầy đủ, `registry.librechat.ai/librechat-ai/librechat-rag-api-dev:latest`. Hãy thực hiện thay đổi này trong [Docker Compose Override File](/docs/configuration/docker_override) của bạn. Xem `docker-compose.override.yml.example` tại thư mục gốc của dự án để biết ví dụ.
...
Thiết lập không sử dụng container đòi hỏi thao tác thủ công nhiều hơn. Hãy làm theo các hướng dẫn trong [RAG API repo](https://github.com/LibreChat-AI/rag_api/).
...
Để xem danh sách đầy đủ các biến và mô tả của chúng, hãy xem [RAG API repo](https://github.com/LibreChat-AI/rag_api/).
