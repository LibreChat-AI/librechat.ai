...
## Exemple propre [#clean-example]

<Callout type="example" title="Exemple" collapsible>

Cet exemple de configuration inclut tous les endpoints documentés (à l'exception d'Azure, LiteLLM, MLX et Ollama, qui nécessitent tous des configurations supplémentaires)
...
interface:
  ...
  # Separate regular-chat retention applies only with retentionMode: "all".
  ...
  # generalChatRetention: 2160 # Regular chats under retentionMode: "all"
...
endpoints:
  ...
  #   # Beta machine-to-machine Agent and Skill management API.
  #   managementApi:
  #     auth:
  #       oidc:
  #         issuer: https://idp.example.com
  #         audience: librechat-management
  #       clients:
...
## Exemple avec commentaires [#example-with-comments]

Cet exemple de fichier de configuration configure LibreChat avec des options détaillées dans plusieurs domaines clés :

- **Mise en cache** : Activée pour améliorer les performances.
- **Gestion des fichiers** :
  - **Stratégie de fichier** : Commenté, mais suggère une intégration possible avec Firebase pour le stockage de fichiers.
  - **Configurations de fichiers** : Personnalise les limites de téléchargement de fichiers et les types MIME autorisés pour différents endpoint, incluant une limite globale de taille de fichier serveur et une limite spécifique pour les images d'avatar des utilisateurs.
- **Rate Limiting** : Définit des seuils pour le nombre maximal de téléchargements de fichiers autorisés par IP et par utilisateur dans une fenêtre temporelle spécifiée, visant à prévenir les abus.
- **Inscription** :
  - Permet l'inscription à partir de fournisseurs de connexion sociale et de domaines d'e-mail spécifiés, améliorant ainsi la sécurité et la gestion des utilisateurs.
- **Endpoints** :
  - **Assistants** : Configure l'endpoint des assistants avec un intervalle d'interrogation et un délai d'expiration pour les opérations, et fournit une option pour désactiver l'interface du constructeur.
  - **Endpoints personnalisés** :
    - Configure deux endpoints de service IA externes, Mistral et OpenRouter, incluant les clés API, les URLs de base, la gestion des modèles et des bascules de fonctionnalités spécifiques comme les titres de conversation, la synthèse et les ajustements de paramètres.
    - Pour Mistral, cela permet la récupération dynamique des modèles, applique des paramètres supplémentaires pour les invites sécurisées et supprime explicitement les paramètres non pris en charge.
    - Pour OpenRouter, cela configure une configuration de base sans récupération dynamique des modèles et spécifie un modèle pour les titres de conversation.

<Callout type="example" title="Exemple commenté" collapsible>
...
# fileConfig:
#   # Unified upload routing; set legacyFileUploadUX: true to restore the destination chooser.
