...
- Supports dynamic environment variable values, which use the format: `"${VARIABLE_NAME}"`.
- Supports template placeholder fields for both user and body (see the ["Headers"](/docs/configuration/librechat_yaml/object_structure/custom_endpoint#headers) section).
