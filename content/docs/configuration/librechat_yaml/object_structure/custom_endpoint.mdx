...
<Callout type="info" title="Automatic pricing for supported endpoints">

For custom endpoints named `"OpenRouter"`, `"Helicone"`, or `"LiteLLM"` (case-insensitive), LibreChat can populate `tokenConfig` automatically by fetching pricing from the provider — you don't need to write it by hand. Adding a `tokenConfig` block yourself disables this fetch entirely for that endpoint, not just for the models you listed, so only add one if you want full manual control over every model's rate. See the [LiteLLM endpoint guide](/docs/configuration/librechat_yaml/ai_endpoints/litellm#automatic-pricing) for the LiteLLM-specific walkthrough.
