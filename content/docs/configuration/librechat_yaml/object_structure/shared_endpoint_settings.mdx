...
**Default:** Provider-dependent. Provider clients from `@librechat/agents` use adaptive smoothing with a 25 ms target; legacy and direct stream handlers can retain their own defaults.

Adaptive smoothing emits the first text token without delay, increases chunk size when a stream falls behind, and preserves tool-call and metadata ordering. A configured `streamRate` sets the target cadence where the endpoint integration forwards it.

For custom endpoints, the endpoint value is preserved when `endpoints.all` exists without `streamRate`. An `endpoints.all.streamRate` value overrides the custom endpoint value, including an explicit `0`; `streamRate: 0` disables adaptive smoothing for custom endpoints.
