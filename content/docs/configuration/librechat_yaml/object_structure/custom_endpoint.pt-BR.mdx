...
Quando um endpoint personalizado é criado através da API de configuração do administrador, o LibreChat criptografa um `apiKey` literal em repouso e retorna apenas um `apiKeyPreview`. Referências `${ENV_VAR}` e `user_provided` permanecem como valores de passagem legíveis. Para endpoints definidos diretamente no `librechat.yaml`, prefira uma referência de ambiente conforme mostrado acima.
...
- Em endpoints personalizados compatíveis com OpenAI, IDs de modelo contendo `claude` utilizam as restrições de documento do Claude: PDFs e documentos de texto são aceitos, enquanto tipos de documentos binários que não sejam PDF são ignorados antes que a solicitação seja enviada.
...
Na etapa final de resolução de saída, os placeholders `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` e `{{LIBRECHAT_OPENID_*}}` reconhecidos que não possuem valor tornam-se strings vazias em vez de serem encaminhados como texto de modelo. Nomes de placeholders desconhecidos permanecem inalterados para que erros de digitação na configuração fiquem visíveis.
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Data e hora da aceitação dos termos                         |

As solicitações de modelo de agente também suportam `{{LIBRECHAT_USER_TENANT_ID}}` e `{{LIBRECHAT_USER_TENANTID}}` em cabeçalhos de endpoint personalizados. Ambos os aliases usam o tenant com escopo de solicitação autoritativo em vez de um registro de usuário potencialmente obsoleto, aplicam-se a Agentes raiz, Subagentes e chamadas de sumarização personalizadas, e são resolvidos como uma string vazia quando nenhum tenant está presente. Eles não são placeholders de campo de usuário gerais para consumidores de modelos que não sejam Agentes.
