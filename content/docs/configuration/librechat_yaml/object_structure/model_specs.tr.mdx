...
Kullanıcının kayıtlı seçimi artık çözümlenmeyen bir Agent'a işaret ediyorsa, LibreChat yumuşak varsayılanı (soft default) yeniden etkinleştirir, böylece yeni bir sohbet hala kullanılabilir bir seçime sahip olur. Agent listesinin yüklenmesindeki geçici bir başarısızlık, kayıtlı seçimi temizlemez veya bu geri dönüşü tetiklemez.
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'Model seçicide ve istemci başlangıç yapılandırmasında tam Model Specs\'in görünüp görünmeyeceğini kontrol eder.',
      'true',
    ],
  ]}
/>

Bir Model Specs öğesini, `spec` alanında `name` değerini açıkça gönderen güvenilir sunucu tarafı çağrıcılar için kullanılabilir tutarken etkileşimli seçimden gizlemek için `showInMenu: false` olarak ayarlayın. Bu, yalnızca simgeyi gizleyen `showIconInMenu` özelliğinden farklıdır.
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Model spec\'in geçici temsilcisini bellek araçlarıyla donatır.',
      'false',
    ],
  ]}
/>

Yapılandırılmış [User Memory](/docs/features/memory), endpoint düzeyinde `memory` yeteneği ve kullanıcı bellek izinleri gerektirir. Etkinleştirildiğinde, model kullanıcı açıkça talep ettiğinde yapılandırılmış anıları kaydedebilir veya silebilir.
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Model spec\'in dört adede kadar ilgili soru sormak için duraklamasına ve yanıtlarla devam etmesine olanak tanır.',
      'false',
    ],
  ]}
/>

Uç nokta düzeyindeki `ask_user_question` yeteneğini gerektirir. Bu, LibreChat'in kalıcı duraklatma/devam ettirme akışını sağlayan Agent sohbet çalıştırmalarında desteklenir.
...
<OptionTable
  options={[
    [
      ...
      'Model spec\'e ekli hangi uygun araçların arka plan görevleri olarak çalışabileceğini kontrol eder.',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

İsteğe bağlı uç nokta düzeyindeki `run_in_background` yeteneğini gerektirir. Bu yetenek ve `executeCode: true` ile, Code Interpreter yürütme ve kabuk çağrıları varsayılan olarak arka planda çalışmaya uygundur. Bunları devre dışı bırakmak için `runInBackground: false` veya `[]` değerini, spesifikasyona bağlı tüm uygun araçları etkinleştirmek için `true` değerini veya yalnızca belirli araçları etkinleştirmek için çözümlenmiş araç kimliklerinden oluşan bir dizi ayarlayın. `execute_code` veya `bash_tool` seçimi, her iki yerel kod aracını da seçer; bunları içermeyen bir liste ise devre dışı bırakır. Desteklenmeyen veya yanlış yazılmış seçimler günlüğe kaydedilir.

Model uygun bir aracı başlatabilir ve çalışmaya devam edebilir. Kaydedilmiş Agent'lar için, desteklenen yalnızca içerik (content-only) tamamlamaları varsayılan olarak otomatik şekilde iletilir; `check_background_task` durum, kontrol, canlı artifact'ler ve kurtarma için kullanılabilir durumda kalır. Tamamlanan kod çıktısı ve oluşturulan dosyalar orijinal kod çağrısına eklenir. Olağan yürütme işlem yerelidir (process-local) ve çalışan işlem (worker process) kaybından sağ çıkamaz, ancak kalıcı bir terminal sonucu başka bir replikadan iletilebilir. Yöneticiler, [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks) ile yalnızca yoklama (poll-only) davranışını geri yükleyebilirler.
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'Hangi uygun araçların, model tarafından yazılan bir amacı canlı araç çağrısı etiketi olarak yayınlayacağını kontrol eder.',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

İsteğe bağlı endpoint düzeyindeki `tool_intents` yeteneğini gerektirir. Web araması, dosya oluşturma, bellek ve desteklenen Code Interpreter araçları gibi yerel araçlar, bu yetenek etkinleştirildiğinde varsayılan olarak intent etiketlerini kullanır. Her uygun aracı etkinleştirmek için `describeIntent: true` olarak ayarlayın veya yalnızca belirli araçları etkinleştirip diğer her şey için etiketleri devre dışı bırakmak üzere çözümlenmiş araç kimliklerini (tool IDs) sağlayın. Boş bir dizi, spec için tüm intent etiketlerini devre dışı bırakır. Alanı atlamak veya `false` olarak ayarlamak daha geniş bir seçim eklemez, bu nedenle yerel varsayılanlar geçerli olmaya devam eder.

Niyet etiketleri araç şemalarına dahil edilir ve bu nedenle model isteklerine şema belirteçleri (tokens) ekler. LibreChat; yanlış yazılmış, spesifikasyon için kullanılamayan veya uygun olmayan seçimleri günlüğe kaydeder.
...
- `agent_ids`: belirli kalıcı ajanların ek alt ajanlar olarak kullanılmasına olanak tanır. Bu liste, varsayılan olarak 10 olan ve [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) ile yapılandırılabilen efektif alt ajan sınırı ile sınırlandırılmıştır. Liste sunucu tarafında kalır; istemcilere gönderilen başlangıç yapılandırması yalnızca herkese açık `enabled` ve `allowSelf` bayraklarını içerir.
...
Google model ayarları artık bu kontrolü paylaşılan şemada sunduğundan, yöneticiler Google ve Vertex AI Model Specs için dosyaların sonraki turlarda yeniden gönderilip gönderilmeyeceğini açıkça seçebilirler.
...
Google model ayarları için Agent Builder, `10` ile `2,000,000` arasındaki değerleri kabul eder ve `1,000`'lik artışlar kullanır.
...
Sunucu ayrıca, ilgili Agent'ı yüklemeden veya çalıştırmadan önce geçerli Model Specs'i çözümler ve onun kalıcı `agent_id` değerini yetkilendirir. Bir çağırıcı, bu erişim kontrolünden sonra farklı bir Agent'ın yerine geçmek için istek alanlarını veya bir preset uyuşmazlığını kullanamaz.
...
> **Google / Anthropic** genellikle `topP`, `topK`, `maxOutputTokens` parametrelerini destekler; Google ayrıca desteklenen Gemini metin modellerinde `url_context` özelliğini de destekler.
> **Anthropic / OpenRouter / Bedrock (Anthropic ve Nova modelleri)** `promptCache` ve `promptCacheTtl` parametrelerini destekler.
...
> **Destekleyenler:** `anthropic`, OpenRouter özel endpoint'leri, `bedrock` (Anthropic ve Nova modelleri)
> (Sağlayıcı istem önbelleğe almayı aç/kapat)
...
> **Destekleyenler:** `anthropic`, OpenRouter özel endpoint'leri, `bedrock` (Anthropic ve Nova modelleri)
> (İstem önbelleğe alma etkinleştirildiğinde istem önbelleği ömrünü ayarlar)
...
<OptionTable
  options={[
    [
      'promptCacheTtl',
      'Enum',
      'Prompt-cache ömrünü ayarlar. Desteklenen değerler `5m` ve `1h` şeklindedir.',
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'Model için düşünme çabası seviyesini kontrol eder. Düşünme çabasını azaltmak, daha hızlı yanıtlar alınmasını ve düşünme için daha az token kullanılmasını sağlayabilir. Desteklenen OpenAI modelleri `xhigh` ve `max` değerlerini sunabilir; Bedrock için kabul edilen değerler `low`, `medium` ve `high` şeklindedir.',
      '',
    ],
  ]}
/>
...
- `""` (boş dizge - ayarlanmamış)
- `"standard"`
- `"pro"`

> **Destekleyenler:** Akıl yürütme modunu açığa çıkaran OpenAI Responses API modelleri (GPT-5.6 dahil).

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'OpenAI Responses API mantık yürütme modunu ayarlar.', ''],
  ]}
/>
...
> **Destekleyenler:** Akıl yürütme bağlamını açığa çıkaran OpenAI Responses API modelleri (GPT-5.6 dahil).
...
<OptionTable
  options={[
    [
      'reasoning_context',
      'String',
      'Modelin muhakeme yeteneği için ne kadar konuşma bağlamının kullanılabilir olduğunu kontrol eder.',
      '',
    ],
  ]}
/>
...
GPT-5.6 için LibreChat, `useResponsesApi: false` açıkça ayarlanmadığı sürece muhakeme (reasoning) isteklerini varsayılan olarak Responses API'ye yönlendirir.
...
<OptionTable
  options={[
    [
      'url_context',
      'Boolean',
      'Modelin kullanıcı mesajında yer alan URL\'leri okuyabilmesi için Google URL Context\'i etkinleştirir. YouTube bağlantıları, mümkün olduğunda yerel video anlama girdilerine dönüştürülür.',
      '',
    ],
  ]}
/>
...
Gemini 2.5 modelleri için Google ayarı modele duyarlıdır: Pro `128`-`32768` aralığını, Flash `0`-`24576` aralığını ve Flash-Lite `512`-`24576` aralığını kabul eder. `-1` otomatik ayar olarak kalmaya devam eder. Gemini 3+ modelleri, sayısal bir bütçe yerine [`thinkingLevel`](#thinkinglevel) kullanır.
