...
Model uygun bir aracı başlatabilir, çalışmaya devam edebilir ve `check_background_task` ile sorgulama yapabilir. Tamamlanan kod çıktısı ve oluşturulan dosyalar orijinal kod çağrısına eklenir. Arka plan görevi durumu süreç-yereldir (process-local) ve sunucu yeniden başlatıldığında veya kopyalar arasında taşındığında varlığını sürdürmez.
