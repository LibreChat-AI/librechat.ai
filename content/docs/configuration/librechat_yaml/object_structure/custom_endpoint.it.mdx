...
Quando un endpoint personalizzato viene scritto tramite l'API di configurazione dell'amministratore, LibreChat crittografa un `apiKey` letterale a riposo e restituisce solo un `apiKeyPreview`. I riferimenti `${ENV_VAR}` e `user_provided` rimangono valori pass-through leggibili. Per gli endpoint definiti direttamente in `librechat.yaml`, preferisci un riferimento all'ambiente come mostrato sopra.
...
- Sugli endpoint personalizzati compatibili con OpenAI, gli ID modello contenenti `claude` utilizzano le restrizioni sui documenti di Claude: i PDF e i documenti di testo sono accettati, mentre i tipi di documenti binari non PDF vengono ignorati prima dell'invio della richiesta.
...
<OptionTable
  options={[
    [
      'addParams',
      'Object/Dictionary',
      'Aggiunge parametri aggiuntivi alle richieste. I valori possono essere stringhe, numeri, booleani, array o oggetti nidificati. Supporta i toggle degli strumenti del provider come `web_search: true` e `url_context: true` di Google.',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Boolean',
      'Riproduce il `reasoning_content` del provider all\'interno dei turni di chiamata agli strumenti per endpoint personalizzati compatibili con OpenAI che lo richiedono.',
      'includeReasoningContent: true',
    ],
    [
      'includeReasoningHistory',
      'Boolean',
      'Ricostruisce `reasoning_content` dalla cronologia delle conversazioni persistenti tra i vari turni. Implica `includeReasoningContent`.',
...
Al passaggio finale di risoluzione in uscita, i segnaposto `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` e `{{LIBRECHAT_OPENID_*}}` riconosciuti e privi di valore diventano stringhe vuote invece di essere inoltrati come testo del modello. I nomi dei segnaposto sconosciuti rimangono invariati, in modo che gli errori di battitura nella configurazione siano visibili.
...
Le richieste del modello Agente supportano anche `{{LIBRECHAT_USER_TENANT_ID}}` e `{{LIBRECHAT_USER_TENANTID}}` nelle intestazioni degli endpoint personalizzati. Entrambi gli alias utilizzano il tenant autoritativo con ambito di richiesta anziché un record utente potenzialmente non aggiornato, si applicano agli Agenti root, ai Sottoagenti e alle chiamate di riepilogo personalizzate, e si risolvono in una stringa vuota quando non è presente alcun tenant. Non sono segnaposto generici per i campi utente per i consumatori di modelli non Agente.
