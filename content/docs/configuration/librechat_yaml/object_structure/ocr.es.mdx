...
2. **`ocr` config exists** — La estrategia configurada (p. ej., `mistral_ocr`) se intenta primero. Si la estrategia configurada **falla en tiempo de ejecución**, se utiliza `document_parser` como respaldo para que la extracción de texto siga teniendo éxito en los tipos de documentos compatibles.
