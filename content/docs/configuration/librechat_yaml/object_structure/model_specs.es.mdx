...
Si la selección almacenada del usuario apunta a un Agent que ya no se resuelve, LibreChat reactiva el valor predeterminado flexible para que un nuevo chat aún tenga una selección utilizable. Un fallo temporal al cargar la lista de Agents no borra la elección almacenada ni activa ese respaldo.
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'Controla si la especificación completa del modelo aparece en el selector de modelos y en la configuración de inicio del cliente.',
      'true',
    ],
  ]}
/>

Establece `showInMenu: false` para ocultar una especificación de la selección interactiva mientras la mantienes disponible para llamadores de confianza del lado del servidor que envíen explícitamente su `name` en el campo `spec`. Esto difiere de `showIconInMenu`, que oculta solo el icono.
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Equipa al agente efímero de Model Specs con herramientas de memoria.',
      'false',
    ],
  ]}
/>

Requiere [User Memory](/docs/features/memory) configurado, la capacidad `memory` a nivel de endpoint y permisos de memoria de usuario. Cuando está habilitado, el modelo puede guardar o eliminar memorias estructuradas cuando el usuario lo solicita explícitamente.
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Permite que el Model Specs haga una pausa para realizar hasta cuatro preguntas relacionadas y reanude con las respuestas.',
      'false',
    ],
  ]}
/>

Requiere la capacidad `ask_user_question` a nivel de endpoint. Esto es compatible con las ejecuciones de chat de Agentes que proporcionan el flujo de pausa/reanudación duradero de LibreChat.
...
<OptionTable
  options={[
    [
      ...
      'Controla qué herramientas elegibles adjuntas a Model Specs pueden ejecutarse como tareas en segundo plano.',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

Requiere la capacidad `run_in_background` a nivel de endpoint que debe ser activada. Con esa capacidad y `executeCode: true`, la ejecución del Intérprete de Código y las llamadas a shell son elegibles para ejecutarse en segundo plano de forma predeterminada. Establezca `runInBackground: false` o `[]` para desactivarlas, `true` para habilitar todas las herramientas elegibles adjuntas a la especificación, o una matriz de IDs de herramientas resueltos para habilitar solo esas herramientas. Seleccionar `execute_code` o `bash_tool` selecciona ambas herramientas de código nativo; una lista que las omita las desactivará. Las selecciones no admitidas o mal escritas se registrarán.

El modelo puede iniciar una herramienta elegible y continuar trabajando. Para los Agents guardados, las finalizaciones de solo contenido compatibles se entregan automáticamente de forma predeterminada; `check_background_task` permanece disponible para el estado, el control, los artefactos en vivo y la recuperación. La salida de código completada y los archivos generados se adjuntan a la llamada de código original. La ejecución ordinaria es local al proceso y no sobrevive a la pérdida de su proceso de trabajo, pero un resultado de terminal persistido puede ser entregado desde otra réplica. Los administradores pueden restaurar el comportamiento de solo sondeo con [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks).
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'Controla qué herramientas elegibles transmiten una intención escrita por el modelo como su etiqueta de llamada de herramienta en vivo.',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

Requiere la capacidad `tool_intents` a nivel de endpoint que debe ser activada explícitamente. Las herramientas nativas como la búsqueda web, la creación de archivos, la memoria y las herramientas de Code Interpreter compatibles utilizan etiquetas de intención de forma predeterminada mientras la capacidad esté habilitada. Establezca `describeIntent: true` para habilitar todas las herramientas elegibles, o proporcione los IDs de las herramientas resueltas para habilitar solo esas herramientas y deshabilitar las etiquetas para todo lo demás. Un arreglo vacío deshabilita todas las etiquetas de intención para la especificación. Omitir el campo o establecerlo en `false` no añade una selección más amplia, por lo que se siguen aplicando los valores predeterminados nativos.

Las etiquetas de intención se incluyen en los esquemas de herramientas y, por lo tanto, añaden tokens de esquema a las solicitudes del modelo. LibreChat registra las selecciones que están mal escritas, no disponibles para la especificación o que no son elegibles.
...
- `agent_ids`: permite agentes persistentes específicos como subagentes adicionales. Esta lista está limitada por el límite efectivo de subagentes, que es de 10 por defecto y es configurable con [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents). La lista permanece en el lado del servidor; la configuración de inicio enviada a los clientes solo incluye las banderas públicas `enabled` y `allowSelf`.
...
La configuración de modelos de Google ahora expone este control en el esquema compartido, por lo que los administradores pueden elegir explícitamente si los archivos se reenvían en turnos posteriores para los Model Specs de Google y Vertex AI.
...
Para la configuración de modelos de Google, el Agent Builder acepta valores desde `10` hasta `2,000,000` y utiliza incrementos de `1,000`.
...
El servidor también resuelve la especificación de modelo efectiva y autoriza su `agent_id` persistente antes de cargar o ejecutar dicho Agent. Un llamador no puede utilizar campos de solicitud o una discrepancia en los ajustes preestablecidos para sustituir un Agent diferente después de esta verificación de acceso.
...
> **Google / Anthropic** normalmente admiten `topP`, `topK`, `maxOutputTokens`; Google también admite `url_context` en modelos de texto Gemini compatibles.
> **Anthropic / OpenRouter / Bedrock (modelos Anthropic y Nova)** admiten `promptCache` y `promptCacheTtl`.
...
> **Compatible con:** `anthropic`, endpoints personalizados de OpenRouter, `bedrock` (modelos de Anthropic y Nova)
> (Alternar almacenamiento en caché de prompts del proveedor)
...
> **Compatible con:** `anthropic`, endpoints personalizados de OpenRouter, `bedrock` (modelos Anthropic y Nova)
> (Establece el tiempo de vida de la caché de prompts cuando el almacenamiento en caché de prompts está habilitado)
...
<OptionTable
  options={[
    [
      'promptCacheTtl',
      'Enum',
      'Establece el tiempo de vida de la caché de prompts. Los valores admitidos son `5m` y `1h`.',
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'Controla el nivel de esfuerzo de razonamiento del modelo. Reducir el esfuerzo de razonamiento puede resultar en respuestas más rápidas y menos tokens utilizados en el razonamiento. Los modelos de OpenAI compatibles pueden exponer `xhigh` y `max`; para Bedrock, los valores aceptados son `low`, `medium`, `high`.',
      '',
    ],
  ]}
/>
...
- `""` (cadena vacía - sin configurar)
- `"standard"`
- `"pro"`

> **Soportado por:** Modelos de la API de respuestas de OpenAI que exponen el modo de razonamiento, incluyendo GPT-5.6

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'Establece el modo de razonamiento de la API de respuestas de OpenAI.', ''],
  ]}
/>
...
> **Soportado por:** Modelos de la API de respuestas de OpenAI que exponen contexto de razonamiento, incluyendo GPT-5.6
...
<OptionTable
  options={[
    [
      'reasoning_context',
      'String',
      'Controla cuánto contexto conversacional está disponible para el razonamiento del modelo.',
      '',
    ],
  ]}
/>
...
Para GPT-5.6, LibreChat utiliza de forma predeterminada la Responses API para las solicitudes de razonamiento, a menos que se establezca explícitamente `useResponsesApi: false`.
...
<OptionTable
  options={[
    [
      'url_context',
      'Boolean',
      'Habilita el contexto de URL de Google para que el modelo pueda leer las URL incluidas en el mensaje del usuario. Los enlaces de YouTube se convierten en entradas de comprensión de video nativas cuando es posible.',
      '',
    ],
  ]}
/>
...
Para los modelos Gemini 2.5, la configuración de Google es consciente del modelo: Pro acepta `128`-`32768`, Flash acepta `0`-`24576`, y Flash-Lite acepta `512`-`24576`. `-1` sigue siendo la configuración automática. Los modelos Gemini 3+ utilizan [`thinkingLevel`](#thinkinglevel) en lugar de un presupuesto numérico.
