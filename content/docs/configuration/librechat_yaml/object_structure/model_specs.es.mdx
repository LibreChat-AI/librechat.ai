...
El modelo puede iniciar una herramienta elegible, continuar trabajando y realizar sondeos con `check_background_task`. La salida de código completada y los archivos generados se adjuntan a la llamada de código original. El estado de la tarea en segundo plano es local al proceso y no sobrevive a un reinicio del servidor ni al movimiento entre réplicas.
