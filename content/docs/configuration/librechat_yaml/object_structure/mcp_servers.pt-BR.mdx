...
<Callout type="warning" title="Servidores baseados em processos são de propriedade do operador">
  Servidores MCP que usam `type: stdio` ou campos de processo como `command`, `args`, `env`, `cwd` ou `stderr` devem ser configurados no `librechat.yaml`. A API de configuração do administrador rejeita entradas baseadas em processo, e substituições no banco de dados ou por principal não podem substituir ou remover um servidor baseado em processo existente. Servidores MCP remotos continuam a seguir a precedência normal de substituição de configuração.
</Callout>
...
<Callout type="warning" title="Use nomes únicos seguros para normalização">
  As chaves de ferramenta voltadas para o modelo preservam letras, números, `_`, `.` e `-`; outros caracteres em um nome de servidor são substituídos por `_`. O LibreChat mapeia essas chaves normalizadas de volta para o nome configurado e corrige referências de ferramentas de Agent salvas anteriormente. Evite dois nomes que sejam normalizados para o mesmo valor, como `Sales Force` e `Sales:Force`: suas chaves de ferramenta são ambíguas, portanto, o LibreChat emite um aviso e exclui as ferramentas do servidor sombreado em vez de arriscar rotear uma chamada para o servidor errado.
</Callout>
...
<OptionTable
  options={[
    ...
    [
      'chatMenu',
      'Boolean',
      '(Opcional) Quando `false`, exclui o servidor MCP do seletor de chat regular e o rejeita de seleções de chat fornecidas pelo seletor. Agentes salvos e Model Specs atribuídas explicitamente ainda podem usá-lo. O padrão é `true`.',
      'chatMenu: false',
    ],
    [
      'serverInstructions',
...
- **Validação:** Deve começar com uma letra ou número Unicode. Os caracteres restantes podem incluir letras Unicode, números, marcas de combinação, espaços, hifens e apóstrofos retos ou curvos.
- **Exemplo:**
  ```yaml
  my-server:
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Carimbo de data/hora da aceitação dos termos               |

**Nota:** Campos ausentes serão substituídos por strings vazias.

Os placeholders `{{LIBRECHAT_BODY_*}}` são de escopo de requisição. O LibreChat cria a conexão MCP para a execução ativa, reutiliza-a entre chamadas de ferramentas nessa execução e a encerra quando a requisição termina. Servidores de escopo de requisição são excluídos do cache persistente de ferramentas para que cabeçalhos e URLs específicos da requisição não sejam reutilizados fora da execução ativa. O Agent Builder anexa o servidor completo através de uma seleção de runtime-tools e resolve suas ferramentas após uma execução fornecer os valores do corpo (body). A conclusão do OAuth armazena a autorização, mas adia a conexão até que esse contexto de requisição exista. Os placeholders `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_OPENID_*}}` e `{{LIBRECHAT_GRAPH_*}}` ainda tornam o servidor de escopo de usuário, mas os transportes HTTP atualizam os cabeçalhos resolvidos antes de cada chamada de ferramenta sem forçar uma reconexão por conta própria.

`{{LIBRECHAT_BODY_PARENTMESSAGEID}}` recebe a mensagem pai do turno raiz em chats de Agentes nativos, solicitações da API de Agentes compatível com OpenAI e execuções de Assistants legados. Solicitações de Open Responses não possuem um modelo de mensagem pai compatível, portanto, um servidor que exija este placeholder falhará de forma fechada nessa interface; `previous_response_id` não é substituído por ele.

Os tokens de acesso e de ID do OpenID expiram de forma independente. `{{LIBRECHAT_OPENID_ACCESS_TOKEN}}` e o legado `{{LIBRECHAT_OPENID_TOKEN}}` usam a expiração do access-token armazenado, enquanto `{{LIBRECHAT_OPENID_ID_TOKEN}}` requer um ID token atual. Se um token de credencial estiver indisponível ou expirado, o LibreChat retorna um erro de reautenticação `401` acionável em vez de enviar um valor bearer vazio ou literal; faça login novamente antes de tentar novamente. Para uma solicitação sem identidade OpenID, um cabeçalho contendo um desses placeholders de credencial não resolvidos é omitido completamente. Placeholders de metadados de identidade mantêm seu comportamento existente de valor vazio.

Operadores confiáveis podem passar o token de acesso OpenID ativo do usuário conectado diretamente para um servidor MCP remoto definido em YAML:
...
Use o passthrough direto apenas quando o servidor MCP for confiável para receber o token de acesso upstream do usuário. OAuth MCP explícito, [troca em nome de](#obo) e autenticação por chave de API de administrador têm precedência sobre este cabeçalho. O LibreChat resolve o token de sessão ativa antes de conectar e atualiza a conexão uma vez após um `401` ou `403`. Ele não repete automaticamente a chamada de ferramenta rejeitada, pois o servidor pode já tê-la executado; tente novamente a ferramenta de forma deliberada após a recuperação ser bem-sucedida. Se o token não puder ser atualizado, o usuário deve entrar novamente.
...
- **Tipo:** Objeto (Opcional, para os tipos `sse` e `streamable-http`)
- **Descrição:** Headers resolvidos apenas para uma solicitação de chat ativa. Eles usam os mesmos placeholders que `headers`, são mesclados sobre `headers` e prevalecem quando ambos os objetos definem o mesmo nome.
- **Comportamento de descoberta:** A descoberta de ferramentas omite `requestHeaders` completamente, portanto, valores exclusivos de solicitação como `{{LIBRECHAT_BODY_*}}` não ocultam o catálogo de ferramentas do servidor. A execução ativa resolve e os envia quando se conecta.
...
- **Descrição:** Quando definido como `false`, exclui o servidor MCP do seletor de chat regular e de solicitações de chat fornecidas pelo seletor. Ele permanece disponível para Agentes salvos e Model Specs que o atribuam explicitamente.
- **Valor Padrão:** `true` (O servidor MCP pode ser selecionado no chat comum)
...
- **Notas:**
  ...
  - O LibreChat armazena o texto anunciado pelo servidor separadamente da declaração configurada. A inspeção não substitui `true` ou uma string criada pelo operador, e as entradas de registro baseadas em Redis de builds anteriores são atualizadas automaticamente durante a inicialização.
- **Exemplo:**
...
#### Coordenação de atualização de OAuth [#oauth-refresh-coordination]

Para implantações com múltiplas réplicas, `oauthRefreshCoordination: true` serializa a atualização de token através da instância Redis compartilhada. `oauthRefreshWaitTimeout` controla por quanto tempo uma réplica aguarda pelo resultado da atualização, e `oauthPersistenceWaitTimeout` controla por quanto tempo ela aguarda para que as credenciais atualizadas se tornem legíveis; ambos possuem o padrão de `15000` milissegundos.

Habilite a coordenação apenas após cada réplica ser atualizada e conectada à mesma implementação do Redis. Use valores idênticos em cada réplica, drene as solicitações ativas antes de alterar a configuração e evite alterná-la durante uma implantação contínua (rolling deployment).
...
- **Subchaves Opcionais:**
  ...
  - `token_exchange_method`: String - Substituição opcional de troca de token. Use `default_post` para enviar credenciais do cliente no corpo POST codificado por formulário, ou `basic_auth_header` para enviá-las em um cabeçalho de autorização HTTP Basic. Quando omitido, o LibreChat descobre os métodos de autenticação de endpoint de token anunciados pelo provedor e utiliza a autenticação Basic como padrão se nenhum for anunciado.
...
Quando `token_exchange_method` e `token_endpoint_auth_methods_supported` são ambos omitidos para um client secret pré-configurado, o LibreChat permite até cinco segundos para a descoberta de metadados do recurso protegido e do servidor de autorização. Ele usa as capacidades anunciadas apenas quando o endpoint de token descoberto corresponde exatamente à `token_url` configurada; a descoberta não pode redirecionar as credenciais de cliente configuradas para outro endpoint. Defina `token_exchange_method` explicitamente quando um provedor não publicar metadados utilizáveis ou precisar de um método forçado.
