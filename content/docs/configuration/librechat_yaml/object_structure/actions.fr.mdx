...
Lorsqu'une Action est créée à partir de métadonnées OpenAPI, LibreChat vérifie également que le domaine de l'Action soumis correspond à l'URL du serveur de la spécification. Si le domaine soumis inclut un port explicite, il doit correspondre au port effectif de l'URL du serveur (`443` pour HTTPS ou `80` pour HTTP lorsque l'URL en omet un).
...
`allowedDomains` est une liste blanche stricte : lorsqu'elle est définie, **seules** les entrées listées sont accessibles. Ajouter une IP privée pour autoriser, par exemple, une API interne auto-hébergée bloque également tous les points de terminaison d'action publics que vous n'avez pas aussi listés.

`allowedAddresses` est utilisé uniquement lorsque `allowedDomains` n'est pas configuré. Il autorise des cibles privées spécifiques `host:port` tout en laissant le reste de l'internet public accessible via la politique SSRF par défaut.
...
- **Noms d'hôte avec port** : `host.docker.internal:11434`, `ollama.internal:8080`, `localhost:11434`
