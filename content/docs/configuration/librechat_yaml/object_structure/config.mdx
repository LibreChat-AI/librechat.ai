...
<OptionTable
  options={[
    [
      'maxWriteAttempts',
      'Integer',
      'Maximum compare-and-swap attempts for each ACL document, including the initial write. Only write conflicts are retried.',
      '3',
    ],
  ]}
/>

Valid values range from `1` through `100`.
...
- **Notes**:
  ...
  - Images and avatars need persistent, stable URLs to render correctly across the UI. S3 presigned URLs expire (AWS cap: 7 days for IAM users, hours for STS/role-based credentials), causing broken images in the model selector and chat UI. See the [related discussion](https://github.com/LibreChat-AI/LibreChat/discussions/10280#discussioncomment-14803903) for full context. Use `"cloudfront"` or `"firebase"` for `avatar` and `image` to avoid this.
...
- **Notes**:
  ...
  - You can find the names of the tools to filter in [`api/app/clients/tools/manifest.json`](https://github.com/LibreChat-AI/LibreChat/blob/main/api/app/clients/tools/manifest.json)
...
<OptionTable
  options={[
    ...
    [
      'skills',
      'Object',
      'Settings for Skill imports, such as the maximum upload size for `.md`, `.zip`, and `.skill` files.',
      '',
    ],
    [
      'fileTokenLimit',
...
<OptionTable
  options={[
    ...
    [
      'catalogRecovery',
      'Object',
      'Bounds passive MCP discovery, OAuth recovery, and cross-replica authorization fencing. See the MCP Settings reference for fields and defaults.',
      '',
    ],
  ]}
/>
