...
<OptionTable
  options={[
    [
      'langfuse',
      'Object',
      'Configures tenant-scoped Langfuse tracing and optional fanout export.',
      '',
    ],
  ]}
/>
...
<OptionTable
  options={[
    ...
    ['publicKey', 'String', 'Langfuse project public key for tenant export.', ''],
    ['secretKey', 'String', 'Langfuse project secret key for tenant export.', ''],
    [
      'destination',
      'String',
      'Selects one deployment-allowed tenant destination key.',
      '',
    ],
    ['fanout.enabled', 'Boolean', 'Enables tenant fanout for this config scope.', ''],
  ]}
/>

`displaySecretKey` is generated for redacted administrator display and should not be configured directly. See [Langfuse Configuration](/docs/configuration/langfuse#tenant-fanout-optional) for deployment variables, Compose and Helm setup, secret handling, and routing behavior.
