...
See [Langfuse Configuration](/docs/configuration/langfuse) for availability rules, environment-managed credentials, and optional fanout deployment.
...
<OptionTable
  options={[
    ...
    [
      'allowedAddresses',
      'Array of Strings',
      'Validated private host:port entries reserved for OCR SSRF exemptions. Accepted and preserved in v0.8.8-rc1, but not yet enforced by OCR requests.',
      '',
    ],
  ]}
/>
...
Both `speech.tts` and `speech.stt` accept an `allowedAddresses` array of validated private host:port entries. In v0.8.8-rc1 these values are preserved but do not yet change outbound speech request authorization. See the detailed Speech reference for entry rules and examples.
