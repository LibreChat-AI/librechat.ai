...
Si la sélection enregistrée de l'utilisateur pointe vers un Agent qui ne peut plus être résolu, LibreChat réactive la valeur par défaut souple afin qu'une nouvelle discussion dispose toujours d'une sélection utilisable. Une erreur temporaire lors du chargement de la liste des Agents ne supprime pas le choix enregistré et ne déclenche pas ce repli.
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'Contrôle si la spécification complète du modèle apparaît dans le sélecteur de modèle et dans la configuration de démarrage du client.',
      'true',
    ],
  ]}
/>

Définissez `showInMenu: false` pour masquer une spec de la sélection interactive tout en la gardant disponible pour les appelants côté serveur de confiance qui envoient explicitement son `name` dans le champ `spec`. Ceci diffère de `showIconInMenu`, qui masque uniquement l'icône.
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Équipe l\'agent éphémère de la Model Specs avec des outils de mémoire.',
      'false',
    ],
  ]}
/>

Nécessite [User Memory](/docs/features/memory) configuré, la capacité `memory` au niveau de l'endpoint, et les autorisations de mémoire utilisateur. Lorsqu'elle est activée, le modèle peut enregistrer ou supprimer des mémoires structurées lorsque l'utilisateur en fait explicitement la demande.
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Permet au Model Specs de faire une pause pour poser jusqu\'à quatre questions connexes et de reprendre avec les réponses.',
      'false',
    ],
  ]}
/>

Nécessite la capacité `ask_user_question` au niveau de l'endpoint. Ceci est pris en charge dans les exécutions de chat d'Agent qui fournissent le flux de pause/reprise durable de LibreChat.
...
<OptionTable
  options={[
    [
      ...
      'Contrôle quels outils éligibles attachés à la Model Specs peuvent s\'exécuter en tant que tâches d\'arrière-plan.',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

Nécessite la fonctionnalité `run_in_background` au niveau de l'endpoint, activable sur demande. Avec cette fonctionnalité et `executeCode: true`, l'exécution de l'interpréteur de code et les appels shell sont éligibles à l'exécution en arrière-plan par défaut. Définissez `runInBackground: false` ou `[]` pour les désactiver, `true` pour activer tous les outils éligibles attachés à la spécification, ou un tableau d'identifiants d'outils résolus pour n'activer que ces outils. La sélection de `execute_code` ou `bash_tool` sélectionne les deux outils de code natifs ; une liste qui les omet les désactive. Les sélections non prises en charge ou mal orthographiées sont consignées dans les journaux.

Le modèle peut démarrer un outil éligible et continuer à travailler. Pour les Agents enregistrés, les complétions de contenu uniquement prises en charge sont livrées automatiquement par défaut ; `check_background_task` reste disponible pour le statut, le contrôle, les artefacts en direct et la récupération. La sortie de code terminée et les fichiers générés sont joints à l'appel de code original. L'exécution ordinaire est locale au processus et ne survit pas à la perte de son processus de travail, mais un résultat de terminal persisté peut être livré depuis une autre réplique. Les administrateurs peuvent restaurer le comportement de sondage uniquement avec [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks).
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'Contrôle quels outils éligibles diffusent une intention écrite par le modèle en tant qu\'étiquette d\'appel d\'outil en direct.',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

Nécessite la fonctionnalité `tool_intents` au niveau de l'endpoint, activable sur demande. Les outils natifs tels que la recherche web, la création de fichiers, la mémoire et les outils Code Interpreter pris en charge utilisent des étiquettes d'intention par défaut lorsque la fonctionnalité est activée. Définissez `describeIntent: true` pour activer tous les outils éligibles, ou fournissez les identifiants d'outils résolus pour activer uniquement ces outils et désactiver les étiquettes pour tout le reste. Un tableau vide désactive toutes les étiquettes d'intention pour la spec. Omettre le champ ou le définir sur `false` n'ajoute aucune sélection plus large, les valeurs par défaut natives continuent donc de s'appliquer.

Les étiquettes d'intention sont incluses dans les schémas d'outils et ajoutent donc des jetons de schéma aux requêtes des modèles. LibreChat enregistre les sélections qui sont mal orthographiées, indisponibles pour la spécification ou inéligibles.
...
- `agent_ids` : permet d'utiliser des agents persistants spécifiques en tant que sous-agents supplémentaires. Cette liste est limitée par la limite effective de sous-agents, qui est définie par défaut à 10 et est configurable via [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents). La liste reste côté serveur ; la configuration de démarrage envoyée aux clients inclut uniquement les indicateurs publics `enabled` et `allowSelf`.
...
Les paramètres des modèles Google exposent désormais ce contrôle dans le schéma partagé, afin que les administrateurs puissent choisir explicitement si les fichiers sont renvoyés lors des tours ultérieurs pour les Model Specs de Google et de Vertex AI.
...
Le serveur résout également la Model Specs effective et autorise son `agent_id` persistant avant de charger ou d'exécuter cet Agent. Un appelant ne peut pas utiliser les champs de requête ou une inadéquation de préréglage pour substituer un Agent différent après cette vérification d'accès.
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'Contrôle le niveau d\'effort de raisonnement pour le modèle. Réduire l\'effort de raisonnement peut entraîner des réponses plus rapides et une utilisation moindre de jetons pour le raisonnement. Les modèles OpenAI pris en charge peuvent exposer `xhigh` et `max` ; pour Bedrock, les valeurs acceptées sont `low`, `medium`, `high`.',
      '',
    ],
  ]}
/>
...
- `""` (chaîne vide - non défini)
- `"standard"`
- `"pro"`

> **Pris en charge par :** les modèles de l'API OpenAI Responses qui exposent le mode de raisonnement, y compris GPT-5.6

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'Définit le mode de raisonnement de l\'API OpenAI Responses.', ''],
  ]}
/>
...
> **Pris en charge par :** les modèles de l'API OpenAI Responses qui exposent le contexte de raisonnement, y compris GPT-5.6
...
<OptionTable
  options={[
    [
      'reasoning_context',
      'String',
      'Contrôle la quantité de contexte conversationnel disponible pour le raisonnement du modèle.',
      '',
    ],
  ]}
/>
...
Pour GPT-5.6, LibreChat utilise par défaut l'API Responses pour les requêtes de raisonnement, sauf si `useResponsesApi: false` est défini explicitement.
...
Pour les modèles Gemini 2.5, le paramètre Google est conscient du modèle : Pro accepte `128`-`32768`, Flash accepte `0`-`24576`, et Flash-Lite accepte `512`-`24576`. `-1` reste le paramètre automatique. Les modèles Gemini 3+ utilisent [`thinkingLevel`](#thinkinglevel) au lieu d'un budget numérique.
