...
<OptionTable
  options={[
    ['allowedDomains', 'Array of Strings', 'Eine Liste zur Festlegung zulässiger Domains für Agenten-/Assistenten-Aktionen.', 'When configured, only listed domains are allowed. When not configured, SSRF targets are blocked but all other domains are allowed.'],
  ]}
/>
...
Wenn eine Action aus OpenAPI-Metadaten erstellt wird, überprüft LibreChat außerdem, ob die übermittelte Action-Domain mit der Server-URL der Spezifikation übereinstimmt. Wenn die übermittelte Domain einen expliziten Port enthält, muss dieser mit dem effektiven Port der Server-URL übereinstimmen (`443` für HTTPS oder `80` für HTTP, wenn der Port in der URL weggelassen wurde).
...
`allowedDomains` ist eine strikte Whitelist: Wenn sie festgelegt ist, sind **nur** die aufgeführten Einträge erreichbar. Das Hinzufügen einer privaten IP-Adresse, um beispielsweise eine selbst gehostete interne API zuzulassen, blockiert auch jeden öffentlichen Aktions-endpoint, den Sie nicht ebenfalls aufgelistet haben.
