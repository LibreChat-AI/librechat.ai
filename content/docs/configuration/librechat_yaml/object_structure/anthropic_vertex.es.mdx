...
> **Importante:** Claude Opus 4.7 y versiones posteriores, Opus 5, Sonnet 5 y Fable/Mythos 5 requieren `global`, `us` o `eu`. Las regiones específicas como `us-east5` sirven a Claude Sonnet 4.6 y versiones anteriores, y devolverán una respuesta de modelo no encontrado (model-not-found) para los modelos más recientes. Utilice `global` para obtener la mayor disponibilidad y evitar primas de precios regionales, o `us`/`eu` cuando necesite residencia de datos multirregional.
...
Cuando se omite `models`, LibreChat utiliza su catálogo integrado de Vertex y elimina los modelos incompatibles con la `region` configurada. Una lista o mapa de `models` explícito nunca se filtra, por lo que cada ID de despliegue debe estar disponible en la ubicación seleccionada.
...
> **Nota:** Al utilizar variables de entorno, el mapeo de modelos no está disponible y LibreChat incluye su catálogo estándar de modelos de Anthropic. Establezca `ANTHROPIC_VERTEX_REGION` en `global`, `us` o `eu` antes de seleccionar un modelo moderno que no esté disponible en una región específica.
...
- Modelos: catálogo Vertex integrado de LibreChat para la ubicación seleccionada
...
- Use `global`, `us`, o `eu` para Opus 4.7+, Opus 5, Sonnet 5 y Fable/Mythos 5
...
- Cambie a `global`, `us` o `eu` para modelos más recientes que Sonnet 4.6
