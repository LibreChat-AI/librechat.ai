...
4. **Service Account Key** (archivo JSON) descargado y accesible para LibreChat
...
Utilice los IDs de modelo de Vertex AI reales directamente. Estos se mostrarán tal cual en la interfaz de usuario:
...
Asignar nombres fáciles de usar a los nombres de despliegue de Vertex AI:
...
Configuración completa con nombres y títulos de modelo personalizados:
...
Solo puede configurar una región por despliegue. Para necesidades multirregión, considere usar instancias de LibreChat separadas o endpoints personalizados.
...
- Asegúrate de que el modelo esté habilitado en tu Vertex AI Model Garden

### Problemas de región [#region-issues]

**"Región no válida" o "Región no admitida"**
- Utilice una de las regiones admitidas enumeradas anteriormente
...
- Considere utilizar una región diferente que tenga una mayor disponibilidad de modelos (por ejemplo, `us-east5`)
...
- Vertex AI y la API directa de Anthropic son mutuamente excluyentes. Cuando una sección de configuración `vertex` está presente, la variable de entorno `ANTHROPIC_API_KEY` es ignorada.
...
- La llamada a funciones (function calling) y el uso de herramientas (tool use) funcionan igual que con la API directa de Anthropic.
