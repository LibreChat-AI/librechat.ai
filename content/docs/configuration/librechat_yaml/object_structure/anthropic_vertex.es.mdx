...
> **Importante:** Claude Opus 4.7 y versiones posteriores, Opus 5, Sonnet 5 y Fable/Mythos 5 o 5.1 requieren `global`, `us` o `eu`. Las regiones específicas como `us-east5` sirven a Claude Sonnet 4.6 y versiones anteriores, y devuelven una respuesta de modelo no encontrado para los modelos más recientes. Utilice `global` para obtener la mayor disponibilidad y sin recargos por precios regionales, o `us`/`eu` cuando necesite residencia de datos en múltiples regiones.
