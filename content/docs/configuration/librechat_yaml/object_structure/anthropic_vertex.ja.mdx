...
> **重要:** Claude Opus 4.7 以降、Opus 5、Sonnet 5、および Fable/Mythos 5 では、`global`、`us`、または `eu` が必要です。`us-east5` などの特定のリージョンは Claude Sonnet 4.6 以前のモデル用であり、新しいモデルを指定すると model-not-found レスポンスが返されます。最も幅広い可用性があり、リージョンごとの価格プレミアムが発生しない `global` を使用するか、マルチリージョンのデータレジデンシーが必要な場合は `us` または `eu` を使用してください。
