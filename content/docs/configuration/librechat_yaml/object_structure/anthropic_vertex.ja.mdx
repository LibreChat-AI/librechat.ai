...
> **重要:** Claude Opus 4.7 以降、Opus 5、Sonnet 5、および Fable/Mythos 5 または 5.1 では、`global`、`us`、または `eu` が必要です。`us-east5` などの特定のリージョンは Claude Sonnet 4.6 以前のモデルを提供しており、新しいモデルに対しては model-not-found レスポンスを返します。最も幅広い可用性を確保し、リージョン別の価格プレミアムを避けるには `global` を使用してください。マルチリージョンのデータレジデンシーが必要な場合は `us` または `eu` を使用してください。
...
`models` が省略された場合、LibreChat は組み込みの Vertex カタログを使用し、設定された `region` と互換性のないモデルを除外します。明示的な `models` リストまたはマップはフィルタリングされないため、すべてのデプロイメント ID は選択された場所で利用可能である必要があります。
...
> **注:** 環境変数を使用する場合、モデルマッピングは利用できず、LibreChatには標準のAnthropicモデルカタログが含まれます。特定のリージョンで利用できない最新のモデルを選択する前に、`ANTHROPIC_VERTEX_REGION` を `global`、`us`、または `eu` に設定してください。
...
- モデル: 選択したロケーションに対応するLibreChatの組み込みVertexカタログ
...
- Opus 4.7+、Opus 5、Sonnet 5、および Fable/Mythos 5 には `global`、`us`、または `eu` を使用してください。
...
- Sonnet 4.6より新しいモデルについては、`global`、`us`、または `eu` に切り替えてください。
