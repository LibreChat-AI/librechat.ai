...
Yönetici yapılandırma API'si aracılığıyla özel bir endpoint yazıldığında, LibreChat değişmez bir `apiKey` değerini bekleme durumunda (at rest) şifreler ve yalnızca bir `apiKeyPreview` döndürür. `${ENV_VAR}` referansları ve `user_provided` değerleri, okunabilir geçiş değerleri olarak kalmaya devam eder. Doğrudan `librechat.yaml` içinde tanımlanan endpoint'ler için, yukarıda gösterildiği gibi bir ortam değişkeni referansı kullanılması tercih edilir.
...
- OpenAI uyumlu özel endpoint'lerde, `claude` içeren model kimlikleri Claude belge kısıtlamalarını kullanır: PDF'ler ve metin belgeleri kabul edilirken, PDF olmayan ikili belge türleri istek gönderilmeden önce atlanır.
...
Son giden çözümleme adımında, değeri olmayan tanınmış `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` ve `{{LIBRECHAT_OPENID_*}}` yer tutucuları, şablon metni olarak iletilmek yerine boş dizelere dönüşür. Bilinmeyen yer tutucu adları ise yapılandırma hatalarının görülebilmesi için değiştirilmeden kalır.
...
Ajan modeli istekleri, özel endpoint başlıklarında `{{LIBRECHAT_USER_TENANT_ID}}` ve `{{LIBRECHAT_USER_TENANTID}}` ifadelerini de destekler. Her iki takma ad da potansiyel olarak güncelliğini yitirmiş bir kullanıcı kaydı yerine yetkili istek kapsamlı kiracıyı (tenant) kullanır, kök Ajanlar, Alt Ajanlar ve özel özetleme çağrıları için geçerlidir ve herhangi bir kiracı mevcut olmadığında boş bir dizgiye çözümlenir. Bunlar, Ajan şablonu tüketicisi olmayanlar için genel kullanıcı alanı yer tutucuları değildir.
