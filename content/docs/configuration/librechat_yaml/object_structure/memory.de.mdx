...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Konfiguriert den optionalen automatischen Memory-Agenten. Kann entweder ein Verweis auf einen bestehenden Agenten per ID oder eine vollständige Agentenkonfiguration sein.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Die automatische Extraktion ist optional. Setzen Sie `agent.enabled: true` in einem der beiden unten aufgeführten Konfigurationsformate. Wenn `enabled` weggelassen oder auf `false` gesetzt wird, bleiben der manuelle Speicher und die Agent-Speicher-Tools verfügbar, ohne dass der automatische Speicher-Agent bei Chat-Anfragen ausgeführt wird.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Aktiviert die automatische Speicherextraktion mit dem konfigurierten Memory-Agenten.',
      'enabled: true',
    ],
  ]}
/>
...
- Wenn `personalize` auf true gesetzt ist, erhalten Benutzer einen Schalter in ihrer Chat-Oberfläche, um die Speichernutzung zu steuern.
- Token-Limits helfen dabei, die Speicherauslastung und die Verarbeitungskosten zu kontrollieren.
- `maxInputTokens` begrenzt den Chat-Kontext, der an den automatischen Memory-Agent gesendet wird, während `tokenLimit` die Nutzung des gespeicherten Speichers steuert.
...
- `agent.enabled: true` ist für die automatische Speicherextraktion erforderlich
- Die Agent-Konfiguration ermöglicht die Anpassung des Verhaltens der automatischen Speicherverarbeitung.
