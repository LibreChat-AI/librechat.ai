...
<Callout type="info" title="Bu ayar yalnızca depolamayı kontrol eder, görüntülemeyi değil">

`transactions`, kullanım kayıtlarının **veritabanına yazılıp yazılmayacağına** karar verir. Uygulamanın hiçbir yerinde token sayılarını göstermez, bu nedenle yalnızca bu ayarı açmak token bilgilerinin arayüzde görünmesini sağlamayacaktır.

Token kullanımını *görmek* için, varsayılan olarak açık olan ve sohbet içi bağlam göstergesini çizen `interface.contextUsage` ayarını kullanın. `interface.contextCost`, varsayılan olarak kapalı olan ve bu göstergeye maliyet rakamlarını ekleyen ayrı bir ayardır; fiyatlandırmayı göstermeden sadece token sayılarını görmek istiyorsanız bu ayarı kapalı bırakın. Bkz. [Token Kullanımı](/docs/configuration/token_usage#viewing-context-usage-and-cost). Kullanıcı başına kredi limitlerini zorunlu kılmak için bkz. [Bakiye](/docs/configuration/librechat_yaml/object_structure/balance).
