...
O modelo pode iniciar uma ferramenta elegível, continuar trabalhando e realizar consultas com `check_background_task`. A saída de código concluída e os arquivos gerados são anexados à chamada de código original. O estado da tarefa em segundo plano é local ao processo e não sobrevive a uma reinicialização do servidor ou movimentação entre réplicas.
