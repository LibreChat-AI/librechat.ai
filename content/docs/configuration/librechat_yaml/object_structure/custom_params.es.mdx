...
#### Valores aceptados [#accepted-values]

`defaultParamsEndpoint` selecciona qué conjunto de parámetros integrados renderiza el panel del endpoint. Estos son los valores que se resuelven a uno:

<OptionTable
  options={[
    ['custom', 'String', 'Conjunto de parámetros de OpenAI. Este es el esquema predeterminado y el que utiliza un endpoint sin `provider` como alternativa.', 'Default'],
    ['openAI', 'String', 'Conjunto de parámetros de OpenAI.', ''],
    ['azureOpenAI', 'String', 'Conjunto de parámetros de OpenAI.', ''],
    ['anthropic', 'String', 'Conjunto de parámetros de Anthropic.', ''],
    ['google', 'String', 'Conjunto de parámetros de Google.', ''],
    ['openrouter', 'String', 'Conjunto de parámetros de OpenRouter. En minúsculas, a diferencia de los otros.', ''],
  ]}
/>

Tenga en cuenta las mayúsculas y minúsculas: `openAI` y `azureOpenAI` están en camelCase, pero `openrouter` está todo en minúsculas.

<Callout type="info" title="Un ajuste del proveedor proporciona este valor para usted">

Si el endpoint personalizado establece `provider`, LibreChat completa `defaultParamsEndpoint` a partir de él, por lo que el valor predeterminado efectivo es el conjunto de parámetros del proveedor en lugar de `custom`. Esa sustitución solo ocurre cuando dejas el campo vacío o lo dejas en `custom`; cualquier otro valor que establezcas explícitamente prevalecerá. Por lo tanto, un endpoint con `provider: 'anthropic'` comienza con el conjunto de parámetros de Anthropic sin que tengas que escribir `defaultParamsEndpoint` en absoluto.

</Callout>

Los valores `assistants`, `azureAssistants`, `agents` y `bedrock` también son reconocidos cuando LibreChat resuelve los esquemas de conversación y ajustes preestablecidos, pero ninguno de ellos se asigna a un conjunto de parámetros que un endpoint personalizado pueda renderizar, por lo que establecer uno deja el panel vacío.

<Callout type="warning" title="Los valores no reconocidos fallan silenciosamente">

El campo es una cadena de texto de formato libre, no un enum cerrado, por lo que un valor fuera de esta lista pasa la validación de configuración. Simplemente no coincide con ningún conjunto de parámetros y el panel del endpoint se renderiza **sin ningún parámetro** en lugar de informar un error. Si su panel de parámetros está vacío inesperadamente, verifique este valor primero.
...
#### Rangos de Sentinel [#sentinel-ranges]

Utilice `range.positiveMin` cuando `range.min` sea un valor centinela especial, pero los valores regulares tengan un límite inferior más alto. Por ejemplo, un presupuesto de pensamiento puede usar `-1` para el comportamiento automático, mientras acepta presupuestos explícitos solo desde `128` hasta `32768`:
...
Con `positiveMin`, los únicos valores aceptados son el centinela `min` o valores desde `positiveMin` hasta `max`; los valores intermedios no son válidos. `positiveMin` no puede exceder `max`, y el valor predeterminado debe ser `min` o al menos `positiveMin`.
...
- Flags de características específicas del modelo basados en el modelo Claude que se esté utilizando

Para endpoints nativos compatibles con Anthropic, prefiera `provider: 'anthropic'` en el endpoint personalizado. Esto enruta los agentes, la sumarización, la gestión de presupuesto de tokens/contexto y los parámetros predeterminados a través de la ruta del proveedor de Anthropic.

### Reproducción de razonamiento [#reasoning-replay]

Algunas pasarelas de razonamiento compatibles con OpenAI requieren que el `reasoning_content` del proveedor se reproduzca en los turnos de llamadas a herramientas del asistente. Utilice flags de endpoint personalizados para optar por esta opción solo en los proveedores que requieran este comportamiento:
...
Establezca `includeReasoningHistory: true` solo cuando el proveedor también requiera que LibreChat reconstruya el `reasoning_content` a partir del historial de conversación persistido en turnos posteriores. Esto implica `includeReasoningContent`.
