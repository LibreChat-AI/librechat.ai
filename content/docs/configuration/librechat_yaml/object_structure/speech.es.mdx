---
title: Configuración de voz
icon: Mic
---

## Descripción general [#overview]

El objeto `speech` le permite configurar proveedores de Texto a Voz (TTS) y Voz a Texto (STT) directamente en su archivo de configuración `librechat.yaml`. Esto habilita servicios de voz del lado del servidor sin requerir que los usuarios configuren sus propias claves de API.

**Campos bajo `speech`:**

- `tts` - Configuraciones del proveedor de Text-to-Speech
- `stt` - Configuraciones del proveedor de Speech-to-Text
- `speechTab` - Configuración predeterminada de la interfaz de usuario para las funciones de voz

**Notas:**

- Se pueden configurar múltiples proveedores simultáneamente
- Los usuarios pueden seleccionar su proveedor preferido de las opciones disponibles
- Las API keys en el archivo de configuración deben usar referencias a variables de entorno por seguridad

## Ejemplo [#example]

```yaml filename="speech"
speech:
  tts:
    allowedAddresses: ["tts.internal:8080"]
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["voice-id-1", "voice-id-2"]
  stt:
    allowedAddresses: ["stt.internal:8080"]
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: true
    advancedMode: false
    speechToText: true
    textToSpeech: true
```

---

## Protección SSRF [#ssrf-protection]

LibreChat protege las URLs de STT y TTS proporcionadas por el operador en el momento de la conexión. Los destinos privados, de loopback, de enlace local y de metadatos en la nube están bloqueados de forma predeterminada, y las redirecciones están deshabilitadas para las solicitudes de voz.

`speech.tts.allowedAddresses` y `speech.stt.allowedAddresses` son listas de exención separadas, no listas de permitidos estrictas. Agregue un `host:port`, `private.ip:port` o `[ipv6]:port` privado exacto solo cuando esa sección deba acceder a un servicio autohospedado de confianza. No utilice una URL, ruta, rango CIDR, host/IP sin puerto o literal de IP pública. Los puertos predeterminados se normalizan, por lo que una URL HTTPS sin un puerto explícito se verifica como puerto `443`.

Un nombre de host exento confía en cualquier dirección privada a la que se resuelva en ese puerto. Prefiera un literal de IP privada cuando sea posible, y enumere solo los nombres cuyo DNS usted controle y que no puedan ser redirigidos por un atacante.

Cuando un proxy de reenvío (forward proxy) gestiona una solicitud de nombre de host, el proxy realiza la resolución DNS de destino y la salida (egress), por lo que el proxy debe aplicar su propia política SSRF. Un destino privado literal sigue siendo bloqueado antes del manejo del proxy, a menos que su dirección y puerto exactos estén exentos.

---

## tts

El objeto `tts` configura los proveedores de Text-to-Speech. Se pueden configurar múltiples proveedores y los usuarios pueden elegir cuál utilizar.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'Exenciones de host:puerto privado de confianza para comprobaciones SSRF en el momento de la conexión TTS. Los destinos públicos permanecen disponibles.',
      'allowedAddresses: ["tts.internal:8080"]',
    ],
  ]}
/>

### openai

Configuración de OpenAI TTS utilizando modelos como `tts-1` o `tts-1-hd`.

<OptionTable
  options={[
    ['url', 'String', 'URL de API personalizada (opcional). Úselo para endpoints compatibles con OpenAI.', ''],
    ['apiKey', 'String', 'Clave de API de OpenAI. Utilice la referencia de variable de entorno.', 'Required'],
    ['model', 'String', 'Modelo TTS a utilizar (p. ej., "tts-1", "tts-1-hd").', 'Required'],
    ['voices', 'Array of Strings', 'Opciones de voz disponibles para que los usuarios seleccionen.', 'Required'],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / tts / openai"
tts:
  openai:
    apiKey: "${TTS_API_KEY}"
    model: "tts-1"
    voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
```

### azureOpenAI

Configuración de Azure OpenAI TTS.

<OptionTable
  options={[
    ['instanceName', 'String', 'Nombre de la instancia de Azure OpenAI.', 'Required'],
    ['apiKey', 'String', 'Clave de API de Azure OpenAI.', 'Required'],
    ['deploymentName', 'String', 'El nombre de despliegue para el modelo TTS.', 'Required'],
    ['apiVersion', 'String', 'Versión de la API de Azure OpenAI.', 'Required'],
    ['model', 'String', 'Identificador del modelo TTS.', 'Required'],
    ['voices', 'Array of Strings', 'Opciones de voz disponibles.', 'Required'],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / tts / azureOpenAI"
tts:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_TTS_API_KEY}"
    deploymentName: "tts-deployment"
    apiVersion: "2024-02-15-preview"
    model: "tts-1"
    voices: ["alloy", "echo", "nova"]
```

### elevenlabs

Configuración de ElevenLabs TTS para síntesis de voz de alta calidad.

<OptionTable
  options={[
    ['url', 'String', 'URL de API personalizada (opcional).', ''],
    ['websocketUrl', 'String', 'URL de WebSocket para streaming (opcional).', ''],
    ['apiKey', 'String', 'Clave de API de ElevenLabs.', 'Required'],
    ['model', 'String', 'Modelo de ElevenLabs (p. ej., "eleven_multilingual_v2").', 'Required'],
    ['voices', 'Array of Strings', 'IDs de voz disponibles para selección.', 'Required'],
    ['voice_settings', 'Object', 'Configuración de personalización de voz (opcional).', ''],
    ['pronunciation_dictionary_locators', 'Array of Strings', 'IDs del diccionario de pronunciación (opcional).', ''],
  ]}
/>

**Sub-claves de voice_settings:**
<OptionTable
  options={[
    ['similarity_boost', 'Number', 'Mejora de similitud de voz (0-1).', ''],
    ['stability', 'Number', 'Estabilidad de voz (0-1).', ''],
    ['style', 'Number', 'Exageración de estilo (0-1).', ''],
    ['use_speaker_boost', 'Boolean', 'Habilitar el aumento de altavoz.', ''],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / tts / elevenlabs"
tts:
  elevenlabs:
    apiKey: "${ELEVENLABS_API_KEY}"
    model: "eleven_multilingual_v2"
    voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
    voice_settings:
      stability: 0.5
      similarity_boost: 0.75
      use_speaker_boost: true
```

### localai

Configuración de LocalAI TTS para síntesis de voz autohospedada.

<OptionTable
  options={[
    ['url', 'String', 'URL del servidor LocalAI.', 'Required'],
    ['apiKey', 'String', 'Clave de API si la autenticación está habilitada (opcional).', ''],
    ['voices', 'Array of Strings', 'Modelos de voz disponibles.', 'Required'],
    ['backend', 'String', 'Backend de TTS a utilizar (p. ej., "piper").', 'Required'],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / tts / localai"
tts:
  localai:
    url: "http://localhost:8080"
    voices: ["en-us-amy-low", "en-us-danny-low"]
    backend: "piper"
```

---

## stt

El objeto `stt` configura los proveedores de Speech-to-Text.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'Exenciones de host:puerto privado de confianza para comprobaciones SSRF en el momento de conexión STT. Los destinos públicos permanecen disponibles.',
      'allowedAddresses: ["stt.internal:8080"]',
    ],
  ]}
/>

### openai [#openai-1]

Configuración de OpenAI Whisper STT.

<OptionTable
  options={[
    ['url', 'String', 'URL de API personalizada (opcional). Úselo para endpoints compatibles con OpenAI.', ''],
    ['apiKey', 'String', 'Clave de API de OpenAI. Utilice la referencia de variable de entorno.', 'Required'],
    ['model', 'String', 'Modelo STT a utilizar (p. ej., "whisper-1").', 'Required'],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / stt / openai"
stt:
  openai:
    apiKey: "${STT_API_KEY}"
    model: "whisper-1"
```

### azureOpenAI [#azureopenai-1]

Configuración de Azure OpenAI Whisper STT.

<OptionTable
  options={[
    ['instanceName', 'String', 'Nombre de la instancia de Azure OpenAI.', 'Required'],
    ['apiKey', 'String', 'Clave de API de Azure OpenAI.', 'Required'],
    ['deploymentName', 'String', 'El nombre de despliegue para el modelo Whisper.', 'Required'],
    ['apiVersion', 'String', 'Versión de la API de Azure OpenAI.', 'Required'],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / stt / azureOpenAI"
stt:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_STT_API_KEY}"
    deploymentName: "whisper-deployment"
    apiVersion: "2024-02-15-preview"
```

---

## speechTab

El objeto `speechTab` configura los ajustes predeterminados de la interfaz de usuario para las funciones de voz. Estos ajustes controlan lo que los usuarios ven de forma predeterminada en el panel de configuración de voz.

<OptionTable
  options={[
    ['conversationMode', 'Boolean', 'Habilitar el modo de conversación de forma predeterminada.', 'false'],
    ['advancedMode', 'Boolean', 'Mostrar la configuración avanzada de voz de forma predeterminada.', 'false'],
    ['speechToText', 'Boolean or Object', 'Habilitar STT de forma predeterminada o configurar ajustes detallados de STT.', 'false'],
    ['textToSpeech', 'Boolean or Object', 'Habilitar TTS de forma predeterminada o configurar ajustes detallados de TTS.', 'false'],
  ]}
/>

### speechToText (Formato de objeto) [#speechtotext-object-format]

Cuando se utiliza un objeto en lugar de un booleano:

<OptionTable
  options={[
    [
      'engineSTT',
      'String',
      'Motor STT predeterminado. Use `"browser"` o `"external"`. Los valores heredados `"openai"` y `"azureOpenAI"` son aceptados y normalizados a `"external"`.',
      'browser',
    ],
    ['languageSTT', 'String', 'Idioma predeterminado para STT.', ''],
    ['autoTranscribeAudio', 'Boolean', 'Mantener el micrófono escuchando en lugar de detenerse en la primera pausa. Con un motor externo, también activa la detección de silencio basada en `decibelValue`.', ''],
    ['decibelValue', 'Number', 'Umbral de decibelios para la detección de silencio. Rango de -100 a -30, valor predeterminado -45.', ''],
    ['autoSendText', 'Number', 'Segundos a esperar después de la transcripción antes del envío automático. `0` envía inmediatamente; `-1` deshabilita el envío automático.', ''],
  ]}
/>

### textToSpeech (Formato de objeto) [#texttospeech-object-format]

Cuando se utiliza un objeto en lugar de un booleano:

<OptionTable
  options={[
    [
      'engineTTS',
      'String',
      'Motor de TTS predeterminado. Use `"browser"` o `"external"`. Se aceptan valores de proveedores heredados y se normalizan a `"external"`.',
      'browser',
    ],
    ['voice', 'String', 'Selección de voz predeterminada.', ''],
    ['languageTTS', 'String', 'Idioma predeterminado para TTS.', ''],
    ['automaticPlayback', 'Boolean', 'Reproducir automáticamente las respuestas TTS.', ''],
    ['playbackRate', 'Number', 'Velocidad de reproducción predeterminada (1.0 = normal). Rango: 0.25–4.0.', ''],
    ['cacheTTS', 'Boolean', 'Almacenar en caché el audio TTS para su reproducción repetida.', ''],
  ]}
/>

**Ejemplo:**
```yaml filename="speech / speechTab"
speechTab:
  conversationMode: false
  advancedMode: false
  speechToText:
    engineSTT: "external"
    autoTranscribeAudio: true
    decibelValue: -45
  textToSpeech:
    engineTTS: "external"
    voice: "nova"
    automaticPlayback: false
    playbackRate: 1.0
    cacheTTS: true
```

---

## Ejemplo completo [#complete-example]

```yaml filename="librechat.yaml"
version: 1.3.15
cache: true

speech:
  tts:
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1-hd"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
      voice_settings:
        stability: 0.5
        similarity_boost: 0.75
  stt:
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: false
    advancedMode: false
    speechToText: true
    textToSpeech:
      engineTTS: "external"
      voice: "nova"
      automaticPlayback: false
```

---

## Notas [#notes]

- Utilice siempre referencias a variables de entorno (p. ej., `${API_KEY}`) para las claves de API en los archivos de configuración
- Se pueden configurar múltiples proveedores de TTS; los usuarios seleccionan su opción preferida en la UI
- Los ajustes de `speechTab` definen los valores predeterminados que los usuarios pueden anular en su configuración personal
- `browser` utiliza el soporte de voz integrado del navegador; `external` utiliza los proveedores del lado del servidor configurados en `speech.stt` o `speech.tts`
- Los valores predeterminados existentes de STT para `openai` y `azureOpenAI`, y los valores predeterminados de TTS para `openai`, `azureOpenAI`, `elevenlabs` y `localai`, se migran a `external`; un motor externo no disponible recurre a `browser`
- Para obtener documentación detallada sobre las funciones, consulta [Speech to Text & Text to Speech](/docs/configuration/stt_tts)
