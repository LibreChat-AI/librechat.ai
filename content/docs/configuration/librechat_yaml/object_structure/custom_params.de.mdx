---
title: Benutzerdefinierte Parameter
icon: SlidersHorizontal
---

### Auswahl eines Standard-Parametersatzes [#picking-a-default-parameters-set]

Standardmäßig verwendet LibreChat die Standardparameter der OpenAI API, wenn Sie einen benutzerdefinierten endpoint in der Konfigurationsdatei `librechat.yaml` angeben. Sie können diese Standardwerte jedoch überschreiben, indem Sie das Feld `customParams.defaultParamsEndpoint` innerhalb der Definition Ihres benutzerdefinierten endpoint angeben. Um beispielsweise Google-Parameter für Ihren benutzerdefinierten endpoint zu verwenden:

```yaml filename="excerpt of librechat.yaml"
endpoints:
  custom:
    - name: 'Google Gemini'
      apiKey: ...
      baseURL: ...
      customParams:
        defaultParamsEndpoint: 'google'
```

Dein "Google Gemini" endpoint zeigt nun Parameter für die Google API an, wenn du einen neuen Agenten oder ein neues Preset erstellst.

#### Zulässige Werte [#accepted-values]

`defaultParamsEndpoint` wählt aus, welcher integrierte Parametersatz im Panel des endpoint gerendert wird. Dies sind die Werte, die zu einem aufgelöst werden:

<OptionTable
  options={[
    ['custom', 'String', 'OpenAI-Parametersatz. Dies ist der Schema-Standard und der Rückfallwert für einen endpoint ohne `provider`.', 'Default'],
    ['openAI', 'String', 'OpenAI-Parametersatz.', ''],
    ['azureOpenAI', 'String', 'OpenAI-Parametersatz.', ''],
    ['anthropic', 'String', 'Anthropic-Parametersatz.', ''],
    ['google', 'String', 'Google-Parametersatz.', ''],
    ['openrouter', 'String', 'OpenRouter-Parametersatz. Kleingeschrieben, im Gegensatz zu den anderen.', ''],
  ]}
/>

Beachten Sie die Groß-/Kleinschreibung: `openAI` und `azureOpenAI` sind in camelCase geschrieben, während `openrouter` komplett kleingeschrieben ist.

<Callout type="info" title="Eine Anbieter-Einstellung liefert diesen Wert für Sie">

Wenn der benutzerdefinierte endpoint `provider` festlegt, füllt LibreChat `defaultParamsEndpoint` automatisch damit aus, sodass der effektive Standardwert der Parametersatz des Anbieters anstelle von `custom` ist. Diese Ersetzung erfolgt nur, wenn Sie das Feld leer lassen oder auf `custom` belassen; jeder andere Wert, den Sie explizit festlegen, hat Vorrang. Ein endpoint mit `provider: 'anthropic'` beginnt daher mit dem Anthropic-Parametersatz, ohne dass Sie `defaultParamsEndpoint` überhaupt angeben müssen.

</Callout>

Die Werte `assistants`, `azureAssistants`, `agents` und `bedrock` werden ebenfalls erkannt, wenn LibreChat Konversations- und Preset-Schemas auflöst, aber keiner davon wird einem Parametersatz zugeordnet, den ein benutzerdefinierter endpoint rendern kann. Daher bleibt das Panel leer, wenn einer dieser Werte festgelegt wird.

<Callout type="warning" title="Nicht erkannte Werte schlagen still fehl">

Das Feld ist eine frei formulierbare Zeichenkette, kein geschlossenes Enum, daher passiert ein Wert außerhalb dieser Liste die Konfigurationsvalidierung. Er entspricht lediglich keinem Parametersatz, und das Panel des endpoint wird **ganz ohne Parameter** gerendert, anstatt einen Fehler zu melden. Wenn Ihr Parameter-Panel unerwartet leer ist, überprüfen Sie zuerst diesen Wert.

</Callout>

### Überschreiben von Parameterdefinitionen [#overriding-parameter-definitions]

Darüber hinaus können Sie auch die Parameter für Ihren benutzerdefinierten endpoint feinabstimmen. Zum Beispiel ist der `temperature`-Parameter für den google endpoint ein Schieberegler mit einem Bereich von 0.0 bis 1.0 und einem Standardwert von 1.0; Sie können die `librechat.yaml`-Datei aktualisieren, um diese Werte zu überschreiben:

```yaml filename="excerpt of librechat.yaml"
endpoints:
  custom:
    - name: 'Google Gemini'
      apiKey: ...
      baseURL: ...
      customParams:
        defaultParamsEndpoint: 'google'
        paramDefinitions:
          - key: temperature
            range:
              min: 0
              max: 0.7
              step: 0.1
            default: 0.5
```

Infolgedessen wird der `Temperature`-Schieberegler auf den Bereich von `0.0` bis `0.7` mit einer Schrittweite von `0.1` und einem Standardwert von `0.5` begrenzt. Die restlichen Parameter werden auf ihre Standardwerte gesetzt.

#### Sentinel-Bereiche [#sentinel-ranges]

Verwenden Sie `range.positiveMin`, wenn `range.min` ein spezieller Sentinel-Wert ist, reguläre Werte jedoch eine höhere Untergrenze haben. Zum Beispiel kann ein Thinking-Budget `-1` für automatisches Verhalten verwenden, während explizite Budgets nur von `128` bis `32768` akzeptiert werden:

```yaml filename="excerpt of librechat.yaml"
endpoints:
  custom:
    - name: 'My Gemini Gateway'
      apiKey: ...
      baseURL: ...
      customParams:
        paramDefinitions:
          - key: thinkingBudget
            type: number
            component: slider
            range:
              min: -1
              positiveMin: 128
              max: 32768
              step: 1
            default: -1
```

Bei `positiveMin` sind die einzigen akzeptierten Werte der Sentinel-Wert `min` oder Werte von `positiveMin` bis `max`; Werte dazwischen sind nicht zulässig. `positiveMin` darf `max` nicht überschreiten, und der Standardwert muss entweder `min` oder mindestens `positiveMin` sein.

### Festlegen von Standard-Parameterwerten [#setting-default-parameter-values]

Sie können Standardwerte für Parameter festlegen, die automatisch angewendet werden, wenn API-Anfragen gestellt werden. Dies ist nützlich, um Basis-Parameterwerte für Ihren benutzerdefinierten endpoint festzulegen, ohne dass Benutzer diese jedes Mal manuell konfigurieren müssen.

Das Feld `default` in `paramDefinitions` ermöglicht es Ihnen, Standardwerte festzulegen, die angewendet werden, wenn Parameter nicht definiert sind. Diese Standardwerte folgen einer Prioritätsreihenfolge, um ein korrektes Überschreibungsverhalten sicherzustellen:

**Prioritätsreihenfolge (niedrigste bis höchste):**

1. **Standardwerte aus `paramDefinitions`** - Werden zuerst angewendet, wenn der Parameter nicht definiert ist
2. **`addParams`** - Kann Standardwerte überschreiben
3. **Vom Benutzer konfigurierte `modelOptions`** – Höchste Priorität, überschreibt alles

```yaml filename="excerpt of librechat.yaml"
endpoints:
  custom:
    - name: 'My Custom LLM'
      apiKey: ...
      baseURL: ...
      customParams:
        defaultParamsEndpoint: 'openAI'
        paramDefinitions:
          - key: temperature
            default: 0.7
          - key: topP
            default: 0.9
          - key: maxTokens
            default: 2000
```

In diesem Beispiel:

- Wenn ein Benutzer keine `temperature` angibt, wird standardmäßig `0.7` verwendet.
- Wenn ein Benutzer `temperature` explizit auf `0.5` setzt, hat sein Wert (`0.5`) Vorrang.
- Das Feld `addParams` (falls konfiguriert) kann diese Standardwerte überschreiben
- Benutzerauswahlen in der UI haben immer die höchste Priorität

### Anthropic [#anthropic]

Es gibt zwei Anthropic-bezogene benutzerdefinierte endpoint-Modi:

- `provider: 'anthropic'` auf dem benutzerdefinierten endpoint verwendet den nativen Anthropic `/v1/messages` Client. Verwenden Sie dies für Anthropic selbst oder Gateways, die die Anthropic Messages API unterstützen.
- `customParams.defaultParamsEndpoint: 'anthropic'` behält den benutzerdefinierten endpoint auf dem OpenAI-kompatiblen Pfad bei, während gleichzeitig Parametermetadaten im Anthropic-Stil und eine Anpassung der Anfrage angewendet werden.

Bei der Verwendung von `defaultParamsEndpoint: 'anthropic'` bietet das System eine spezielle Handhabung, die über die bloße Anzeige und Verwendung von Anthropic-Parametersätzen hinausgeht:

<Callout type="info">
**Anthropic API-Kompatibilität**

Das Setzen von `defaultParamsEndpoint: 'anthropic'` passt Parameter, Header und Payload-Formatierung für Anfragen im Anthropic-Format auf dem benutzerdefinierten Endpunkt-Pfad an:

- Parameter werden exakt so an Ihren benutzerdefinierten endpoint gesendet, wie es die Anthropic API erwartet.
- Dies ist unerlässlich für Proxy-Dienste wie LiteLLM, die Nicht-OpenAI-Spezifikationsparameter direkt an den zugrunde liegenden Anbieter weiterleiten.
- Anthropic-spezifische Parameter wie `thinking` sind korrekt formatiert
- Die `messages`-Nutzlast ist gemäß den Anforderungen von Anthropic formatiert (Thinking-Blöcke und Prompt-Caching)
- Entsprechende Beta-Header werden automatisch basierend auf dem Modell hinzugefügt, genau wie bei der direkten Verwendung von Anthropic.

</Callout>

Dies ist hauptsächlich erforderlich, um den `thinking`-Parameter korrekt zu formatieren, da dieser nicht OpenAI-kompatibel ist:

```json
{
  "thinking": {
    "type": "enabled",
    "budget_tokens": 10000
  }
}
```

Zusätzlich fügt das System automatisch modellspezifische Anthropic Beta-Header hinzu, wie zum Beispiel:

- `anthropic-beta: prompt-caching-2024-07-31` für Unterstützung von Prompt Caching
- `anthropic-beta: context-1m-2025-08-07` für Modelle mit erweitertem Kontext
- Modellspezifische Feature-Flags basierend auf dem verwendeten Claude-Modell

Für native Anthropic-kompatible endpoints, bevorzugen Sie `provider: 'anthropic'` im benutzerdefinierten endpoint. Dies leitet Agents, Zusammenfassungen, Token-/Kontext-Budgetierung und Parameter-Standardwerte über den Anthropic-Provider-Pfad.

### Reasoning-Wiedergabe [#reasoning-replay]

Einige OpenAI-kompatible Reasoning-Gateways erfordern, dass `reasoning_content` des Anbieters bei Assistant-Tool-Call-Turns erneut wiedergegeben wird. Verwenden Sie benutzerdefinierte Endpoint-Flags, um sich nur für Anbieter anzumelden, die dieses Verhalten benötigen:

```yaml filename="endpoints / custom / customParams"
customParams:
  reasoningFormat: reasoning_object
  reasoningKey: reasoning_content
  includeReasoningContent: true
```

Setzen Sie `includeReasoningHistory: true` nur dann, wenn der Anbieter ebenfalls erfordert, dass LibreChat `reasoning_content` aus dem gespeicherten Konversationsverlauf über spätere Turns hinweg rekonstruiert. Dies impliziert `includeReasoningContent`.

<Callout type="note">
**Implementierungsstatus**

Derzeit ist diese automatische Parameter- und Header-Verarbeitung vollständig für benutzerdefinierte Endpunkte im Anthropic-Stil implementiert. Ein ähnliches Verhalten für andere `defaultParamsEndpoint`-Werte (z. B. `google`, `bedrock`) ist für zukünftige Updates geplant.

</Callout>
