...
Ketika endpoint kustom ditulis melalui API konfigurasi administrator, LibreChat mengenkripsi `apiKey` literal saat tidak digunakan (at rest) dan hanya mengembalikan `apiKeyPreview`. Referensi `${ENV_VAR}` dan `user_provided` tetap menjadi nilai passthrough yang dapat dibaca. Untuk endpoint yang didefinisikan secara langsung di dalam `librechat.yaml`, lebih disarankan menggunakan referensi lingkungan seperti yang ditunjukkan di atas.
...
- Pada endpoint kustom yang kompatibel dengan OpenAI, ID model yang mengandung `claude` menggunakan batasan dokumen Claude: PDF dan dokumen teks diterima, sementara tipe dokumen biner non-PDF akan dilewati sebelum permintaan dikirim.
...
<OptionTable
  options={[
    [
      'addParams',
      'Object/Dictionary',
      'Menambahkan parameter tambahan ke permintaan. Nilai dapat berupa string, angka, boolean, array, atau objek bersarang. Mendukung toggle alat penyedia seperti `web_search: true` dan `url_context: true` untuk Google.',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Boolean',
      'Memutar ulang `reasoning_content` penyedia dalam giliran pemanggilan alat untuk endpoint kustom yang kompatibel dengan OpenAI yang memerlukannya.',
      'includeReasoningContent: true',
    ],
    [
      'includeReasoningHistory',
      'Boolean',
      'Merekonstruksi `reasoning_content` dari riwayat percakapan yang tersimpan di seluruh giliran. Menyiratkan `includeReasoningContent`.',
...
Pada langkah resolusi keluar akhir, `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}`, dan `{{LIBRECHAT_OPENID_*}}` yang dikenali tanpa nilai akan menjadi string kosong alih-alih diteruskan sebagai teks templat. Nama placeholder yang tidak dikenal akan tetap tidak berubah sehingga kesalahan pengetikan konfigurasi dapat terlihat.
...
Permintaan model Agen juga mendukung `{{LIBRECHAT_USER_TENANT_ID}}` dan `{{LIBRECHAT_USER_TENANTID}}` dalam header endpoint kustom. Kedua alias tersebut menggunakan tenant yang berwenang dalam cakupan permintaan alih-alih catatan pengguna yang mungkin sudah usang, berlaku untuk Agen root, Subagen, dan panggilan peringkasan kustom, serta akan menjadi string kosong jika tidak ada tenant yang tersedia. Placeholder ini bukan placeholder bidang pengguna umum untuk konsumen template non-Agen.
