...
<Callout type="info" title="Questa impostazione controlla solo l'archiviazione, non la visualizzazione">

`transactions` decide se i record di utilizzo vengono **scritti nel database**. Non mostra il conteggio dei token da nessuna parte nell'app, quindi attivarlo da solo non farà apparire le informazioni sui token nell'interfaccia utente.

Per *vedere* l'utilizzo dei token, utilizza `interface.contextUsage`, che disegna l'indicatore di contesto nella conversazione ed è attivo per impostazione predefinita. `interface.contextCost` è un'impostazione separata, disattivata per impostazione predefinita, che aggiunge le cifre dei costi a tale indicatore; lasciala disattivata se desideri il conteggio dei token senza mostrare i prezzi. Vedi [Token Usage](/docs/configuration/token_usage#viewing-context-usage-and-cost). Per imporre limiti di credito per utente, vedi [Balance](/docs/configuration/librechat_yaml/object_structure/balance).
