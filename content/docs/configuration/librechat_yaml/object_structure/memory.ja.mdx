...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'オプションの自動メモリ・エージェントを設定します。既存のエージェントのIDへの参照、または完全なエージェント設定のいずれかを指定できます。',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

自動抽出はオプトイン方式です。以下のいずれかの設定形式で `agent.enabled: true` を設定してください。`enabled` を省略するか `false` に設定した場合、チャットリクエスト時に自動メモリ・エージェントを実行することなく、手動メモリおよびエージェントメモリツールが利用可能な状態に維持されます。
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      '設定されたメモリ・エージェントによる自動メモリ抽出を有効にします。',
      'enabled: true',
    ],
  ]}
/>
...
- 自動メモリ抽出には `agent.enabled: true` が必要です。
- エージェント設定により、自動メモリ処理の動作をカスタマイズできます。
