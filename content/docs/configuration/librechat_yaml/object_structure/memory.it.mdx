...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Configura l\'agente di memoria automatico opzionale. Può essere un riferimento a un agente esistente tramite ID o una configurazione completa dell\'agente.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

L'estrazione automatica è facoltativa. Imposta `agent.enabled: true` in uno dei formati di configurazione sottostanti. Omettendo `enabled`, o impostandolo su `false`, la memoria manuale e gli strumenti di memoria dell'Agent rimarranno disponibili senza eseguire l'agente di memoria automatico sulle richieste di chat.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Abilita l\'estrazione automatica della memoria con l\'agente di memoria configurato.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` è richiesto per l'estrazione automatica della memoria
- La configurazione dell'Agent consente la personalizzazione del comportamento di elaborazione automatica della memoria
