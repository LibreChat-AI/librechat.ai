...
Quando un'Action viene creata dai metadati OpenAPI, LibreChat verifica anche che il dominio dell'Action inviato corrisponda all'URL del server della specifica. Se il dominio inviato include una porta esplicita, questa deve corrispondere alla porta effettiva dell'URL del server (`443` per HTTPS o `80` per HTTP quando l'URL ne omette una).
