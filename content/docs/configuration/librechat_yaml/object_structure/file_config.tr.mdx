...
<Callout type="info" title="MIME deseni sözdizimi">
  LibreChat, yönetici tarafından yapılandırılan `supportedMimeTypes` desenlerini sunucuda RE2 uyumlu, doğrusal zamanlı bir düzenli ifade motoru ile değerlendirir. Geriye dönük başvurular (backreferences) ve ileriye/geriye bakış (lookaround) desteklenmez. Geçersiz desenler günlüğe kaydedilir ve atlanır; yapılandırılmış bir listedeki tüm desenler geçersizse, liste kısıtlamasız hale gelmek yerine kapalı kalır ve her dosyayı reddeder.
</Callout>
## skills
...
`skills` tek bir parametreyi destekler:
