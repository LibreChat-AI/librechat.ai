...
When `summarization.enabled` is not set to `false`, Agent conversations expose **Compact context** from the conversation controls. It starts a summarize-only turn at the current branch leaf: LibreChat summarizes the eligible history, stores the new summary checkpoint, and ends without asking the primary Agent or any Subagent to produce an answer.

Earlier messages remain visible in the transcript, but later model calls use the compacted summary plus content retained by [`retainRecent`](#retainrecent). The resulting marker identifies a user-initiated compaction, and token usage from the summarizer is recorded separately. Compaction is unavailable for a new conversation, an active generation, a branch that has nothing eligible to summarize, or a deployment where summarization is disabled. If the summarizer fails or produces no summary, LibreChat leaves the prior context unchanged and reports the reason.
