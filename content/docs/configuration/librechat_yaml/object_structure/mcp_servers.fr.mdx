...
<Callout type="warning" title="Les serveurs basés sur des processus sont détenus par l'opérateur">
  Les serveurs MCP qui utilisent `type: stdio` ou des champs de processus tels que `command`, `args`, `env`, `cwd` ou `stderr` doivent être configurés dans `librechat.yaml`. L'API de configuration administrateur rejette les entrées basées sur des processus, et les remplacements au niveau de la base de données ou par principal ne peuvent ni remplacer ni supprimer un serveur basé sur un processus existant. Les serveurs MCP distants continuent de suivre la priorité de remplacement de configuration normale.
</Callout>
...
<Callout type="warning" title="Utilisez des noms uniques sécurisés pour la normalisation">
  Les clés d'outils destinées aux modèles conservent les lettres, les chiffres, `_`, `.` et `-` ; les autres caractères d'un nom de serveur sont remplacés par `_`. LibreChat fait correspondre ces clés normalisées au nom configuré et répare les anciennes références d'outils d'Agent enregistrées. Évitez deux noms qui se normalisent vers la même valeur, tels que `Sales Force` et `Sales:Force` : leurs clés d'outils sont ambiguës, donc LibreChat affiche un avertissement et exclut les outils du serveur masqué plutôt que de risquer d'acheminer un appel vers le mauvais serveur.
</Callout>
...
<OptionTable
  options={[
    ...
    [
      'chatMenu',
      'Boolean',
      '(Facultatif) Lorsqu\'il est défini sur `false`, exclut le serveur MCP du sélecteur de chat habituel et le rejette des sélections de chat fournies par le sélecteur. Les agents enregistrés et les Model Specs explicitement assignées peuvent toujours l\'utiliser. La valeur par défaut est `true`.',
      'chatMenu: false',
    ],
    [
      'serverInstructions',
...
- **Validation :** Doit commencer par une lettre ou un chiffre Unicode. Les caractères restants peuvent inclure des lettres Unicode, des chiffres, des signes diacritiques, des espaces, des traits d'union et des apostrophes droites ou courbes.
- **Exemple :**
  ```yaml
  my-server:
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Horodatage de l'acceptation des conditions                  |

**Remarque :** Les champs manquants seront remplacés par des chaînes vides.

Les espaces réservés `{{LIBRECHAT_BODY_*}}` sont limités à la portée de la requête. LibreChat crée la connexion MCP pour l'exécution active, la réutilise lors des appels d'outils au sein de cette exécution, et la nettoie lorsque la requête se termine. Les serveurs limités à la portée de la requête sont exclus du cache d'outils persistant afin que les en-têtes et les URL spécifiques à la requête ne soient pas réutilisés en dehors de l'exécution active. L'Agent Builder attache le serveur complet via une sélection d'outils d'exécution et résout ses outils après qu'une exécution a fourni les valeurs du corps. La finalisation de l'OAuth stocke l'autorisation mais diffère la connexion jusqu'à ce que ce contexte de requête existe. Les espaces réservés `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_OPENID_*}}` et `{{LIBRECHAT_GRAPH_*}}` rendent toujours le serveur limité à la portée de l'utilisateur, mais les transports HTTP actualisent les en-têtes résolus avant chaque appel d'outil sans forcer eux-mêmes une reconnexion.

`{{LIBRECHAT_BODY_PARENTMESSAGEID}}` reçoit le message parent du tour racine dans les conversations d'agents natives, les requêtes de l'API Agents compatibles avec OpenAI et les exécutions d'Assistants héritées. Les requêtes Open Responses n'ont pas de modèle de message parent compatible, donc un serveur qui nécessite cet espace réservé échoue par défaut sur cette interface ; `previous_response_id` n'est pas substitué à sa place.

Les jetons d'accès et d'ID OpenID expirent indépendamment. `{{LIBRECHAT_OPENID_ACCESS_TOKEN}}` et l'ancien `{{LIBRECHAT_OPENID_TOKEN}}` utilisent l'expiration du jeton d'accès stocké, tandis que `{{LIBRECHAT_OPENID_ID_TOKEN}}` nécessite un jeton d'ID actuel. Si un jeton d'identification n'est pas disponible ou a expiré, LibreChat renvoie une erreur de ré-authentification `401` exploitable au lieu d'envoyer une valeur bearer vide ou littérale ; connectez-vous à nouveau avant de réessayer. Pour une requête sans identité OpenID, un en-tête contenant l'un de ces espaces réservés d'identification non résolus est entièrement omis. Les espaces réservés de métadonnées d'identité conservent leur comportement actuel de valeur vide.

Les opérateurs de confiance peuvent transmettre le jeton d'accès OpenID en direct de l'utilisateur connecté directement à un serveur MCP distant défini dans le fichier YAML :
...
Utilisez le passthrough direct uniquement lorsque le serveur MCP est approuvé pour recevoir le jeton d'accès amont de l'utilisateur. L'OAuth MCP explicite, l'échange [on-behalf-of](#obo) et l'authentification par clé API administrateur prévalent sur cet en-tête. LibreChat résout le jeton de session actif avant la connexion et actualise la connexion une fois après une erreur `401` ou `403`. Il ne rejoue pas automatiquement l'appel d'outil rejeté car le serveur pourrait déjà l'avoir exécuté ; réessayez l'appel d'outil délibérément une fois la récupération réussie. Si le jeton ne peut pas être actualisé, l'utilisateur doit se reconnecter.
...
- **Type :** Objet (Optionnel, pour les types `sse` et `streamable-http`)
- **Description :** Les en-têtes ne sont résolus que pour une requête de chat active. Ils utilisent les mêmes espaces réservés que `headers`, sont fusionnés par-dessus `headers` et prévalent lorsque les deux objets définissent le même nom.
- **Comportement de découverte :** La découverte d'outils omet entièrement `requestHeaders`, de sorte que les valeurs réservées aux requêtes telles que `{{LIBRECHAT_BODY_*}}` ne masquent pas le catalogue d'outils du serveur. L'exécution active les résout et les envoie lors de la connexion.
...
- **Description :** Lorsqu'il est défini sur `false`, exclut le serveur MCP du sélecteur de chat habituel et des requêtes de chat fournies par le sélecteur. Il reste disponible pour les Agents enregistrés et les Model Specs qui l'assignent explicitement.
- **Valeur par défaut :** `true` (Le serveur MCP peut être sélectionné dans une discussion classique)
...
- **Notes :**
  ...
  - LibreChat stocke le texte annoncé par le serveur séparément de la déclaration configurée. L'inspection ne remplace pas `true` ou une chaîne définie par l'opérateur, et les entrées du registre basées sur Redis provenant de versions antérieures sont actualisées automatiquement lors de l'initialisation.
- **Exemple :**
...
#### Coordination du rafraîchissement OAuth [#oauth-refresh-coordination]

Pour les déploiements à réplicas multiples, `oauthRefreshCoordination: true` sérialise le rafraîchissement des jetons via l'instance Redis partagée. `oauthRefreshWaitTimeout` contrôle la durée pendant laquelle un réplica attend le résultat du rafraîchissement, et `oauthPersistenceWaitTimeout` contrôle la durée pendant laquelle il attend que les identifiants rafraîchis deviennent lisibles ; les deux ont une valeur par défaut de `15000` millisecondes.

Activez la coordination uniquement après que chaque réplique a été mise à niveau et connectée au même déploiement Redis. Utilisez des valeurs identiques sur chaque réplique, videz les requêtes actives avant de modifier le paramètre et évitez de le basculer pendant un déploiement progressif.
...
- **Sous-clés optionnelles :**
  ...
  - `token_exchange_method` : String - Remplacement facultatif de l'échange de jetons. Utilisez `default_post` pour envoyer les identifiants du client dans le corps de la requête POST encodé sous forme de formulaire, ou `basic_auth_header` pour les envoyer dans un en-tête d'autorisation HTTP Basic. Lorsqu'il est omis, LibreChat détecte les méthodes d'authentification du point de terminaison de jeton annoncées par le fournisseur et utilise par défaut l'authentification Basic si aucune n'est annoncée.
...
Lorsque `token_exchange_method` et `token_endpoint_auth_methods_supported` sont tous deux omis pour un secret client préconfiguré, LibreChat accorde jusqu'à cinq secondes pour la découverte des métadonnées de la ressource protégée et du serveur d'autorisation. Il n'utilise les capacités annoncées que lorsque le point de terminaison de jeton découvert correspond exactement à l'URL `token_url` configurée ; la découverte ne peut pas rediriger les identifiants client configurés vers un autre point de terminaison. Définissez explicitement `token_exchange_method` lorsqu'un fournisseur ne publie pas de métadonnées utilisables ou nécessite une méthode forcée.
