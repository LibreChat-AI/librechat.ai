...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Configura o agente de memória automático opcional. Pode ser uma referência a um agente existente por ID ou uma configuração completa de agente.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

A extração automática é opcional (opt-in). Defina `agent.enabled: true` em qualquer um dos formatos de configuração abaixo. Omitir `enabled`, ou defini-lo como `false`, mantém a memória manual e as ferramentas de memória do Agent disponíveis sem executar o agente de memória automático nas solicitações de chat.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Habilita a extração automática de memória com o agente de memória configurado.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` é necessário para a extração automática de memória
- A configuração de Agent permite a personalização do comportamento de processamento automático de memória
