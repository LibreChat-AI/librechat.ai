...
<OptionTable
  options={[
    ...
    ['catalogRecovery', 'Object', 'Limiti opzionali per il rilevamento passivo MCP, il ripristino dell\'autenticazione OAuth e la protezione dell\'autorizzazione tra repliche.', 'Uses the defaults shown in the example.'],
  ]}
/>

## catalogRecovery

`catalogRecovery` impedisce alle richieste di catalogo passive di riscoprire ripetutamente server OAuth disconnessi e limita i controlli di generazione dell'autorizzazione utilizzati tra le repliche. La maggior parte delle distribuzioni dovrebbe mantenere i valori predefiniti.

<OptionTable
  options={[
    ['discoveryBackoffMs', 'Array of Numbers', 'Ritardi progressivi dopo il rilevamento passivo fallito, in millisecondi (1-8 voci).', '[300000, 600000, 1200000, 1800000]'],
    ['discoveryTimeoutMs', 'Number', 'Timeout per un tentativo di rilevamento passivo.', '3000'],
    ['discoverySettleGraceMs', 'Number', 'Tempo aggiuntivo che una discovery scaduta può mantenere il suo slot coalescente mentre si stabilizza.', '10000'],
    ['reauthRetryMs', 'Number', 'Ritardo prima di riprovare con un server che richiede l\'autorizzazione OAuth interattiva.', '1800000'],
    ['maxStateEntries', 'Number', 'Stati di ripristino del server locali al processo massimi conservati in memoria.', '10000'],
    ['maxDetachedDiscoveries', 'Number', 'Numero massimo di rilevamenti scaduti ancora in fase di completamento prima che il ripristino smetta di avviare nuovi rilevamenti.', '3'],
    ['generationReadTimeoutMs', 'Number', 'Scadenza per la lettura della generazione delle credenziali cross-replica.', '500'],
    ['authorizationFenceRetryMs', 'Array of Numbers', 'Ritardi di riprova per la pubblicazione di un fence di generazione dell\'autorizzazione (1-8 voci).', '[0, 50, 200]'],
    ['authorizationFenceTimeoutMs', 'Number', 'Timeout per ogni tentativo di scrittura authorization-fence.', '1000'],
    ['authorizationFenceRetryIntervalMs', 'Number', 'Intervallo per la riesecuzione delle scritture fence durevoli non riuscite.', '30000'],
    ['authorizationFenceRetryBatchSize', 'Number', 'Numero massimo di record di fence durevoli riprodotti per intervallo.', '100'],
  ]}
/>

La scoperta passiva simultanea viene unificata per utente e server. Le richieste di autenticazione OAuth si stabilizzano in uno stato di riautenticazione richiesta, mentre le interruzioni temporanee dei token o della cache rimangono ritentabili. Le modifiche alle credenziali pubblicano un limite di generazione (generation fence), in modo che i risultati di scoperta obsoleti vengano scartati invece di essere riutilizzati su un'altra replica.

`discoverySettleGraceMs` può variare da `0` a `300000`; il discovery tardivo non viene mai annullato, quindi un token OAuth riscattato può ancora persistere. `maxDetachedDiscoveries` varia da `1` a `1000` e limita quanti di questi discovery rilasciati possono continuare prima che il nuovo lavoro di recupero attenda che uno di essi si stabilizzi.
