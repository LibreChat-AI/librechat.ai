...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Konfiguruje opcjonalnego agenta pamięci automatycznej. Może być odniesieniem do istniejącego agenta według ID lub pełną konfiguracją agenta.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Automatyczna ekstrakcja jest opcjonalna (opt-in). Ustaw `agent.enabled: true` w jednym z poniższych formatów konfiguracji. Pominięcie `enabled` lub ustawienie go na `false` sprawia, że pamięć ręczna oraz narzędzia pamięci Agent pozostają dostępne bez uruchamiania automatycznego agenta pamięci podczas żądań czatu.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Włącza automatyczną ekstrakcję pamięci za pomocą skonfigurowanego agenta pamięci.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` jest wymagane do automatycznej ekstrakcji pamięci
- Konfiguracja agenta umożliwia dostosowanie zachowania automatycznego przetwarzania pamięci
