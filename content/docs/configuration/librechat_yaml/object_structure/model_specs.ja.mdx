...
モデルは適切なツールを開始し、作業を継続し、`check_background_task` でポーリングを行うことができます。完了したコードの出力と生成されたファイルは、元のコード呼び出しに添付されます。バックグラウンドタスクの状態はプロセスローカルであり、サーバーの再起動やレプリカ間での移動をまたいで保持されることはありません。
