...
- `enforce` が true に設定されている場合、Model Specs は `modelSelect`、`presets`、`parameters` といった他のインターフェース設定と競合する可能性があります。
...
ユーザーが保存した選択内容が解決できなくなったエージェントを指している場合、LibreChatはソフトデフォルトを再設定し、新しいチャットで引き続き使用可能な選択肢が選ばれるようにします。エージェントリストの読み込みが一時的に失敗しても、保存された選択内容が消去されたり、そのフォールバックがトリガーされたりすることはありません。
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'モデルセレクターおよびクライアントの起動設定に完全なModel Specsを表示するかどうかを制御します。',
      'true',
    ],
  ]}
/>

`showInMenu: false` を設定すると、対話的な選択肢から Model Specs を非表示にできます。ただし、`spec` フィールドにその `name` を明示的に送信する信頼されたサーバーサイドの呼び出し元に対しては、引き続き利用可能です。これはアイコンのみを非表示にする `showIconInMenu` とは異なります。
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Model Specsの一時的なエージェントにメモリツールを装備します。',
      'false',
    ],
  ]}
/>

設定済みの [User Memory](/docs/features/memory)、endpointレベルの `memory` 機能、およびユーザーのメモリ権限が必要です。有効にすると、ユーザーが明示的に要求した際に、モデルは構造化されたメモリを保存または削除できるようになります。
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Model Specsが一時停止して最大4つの関連質問を行い、その回答を得て再開できるようにします。',
      'false',
    ],
  ]}
/>

エンドポイントレベルの `ask_user_question` 機能が必要です。これは、LibreChatの永続的な一時停止/再開フローを提供するエージェントチャットの実行でサポートされています。
...
<OptionTable
  options={[
    [
      ...
      'Model Specsにアタッチされた、バックグラウンドタスクとして実行可能なツールを制御します。',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

オプトイン形式のエンドポイントレベルの `run_in_background` 機能が必要です。その機能と `executeCode: true` が有効な場合、Code Interpreter の実行およびシェル呼び出しはデフォルトでバックグラウンド実行の対象となります。これらを無効にするには `runInBackground: false` または `[]` を設定し、スペックに付随するすべての対象ツールを有効にするには `true` を設定します。また、特定のツールのみを有効にするには、解決済みのツールIDの配列を指定します。`execute_code` または `bash_tool` を選択すると両方のネイティブコードツールが選択され、リストから除外するとそれらは無効になります。サポートされていない選択やスペルミスはログに記録されます。

モデルは対象となるツールを開始し、作業を継続できます。保存されたAgentsの場合、サポートされているコンテンツのみの補完はデフォルトで自動的に配信されます。`check_background_task` は、ステータス、制御、ライブアーティファクト、およびリカバリのために引き続き利用可能です。完了したコード出力と生成されたファイルは、元のコード呼び出しに添付されます。通常の実行はプロセスローカルであり、ワーカープロセスの喪失を生き延びることはできませんが、永続化されたターミナル結果が別のレプリカから配信される場合があります。管理者は、[`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks) を使用して、ポーリングのみの動作を復元できます。
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'モデルが記述したインテントをライブツール呼び出しラベルとしてストリーミングする対象のツールを制御します。',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

オプトインのendpointレベルの `tool_intents` 機能が必要です。Web検索、ファイル作成、メモリ、およびサポートされているCode Interpreterツールなどのネイティブツールは、この機能が有効な間、デフォルトでインテントラベルを使用します。すべての対象ツールを有効にするには `describeIntent: true` を設定するか、特定のツールIDを指定してそれらのツールのみを有効にし、他のすべてのラベルを無効にしてください。空の配列を指定すると、仕様に対するすべてのインテントラベルが無効になります。このフィールドを省略するか `false` に設定した場合、広範な選択は追加されないため、ネイティブのデフォルト設定が引き続き適用されます。

Intentラベルはツールスキーマに含まれるため、モデルリクエストにスキーマトークンを追加します。LibreChatは、スペルミスがあるもの、仕様で利用できないもの、または不適格な選択をログに記録します。
...
- `agent_ids`: 特定の永続化されたエージェントをサブエージェントとして追加することを許可します。このリストは有効なサブエージェントの制限数（デフォルトは10で、[`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) で設定可能）によって上限が定められています。このリストはサーバー側に保持され、クライアントに送信される起動設定には、公開されている `enabled` および `allowSelf` フラグのみが含まれます。
...
Googleモデルの設定でこの制御が共有スキーマに公開されるようになったため、管理者はGoogleおよびVertex AIのModel Specsにおいて、後のターンでファイルを再送信するかどうかを明示的に選択できるようになりました。
...
Googleモデルの設定において、Agent Builderは `10` から `2,000,000` までの値を受け入れ、`1,000` 単位で増減します。
...
サーバーは、有効な Model Specs を解決し、その Agent を読み込むか実行する前に、永続的な `agent_id` を認証します。呼び出し元は、このアクセスチェックの後に、リクエストフィールドやプリセットの不一致を使用して別の Agent に置き換えることはできません。
...
> **Google / Anthropic** は通常、`topP`、`topK`、`maxOutputTokens` をサポートしています。また、Google はサポートされている Gemini テキストモデルにおいて `url_context` もサポートしています。
> **Anthropic / OpenRouter / Bedrock (Anthropic および Nova モデル)** は、`promptCache` および `promptCacheTtl` をサポートしています。
...
> **サポート対象:** `anthropic`、OpenRouterカスタムエンドポイント、`bedrock` (AnthropicおよびNovaモデル)
> (プロバイダーのプロンプトキャッシングを切り替え)
...
> **サポート対象:** `anthropic`、OpenRouter カスタムエンドポイント、`bedrock` (Anthropic および Nova モデル)
> (プロンプトキャッシングが有効な場合のプロンプトキャッシュの有効期間を設定します)
...
<OptionTable
  options={[
    [
      'promptCacheTtl',
      'Enum',
      'prompt-cacheの有効期間を設定します。サポートされている値は `5m` および `1h` です。',
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'モデルの推論努力レベルを制御します。推論努力を減らすことで、応答の高速化や推論に使用されるトークン数の削減が可能になります。サポートされているOpenAIモデルでは `xhigh` および `max` が利用可能で、Bedrockの場合は `low`、`medium`、`high` が指定可能です。',
      '',
    ],
  ]}
/>
...
> **サポート対象:** 推論モードを公開しているOpenAI Responses APIモデル（GPT-5.6を含む）

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'OpenAI Responses APIの推論モードを設定します。', ''],
  ]}
/>
...
> **サポート対象:** 推論コンテキストを公開するOpenAI Responses APIモデル（GPT-5.6を含む）
...
GPT-5.6の場合、LibreChatは `useResponsesApi: false` が明示的に設定されていない限り、推論リクエストをResponses APIにデフォルトで送信します。
...
<OptionTable
  options={[
    [
      'url_context',
      'Boolean',
      'Google URL Contextを有効にすることで、モデルがユーザーメッセージに含まれるURLを読み取れるようになります。YouTubeリンクは、可能な場合にネイティブの動画理解入力に変換されます。',
      '',
    ],
  ]}
/>
...
Gemini 2.5モデルの場合、Google設定はモデルを認識します。Proは `128`-`32768`、Flashは `0`-`24576`、Flash-Liteは `512`-`24576` を受け入れます。`-1` は引き続き自動設定となります。Gemini 3以降のモデルでは、数値の予算の代わりに [`thinkingLevel`](#thinkinglevel) を使用します。
