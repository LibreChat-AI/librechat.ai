...
<OptionTable
  options={[
    ...
    ['catalogRecovery', 'Object', 'Opcjonalne granice dla pasywnego wykrywania MCP, odzyskiwania po ponownej autoryzacji OAuth oraz zabezpieczeń autoryzacji między replikami.', 'Uses the defaults shown in the example.'],
  ]}
/>

## catalogRecovery

`catalogRecovery` zapobiega wielokrotnemu ponownemu wykrywaniu rozłączonych serwerów OAuth przez pasywne żądania katalogu i ogranicza sprawdzanie generowania autoryzacji używane w replikach. Większość wdrożeń powinna zachować ustawienia domyślne.

<OptionTable
  options={[
    ['discoveryBackoffMs', 'Array of Numbers', 'Progresywne opóźnienia po nieudanym pasywnym wykrywaniu, w milisekundach (1-8 wpisów).', '[300000, 600000, 1200000, 1800000]'],
    ['discoveryTimeoutMs', 'Number', 'Limit czasu dla jednej próby pasywnego wykrywania.', '3000'],
    ['discoverySettleGraceMs', 'Number', 'Dodatkowy czas, przez który wykrywanie z przekroczonym limitem czasu może zachować swój połączony slot podczas stabilizacji.', '10000'],
    ['reauthRetryMs', 'Number', 'Opóźnienie przed ponowną próbą połączenia z serwerem wymagającym interaktywnej autoryzacji OAuth.', '1800000'],
    ['maxStateEntries', 'Number', 'Maksymalna liczba stanów odzyskiwania serwera lokalnego procesu przechowywanych w pamięci.', '10000'],
    ['maxDetachedDiscoveries', 'Number', 'Maksymalna liczba wykryć, których czas oczekiwania upłynął, a które nadal trwają, zanim proces odzyskiwania przestanie inicjować nowe wykrycia.', '3'],
    ['generationReadTimeoutMs', 'Number', 'Termin zapoznania się z generowaniem poświadczeń między replikami.', '500'],
    ['authorizationFenceRetryMs', 'Array of Numbers', 'Opóźnienia ponawiania prób dla publikowania bariery generowania autoryzacji (1-8 wpisów).', '[0, 50, 200]'],
    ['authorizationFenceTimeoutMs', 'Number', 'Limit czasu dla każdej próby zapisu authorization-fence.', '1000'],
    ['authorizationFenceRetryIntervalMs', 'Number', 'Interwał ponawiania trwałych, nieudanych zapisów typu fence.', '30000'],
    ['authorizationFenceRetryBatchSize', 'Number', 'Maksymalna liczba trwałych rekordów ogrodzenia odtwarzanych na interwał.', '100'],
  ]}
/>

Współbieżne pasywne wykrywanie jest łączone dla każdego użytkownika i serwera. Wyzwania OAuth przechodzą w stabilny stan wymagający ponownego uwierzytelnienia, podczas gdy przejściowe awarie tokenów lub pamięci podręcznej pozostają możliwe do ponowienia. Zmiany poświadczeń publikują barierę generacji (generation fence), dzięki czemu nieaktualne wyniki wykrywania są odrzucane zamiast ponownego użycia na innej replice.

`discoverySettleGraceMs` może wynosić od `0` do `300000`; opóźnione wykrywanie nigdy nie jest anulowane, więc zrealizowany token OAuth może nadal istnieć. `maxDetachedDiscoveries` mieści się w zakresie od `1` do `1000` i ogranicza, ile takich zwolnionych wykryć może być kontynuowanych, zanim nowe zadania odzyskiwania będą musiały poczekać na zakończenie jednego z nich.
