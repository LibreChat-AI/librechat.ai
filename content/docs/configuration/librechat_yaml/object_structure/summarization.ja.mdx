...
<OptionTable
  options={[
    [
      'model',
      'String',
      "要約呼び出しに使用するモデル。省略した場合は、エージェント自身のモデルが使用されます。",
      'model: "gpt-4o-mini"',
    ],
  ]}
/>
...
<OptionTable
  options={[
    [
      'reserveRatio',
      'Number',
      'ヘッドルームとして確保するトークンバジェットの割合（0～1）。コンテキストが最大容量まで埋まるのを防ぎます。',
      'reserveRatio: 0.05',
    ],
  ]}
/>
...
<OptionTable
  options={[
    [
      'contextPruning',
      'Object',
      '位置ベースのツール結果の劣化を設定します。古いメッセージ内の大きなツール結果は、トークン領域を再利用するために段階的にトリミングまたはクリアされます。',
      '',
    ],
  ]}
/>
...
<OptionTable
  options={[
    ...
    [
      ...
      'Number',
      'プルーニング（切り詰め）が適用される前のツール結果の最小文字数。これより小さい結果はそのまま保持されます。',
      'minPrunableToolChars: 50000',
    ],
...
<OptionTable
  options={[
    ...
    [
      'placeholder',
      'String',
      'ハードクリアされた際にツール実行結果の全コンテンツを置き換えるプレースホルダーテキスト。',
      'placeholder: "[Old tool result content cleared]"',
    ],
  ]}
/>
...
## 設定の完全な例 [#complete-configuration-example]
