...
Wanneer een aangepast endpoint wordt geschreven via de administrator configuratie-API, versleutelt LibreChat een letterlijke `apiKey` in rust en retourneert deze alleen als een `apiKeyPreview`. `${ENV_VAR}`-verwijzingen en `user_provided` blijven leesbare doorgeefwaarden. Voor endpoints die direct in `librechat.yaml` zijn gedefinieerd, heeft een omgevingsverwijzing de voorkeur, zoals hierboven getoond.
...
- Op OpenAI-compatibele aangepaste endpoints gebruiken model-ID's die `claude` bevatten de documentbeperkingen van Claude: PDF's en tekstdocumenten worden geaccepteerd, terwijl binaire documenttypen die geen PDF zijn, worden overgeslagen voordat het verzoek wordt verzonden.
...
<OptionTable
  options={[
    [
      'addParams',
      'Object/Dictionary',
      'Voegt extra parameters toe aan verzoeken. Waarden kunnen strings, getallen, booleans, arrays of geneste objecten zijn. Ondersteunt provider-tool-toggles zoals `web_search: true` en Google `url_context: true`.',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Boolean',
      'Speelt `reasoning_content` opnieuw af binnen tool-call turns voor OpenAI-compatibele aangepaste endpoints die dit vereisen.',
      'includeReasoningContent: true',
    ],
    [
      'includeReasoningHistory',
      'Boolean',
      'Reconstrueert `reasoning_content` uit de opgeslagen gespreksgeschiedenis over verschillende beurten heen. Impliceert `includeReasoningContent`.',
...
Bij de laatste uitgaande resolutiestap worden herkende `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` en `{{LIBRECHAT_OPENID_*}}` placeholders zonder waarde lege strings in plaats van dat ze worden doorgegeven als sjabloontekst. Onbekende placeholder-namen blijven ongewijzigd, zodat typefouten in de configuratie zichtbaar zijn.
...
Agent-modelverzoeken ondersteunen ook `{{LIBRECHAT_USER_TENANT_ID}}` en `{{LIBRECHAT_USER_TENANTID}}` in aangepaste endpoint-headers. Beide aliassen gebruiken de gezaghebbende tenant met verzoekbereik in plaats van een mogelijk verouderd gebruikersrecord, zijn van toepassing op root Agents, Subagents en aangepaste samenvattingsaanroepen, en worden omgezet naar een lege tekenreeks wanneer er geen tenant aanwezig is. Het zijn geen algemene tijdelijke aanduidingen voor gebruikersvelden voor niet-Agent sjabloongebruikers.
