...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      '선택적 자동 메모리 에이전트를 구성합니다. ID를 통한 기존 에이전트 참조 또는 전체 에이전트 구성 중 하나를 사용할 수 있습니다.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

자동 추출은 옵트인(opt-in) 방식입니다. 아래의 구성 형식 중 하나에서 `agent.enabled: true`를 설정하세요. `enabled`를 생략하거나 `false`로 설정하면, 채팅 요청 시 자동 메모리 에이전트를 실행하지 않고 수동 메모리 및 에이전트 메모리 도구를 계속 사용할 수 있습니다.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      '구성된 memory agent를 사용하여 자동 메모리 추출을 활성화합니다.',
      'enabled: true',
    ],
  ]}
/>
...
- 자동 메모리 추출을 위해서는 `agent.enabled: true`가 필요합니다.
- 에이전트 구성(Agent configuration)을 통해 자동 메모리 처리 동작을 사용자 지정할 수 있습니다.
