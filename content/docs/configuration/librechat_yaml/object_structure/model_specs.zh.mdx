...
如果用户保存的选择指向了一个不再有效的 Agent，LibreChat 会重新启用软默认设置，以便新对话仍有可用的选择。加载 Agent 列表时的临时故障不会清除已保存的选择，也不会触发该回退机制。
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      '控制完整的 Model Specs 是否出现在模型选择器和客户端启动配置中。',
      'true',
    ],
  ]}
/>

设置 `showInMenu: false` 可将 Model Specs 从交互式选择中隐藏，但仍可供明确在 `spec` 字段中发送其 `name` 的受信任服务器端调用者使用。这与 `showIconInMenu` 不同，后者仅隐藏图标。
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      '为 Model Specs 的临时智能体配备记忆工具。',
      'false',
    ],
  ]}
/>

需要配置 [User Memory](/docs/features/memory)、endpoint 级别的 `memory` 功能以及用户内存权限。启用后，当用户明确请求时，模型可以保存或删除结构化内存。
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      '允许 Model Specs 暂停并提出最多四个相关问题，并在获得回答后继续执行。',
      'false',
    ],
  ]}
/>

需要 endpoint 级别的 `ask_user_question` 能力。这在提供 LibreChat 持久化暂停/恢复流程的 Agent 聊天运行中受支持。
...
<OptionTable
  options={[
    [
      ...
      '控制哪些附加到 Model Specs 的合格工具可以作为后台任务运行。',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

需要启用端点级别的 `run_in_background` 功能。在具备该功能且 `executeCode: true` 的情况下，Code Interpreter 执行和 shell 调用默认符合后台运行条件。设置 `runInBackground: false` 或 `[]` 可将其禁用，设置为 `true` 可启用附加到该规范的所有符合条件的工具，或者通过提供一组已解析的工具 ID 来仅启用这些工具。选择 `execute_code` 或 `bash_tool` 将同时选中这两个原生代码工具；列表中省略它们则表示将其禁用。不支持或拼写错误的选项将被记录到日志中。

模型可以启动符合条件的工具并继续工作。对于已保存的 Agents，支持的内容补全默认会自动交付；`check_background_task` 仍可用于状态、控制、实时工件和恢复。已完成的代码输出和生成的文件会附加到原始代码调用中。普通执行是进程局部的，不会在工作进程丢失后存活，但持久化的终端结果可能会从另一个副本交付。管理员可以通过 [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks) 恢复仅轮询行为。
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      '控制哪些符合条件的工具将模型编写的意图作为其实时工具调用标签进行流式传输。',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

需要选择加入 endpoint 级别的 `tool_intents` 功能。当启用该功能时，诸如网页搜索、文件创作、记忆以及受支持的 Code Interpreter 工具等原生工具默认会使用意图标签。设置 `describeIntent: true` 可启用所有符合条件的工具，或者提供已解析的工具 ID 以仅启用这些工具并禁用其他所有工具的标签。空数组将禁用该 spec 的所有意图标签。省略该字段或将其设置为 `false` 不会增加更广泛的选择，因此仍适用原生默认设置。

意图标签包含在工具 schema 中，因此会向模型请求添加 schema token。LibreChat 会记录那些拼写错误、在 spec 中不可用或不符合条件的选项。
...
- `agent_ids`：允许将特定的持久化 agent 作为额外的子 agent。此列表受有效子 agent 数量限制，默认上限为 10，可通过 [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) 进行配置。该列表保留在服务器端；发送给客户端的启动配置仅包含公开的 `enabled` 和 `allowSelf` 标志。
...
Google 模型设置现在在共享 schema 中公开了此控件，因此管理员可以明确选择是否在后续轮次中为 Google 和 Vertex AI Model Specs 重新发送文件。
...
对于 Google 模型设置，Agent Builder 接受从 `10` 到 `2,000,000` 的值，并以 `1,000` 为增量。
...
服务器还会解析有效的 Model Specs 并授权其持久化的 `agent_id`，然后再加载或运行该 Agent。调用者无法在通过此访问检查后，通过请求字段或预设不匹配来替换为其他 Agent。
...
> **Google / Anthropic** 通常支持 `topP`、`topK`、`maxOutputTokens`；Google 在受支持的 Gemini 文本模型上还支持 `url_context`。
> **Anthropic / OpenRouter / Bedrock (Anthropic 和 Nova 模型)** 支持 `promptCache` 和 `promptCacheTtl`。
> **Bedrock** 支持 `region`、`maxTokens` 以及其他少数参数。
...
> **支持的提供商：** `anthropic`，OpenRouter 自定义端点，`bedrock`（Anthropic 和 Nova 模型）
> （切换提供商提示词缓存）

<OptionTable
  options={[
    ['promptCache', 'Boolean', '启用或禁用提供商提示词缓存。', ''],
  ]}
/>
...
> **支持的端点：** `anthropic`，OpenRouter 自定义端点，`bedrock`（Anthropic 和 Nova 模型）
> （当启用提示词缓存时，设置提示词缓存的生命周期）
...
<OptionTable
  options={[
    [
      'promptCacheTtl',
      'Enum',
      '设置 prompt-cache 的生命周期。支持的值为 `5m` 和 `1h`。',
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      '控制模型的推理努力水平。降低推理努力可以加快响应速度并减少推理所消耗的 token。受支持的 OpenAI 模型可能提供 `xhigh` 和 `max`；对于 Bedrock，接受的值为 `low`、`medium` 和 `high`。',
      '',
    ],
  ]}
/>
...
> **支持：** 公开推理模式的 OpenAI Responses API 模型，包括 GPT-5.6

<OptionTable
  options={[
    ['reasoning_mode', 'String', '设置 OpenAI Responses API 的推理模式。', ''],
  ]}
/>
...
> **支持：** 公开推理上下文的 OpenAI Responses API 模型，包括 GPT-5.6
...
对于 GPT-5.6，LibreChat 默认将推理请求发送至 Responses API，除非显式设置了 `useResponsesApi: false`。
...
<OptionTable
  options={[
    [
      'url_context',
      'Boolean',
      '启用 Google URL Context，以便模型能够读取用户消息中包含的 URL。YouTube 链接在可能的情况下会被转换为原生视频理解输入。',
      '',
    ],
  ]}
/>
...
对于 Gemini 2.5 模型，Google 设置具有模型感知能力：Pro 接受 `128`-`32768`，Flash 接受 `0`-`24576`，Flash-Lite 接受 `512`-`24576`。`-1` 保持为自动设置。Gemini 3+ 模型使用 [`thinkingLevel`](#thinkinglevel) 而非数值预算。
