...
OpenAPIメタデータからActionが作成される際、LibreChatは送信されたActionのドメインが仕様書のサーバーURLと一致することも検証します。送信されたドメインに明示的なポートが含まれている場合、それはサーバーURLの有効なポート（URLで省略されている場合はHTTPSなら `443`、HTTPなら `80`）と一致しなければなりません。
