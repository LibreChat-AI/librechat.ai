...
<OptionTable
  options={[
    [
      'inferenceProfiles',
      'Object (Record)',
      'リージョン間推論のために、モデルIDを推論プロファイルARNにマッピングします。キーはモデルIDで、値は推論プロファイルARNまたは環境変数参照です。',
...
- AWS Bedrockの認証は環境変数を通じて設定されます。`BEDROCK_AWS_PROFILE`、AWS SDKのデフォルト認証情報プロバイダーチェーン、Bedrock APIキー用の`BEDROCK_AWS_BEARER_TOKEN`、またはBedrock固有の静的認証情報を使用できます。詳細は[AWS Bedrockセットアップガイド](/docs/configuration/pre_configured_ai/bedrock#authentication)を参照してください。
