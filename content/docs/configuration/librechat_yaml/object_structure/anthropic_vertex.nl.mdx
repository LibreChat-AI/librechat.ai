...
> **Belangrijk:** Claude Opus 4.7 en nieuwer, Opus 5, Sonnet 5 en Fable/Mythos 5 vereisen `global`, `us` of `eu`. Specifieke regio's zoals `us-east5` ondersteunen Claude Sonnet 4.6 en ouder en retourneren een model-not-found-foutmelding voor nieuwere modellen. Gebruik `global` voor de breedste beschikbaarheid en om regionale prijstoeslagen te vermijden, of `us`/`eu` wanneer je gegevensresidentie in meerdere regio's nodig hebt.
...
Wanneer `models` wordt weggelaten, gebruikt LibreChat zijn ingebouwde Vertex-catalogus en verwijdert het modellen die niet compatibel zijn met de geconfigureerde `region`. Een expliciete `models` lijst of map wordt nooit gefilterd, dus elk deployment ID moet beschikbaar zijn op de geselecteerde locatie.
...
> **Let op:** Bij het gebruik van omgevingsvariabelen is model mapping niet beschikbaar en bevat LibreChat zijn standaard Anthropic modelcatalogus. Stel `ANTHROPIC_VERTEX_REGION` in op `global`, `us` of `eu` voordat je een modern model selecteert dat niet beschikbaar is vanuit een specifieke regio.
...
- Modellen: LibreChat's ingebouwde Vertex-catalogus voor de geselecteerde locatie
...
- Gebruik `global`, `us` of `eu` voor Opus 4.7+, Opus 5, Sonnet 5 en Fable/Mythos 5
...
- Schakel over naar `global`, `us` of `eu` voor modellen die nieuwer zijn dan Sonnet 4.6
