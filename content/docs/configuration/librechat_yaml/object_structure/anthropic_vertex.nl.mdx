...
> **Belangrijk:** Claude Opus 4.7 en nieuwer, Opus 5, Sonnet 5 en Fable/Mythos 5 of 5.1 vereisen `global`, `us` of `eu`. Specifieke regio's zoals `us-east5` ondersteunen Claude Sonnet 4.6 en ouder en retourneren een model-not-found-foutmelding voor nieuwere modellen. Gebruik `global` voor de breedste beschikbaarheid zonder regionale prijstoeslag, of `us`/`eu` wanneer je datalocatie in meerdere regio's nodig hebt.
