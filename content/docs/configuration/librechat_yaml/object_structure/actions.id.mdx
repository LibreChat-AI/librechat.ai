...
Ketika sebuah Action dibuat dari metadata OpenAPI, LibreChat juga memverifikasi bahwa domain Action yang dikirimkan cocok dengan URL server pada spesifikasi tersebut. Jika domain yang dikirimkan menyertakan port eksplisit, port tersebut harus cocok dengan port efektif URL server (`443` untuk HTTPS atau `80` untuk HTTP jika URL tidak menyertakannya).
