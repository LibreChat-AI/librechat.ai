...
Wenn die gespeicherte Auswahl des Benutzers auf einen Agent verweist, der nicht mehr aufgelöst werden kann, reaktiviert LibreChat den Soft-Standardwert, sodass ein neuer Chat weiterhin eine verwendbare Auswahl hat. Ein vorübergehender Fehler beim Laden der Agent-Liste löscht die gespeicherte Auswahl nicht und löst diesen Fallback nicht aus.
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'Steuert, ob die vollständige Model Specs im Modellauswahlmenü und in der Client-Startkonfiguration erscheint.',
      'true',
    ],
  ]}
/>

Setzen Sie `showInMenu: false`, um eine Spec von der interaktiven Auswahl zu verbergen, während sie für vertrauenswürdige serverseitige Aufrufer verfügbar bleibt, die ihren `name` explizit im `spec`-Feld senden. Dies unterscheidet sich von `showIconInMenu`, welches nur das Icon verbirgt.
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Statte den flüchtigen Agenten der Model Specs mit Memory-Tools aus.',
      'false',
    ],
  ]}
/>

Erfordert konfigurierten [User Memory](/docs/features/memory), die `memory`-Fähigkeit auf endpoint-Ebene sowie Berechtigungen für den Benutzerspeicher. Wenn diese Funktion aktiviert ist, kann das Modell strukturierte Erinnerungen speichern oder löschen, wenn der Benutzer dies ausdrücklich anfordert.
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Ermöglicht es dem Model Spec, bis zu vier verwandte Fragen zu stellen und mit den Antworten fortzufahren.',
      'false',
    ],
  ]}
/>

Erfordert die `ask_user_question`-Funktion auf endpoint-Ebene. Dies wird in Agent-Chat-Ausführungen unterstützt, die den dauerhaften Pause/Fortsetzen-Ablauf von LibreChat bereitstellen.
...
<OptionTable
  options={[
    [
      ...
      'Steuert, welche berechtigten, an die Model Specs angehängten Tools als Hintergrundaufgaben ausgeführt werden dürfen.',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

Erfordert die opt-in `run_in_background`-Funktion auf endpoint-Ebene. Mit dieser Funktion und `executeCode: true` sind die Ausführung des Code Interpreter und Shell-Aufrufe standardmäßig für den Hintergrund qualifiziert. Setzen Sie `runInBackground: false` oder `[]`, um sie zu deaktivieren, `true`, um jedes qualifizierte Tool zu aktivieren, das an die Spec angehängt ist, oder ein Array von aufgelösten Tool-IDs, um nur diese Tools zu aktivieren. Die Auswahl von `execute_code` oder `bash_tool` wählt beide nativen Code-Tools aus; eine Liste, die diese auslässt, deaktiviert sie. Nicht unterstützte oder falsch geschriebene Auswahlen werden protokolliert.

Das Modell kann ein geeignetes Tool starten und weiterarbeiten. Bei gespeicherten Agents werden unterstützte „Content-only“-Vervollständigungen standardmäßig automatisch bereitgestellt; `check_background_task` bleibt für Status, Steuerung, Live-Artefakte und Wiederherstellung verfügbar. Abgeschlossene Code-Ausgaben und generierte Dateien werden an den ursprünglichen Code-Aufruf angehängt. Die normale Ausführung ist prozesslokal und überlebt den Verlust ihres Worker-Prozesses nicht, aber ein persistiertes Terminal-Ergebnis kann von einem anderen Replikat geliefert werden. Administratoren können das reine Abfrageverhalten (Poll-only) mit [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks) wiederherstellen.
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'Steuert, welche berechtigten Tools eine vom Modell geschriebene Absicht als ihr Live-Tool-Call-Label streamen.',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

Erfordert die Opt-in-Funktion `tool_intents` auf Endpoint-Ebene. Native Tools wie Websuche, Dateierstellung, Memory und unterstützte Code Interpreter-Tools verwenden standardmäßig Intent-Labels, solange die Funktion aktiviert ist. Setzen Sie `describeIntent: true`, um jedes geeignete Tool zu aktivieren, oder geben Sie aufgelöste Tool-IDs an, um nur diese Tools zu aktivieren und die Labels für alle anderen zu deaktivieren. Ein leeres Array deaktiviert alle Intent-Labels für die Spec. Das Weglassen des Feldes oder das Setzen auf `false` fügt keine breitere Auswahl hinzu, sodass weiterhin die nativen Standardeinstellungen gelten.

Intent-Labels sind in Tool-Schemas enthalten und fügen daher Schema-Token zu Modell-Anfragen hinzu. LibreChat protokolliert Auswahlen, die falsch geschrieben, für die Spezifikation nicht verfügbar oder unzulässig sind.
...
- `agent_ids`: ermöglicht spezifische persistente Agents als zusätzliche Subagents. Diese Liste ist durch das effektive Subagent-Limit begrenzt, das standardmäßig 10 beträgt und über [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents) konfigurierbar ist. Die Liste verbleibt serverseitig; die an Clients gesendete Startkonfiguration enthält nur die öffentlichen `enabled`- und `allowSelf`-Flags.
...
Die Google-Modelleinstellungen machen diese Steuerung nun im gemeinsamen Schema verfügbar, sodass Administratoren explizit wählen können, ob Dateien bei späteren Abfragen für Google- und Vertex AI-Model Specs erneut gesendet werden sollen.
...
Der Server löst außerdem die effektive Model Specs auf und autorisiert deren persistente `agent_id`, bevor dieser Agent geladen oder ausgeführt wird. Ein Aufrufer kann nach dieser Zugriffsprüfung keine Anforderungsfelder oder eine Diskrepanz bei den Presets verwenden, um einen anderen Agenten einzusetzen.
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'Steuert den Grad des Reasoning-Aufwands für das Modell. Eine Verringerung des Reasoning-Aufwands kann zu schnelleren Antworten und einem geringeren Token-Verbrauch beim Reasoning führen. Unterstützte OpenAI-Modelle können `xhigh` und `max` bereitstellen; für Bedrock sind die akzeptierten Werte `low`, `medium` und `high`.',
      '',
    ],
  ]}
/>
...
- `""` (leerer String – nicht gesetzt)
- `"standard"`
- `"pro"`

> **Unterstützt durch:** OpenAI Responses API-Modelle, die den Reasoning-Modus offenlegen, einschließlich GPT-5.6

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'Legt den Reasoning-Modus der OpenAI Responses API fest.', ''],
  ]}
/>
...
> **Unterstützt durch:** OpenAI Responses API-Modelle, die Reasoning-Kontext bereitstellen, einschließlich GPT-5.6
...
<OptionTable
  options={[
    [
      'reasoning_context',
      'String',
      'Steuert, wie viel Konversationskontext für die Modell-Logik verfügbar ist.',
      '',
    ],
  ]}
/>
...
Für GPT-5.6 leitet LibreChat Reasoning-Anfragen standardmäßig an die Responses API weiter, sofern nicht explizit `useResponsesApi: false` gesetzt ist.
...
Für Gemini 2.5-Modelle ist die Google-Einstellung modellabhängig: Pro akzeptiert `128`-`32768`, Flash akzeptiert `0`-`24576` und Flash-Lite akzeptiert `512`-`24576`. `-1` bleibt die automatische Einstellung. Gemini 3+-Modelle verwenden [`thinkingLevel`](#thinkinglevel) anstelle eines numerischen Budgets.
