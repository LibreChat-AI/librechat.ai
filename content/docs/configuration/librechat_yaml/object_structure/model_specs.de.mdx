...
> **Hinweis:** Jeder der unten aufgeführten Parameter enthält einen Hinweis darauf, welche endpoint sie unterstützen.  
> **OpenAI / AzureOpenAI / Custom** unterstützen in der Regel `temperature`, `presence_penalty`, `frequency_penalty`, `stop`, `top_p`, `max_tokens`.  
> **Google / Anthropic** unterstützen in der Regel `topP`, `topK`, `maxOutputTokens`.  
> **Anthropic / Bedrock (Anthropic und Nova Modelle)** unterstützen `promptCache`.  
...
> **Unterstützt von:** `anthropic`, `bedrock` (Anthropic- und Nova-Modelle)  
> (Schaltet die „prompt-caching“-Funktion von Anthropic um)

<OptionTable
  options={[
    ['promptCache', 'Boolean', 'Aktiviert oder deaktiviert das integrierte Prompt-Caching von Anthropic.', ''],
  ]}
/>
