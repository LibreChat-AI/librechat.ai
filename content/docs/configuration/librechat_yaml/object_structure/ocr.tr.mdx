...
`ocr` altında 5 ana alan bulunmaktadır:
...
<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'OCR bağlantı zamanı SSRF kontrolleri için güvenilir özel host:port muafiyetleri. Genel hedefler erişilebilir kalmaya devam eder. Girdiler bir port gerektirir ve URL, yol, CIDR aralığı, çıplak host veya genel IP değişmezleri olamaz.',
      'allowedAddresses: ["ocr.internal:8080"]',
    ],
  ]}
/>
...
OCR istekleri, bağlantı sırasında özel (private), geri döngü (loopback), bağlantı yerel (link-local) ve bulut meta verisi (cloud-metadata) hedeflerini engeller ve yönlendirmeleri devre dışı bırakır. `allowedAddresses`, tam olarak güvenilen özel `host:port`, `private.ip:port` veya `[ipv6]:port` hedefleri için bir muafiyet listesidir; genel (public) hedeflere erişimi kısıtlamaz.

<Callout type="warning" title="Güven muafiyetlerini dikkatli kullanın">
  `allowedAddresses` ifadesini yalnızca kontrolünüz altındaki özel bir OCR hizmeti için kullanın. Muaf tutulan bir ana bilgisayar adı (hostname), o portta çözümlendiği her türlü özel adrese güvenir, bu nedenle mümkün olduğunda özel bir IP değişmezi (literal) tercih edin. Bir ileri vekil sunucu (forward proxy) ile, vekil sunucu hedef SSRF politikasını uygulamalıdır; değişmez özel hedefler, açıkça muaf tutulmadıkça engellenmeye devam eder.
</Callout>
