...
<OptionTable
  options={[
    ...
    ['autoTranscribeAudio', 'Boolean', '最初の休止で停止せず、マイクの入力を継続します。外部エンジンを使用している場合は、`decibelValue`に基づいた無音検知も有効になります。', ''],
    ['decibelValue', 'Number', '無音検知のデシベルしきい値。範囲は -100 から -30、デフォルトは -45。', ''],
    ['autoSendText', 'Number', '文字起こし後に自動送信するまでの待機秒数。`0` は即時送信、`-1` は自動送信を無効にします。', ''],
  ]}
/>
