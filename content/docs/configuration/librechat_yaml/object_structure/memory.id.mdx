...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Mengonfigurasi agen memori otomatis opsional. Dapat berupa referensi ke agen yang sudah ada berdasarkan ID atau konfigurasi agen lengkap.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Ekstraksi otomatis bersifat opsional (opt-in). Atur `agent.enabled: true` dalam salah satu format konfigurasi di bawah ini. Menghilangkan `enabled`, atau mengaturnya ke `false`, akan tetap membuat memori manual dan alat memori Agent tersedia tanpa menjalankan agen memori otomatis pada permintaan obrolan.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Mengaktifkan ekstraksi memori otomatis dengan agen memori yang dikonfigurasi.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` diperlukan untuk ekstraksi memori otomatis
- Konfigurasi agen memungkinkan penyesuaian perilaku pemrosesan memori otomatis
