...
Controls what happens to the answers a user gives to the `ask_user_question` tool after the turn that received them. The tool itself is made available through the usual tool allowlists (`includedTools` and `filteredTools`); this block does not enable or disable it.

<OptionTable
  options={[
    [
      'retainedAnswers.enabled',
      'Boolean',
      'Quote every answer the user gave in the conversation, verbatim, at the top of the current user turn on each model call, so the answers survive after the messages that carried them were summarized or pruned. Set to false to leave the answers only in the messages.',
      'true',
    ],
    [
      'retainedAnswers.maxTokens',
      'Number',
      'Token ceiling for the quoted block. Older answers drop first once it is exceeded; the newest answers are always kept.',
      '4096',
    ],
  ]}
/>

```yaml filename="endpoints / agents / askUserQuestion"
askUserQuestion:
  retainedAnswers:
    enabled: true
    maxTokens: 4096
...
Retention is on even when this block is omitted. Without it, a checkpoint summary, the tool-output pruner, or a full context window eventually removes the tool result that received an answer, and the Agent asks the question again or continues on the summary's paraphrase of a decision the user already made.

The block is rebuilt from the stored conversation on every request and is never persisted: it appears in the prompt only, in the user's own role rather than in the system instructions, and it is left out of memory extraction. It counts toward the context budget of the turn that carries it, measured with the model's tokenizer. Answers are quoted on every Agent turn after the one that received them; within that first turn the answer lives in the tool result like any other tool output.
