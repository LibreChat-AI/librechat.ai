...
endpoints:
  agents:
    ...
    # Opt-in capabilities: "programmatic_tools", "stateful_code_sessions", and "run_in_background".
...
`programmatic_tools`, `stateful_code_sessions`, and `run_in_background` are opt-in and must be added explicitly. Programmatic tools and stateful sessions also require `execute_code` and a compatible Code Interpreter deployment. Background code execution requires `run_in_background`, `execute_code`, and Code Interpreter.
...
- **run_in_background**: Lets eligible MCP and Code Interpreter tools marked Background return immediately while the agent continues and polls for completion. The setting permits background execution; the model decides per call whether to use it. Background code also requires `execute_code` and a configured Code Interpreter deployment. This capability is not enabled by default.
...
endpoints:
  agents:
    ...
    capabilities:
      ...
      # - 'programmatic_tools'
      - 'execute_code'
      # Optional: allows selected MCP and Code Interpreter tools to run in the background.
      # - 'run_in_background'
      - 'file_search'
      - 'skills'
      - 'subagents'
...
- Background tool calls remain disabled unless you add the `run_in_background` capability and mark eligible tools as Background
