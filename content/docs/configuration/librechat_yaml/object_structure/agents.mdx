...
Sets the maximum number of explicit subagents that one parent Agent or model spec can reference. The limit applies to both flat `agent_ids` lists and Agent team graph definitions.
...
<OptionTable
  options={[
    [
      'maxSubagents',
      'Number',
      'Maximum explicit subagents accepted in an Agent or model-spec configuration.',
      '10',
    ],
  ]}
/>

**Range:** `1-50`. LibreChat enforces a hard ceiling of 50.

Configure this field in the base `librechat.yaml`; request validation uses the process-wide value loaded at startup.
...
See [Subagents](/docs/features/subagents#limits) for the related runtime graph limits.
...
Restricts which highly experimental stateful Code Interpreter workspace scopes users can select. Omit this block to allow all three scopes. The allowlist must contain at least one value and is displayed in the stable order `user`, `agent-user`, `conversation`.
...
<OptionTable
  options={[
    ...
    [
      'graphs',
      'Array of Objects',
      'Saved Agent teams that can run as one isolated child graph. Maximum: 10 teams, 32 members per team.',
      '',
    ],
  ]}
/>
