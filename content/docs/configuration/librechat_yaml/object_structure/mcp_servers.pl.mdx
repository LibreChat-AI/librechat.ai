...
<Callout type="warning" title="Serwery oparte na procesach są własnością operatora">
  Serwery MCP, które używają `type: stdio` lub pól procesowych, takich jak `command`, `args`, `env`, `cwd` lub `stderr`, muszą być skonfigurowane w `librechat.yaml`. API konfiguracji administratora odrzuca wpisy oparte na procesach, a nadpisania w bazie danych lub dla poszczególnych podmiotów (per-principal) nie mogą zastąpić ani usunąć istniejącego serwera opartego na procesie. Zdalne serwery MCP nadal podlegają normalnej kolejności pierwszeństwa nadpisywania konfiguracji.
</Callout>
...
<Callout type="warning" title="Używaj unikalnych nazw bezpiecznych dla normalizacji">
  Klucze narzędzi widoczne dla modelu zachowują litery, cyfry, `_`, `.` oraz `-`; pozostałe znaki w nazwie serwera są zastępowane przez `_`. LibreChat mapuje te znormalizowane klucze z powrotem na skonfigurowaną nazwę i naprawia starsze zapisane odwołania do narzędzi Agentów. Unikaj dwóch nazw, które normalizują się do tej samej wartości, takich jak `Sales Force` oraz `Sales:Force`: ich klucze narzędzi są niejednoznaczne, więc LibreChat wyświetla ostrzeżenie i wyklucza narzędzia przesłoniętego serwera, zamiast ryzykować skierowanie wywołania do niewłaściwego serwera.
</Callout>
...
<OptionTable
  options={[
    ...
    [
      'chatMenu',
      'Boolean',
      '(Opcjonalnie) Gdy `false`, wyklucza serwer MCP z regularnego wyboru czatu i odrzuca go w przypadku wyborów czatu dostarczonych przez selektor. Zapisani agenci i jawnie przypisane Model Specs nadal mogą z niego korzystać. Wartość domyślna to `true`.',
      'chatMenu: false',
    ],
    [
      'serverInstructions',
...
- **Weryfikacja:** Musi zaczynać się od litery lub cyfry Unicode. Pozostałe znaki mogą obejmować litery Unicode, cyfry, znaki łączące, spacje, myślniki oraz proste lub zakrzywione apostrofy.
- **Przykład:**
  ```yaml
  my-server:
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Sygnatura czasowa akceptacji regulaminu |

**Uwaga:** Brakujące pola zostaną zastąpione pustymi ciągami znaków.

Symbole zastępcze `{{LIBRECHAT_BODY_*}}` mają zasięg ograniczony do żądania (request-scoped). LibreChat tworzy połączenie MCP dla aktywnego uruchomienia, używa go ponownie w ramach wywołań narzędzi w tym uruchomieniu i zamyka je po zakończeniu żądania. Serwery o zasięgu żądania są wykluczone z trwałej pamięci podręcznej narzędzi, dzięki czemu nagłówki i adresy URL specyficzne dla żądania nie są używane ponownie poza aktywnym uruchomieniem. Agent Builder dołącza kompletny serwer poprzez jeden wybór runtime-tools i rozwiązuje jego narzędzia po dostarczeniu wartości body w ramach uruchomienia. Ukończenie OAuth zapisuje autoryzację, ale odracza połączenie do momentu, aż zaistnieje kontekst danego żądania. Symbole zastępcze `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_OPENID_*}}` oraz `{{LIBRECHAT_GRAPH_*}}` nadal sprawiają, że serwer ma zasięg użytkownika (user-scoped), jednak transporty HTTP odświeżają rozwiązane nagłówki przed każdym wywołaniem narzędzia, nie wymuszając przy tym samodzielnie ponownego połączenia.

`{{LIBRECHAT_BODY_PARENTMESSAGEID}}` otrzymuje identyfikator nadrzędnej wiadomości (root-turn) w natywnych czatach Agentów, żądaniach API Agentów zgodnych z OpenAI oraz starszych uruchomieniach Assistants. Żądania Open Responses nie posiadają kompatybilnego modelu wiadomości nadrzędnej, więc serwer wymagający tego symbolu zastępczego kończy działanie błędem na tym interfejsie; `previous_response_id` nie jest dla niego podstawiane.

Tokeny dostępu i identyfikacyjne OpenID wygasają niezależnie. `{{LIBRECHAT_OPENID_ACCESS_TOKEN}}` oraz starszy `{{LIBRECHAT_OPENID_TOKEN}}` korzystają z zapisanego czasu wygaśnięcia tokena dostępu, podczas gdy `{{LIBRECHAT_OPENID_ID_TOKEN}}` wymaga aktualnego tokena identyfikacyjnego. Jeśli token uwierzytelniający jest niedostępny lub wygasł, LibreChat zwraca możliwy do obsłużenia błąd `401` wymagający ponownego uwierzytelnienia, zamiast wysyłać pustą lub dosłowną wartość bearer; przed ponowieniem próby należy zalogować się ponownie. W przypadku żądania bez tożsamości OpenID, nagłówek zawierający jeden z tych nierozwiązanych symboli zastępczych poświadczeń jest całkowicie pomijany. Symbole zastępcze metadanych tożsamości zachowują swoje dotychczasowe zachowanie dla pustych wartości.

Zaufani operatorzy mogą przekazywać aktywny token dostępu OpenID zalogowanego użytkownika bezpośrednio do zdalnego serwera MCP zdefiniowanego w pliku YAML:
...
Używaj bezpośredniego przekazywania (direct passthrough) tylko wtedy, gdy serwer MCP jest zaufany w zakresie otrzymywania tokena dostępu użytkownika typu upstream. Jawne OAuth MCP, [wymiana w imieniu użytkownika (on-behalf-of exchange)](#obo) oraz uwierzytelnianie za pomocą klucza API administratora mają pierwszeństwo przed tym nagłówkiem. LibreChat rozpoznaje aktywny token sesji przed nawiązaniem połączenia i odświeża połączenie jednokrotnie po wystąpieniu błędu `401` lub `403`. Nie ponawia automatycznie odrzuconego wywołania narzędzia, ponieważ serwer mógł je już wykonać; ponów wywołanie narzędzia celowo po pomyślnym odzyskaniu połączenia. Jeśli tokena nie można odświeżyć, użytkownik musi zalogować się ponownie.
...
- **Typ:** Obiekt (Opcjonalny, dla typów `sse` i `streamable-http`)
- **Opis:** Nagłówki są rozwiązywane tylko dla aktywnego żądania czatu. Używają tych samych symboli zastępczych co `headers`, są scalane z `headers` i mają pierwszeństwo, gdy oba obiekty definiują tę samą nazwę.
- **Zachowanie podczas wykrywania:** Wykrywanie narzędzi całkowicie pomija `requestHeaders`, dzięki czemu wartości przeznaczone wyłącznie dla żądań, takie jak `{{LIBRECHAT_BODY_*}}`, nie ukrywają katalogu narzędzi serwera. Aktywne uruchomienie rozwiązuje je i wysyła w momencie nawiązania połączenia.
...
- **Opis:** Gdy ustawiono na `false`, wyklucza serwer MCP z regularnego wyboru czatu oraz żądań czatu dostarczanych przez selektor. Pozostaje on dostępny dla zapisanych Agentów oraz Model Specs, które przypisują go jawnie.
- **Wartość domyślna:** `true` (Serwer MCP może być wybrany w zwykłym czacie)
...
- **Uwagi:**
  ...
  - LibreChat przechowuje tekst reklamowany przez serwer oddzielnie od skonfigurowanej deklaracji. Inspekcja nie zastępuje `true` ani ciągu znaków utworzonego przez operatora, a wpisy w rejestrze oparte na Redis z wcześniejszych kompilacji są automatycznie odświeżane podczas inicjalizacji.
- **Przykład:**
...
#### Koordynacja odświeżania OAuth [#oauth-refresh-coordination]

W przypadku wdrożeń z wieloma replikami, `oauthRefreshCoordination: true` szereguje odświeżanie tokenów za pośrednictwem współdzielonej instancji Redis. `oauthRefreshWaitTimeout` kontroluje, jak długo replika czeka na wynik odświeżenia, a `oauthPersistenceWaitTimeout` kontroluje, jak długo czeka na to, aż odświeżone poświadczenia staną się możliwe do odczytania; obie wartości domyślnie wynoszą `15000` milisekund.

Włącz koordynację dopiero po zaktualizowaniu każdej repliki i połączeniu jej z tym samym wdrożeniem Redis. Używaj identycznych wartości na każdej replice, opróżnij aktywne żądania przed zmianą ustawienia i unikaj przełączania go podczas wdrażania kroczącego (rolling deployment).
...
- **Opcjonalne podklucze:**
  ...
  - `token_exchange_method`: String - Opcjonalne zastąpienie metody wymiany tokenów. Użyj `default_post`, aby wysłać dane uwierzytelniające klienta w treści POST zakodowanej jako formularz, lub `basic_auth_header`, aby wysłać je w nagłówku autoryzacji HTTP Basic. W przypadku pominięcia, LibreChat wykrywa metody uwierzytelniania punktu końcowego tokena reklamowane przez dostawcę i domyślnie używa autoryzacji Basic, jeśli żadne nie są reklamowane.
...
Gdy zarówno `token_exchange_method`, jak i `token_endpoint_auth_methods_supported` są pominięte dla wstępnie skonfigurowanego klucza tajnego klienta (client secret), LibreChat przeznacza do pięciu sekund na wykrycie metadanych chronionego zasobu i serwera autoryzacji. Wykorzystuje on ogłoszone możliwości tylko wtedy, gdy wykryty punkt końcowy tokena dokładnie odpowiada skonfigurowanemu `token_url`; wykrywanie nie może przekierować skonfigurowanych poświadczeń klienta do innego punktu końcowego. Ustaw `token_exchange_method` jawnie, gdy dostawca nie publikuje użytecznych metadanych lub wymaga wymuszonej metody.
