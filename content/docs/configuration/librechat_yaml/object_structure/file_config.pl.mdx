...
<Callout type="info" title="Składnia wzorców MIME">
  LibreChat sprawdza wzorce `supportedMimeTypes` skonfigurowane przez administratora po stronie serwera za pomocą silnika wyrażeń regularnych czasu liniowego, zgodnego z RE2. Referencje wsteczne (backreferences) oraz mechanizmy lookaround nie są obsługiwane. Nieprawidłowe wzorce są logowane i pomijane; jeśli każdy wzorzec w skonfigurowanej liście jest nieprawidłowy, lista ta przyjmuje domyślne ustawienie bezpieczeństwa i odrzuca każdy plik, zamiast stać się nieograniczoną.
</Callout>
## skills
...
`skills` obsługuje jeden parametr:
