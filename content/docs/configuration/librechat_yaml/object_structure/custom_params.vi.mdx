...
#### Các giá trị được chấp nhận [#accepted-values]

`defaultParamsEndpoint` chọn tập hợp tham số tích hợp sẵn nào mà bảng điều khiển của endpoint sẽ hiển thị. Đây là các giá trị phân giải thành một:

<OptionTable
  options={[
    ['custom', 'String', 'Tập tham số OpenAI. Đây là lược đồ mặc định và là giá trị dự phòng cho một endpoint không có `provider`.', 'Default'],
    ['openAI', 'String', 'Thiết lập tham số OpenAI.', ''],
    ['azureOpenAI', 'String', 'Thiết lập tham số OpenAI.', ''],
    ['anthropic', 'String', 'Bộ tham số Anthropic.', ''],
    ['google', 'String', 'Thiết lập tham số Google.', ''],
    ['openrouter', 'String', 'Tập tham số OpenRouter. Viết thường, không giống như các tham số khác.', ''],
  ]}
/>

Lưu ý về cách viết hoa: `openAI` và `azureOpenAI` là camelCase, nhưng `openrouter` thì viết thường hoàn toàn.

<Callout type="info" title="Một cài đặt nhà cung cấp sẽ cung cấp giá trị này cho bạn">

Nếu endpoint tùy chỉnh thiết lập `provider`, LibreChat sẽ tự động điền `defaultParamsEndpoint` từ đó, vì vậy giá trị mặc định hiệu dụng sẽ là tập tham số của nhà cung cấp thay vì `custom`. Việc thay thế này chỉ xảy ra khi bạn bỏ trống trường đó hoặc để là `custom`; bất kỳ giá trị nào khác mà bạn thiết lập một cách rõ ràng sẽ được ưu tiên. Do đó, một endpoint có `provider: 'anthropic'` sẽ bắt đầu với tập tham số của Anthropic mà bạn không cần phải viết `defaultParamsEndpoint`.

</Callout>

Các giá trị `assistants`, `azureAssistants`, `agents` và `bedrock` cũng được nhận diện khi LibreChat phân giải các lược đồ hội thoại và cài đặt sẵn (preset), nhưng không có giá trị nào trong số đó ánh xạ tới một tập hợp tham số mà một endpoint tùy chỉnh có thể hiển thị, vì vậy việc thiết lập một trong các giá trị này sẽ khiến bảng điều khiển bị trống.

<Callout type="warning" title="Các giá trị không được nhận dạng sẽ bị bỏ qua mà không thông báo">

Trường này là một chuỗi tự do, không phải là một enum đóng, vì vậy một giá trị nằm ngoài danh sách này vẫn vượt qua quá trình xác thực cấu hình. Nó chỉ đơn giản là không khớp với bất kỳ tập tham số nào, và bảng điều khiển của endpoint sẽ hiển thị **hoàn toàn không có tham số nào** thay vì báo lỗi. Nếu bảng tham số của bạn trống một cách bất ngờ, hãy kiểm tra giá trị này trước tiên.
...
#### Các phạm vi Sentinel [#sentinel-ranges]

Sử dụng `range.positiveMin` khi `range.min` là một giá trị sentinel đặc biệt nhưng các giá trị thông thường lại có giới hạn dưới cao hơn. Ví dụ, ngân sách suy nghĩ (thinking budget) có thể sử dụng `-1` cho hành vi tự động trong khi chỉ chấp nhận các ngân sách rõ ràng từ `128` đến `32768`:
...
Với `positiveMin`, các giá trị được chấp nhận duy nhất là giá trị sentinel `min` hoặc các giá trị từ `positiveMin` đến `max`; các giá trị nằm giữa chúng là không hợp lệ. `positiveMin` không được vượt quá `max` và giá trị mặc định phải là `min` hoặc ít nhất là `positiveMin`.
...
### Phát lại suy luận [#reasoning-replay]
