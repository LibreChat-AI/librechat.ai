...
Gdy niestandardowy endpoint jest tworzony za pomocą API konfiguracji administratora, LibreChat szyfruje dosłowny `apiKey` w spoczynku i zwraca jedynie `apiKeyPreview`. Odwołania `${ENV_VAR}` oraz `user_provided` pozostają czytelnymi wartościami przekazywanymi dalej. W przypadku endpointów zdefiniowanych bezpośrednio w `librechat.yaml`, preferuj odwołanie do zmiennej środowiskowej, jak pokazano powyżej.
...
- W przypadku niestandardowych endpointów zgodnych z OpenAI, identyfikatory modeli zawierające `claude` podlegają ograniczeniom dokumentów Claude: akceptowane są pliki PDF i dokumenty tekstowe, podczas gdy binarne typy dokumentów inne niż PDF są pomijane przed wysłaniem żądania.
...
<OptionTable
  options={[
    [
      'addParams',
      'Object/Dictionary',
      'Dodaje dodatkowe parametry do żądań. Wartościami mogą być ciągi znaków, liczby, wartości logiczne, tablice lub zagnieżdżone obiekty. Obsługuje przełączniki narzędzi dostawcy, takie jak `web_search: true` oraz `url_context: true` dla Google.',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Boolean',
      'Odtwarza `reasoning_content` dostawcy w turach wywołań narzędzi dla zgodnych z OpenAI niestandardowych endpoint, które tego wymagają.',
      'includeReasoningContent: true',
    ],
    [
      'includeReasoningHistory',
      'Boolean',
      'Rekonstruuje `reasoning_content` z utrwalonej historii konwersacji pomiędzy turami. Implikuje `includeReasoningContent`.',
...
W końcowym kroku rozpoznawania ruchu wychodzącego, rozpoznane symbole zastępcze `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` oraz `{{LIBRECHAT_OPENID_*}}`, które nie posiadają wartości, stają się pustymi ciągami znaków zamiast być przekazywane jako tekst szablonu. Nieznane nazwy symboli zastępczych pozostają niezmienione, dzięki czemu literówki w konfiguracji są widoczne.
...
Żądania modelu agenta obsługują również `{{LIBRECHAT_USER_TENANT_ID}}` oraz `{{LIBRECHAT_USER_TENANTID}}` w niestandardowych nagłówkach endpoint. Oba aliasy używają autorytatywnego identyfikatora dzierżawcy (tenant) w zakresie żądania, zamiast potencjalnie nieaktualnego rekordu użytkownika, mają zastosowanie do głównych agentów (root Agents), podagentów (Subagents) oraz niestandardowych wywołań podsumowań i są rozwiązywane do pustego ciągu znaków, gdy dzierżawca nie jest obecny. Nie są to ogólne symbole zastępcze pól użytkownika dla konsumentów szablonów niebędących agentami.
