...
> **重要提示：** Claude Opus 4.7 及更高版本、Opus 5、Sonnet 5 以及 Fable/Mythos 5 需要使用 `global`、`us` 或 `eu`。特定的区域（如 `us-east5`）仅支持 Claude Sonnet 4.6 及更早版本，对于较新的模型会返回 model-not-found 响应。请使用 `global` 以获得最广泛的可用性且无区域定价溢价，或者在需要多区域数据驻留时使用 `us`/`eu`。
