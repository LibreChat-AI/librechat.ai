---
title: Anthropic Vertex AI 对象结构
icon: Anthropic
---

LibreChat 支持通过 **Google Cloud Vertex AI** 运行 Anthropic Claude 模型。这使您能够利用现有的 Google Cloud 基础设施、账单和凭据来使用 Claude 模型。

**[如需使用环境变量进行快速设置，请参阅 Anthropic 配置指南](/docs/configuration/pre_configured_ai/anthropic#vertex-ai)**

## 优势 [#benefits]

- **统一计费：** 使用您现有的 Google Cloud 计费账户
- **企业级功能：** 访问 Google Cloud 的企业级安全和合规功能
- **区域合规性：** 在特定区域部署以满足数据驻留要求
- **现有基础设施：** 利用您当前的 GCP 服务账号和 IAM 策略

## 先决条件 [#prerequisites]

在配置 Anthropic Vertex AI 之前，请确保您已完成以下准备工作：

1. 已启用 Vertex AI API 的 **Google Cloud Project**
2. **服务账号**，需具备 `Vertex AI User` 角色 (`roles/aiplatform.user`)
3. 在您的 [Vertex AI Model Garden](https://console.cloud.google.com/vertex-ai/model-garden) 中启用的 **Claude models**
4. **Service Account Key** (JSON 文件)，已下载并可供 LibreChat 访问

## 示例配置 [#example-configuration]

```yaml filename="Example Anthropic Vertex AI Configuration"
endpoints:
  anthropic:
    streamRate: 20
    titleModel: "claude-3.5-haiku"  # Use the visible model name (key from models config)

    vertex:
      region: "global"
      # serviceKeyFile: "/path/to/service-account.json"  # Optional, defaults to api/data/auth.json
      # projectId: "${VERTEX_PROJECT_ID}"  # Optional, auto-detected from service key

      # Model mapping: visible name -> Vertex AI deployment name
      models:
        claude-fable-5-1:
          deploymentName: claude-fable-5-1
        claude-opus-5:
          deploymentName: claude-opus-5
        claude-sonnet-5:
          deploymentName: claude-sonnet-5
        claude-3.7-sonnet:
          deploymentName: claude-3-7-sonnet-20250219
        claude-3.5-sonnet:
          deploymentName: claude-3-5-sonnet-v2@20241022
        claude-3.5-haiku:
          deploymentName: claude-3-5-haiku@20241022
```

> **注意：** Anthropic endpoint 支持所有 [Shared Endpoint Settings](/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings)，包括 `streamRate`、`titleModel`、`titleMethod`、`titlePrompt`、`titlePromptTemplate` 和 `titleEndpoint`。

---

## vertex

`vertex` 对象包含所有 Vertex AI 特定的配置选项。

### region

**密钥：**
<OptionTable
  options={[
    ['region', 'String', '部署您的 Vertex AI endpoint 的 Google Cloud 区域。', 'Must be a region where Claude models are available on Vertex AI.'],
  ]}
/>

**默认值：** `us-east5`

**可用区域：**

- `global`
- `us`
- `eu`
- `us-east5`
- `us-central1`
- `europe-west1`
- `europe-west4`
- `asia-southeast1`

> **重要提示：** Claude Opus 4.7 及更新版本、Opus 5、Sonnet 5 以及 Fable/Mythos 5 或 5.1 需要使用 `global`、`us` 或 `eu`。诸如 `us-east5` 之类的特定区域仅支持 Claude Sonnet 4.6 及更早版本，对于较新的模型会返回 model-not-found 响应。请使用 `global` 以获得最广泛的可用性且无区域定价溢价，或者在需要多区域数据驻留时使用 `us` 或 `eu`。

**示例：**
```yaml filename="endpoints / anthropic / vertex / region"
region: "global"
```

### projectId

**密钥：**
<OptionTable
  options={[
    ['projectId', 'String', 'Google Cloud 项目 ID。支持环境变量引用。', 'Optional. If not specified, auto-detected from the service account key file.'],
  ]}
/>

**默认值：** 从服务密钥文件中自动检测

**示例：**
```yaml filename="endpoints / anthropic / vertex / projectId"
projectId: "${GOOGLE_PROJECT_ID}"
```

### serviceKeyFile

**密钥：**
<OptionTable
  options={[
    ['serviceKeyFile', 'String', 'Google Cloud 服务账号密钥 JSON 文件的路径。', 'Can be absolute or relative to the LibreChat root directory.'],
  ]}
/>

**默认值：** `api/data/auth.json`（或 `GOOGLE_SERVICE_KEY_FILE` 环境变量）

**示例：**
```yaml filename="endpoints / anthropic / vertex / serviceKeyFile"
serviceKeyFile: "/etc/secrets/gcp-service-account.json"
```

---

## models

`models` 字段定义了可用的 Claude 模型，并将用户友好的名称映射到 Vertex AI 部署 ID。其工作方式类似于 [Azure OpenAI 模型映射](/docs/configuration/librechat_yaml/object_structure/azure_openai#group-object-structure)。

当省略 `models` 时，LibreChat 会使用其内置的 Vertex 目录，并移除与所配置 `region` 不兼容的模型。显式的 `models` 列表或映射永远不会被过滤，因此每个部署 ID 都必须在所选位置可用。

### 格式选项 [#format-options]

您可以通过三种方式配置模型：

#### 选项 1：简单数组 [#option-1-simple-array]

直接使用实际的 Vertex AI 模型 ID。这些 ID 将按原样显示在 UI 中：

```yaml filename="Simple array format"
models:
  - "claude-fable-5-1"
  - "claude-opus-5"
  - "claude-sonnet-5"
  - "claude-sonnet-4-20250514"
  - "claude-3-7-sonnet-20250219"
  - "claude-3-5-haiku@20241022"
```

#### 选项 2：使用自定义名称的对象（推荐） [#option-2-object-with-custom-names-recommended]

将用户友好的名称映射到 Vertex AI 部署名称：

```yaml filename="Object format with custom names"
models:
  claude-fable-5-1:        # Visible in UI
    deploymentName: claude-fable-5-1  # Actual Vertex AI model ID
  claude-opus-5:           # Visible in UI
    deploymentName: claude-opus-5  # Actual Vertex AI model ID
  claude-sonnet-5:
    deploymentName: claude-sonnet-5
  claude-3.5-haiku:
    deploymentName: claude-3-5-haiku@20241022
```

#### 选项 3：混合格式及默认值 [#option-3-mixed-format-with-default]

设置一个默认的部署名称，并对继承该名称的模型使用布尔值：

```yaml filename="Mixed format"
deploymentName: claude-sonnet-4-20250514  # Default deployment
models:
  claude-sonnet-4: true  # Uses default deploymentName
  claude-3.5-haiku:
    deploymentName: claude-3-5-haiku@20241022  # Override for this model
```

### 模型对象属性 [#model-object-properties]

<OptionTable
  options={[
    ['deploymentName', 'String', '用于 API 调用的实际 Vertex AI 模型 ID。', 'Required for each model unless using boolean `true` with a group-level default.'],
  ]}
/>

**示例：**
```yaml filename="Model with deploymentName"
models:
  claude-sonnet-4:
    deploymentName: claude-sonnet-4-20250514
```

---

## 环境变量替代方案 [#environment-variable-alternative]

对于更简单的设置，您可以使用环境变量而不是 YAML 来配置 Vertex AI：

```bash filename=".env"
# Enable Vertex AI mode
ANTHROPIC_USE_VERTEX=true

# Vertex AI region (optional, defaults to us-east5)
ANTHROPIC_VERTEX_REGION=global

# Path to service account key (optional, defaults to api/data/auth.json)
GOOGLE_SERVICE_KEY_FILE=/path/to/service-account.json
```

> **注意：** 当使用环境变量时，模型映射功能不可用，且 LibreChat 会包含其标准的 Anthropic 模型目录。在选择特定区域不可用的现代模型之前，请将 `ANTHROPIC_VERTEX_REGION` 设置为 `global`、`us` 或 `eu`。

---

## 完整示例 [#complete-examples]

### 基本设置 [#basic-setup]

使用默认值的最小配置（Vertex AI 因 `vertex` 部分的存在而被启用）：

```yaml filename="Basic Vertex AI Setup"
endpoints:
  anthropic:
    vertex:
      region: global
```

这使用了：

- 区域：`global`
- 服务密钥：`api/data/auth.json` (或 `GOOGLE_SERVICE_KEY_FILE` 环境变量)
- 项目 ID：从服务密钥自动检测
- 模型：LibreChat 为所选位置内置的 Vertex 目录

### 使用 Model Mapping 的生产环境设置 [#production-setup-with-model-mapping]

包含自定义模型名称和标题的完整配置：

```yaml filename="Production Vertex AI Setup"
endpoints:
  anthropic:
    streamRate: 20
    titleModel: "haiku"
    titleMethod: "completion"

    vertex:
      region: "global"
      serviceKeyFile: "${GOOGLE_SERVICE_KEY_FILE}"

      models:
        fable:
          deploymentName: claude-fable-5-1
        opus:
          deploymentName: claude-opus-5
        sonnet:
          deploymentName: claude-sonnet-5
        haiku:
          deploymentName: claude-3-5-haiku@20241022
```

### 多区域设置 [#multi-region-setup]

每个部署只能配置一个区域。如果需要多区域支持，请考虑使用独立的 LibreChat 实例或自定义 endpoint。

---

## 故障排除 [#troubleshooting]

### 常见错误 [#common-errors]

**"无法加载默认凭据"**
- 确保服务账号密钥文件存在于指定路径
- 检查文件权限（必须可被 LibreChat 进程读取）
- 验证 JSON 文件是否有效且未损坏

**“Permission denied” 或 “403 Forbidden”**
- 验证服务账号是否拥有 `Vertex AI User` 角色
- 确保在您的 Vertex AI Model Garden 中启用了 Claude 模型
- 检查服务账号是否属于正确的项目

**"未找到模型"**
- 检查 `deploymentName` 中的模型 ID 是否正确
- 验证该模型在您选择的区域内是否可用
- 对于 Opus 4.7+、Opus 5、Sonnet 5 以及 Fable/Mythos 5，请使用 `global`、`us` 或 `eu`。
- 确保该模型已在您的 Vertex AI Model Garden 中启用

### 区域问题 [#region-issues]

**“Invalid region” 或 “Region not supported”**
- 使用上述列出的受支持区域之一
- 尝试使用 `global` 区域，它提供自动路由功能
- 请查看 [Google Cloud 的文档](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude#regions) 以获取 Claude 可用区域的最新列表。

**“模型在当前地区不可用”**
- 并非所有 Claude 模型在所有地区都可用
- 对于比 Sonnet 4.6 更新的模型，请切换至 `global`、`us` 或 `eu`
- 请查看 [Vertex AI Model Garden](https://console.cloud.google.com/vertex-ai/model-garden) 以了解您所在区域可用的模型。

**延迟问题**
- 如果您遇到高延迟，请尝试使用地理位置更靠近您用户的区域。
- `global` 区域会自动路由到最近的可用区域
- 对于有严格延迟要求的生产工作负载，请测试不同的区域并选择最适合您用例的区域以获得最佳性能。

### 验证设置 [#verifying-setup]

1. 确保您的服务账号密钥有效：
   ```bash
   gcloud auth activate-service-account --key-file=/path/to/key.json
   gcloud auth list
   ```

2. 测试 Vertex AI 访问：
   ```bash
   gcloud ai models list --region=us-east5
   ```

3. 验证 Claude 模型访问权限：
   ```bash
   curl -X POST \
     -H "Authorization: Bearer $(gcloud auth print-access-token)" \
     -H "Content-Type: application/json" \
     "https://us-east5-aiplatform.googleapis.com/v1/projects/YOUR_PROJECT/locations/us-east5/publishers/anthropic/models/claude-3-5-haiku@20241022:rawPredict" \
     -d '{"anthropic_version": "vertex-2023-10-16", "max_tokens": 100, "messages": [{"role": "user", "content": "Hello"}]}'
   ```

---

## 注意事项 [#notes]

- Vertex AI 和直接的 Anthropic API 是互斥的。当存在 `vertex` 配置部分时，`ANTHROPIC_API_KEY` 环境变量将被忽略。
- Vertex AI 完全支持网络搜索功能。
- 通过自动标头过滤支持 Prompt caching，以实现与 Vertex AI 的兼容性。
- Function calling 和 tool use 的工作方式与直接使用 Anthropic API 时相同。
