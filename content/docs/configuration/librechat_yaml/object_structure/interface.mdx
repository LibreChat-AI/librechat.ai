...
<OptionTable
  options={[
    ...
    [
      ...
      'Number',
      'Polling interval in milliseconds for refreshing MCP tool catalogs while MCP controls are visible. Set to 0 to disable polling.',
      '300000',
    ],
    [
      'statusRefreshInterval',
      'Number',
      'Polling interval in milliseconds for refreshing MCP connection status while MCP controls are visible. Set to 0 to disable polling.',
      '30000',
    ],
    [
      'trustCheckbox',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Number',
      'Maximum occurrences concurrently performing readiness admission per replica (1-100).',
      '20',
    ],
    [
      'fireConcurrency',
    ...
    [
      ...
      'Number',
      'Maximum MCP readiness connections opened concurrently by one schedule admission (1-10).',
      '3',
    ],
    [
      'mcpPreflightTimeoutMs',
      'Number',
      'Maximum wall-clock time for one schedule MCP readiness admission (1000-600000 ms).',
      '300000',
    ],
    [
      'requireProject',
...
<OptionTable
  options={[
    [
      'generalChatRetention',
      'Number',
      'Sets the retention period for regular chats in hours under all-data retention.',
      'Defaults to temporaryChatRetention when omitted',
    ],
  ]}
/>

The value must be between `1` and `8760` hours. This setting lets regular and temporary chats use different lifetimes. Changing either retention value affects deadlines assigned to new or updated records; LibreChat does not rewrite existing stored deadlines automatically.
...
Controls the minimum interval between syntax-highlighting passes while a code block is changing during a streamed response. Throttling reduces repeated highlighting work without delaying the streamed text itself. The same value applies when rendering shared conversations.

**Default:** `300` milliseconds

Set the value to `0` to highlight every streamed update. Valid values range from `0` through `60000` milliseconds.
...
<OptionTable
  options={[
    ['enableDefaultCategories', 'Boolean', "When true, LibreChat's built-in default categories are kept active and appear before custom categories. When false they are hidden, except general. Default: true"],
    ['list', 'Array of Objects', 'Ordered list of categories. The declared order is preserved in the marketplace UI (after default categories when enableDefaultCategories is true).'],
  ]}
/>
...
<OptionTable
  options={[
    ['value', 'String', 'Required. Identifier for the category, and the display name when label is omitted. Stored lowercase internally and treated case-insensitively when matching existing categories.'],
    ['label', 'String', 'Optional. Display name for the category. Falls back to value when omitted.'],
    ['description', 'String', 'Optional. Short description shown alongside the category in the marketplace.'],
  ]}
/>
  ...
  marketplace:
    ...
    categories:
      ...
      list:                          # Order here determines marketplace order
        ...
        - value: "productivity"
          label: "Productivity & Focus"  # Optional, defaults to value
          description: "Productivity agents."
```

#### Localized labels and descriptions

`label` and `description` are displayed verbatim unless they start with `com_`, in which case the client resolves them as UI translation keys and renders the string for each user's own language. Omitting `label` displays `value` exactly as typed, in every locale.

```yaml
list:
  - value: "support"
    label: "com_agents_category_aftersales"              # localized per user
    description: "com_agents_category_aftersales_description"
  - value: "Education"                                   # shown as "Education" everywhere
    description: "Educational agents."
```

<Callout type="warning">
  A `com_` key must already exist in LibreChat's bundled locale files. Unknown keys are rendered
  literally, so a typo shows up in the UI as `com_agents_category_typo` rather than falling back to
  the category name.
</Callout>

#### Overriding built-in categories

Declaring a `value` that matches a built-in category overrides that category's `label` and `description` instead of creating a new one. Fields you omit keep their existing values, so declaring only `value` changes nothing but visibility.

An overridden category stays a built-in: it keeps its original position in the marketplace, is never deleted when removed from `list`, and remains active even when `enableDefaultCategories` is `false`. That last point is how you keep a subset of the built-in categories while hiding the rest.
...
interface:
  marketplace:
    use: true
    categories:
      enableDefaultCategories: false   # hide the built-in categories...
      list:
        - value: "aftersales"          # ...except this one, renamed and kept visible
          label: "Customer Success"
        - value: "Education"
          description: "Educational agents."
```

> **Note:** Removing a custom category from `list` deletes it from the database on the next startup. Built-in categories are never deleted, `enableDefaultCategories: false` only hides them.

> **Note:** The `general` category stays active even when `enableDefaultCategories` is `false`, because it is the category assigned to agents created without one. Hiding it would leave those agents reachable only from the "All" tab. You can still rename it by declaring `general` in `list`.
