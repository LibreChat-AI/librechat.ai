...
<Callout type="info" title="Deze instelling regelt alleen de opslag, niet de weergave">

`transactions` bepaalt of gebruiksgegevens naar de **database worden geschreven**. Het toont nergens in de app het aantal tokens, dus het inschakelen hiervan alleen zorgt er niet voor dat tokeninformatie in de UI verschijnt.

Om tokengebruik te *zien*, gebruik je `interface.contextUsage`, die de contextmeter in het gesprek tekent en standaard is ingeschakeld. `interface.contextCost` is een afzonderlijke instelling, standaard uitgeschakeld, die kostenbedragen aan die meter toevoegt; laat deze uitgeschakeld als je tokenaantallen wilt zonder prijzen weer te geven. Zie [Token Usage](/docs/configuration/token_usage#viewing-context-usage-and-cost). Om kredietlimieten per gebruiker af te dwingen, zie [Balance](/docs/configuration/librechat_yaml/object_structure/balance).
