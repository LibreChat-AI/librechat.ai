---
title: Konfiguracja mowy
icon: Mic
---

## Przegląd [#overview]

Obiekt `speech` umożliwia konfigurację dostawców zamiany tekstu na mowę (TTS) oraz mowy na tekst (STT) bezpośrednio w pliku konfiguracyjnym `librechat.yaml`. Pozwala to na korzystanie z usług głosowych po stronie serwera bez konieczności konfigurowania przez użytkowników własnych kluczy API.

**Pola w `speech`:**

- `tts` - Konfiguracje dostawców Text-to-Speech
- `stt` - Konfiguracje dostawcy Speech-to-Text
- `speechTab` - Domyślne ustawienia interfejsu użytkownika dla funkcji mowy

**Uwagi:**

- Wiele dostawców może być skonfigurowanych jednocześnie
- Użytkownicy mogą wybrać preferowanego dostawcę spośród dostępnych opcji
- Klucze API w pliku konfiguracyjnym powinny używać odniesień do zmiennych środowiskowych ze względów bezpieczeństwa

## Przykład [#example]

```yaml filename="speech"
speech:
  tts:
    allowedAddresses: ["tts.internal:8080"]
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["voice-id-1", "voice-id-2"]
  stt:
    allowedAddresses: ["stt.internal:8080"]
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: true
    advancedMode: false
    speechToText: true
    textToSpeech: true
```

---

## Ochrona SSRF [#ssrf-protection]

LibreChat chroni dostarczone przez operatora adresy URL STT i TTS w momencie nawiązywania połączenia. Miejsca docelowe typu private, loopback, link-local oraz cloud-metadata są domyślnie blokowane, a przekierowania dla żądań mowy są wyłączone.

`speech.tts.allowedAddresses` oraz `speech.stt.allowedAddresses` to oddzielne listy wyjątków, a nie ścisłe listy dozwolonych adresów (allowlists). Dodawaj dokładny prywatny `host:port`, `private.ip:port` lub `[ipv6]:port` tylko wtedy, gdy dana sekcja musi uzyskać dostęp do zaufanej usługi hostowanej samodzielnie. Nie używaj adresu URL, ścieżki, zakresu CIDR, samego hosta/IP ani publicznego literału IP. Domyślne porty są normalizowane, więc adres URL HTTPS bez jawnie podanego portu jest sprawdzany jako port `443`.

Wyłączona z ograniczeń nazwa hosta ufa każdemu prywatnemu adresowi, na który rozwiązuje się na danym porcie. Jeśli to możliwe, preferuj literał prywatnego adresu IP i wymieniaj tylko te nazwy, których DNS kontrolujesz i których nie można przekierować przez atakującego.

Gdy serwer proxy przekazuje żądanie z nazwą hosta, wykonuje on rozpoznawanie DNS miejsca docelowego oraz ruch wychodzący, dlatego musi egzekwować własną politykę SSRF. Literalny prywatny adres docelowy jest nadal blokowany przed przetworzeniem przez proxy, chyba że jego dokładny adres i port zostaną wyłączone z tego ograniczenia.

---

## tts

Obiekt `tts` konfiguruje dostawców zamiany tekstu na mowę (Text-to-Speech). Można skonfigurować wielu dostawców, a użytkownicy mogą wybrać, którego z nich chcą używać.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'Zaufane prywatne host:port wykluczenia dla kontroli SSRF podczas łączenia TTS. Publiczne miejsca docelowe pozostają dostępne.',
      'allowedAddresses: ["tts.internal:8080"]',
    ],
  ]}
/>

### openai

Konfiguracja OpenAI TTS przy użyciu modeli takich jak `tts-1` lub `tts-1-hd`.

<OptionTable
  options={[
    ['url', 'String', 'Niestandardowy adres URL API (opcjonalnie). Użyj dla endpointów zgodnych z OpenAI.', ''],
    ['apiKey', 'String', 'Klucz API OpenAI. Użyj odniesienia do zmiennej środowiskowej.', 'Required'],
    ['model', 'String', 'Model TTS do użycia (np. "tts-1", "tts-1-hd").', 'Required'],
    ['voices', 'Array of Strings', 'Dostępne opcje głosowe do wyboru przez użytkowników.', 'Required'],
  ]}
/>

**Przykład:**
```yaml filename="speech / tts / openai"
tts:
  openai:
    apiKey: "${TTS_API_KEY}"
    model: "tts-1"
    voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
```

### azureOpenAI

Konfiguracja Azure OpenAI TTS.

<OptionTable
  options={[
    ['instanceName', 'String', 'Nazwa instancji Azure OpenAI.', 'Required'],
    ['apiKey', 'String', 'Klucz API Azure OpenAI.', 'Required'],
    ['deploymentName', 'String', 'Nazwa wdrożenia dla modelu TTS.', 'Required'],
    ['apiVersion', 'String', 'Wersja API Azure OpenAI.', 'Required'],
    ['model', 'String', 'Identyfikator modelu TTS.', 'Required'],
    ['voices', 'Array of Strings', 'Dostępne opcje głosowe.', 'Required'],
  ]}
/>

**Przykład:**
```yaml filename="speech / tts / azureOpenAI"
tts:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_TTS_API_KEY}"
    deploymentName: "tts-deployment"
    apiVersion: "2024-02-15-preview"
    model: "tts-1"
    voices: ["alloy", "echo", "nova"]
```

### elevenlabs

Konfiguracja ElevenLabs TTS dla wysokiej jakości syntezy mowy.

<OptionTable
  options={[
    ['url', 'String', 'Niestandardowy adres URL API (opcjonalnie).', ''],
    ['websocketUrl', 'String', 'Adres URL WebSocket do przesyłania strumieniowego (opcjonalnie).', ''],
    ['apiKey', 'String', 'Klucz API ElevenLabs.', 'Required'],
    ['model', 'String', 'Model ElevenLabs (np. "eleven_multilingual_v2").', 'Required'],
    ['voices', 'Array of Strings', 'Dostępne identyfikatory głosów do wyboru.', 'Required'],
    ['voice_settings', 'Object', 'Ustawienia personalizacji głosu (opcjonalne).', ''],
    ['pronunciation_dictionary_locators', 'Array of Strings', 'Identyfikatory słownika wymowy (opcjonalne).', ''],
  ]}
/>

**Podklucze voice_settings:**
<OptionTable
  options={[
    ['similarity_boost', 'Number', 'Ulepszenie podobieństwa głosu (0-1).', ''],
    ['stability', 'Number', 'Stabilność głosu (0-1).', ''],
    ['style', 'Number', 'Przesada stylu (0-1).', ''],
    ['use_speaker_boost', 'Boolean', 'Włącz wzmocnienie głośnika.', ''],
  ]}
/>

**Przykład:**
```yaml filename="speech / tts / elevenlabs"
tts:
  elevenlabs:
    apiKey: "${ELEVENLABS_API_KEY}"
    model: "eleven_multilingual_v2"
    voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
    voice_settings:
      stability: 0.5
      similarity_boost: 0.75
      use_speaker_boost: true
```

### localai

Konfiguracja LocalAI TTS dla samodzielnie hostowanej syntezy mowy.

<OptionTable
  options={[
    ['url', 'String', 'Adres URL serwera LocalAI.', 'Required'],
    ['apiKey', 'String', 'Klucz API, jeśli uwierzytelnianie jest włączone (opcjonalnie).', ''],
    ['voices', 'Array of Strings', 'Dostępne modele głosowe.', 'Required'],
    ['backend', 'String', 'Backend TTS do użycia (np. "piper").', 'Required'],
  ]}
/>

**Przykład:**
```yaml filename="speech / tts / localai"
tts:
  localai:
    url: "http://localhost:8080"
    voices: ["en-us-amy-low", "en-us-danny-low"]
    backend: "piper"
```

---

## stt

Obiekt `stt` konfiguruje dostawców funkcji Speech-to-Text.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'Zaufane prywatne host:port wykluczenia dla kontroli SSRF podczas łączenia STT. Publiczne miejsca docelowe pozostają dostępne.',
      'allowedAddresses: ["stt.internal:8080"]',
    ],
  ]}
/>

### openai [#openai-1]

Konfiguracja OpenAI Whisper STT.

<OptionTable
  options={[
    ['url', 'String', 'Niestandardowy adres URL API (opcjonalnie). Użyj dla endpointów zgodnych z OpenAI.', ''],
    ['apiKey', 'String', 'Klucz API OpenAI. Użyj odniesienia do zmiennej środowiskowej.', 'Required'],
    ['model', 'String', 'Model STT do użycia (np. "whisper-1").', 'Required'],
  ]}
/>

**Przykład:**
```yaml filename="speech / stt / openai"
stt:
  openai:
    apiKey: "${STT_API_KEY}"
    model: "whisper-1"
```

### azureOpenAI [#azureopenai-1]

Konfiguracja Azure OpenAI Whisper STT.

<OptionTable
  options={[
    ['instanceName', 'String', 'Nazwa instancji Azure OpenAI.', 'Required'],
    ['apiKey', 'String', 'Klucz API Azure OpenAI.', 'Required'],
    ['deploymentName', 'String', 'Nazwa wdrożenia dla modelu Whisper.', 'Required'],
    ['apiVersion', 'String', 'Wersja API Azure OpenAI.', 'Required'],
  ]}
/>

**Przykład:**
```yaml filename="speech / stt / azureOpenAI"
stt:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_STT_API_KEY}"
    deploymentName: "whisper-deployment"
    apiVersion: "2024-02-15-preview"
```

---

## speechTab

Obiekt `speechTab` konfiguruje domyślne ustawienia interfejsu użytkownika dla funkcji mowy. Ustawienia te określają, co użytkownicy widzą domyślnie w panelu ustawień mowy.

<OptionTable
  options={[
    ['conversationMode', 'Boolean', 'Włącz tryb konwersacji domyślnie.', 'false'],
    ['advancedMode', 'Boolean', 'Domyślnie pokazuj zaawansowane ustawienia mowy.', 'false'],
    ['speechToText', 'Boolean or Object', 'Włącz STT domyślnie lub skonfiguruj szczegółowe ustawienia STT.', 'false'],
    ['textToSpeech', 'Boolean or Object', 'Włącz TTS domyślnie lub skonfiguruj szczegółowe ustawienia TTS.', 'false'],
  ]}
/>

### speechToText (Format obiektu) [#speechtotext-object-format]

W przypadku użycia obiektu zamiast wartości logicznej:

<OptionTable
  options={[
    [
      'engineSTT',
      'String',
      'Domyślny silnik STT. Użyj `"browser"` lub `"external"`. Starsze wartości `"openai"` oraz `"azureOpenAI"` są akceptowane i normalizowane do `"external"`.',
      'browser',
    ],
    ['languageSTT', 'String', 'Domyślny język dla STT.', ''],
    ['autoTranscribeAudio', 'Boolean', 'Utrzymuj mikrofon włączony zamiast zatrzymywać się przy pierwszej pauzie. W przypadku zewnętrznego silnika włącza to również wykrywanie ciszy w oparciu o `decibelValue`.', ''],
    ['decibelValue', 'Number', 'Próg decybeli dla wykrywania ciszy. Zakres od -100 do -30, domyślnie -45.', ''],
    ['autoSendText', 'Number', 'Liczba sekund oczekiwania po transkrypcji przed automatycznym wysłaniem. `0` wysyła natychmiast; `-1` wyłącza automatyczne wysyłanie.', ''],
  ]}
/>

### textToSpeech (Format obiektu) [#texttospeech-object-format]

W przypadku użycia obiektu zamiast wartości logicznej:

<OptionTable
  options={[
    [
      'engineTTS',
      'String',
      'Domyślny silnik TTS. Użyj `"browser"` lub `"external"`. Starsze wartości dostawców są akceptowane i normalizowane do `"external"`.',
      'browser',
    ],
    ['voice', 'String', 'Domyślny wybór głosu.', ''],
    ['languageTTS', 'String', 'Domyślny język dla TTS.', ''],
    ['automaticPlayback', 'Boolean', 'Automatycznie odtwarzaj odpowiedzi TTS.', ''],
    ['playbackRate', 'Number', 'Domyślna prędkość odtwarzania (1.0 = normalna). Zakres: 0.25–4.0.', ''],
    ['cacheTTS', 'Boolean', 'Buforuj dźwięk TTS dla powtarzanego odtwarzania.', ''],
  ]}
/>

**Przykład:**
```yaml filename="speech / speechTab"
speechTab:
  conversationMode: false
  advancedMode: false
  speechToText:
    engineSTT: "external"
    autoTranscribeAudio: true
    decibelValue: -45
  textToSpeech:
    engineTTS: "external"
    voice: "nova"
    automaticPlayback: false
    playbackRate: 1.0
    cacheTTS: true
```

---

## Kompletny przykład [#complete-example]

```yaml filename="librechat.yaml"
version: 1.3.15
cache: true

speech:
  tts:
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1-hd"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
      voice_settings:
        stability: 0.5
        similarity_boost: 0.75
  stt:
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: false
    advancedMode: false
    speechToText: true
    textToSpeech:
      engineTTS: "external"
      voice: "nova"
      automaticPlayback: false
```

---

## Uwagi [#notes]

- Zawsze używaj odwołań do zmiennych środowiskowych (np. `${API_KEY}`) dla kluczy API w plikach konfiguracyjnych
- Można skonfigurować wielu dostawców TTS; użytkownicy wybierają preferowaną opcję w interfejsie użytkownika.
- Ustawienia `speechTab` definiują wartości domyślne, które użytkownicy mogą nadpisać w swoich ustawieniach osobistych.
- `browser` korzysta z wbudowanej obsługi mowy w przeglądarce; `external` korzysta z dostawców po stronie serwera skonfigurowanych w `speech.stt` lub `speech.tts`
- Istniejące domyślne ustawienia STT dla `openai` i `azureOpenAI` oraz domyślne ustawienia TTS dla `openai`, `azureOpenAI`, `elevenlabs` i `localai` zostały przeniesione do `external`; w przypadku niedostępności zewnętrznego silnika następuje powrót do `browser`.
- Aby uzyskać szczegółową dokumentację funkcji, zobacz [Speech to Text & Text to Speech](/docs/configuration/stt_tts)
