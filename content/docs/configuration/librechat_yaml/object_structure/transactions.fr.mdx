...
<Callout type="info" title="Ce paramètre contrôle uniquement le stockage, pas l'affichage">

`transactions` détermine si les enregistrements d'utilisation sont **écrits dans la base de données**. Cela n'affiche pas le nombre de jetons nulle part dans l'application ; par conséquent, l'activer seul ne fera pas apparaître les informations sur les jetons dans l'interface utilisateur.

Pour *voir* l'utilisation des jetons, utilisez `interface.contextUsage`, qui affiche la jauge de contexte dans la conversation et qui est activée par défaut. `interface.contextCost` est un paramètre distinct, désactivé par défaut, qui ajoute des chiffres de coût à cette jauge ; laissez-le désactivé si vous souhaitez obtenir le nombre de jetons sans afficher la tarification. Voir [Token Usage](/docs/configuration/token_usage#viewing-context-usage-and-cost). Pour appliquer des limites de crédit par utilisateur, voir [Balance](/docs/configuration/librechat_yaml/object_structure/balance).
