...
<OptionTable
    options={[
        ['enabled', 'Boolean', 'Détermine s\'il faut enregistrer les relevés de transaction dans la base de données.', 'Default: true. Set to false to disable transaction recording (unless balance.enabled is true).'],
    ]}
/>
