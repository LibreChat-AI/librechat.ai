...
> **Important :** Claude Opus 4.7 et versions ultérieures, Opus 5, Sonnet 5, et Fable/Mythos 5 ou 5.1 nécessitent `global`, `us`, ou `eu`. Les régions spécifiques telles que `us-east5` prennent en charge Claude Sonnet 4.6 et versions antérieures et renvoient une réponse « model-not-found » pour les modèles plus récents. Utilisez `global` pour une disponibilité maximale et sans surcoût lié à la région, ou `us`/`eu` lorsque vous avez besoin d'une résidence des données multi-régionale.
