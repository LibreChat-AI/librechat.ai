...
> **Important :** Claude Opus 4.7 et versions ultérieures, Opus 5, Sonnet 5, et Fable/Mythos 5 nécessitent `global`, `us`, ou `eu`. Les régions spécifiques telles que `us-east5` prennent en charge Claude Sonnet 4.6 et versions antérieures et renvoient une réponse « model-not-found » pour les modèles plus récents. Utilisez `global` pour une disponibilité maximale et éviter les surcoûts liés aux régions, ou `us`/`eu` lorsque vous avez besoin d'une résidence des données multi-régionale.
...
Lorsque `models` est omis, LibreChat utilise son catalogue Vertex intégré et supprime les modèles incompatibles avec la `region` configurée. Une liste ou une carte `models` explicite n'est jamais filtrée, par conséquent chaque ID de déploiement doit être disponible dans l'emplacement sélectionné.
...
> **Note :** Lors de l'utilisation de variables d'environnement, le mappage de modèles n'est pas disponible et LibreChat inclut son catalogue de modèles Anthropic standard. Définissez `ANTHROPIC_VERTEX_REGION` sur `global`, `us` ou `eu` avant de sélectionner un modèle moderne qui n'est pas disponible dans une région spécifique.
...
- Modèles : catalogue Vertex intégré de LibreChat pour l'emplacement sélectionné
...
- Utilisez `global`, `us` ou `eu` pour Opus 4.7+, Opus 5, Sonnet 5 et Fable/Mythos 5
...
- Passez à `global`, `us` ou `eu` pour les modèles plus récents que Sonnet 4.6
