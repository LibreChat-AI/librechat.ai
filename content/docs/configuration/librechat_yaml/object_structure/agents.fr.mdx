...
Pour plus d'informations sur les étapes des agents, consultez [Max Agent Steps](/docs/features/agents#max-agent-steps).
...
**Note :** Doit être l'un des éléments suivants, ou un nom de endpoint personnalisé tel que défini dans votre [configuration](/docs/configuration/librechat_yaml/object_structure/custom_endpoint#name) : - `openAI, azureOpenAI, google, anthropic, assistants, azureAssistants, bedrock`
...
<OptionTable
  options={[
    [
      'maxCitations',
      'Number',
      'Contrôle le nombre total maximal de citations pouvant être incluses dans une seule réponse d\'agent.',
...
- **subagents** : Permet des exécutions d'agents enfants dans un contexte isolé. Voir [Subagents](/docs/features/subagents).
...
<OptionTable
  options={[
    [
      'auth',
      'Object',
      'Configuration d\'authentification pour l\'API Remote Agent.',
...
<OptionTable
  options={[
    ...
    [
      'allowSelf',
      'Boolean',
      'Permet à l\'agent de se générer dans un nouveau contexte isolé. Par défaut : true.',
      'allowSelf: true',
    ],
    [
      'agent_ids',
