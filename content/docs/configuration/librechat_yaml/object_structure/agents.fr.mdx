...
L'appel d'outils programmatique (Programmatic Tool Calling) est une fonctionnalité optionnelle. Ajoutez `programmatic_tools` aux côtés de `execute_code` uniquement lorsque votre déploiement de l'interpréteur de code inclut le composant Tool Call Server.
