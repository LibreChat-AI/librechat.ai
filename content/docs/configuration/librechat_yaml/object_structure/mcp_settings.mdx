...
mcpSettings:
  ...
  # Withhold an MCP tool's argument schema from the model when it is larger than
  # this many bytes. 0, the default, disables the rule.
  deferSchemaBytes: 4096
...
Setting `deferSchemaBytes` withholds the argument schema of any tool above that
size. The model still sees the tool's name and description in the `tool_search`
...
2. `mcpSettings.deferSchemaBytes`, when the tool has no explicit toggle.
