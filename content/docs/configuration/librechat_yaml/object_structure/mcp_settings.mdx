...
mcpSettings:
  ...
  # Withhold an MCP tool's argument schema from the model when its JSON is longer
  # than this many characters. 0, the default, disables the rule.
  deferSchemaChars: 4096
...
<OptionTable
  options={[
    ...
    ['deferSchemaChars', 'Number', 'Argument-schema size, in characters of its JSON, above which an MCP tool starts deferred: the model sees only its name and loads the schema through `tool_search` when it needs it. Needs the agents `deferred_tools` capability. A per-tool `defer_loading` toggle still takes precedence in either direction.', '0, which disables the rule and leaves every tool exactly as its own options configure it.'],
    ['catalogRecovery', 'Object', 'Optional bounds for passive MCP discovery, OAuth reauthentication recovery, and cross-replica authorization fencing.', 'Uses the defaults shown in the example.'],
  ]}
/>

## catalogRecovery

`catalogRecovery` prevents passive catalog requests from repeatedly rediscovering disconnected OAuth servers and bounds the authorization-generation checks used across replicas. Most deployments should retain the defaults.

<OptionTable
  options={[
    ['discoveryBackoffMs', 'Array of Numbers', 'Progressive delays after failed passive discovery, in milliseconds (1-8 entries).', '[300000, 600000, 1200000, 1800000]'],
    ['discoveryTimeoutMs', 'Number', 'Timeout for one passive discovery attempt.', '3000'],
    ['discoverySettleGraceMs', 'Number', 'Additional time a timed-out discovery may retain its coalesced slot while it settles.', '10000'],
    ['reauthRetryMs', 'Number', 'Delay before retrying a server that requires interactive OAuth authorization.', '1800000'],
    ['maxStateEntries', 'Number', 'Maximum process-local server recovery states retained in memory.', '10000'],
    ['maxDetachedDiscoveries', 'Number', 'Maximum timed-out discoveries still settling before recovery stops starting new discoveries.', '3'],
    ['generationReadTimeoutMs', 'Number', 'Deadline for reading the cross-replica credential generation.', '500'],
    ['authorizationFenceRetryMs', 'Array of Numbers', 'Retry delays for publishing an authorization-generation fence (1-8 entries).', '[0, 50, 200]'],
    ['authorizationFenceTimeoutMs', 'Number', 'Timeout for each authorization-fence write attempt.', '1000'],
    ['authorizationFenceRetryIntervalMs', 'Number', 'Interval for replaying durable failed fence writes.', '30000'],
    ['authorizationFenceRetryBatchSize', 'Number', 'Maximum durable fence records replayed per interval.', '100'],
  ]}
/>

Concurrent passive discovery is coalesced per user and server. OAuth challenges settle into a stable reauthentication-required state, while transient token or cache outages remain retryable. Credential changes publish a generation fence so stale discovery results are discarded instead of being reused on another replica.

`discoverySettleGraceMs` may be `0` through `300000`; the late discovery is never cancelled so a redeemed OAuth token can still persist. `maxDetachedDiscoveries` ranges from `1` to `1000` and bounds how many such released discoveries may continue before new recovery work waits for one to settle.
...
A small number of MCP tools can account for most of a catalog's size. A tool whose
argument schema is very large costs those tokens on every turn, even when the
conversation has nothing to do with it, while its description may be only a line.

Setting `deferSchemaChars` withholds the argument schema of any tool whose JSON is
longer than that many characters. Characters track the tokens the rule saves more
closely than bytes would. The model still sees the tool's name in the `tool_search`
listing and loads the schema when it decides to use it, so nothing becomes
unreachable. The rule only applies when the agents `deferred_tools` capability is
enabled; with it off, every tool loads in full.

Precedence, highest first:

1. A per-tool `defer_loading` toggle in the agent builder, in either direction.
2. `mcpSettings.deferSchemaChars`, when the tool has no explicit toggle.
3. Otherwise the tool is loaded in full.

Because an explicit per-tool choice always wins, the rule is safe to apply
instance-wide without overriding tools an author deliberately pinned open.

In the agent builder, a tool deferred by this rule shows its defer toggle as on,
with a tooltip saying it was deferred automatically. Switching it off keeps that
tool loaded for the agent; switching it back on returns the tool to the rule.
