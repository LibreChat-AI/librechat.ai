...
<Callout type="info" title="Cú pháp mẫu MIME">
  LibreChat đánh giá các mẫu `supportedMimeTypes` do quản trị viên cấu hình trên máy chủ bằng công cụ biểu thức chính quy thời gian tuyến tính tương thích với RE2. Các tham chiếu ngược (backreferences) và kiểm tra xung quanh (lookaround) không được hỗ trợ. Các mẫu không hợp lệ sẽ được ghi nhật ký và bỏ qua; nếu mọi mẫu trong danh sách được cấu hình đều không hợp lệ, danh sách đó sẽ chuyển sang trạng thái đóng và từ chối mọi tệp thay vì trở nên không bị hạn chế.
</Callout>
## skills
