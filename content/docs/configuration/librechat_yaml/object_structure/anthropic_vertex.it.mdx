...
> **Importante:** Claude Opus 4.7 e versioni successive, Opus 5, Sonnet 5 e Fable/Mythos 5 richiedono `global`, `us` o `eu`. Regioni specifiche come `us-east5` supportano Claude Sonnet 4.6 e versioni precedenti e restituiscono una risposta di modello non trovato per i modelli più recenti. Utilizza `global` per la massima disponibilità e per evitare sovrapprezzi regionali, oppure `us`/`eu` quando hai bisogno della residenza dei dati multi-regione.
