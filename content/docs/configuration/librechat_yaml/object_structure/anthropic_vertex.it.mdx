...
> **Importante:** Claude Opus 4.7 e versioni successive, Opus 5, Sonnet 5 e Fable/Mythos 5 o 5.1 richiedono `global`, `us` o `eu`. Regioni specifiche come `us-east5` supportano Claude Sonnet 4.6 e versioni precedenti e restituiscono una risposta di modello non trovato per i modelli più recenti. Utilizza `global` per la massima disponibilità e per evitare sovrapprezzi regionali, oppure `us`/`eu` quando hai bisogno della residenza dei dati in più regioni.
...
Quando `models` viene omesso, LibreChat utilizza il suo catalogo Vertex integrato e rimuove i modelli incompatibili con la `region` configurata. Un elenco o una mappa `models` esplicita non viene mai filtrata, pertanto ogni ID di distribuzione deve essere disponibile nella posizione selezionata.
...
> **Nota:** Quando si utilizzano le variabili d'ambiente, il model mapping non è disponibile e LibreChat include il suo catalogo standard di modelli Anthropic. Impostare `ANTHROPIC_VERTEX_REGION` su `global`, `us` o `eu` prima di selezionare un modello moderno non disponibile in una specifica regione.
...
- Modelli: catalogo Vertex integrato di LibreChat per la posizione selezionata
...
- Usa `global`, `us` o `eu` per Opus 4.7+, Opus 5, Sonnet 5 e Fable/Mythos 5
...
- Passa a `global`, `us` o `eu` per modelli più recenti di Sonnet 4.6
