...
<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'OCR 연결 시 SSRF 검사를 면제할 신뢰할 수 있는 비공개 host:port입니다. 공용 대상은 계속 사용 가능합니다. 항목은 반드시 포트를 포함해야 하며 URL, 경로, CIDR 범위, 호스트 이름만 단독으로 사용하거나 공용 IP 리터럴을 사용할 수 없습니다.',
      'allowedAddresses: ["ocr.internal:8080"]',
    ],
  ]}
/>
...
OCR 요청은 연결 시점에 private, loopback, link-local 및 cloud-metadata 대상을 차단하며 리디렉션을 비활성화합니다. `allowedAddresses`는 신뢰할 수 있는 정확한 private `host:port`, `private.ip:port` 또는 `[ipv6]:port` 대상을 위한 예외 목록이며, 공용 대상에 대한 액세스를 제한하지는 않습니다.

<Callout type="warning" title="신중하게 면제 항목을 신뢰하세요">
  `allowedAddresses`는 직접 제어하는 비공개 OCR 서비스에만 사용하세요. 예외 처리된 호스트 이름은 해당 포트에서 확인되는 모든 비공개 주소를 신뢰하므로, 가능하면 비공개 IP 리터럴을 사용하는 것이 좋습니다. 포워드 프록시를 사용하는 경우, 프록시가 대상 SSRF 정책을 강제해야 합니다. 명시적으로 예외 처리되지 않는 한 리터럴 비공개 대상은 계속 차단됩니다.
</Callout>
