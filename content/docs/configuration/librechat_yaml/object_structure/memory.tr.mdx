...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'İsteğe bağlı otomatik bellek aracısını yapılandırır. Mevcut bir aracıya ID ile referans olabilir veya tam bir aracı yapılandırması olabilir.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Otomatik çıkarma isteğe bağlıdır (opt-in). Aşağıdaki yapılandırma formatlarından birinde `agent.enabled: true` ayarını yapın. `enabled` değerini atlamak veya `false` olarak ayarlamak, sohbet isteklerinde otomatik bellek aracını çalıştırmadan manuel belleği ve Agent bellek araçlarını kullanılabilir durumda tutar.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Yapılandırılmış memory agent ile otomatik bellek çıkarımını etkinleştirir.',
      'enabled: true',
    ],
  ]}
/>
...
- Otomatik bellek çıkarımı için `agent.enabled: true` gereklidir
- Ajan yapılandırması, otomatik bellek işleme davranışının özelleştirilmesine olanak tanır.
