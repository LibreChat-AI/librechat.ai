...
<Callout type="info" title="To ustawienie kontroluje tylko przechowywanie, a nie wyświetlanie">

`transactions` decyduje o tym, czy rekordy użycia są **zapisywane w bazie danych**. Nie wyświetla on liczby tokenów w żadnym miejscu aplikacji, więc samo jego włączenie nie spowoduje pojawienia się informacji o tokenach w interfejsie użytkownika.

Aby *zobaczyć* zużycie tokenów, użyj `interface.contextUsage`, co powoduje wyświetlenie wskaźnika kontekstu wewnątrz konwersacji; opcja ta jest domyślnie włączona. `interface.contextCost` to oddzielne ustawienie, domyślnie wyłączone, które dodaje wartości kosztów do tego wskaźnika; pozostaw je wyłączone, jeśli chcesz widzieć liczbę tokenów bez ujawniania cen. Zobacz [Token Usage](/docs/configuration/token_usage#viewing-context-usage-and-cost). Aby wymusić limity kredytów dla użytkownika, zobacz [Balance](/docs/configuration/librechat_yaml/object_structure/balance).
