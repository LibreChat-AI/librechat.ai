...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Configureert de optionele automatische memory agent. Kan een verwijzing naar een bestaande agent op ID zijn of een volledige agentconfiguratie.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Automatische extractie is opt-in. Stel `agent.enabled: true` in in een van de onderstaande configuratieformaten. Het weglaten van `enabled`, of het instellen op `false`, houdt handmatig geheugen en Agent-geheugentools beschikbaar zonder de automatische geheugenagent uit te voeren bij chatverzoeken.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Schakelt automatische geheugenextractie in met de geconfigureerde memory agent.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` is vereist voor automatische geheugenextractie
- Agent-configuratie maakt aanpassing van het gedrag voor automatische geheugenverwerking mogelijk
