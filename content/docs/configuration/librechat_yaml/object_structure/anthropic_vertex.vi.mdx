...
> **Quan trọng:** Claude Opus 4.7 và mới hơn, Opus 5, Sonnet 5, và Fable/Mythos 5 yêu cầu `global`, `us`, hoặc `eu`. Các khu vực cụ thể như `us-east5` chỉ phục vụ Claude Sonnet 4.6 trở về trước và sẽ trả về phản hồi model-not-found đối với các model mới hơn. Hãy sử dụng `global` để có độ khả dụng rộng nhất và không bị tính phí chênh lệch theo khu vực, hoặc sử dụng `us`/`eu` khi bạn cần lưu trữ dữ liệu đa khu vực.
