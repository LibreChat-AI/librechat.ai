...
- Weryfikuje i publikuje Skills niezależnie, dzięki czemu jeden nieprawidłowy lub powodujący konflikt Skill nie ukrywa prawidłowych Skills z tego samego źródła.
- Zachowuje ostatnią znaną poprawną kopię pominiętej umiejętności (Skill) i wycofuje niekompletne zapisy dla tej umiejętności.
- Oznacza uruchomienie jako `partial`, gdy publikuje ono przynajmniej jedną umiejętność (Skill), ale pomija inną, oraz rejestruje ograniczoną liczbę pominięć i szczegóły dla odczytów statusu administratora.
- Używa blokady synchronizacji, dzięki czemu w danym momencie może być uruchomiona tylko jedna synchronizacja z GitHub.

Nieznane klucze frontmatter w `SKILL.md` są akceptowane, o ile ich wartości mieszczą się w ograniczonych, bezpiecznych dla JSON limitach rozszerzeń. Nieprawidłowe rozpoznane pola, niebezpieczne wartości rozszerzeń, zduplikowane nazwy oraz konflikty własności powodują pominięcie tylko danego Skill. Błędy uwierzytelniania, limitów szybkości (rate-limit), utraty blokady (lock-loss), dostępu do repozytorium oraz wycofania zmian (rollback) pozostają błędami na poziomie źródłowym.
...
<OptionTable
  options={[
    [
      ...
      'Endpoint',
      'Zwraca stan włączenia, ustawienia harmonogramu, status źródła, obecność poświadczeń, liczbę ostatnich synchronizacji oraz szczegóły pominiętych umiejętności (Skill) dla częściowych uruchomień.',
      '',
    ],
    [
