...
- [`classification.memoryGate`](/docs/configuration/librechat_yaml/object_structure/classification#memorygate) can skip the automatic memory agent on turns that carry nothing durable, and suggest which valid key a memory belongs under
