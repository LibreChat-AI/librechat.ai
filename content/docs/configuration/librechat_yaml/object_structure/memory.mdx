...
- [`classification.memoryGate`](/docs/configuration/librechat_yaml/object_structure/classification#memorygate) can skip the automatic memory agent on turns that do not ask to remember, update or forget anything, and suggest which valid key a request concerns
