...
> **Nota:** Ogni parametro sottostante include una nota su quali endpoint lo supportano.  
> **OpenAI / AzureOpenAI / Custom** supportano solitamente `temperature`, `presence_penalty`, `frequency_penalty`, `stop`, `top_p`, `max_tokens`.  
> **Google / Anthropic** supportano solitamente `topP`, `topK`, `maxOutputTokens`.  
> **Anthropic / Bedrock (modelli Anthropic e Nova)** supportano `promptCache`.  
...
> **Supportato da:** `anthropic`, `bedrock` (modelli Anthropic e Nova)  
> (Attiva la funzione “prompt-caching” di Anthropic)

<OptionTable
  options={[
    ['promptCache', 'Boolean', 'Abilita o disabilita la memorizzazione nella cache dei prompt integrata di Anthropic.', ''],
  ]}
/>
