...
Se la selezione salvata dell'utente punta a un Agent che non viene più risolto, LibreChat riattiva il soft default in modo che una nuova chat abbia comunque una selezione utilizzabile. Un errore temporaneo nel caricamento dell'elenco degli Agent non cancella la scelta salvata né attiva tale fallback.
...
<OptionTable
  options={[
    [
      'showInMenu',
      'Boolean',
      'Controlla se la specifica completa del modello appare nel selettore del modello e nella configurazione di avvio del client.',
      'true',
    ],
  ]}
/>

Imposta `showInMenu: false` per nascondere una spec dalla selezione interattiva, mantenendola comunque disponibile per i chiamanti lato server attendibili che inviano esplicitamente il suo `name` nel campo `spec`. Questo differisce da `showIconInMenu`, che nasconde solo l'icona.
...
<OptionTable
  options={[
    [
      'memory',
      'Boolean',
      'Equipa l\'agente effimero della Model Specs con strumenti di memoria.',
      'false',
    ],
  ]}
/>

Richiede [User Memory](/docs/features/memory) configurata, la funzionalità `memory` a livello di endpoint e le autorizzazioni per la memoria utente. Quando abilitato, il modello può salvare o eliminare memorie strutturate quando l'utente lo richiede esplicitamente.
...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Consente al Model Specs di mettere in pausa per porre fino a quattro domande correlate e riprendere con le risposte.',
      'false',
    ],
  ]}
/>

Richiede la funzionalità `ask_user_question` a livello di endpoint. Questa è supportata nelle esecuzioni di chat degli Agent che forniscono il flusso di pausa/ripresa persistente di LibreChat.
...
<OptionTable
  options={[
    [
      ...
      'Controlla quali strumenti idonei collegati alla Model Specs possono essere eseguiti come attività in background.',
      'Unset (background-native code defaults apply)',
    ],
  ]}
/>

Richiede la funzionalità `run_in_background` a livello di endpoint con consenso esplicito. Con tale funzionalità e `executeCode: true`, l'esecuzione di Code Interpreter e le chiamate shell sono idonee per l'esecuzione in background per impostazione predefinita. Imposta `runInBackground: false` o `[]` per escluderli, `true` per abilitare ogni strumento idoneo collegato alla specifica, oppure un array di ID strumento risolti per abilitare solo quegli strumenti. Selezionare `execute_code` o `bash_tool` seleziona entrambi gli strumenti di codice nativi; un elenco che li omette li esclude. Le selezioni non supportate o errate vengono registrate.

Il modello può avviare uno strumento idoneo e continuare a lavorare. Per gli Agent salvati, i completamenti basati solo sul contenuto supportati vengono forniti automaticamente per impostazione predefinita; `check_background_task` rimane disponibile per lo stato, il controllo, gli artefatti live e il ripristino. L'output del codice completato e i file generati vengono allegati alla chiamata di codice originale. L'esecuzione ordinaria è locale al processo e non sopravvive alla perdita del suo processo worker, ma un risultato del terminale persistente può essere fornito da un'altra replica. Gli amministratori possono ripristinare il comportamento basato solo sul polling con [`endpoints.agents.backgroundTasks.completionWakeups: false`](/docs/configuration/librechat_yaml/object_structure/agents#backgroundtasks).
...
<OptionTable
  options={[
    [
      'describeIntent',
      'Boolean or Array of Strings',
      'Controlla quali strumenti idonei trasmettono un intento scritto dal modello come etichetta di chiamata dello strumento in tempo reale.',
      'Unset (native intent defaults apply)',
    ],
  ]}
/>

Richiede la funzionalità `tool_intents` a livello di endpoint, soggetta a consenso esplicito. Gli strumenti nativi come la ricerca web, la creazione di file, la memoria e gli strumenti Code Interpreter supportati utilizzano le etichette di intento per impostazione predefinita quando la funzionalità è abilitata. Imposta `describeIntent: true` per abilitare ogni strumento idoneo, oppure fornisci gli ID degli strumenti risolti per abilitare solo quelli specifici e disabilitare le etichette per tutto il resto. Un array vuoto disabilita tutte le etichette di intento per la spec. L'omissione del campo o l'impostazione su `false` non aggiunge alcuna selezione più ampia, pertanto continuano ad applicarsi le impostazioni predefinite native.

Le etichette di intento sono incluse negli schemi degli strumenti e pertanto aggiungono token di schema alle richieste del modello. LibreChat registra le selezioni che sono scritte in modo errato, non disponibili per la specifica o non idonee.
...
- `agent_ids`: consente l'utilizzo di agenti persistenti specifici come sottoagenti aggiuntivi. Questo elenco è limitato dal limite effettivo dei sottoagenti, che è impostato di default a 10 ed è configurabile tramite [`endpoints.agents.maxSubagents`](/docs/configuration/librechat_yaml/object_structure/agents#maxsubagents). L'elenco rimane lato server; la configurazione di avvio inviata ai client include solo i flag pubblici `enabled` e `allowSelf`.
...
Le impostazioni dei modelli Google ora espongono questo controllo nello schema condiviso, in modo che gli amministratori possano scegliere esplicitamente se i file debbano essere reinviati nei turni successivi per le Model Specs di Google e Vertex AI.
...
Il server risolve inoltre la Model Specs effettiva e autorizza il suo `agent_id` persistente prima di caricare o eseguire tale Agent. Un chiamante non può utilizzare campi di richiesta o una mancata corrispondenza di preset per sostituire un Agent diverso dopo questo controllo di accesso.
...
<OptionTable
  options={[
    [
      'reasoning_effort',
      'String',
      'Controlla il livello di sforzo di ragionamento per il modello. Ridurre lo sforzo di ragionamento può portare a risposte più rapide e a un minor numero di token utilizzati per il ragionamento. I modelli OpenAI supportati possono esporre `xhigh` e `max`; per Bedrock, i valori accettati sono `low`, `medium`, `high`.',
      '',
    ],
  ]}
/>
...
> **Supportato da:** modelli OpenAI Responses API che espongono la modalità di ragionamento, incluso GPT-5.6

<OptionTable
  options={[
    ['reasoning_mode', 'String', 'Imposta la modalità di ragionamento dell\'API OpenAI Responses.', ''],
  ]}
/>
...
> **Supportato da:** modelli OpenAI Responses API che espongono il contesto di ragionamento, incluso GPT-5.6
...
<OptionTable
  options={[
    [
      'reasoning_context',
      'String',
      'Controlla quanto contesto conversazionale è disponibile per il ragionamento del modello.',
      '',
    ],
  ]}
/>
...
Per GPT-5.6, LibreChat imposta le richieste di ragionamento (reasoning) predefinite sulla Responses API, a meno che `useResponsesApi: false` non sia impostato esplicitamente.
...
Per i modelli Gemini 2.5, l'impostazione Google è consapevole del modello: Pro accetta `128`-`32768`, Flash accetta `0`-`24576` e Flash-Lite accetta `512`-`24576`. `-1` rimane l'impostazione automatica. I modelli Gemini 3+ utilizzano [`thinkingLevel`](#thinkinglevel) invece di un budget numerico.
