...
当通过管理员配置 API 编写自定义 endpoint 时，LibreChat 会在静态存储时加密字面量 `apiKey`，并仅返回 `apiKeyPreview`。`${ENV_VAR}` 引用和 `user_provided` 将保持为可读的透传值。对于直接在 `librechat.yaml` 中定义的 endpoint，请优先使用如上所示的环境变量引用。
...
- 在兼容 OpenAI 的自定义 endpoint 上，包含 `claude` 的模型 ID 会使用 Claude 文档限制：系统会接受 PDF 和文本文档，而其他非 PDF 的二进制文档类型会在请求发送前被跳过。
...
在最终的出站解析步骤中，无法识别的 `{{LIBRECHAT_USER_*}}`、`{{LIBRECHAT_BODY_*}}` 和 `{{LIBRECHAT_OPENID_*}}` 占位符将变为空字符串，而不是作为模板文本转发。未知的占位符名称将保持不变，以便能够发现配置中的拼写错误。
...
Agent 模型请求在自定义 endpoint 标头中也支持 `{{LIBRECHAT_USER_TENANT_ID}}` 和 `{{LIBRECHAT_USER_TENANTID}}`。这两个别名均使用权威的请求范围租户，而非可能过期的用户记录；它们适用于根 Agent、子 Agent 以及自定义摘要调用，且在没有租户时解析为空字符串。它们不是用于非 Agent 模板消费者的通用用户字段占位符。
