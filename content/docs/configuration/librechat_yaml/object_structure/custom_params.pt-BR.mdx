...
`defaultParamsEndpoint` seleciona qual conjunto de parâmetros integrados o painel do endpoint renderiza. Estes são os valores que correspondem a um:

<OptionTable
  options={[
    ['custom', 'String', 'Conjunto de parâmetros da OpenAI. Este é o padrão do esquema e o que um endpoint sem `provider` utiliza como fallback.', 'Default'],
    ['openAI', 'String', 'Conjunto de parâmetros da OpenAI.', ''],
    ['azureOpenAI', 'String', 'Conjunto de parâmetros da OpenAI.', ''],
    ['anthropic', 'String', 'Conjunto de parâmetros da Anthropic.', ''],
    ['google', 'String', 'Conjunto de parâmetros do Google.', ''],
    ['openrouter', 'String', 'Conjunto de parâmetros do OpenRouter. Em letras minúsculas, ao contrário dos outros.', ''],
  ]}
/>

Observe o uso de maiúsculas e minúsculas: `openAI` e `azureOpenAI` estão em camelCase, mas `openrouter` está totalmente em letras minúsculas.

<Callout type="info" title="Uma configuração de provedor fornece este valor para você">

Se o endpoint personalizado definir `provider`, o LibreChat preencherá `defaultParamsEndpoint` a partir dele, portanto, o padrão efetivo é o conjunto de parâmetros do provedor em vez de `custom`. Essa substituição ocorre apenas quando você omite o campo ou o deixa como `custom`; qualquer outro valor que você definir explicitamente terá prioridade. Um endpoint `provider: 'anthropic'`, portanto, inicia com o conjunto de parâmetros da Anthropic sem que você precise escrever `defaultParamsEndpoint`.

</Callout>

Os valores `assistants`, `azureAssistants`, `agents` e `bedrock` também são reconhecidos quando o LibreChat resolve esquemas de conversação e predefinições, mas nenhum deles é mapeado para um conjunto de parâmetros que um endpoint personalizado possa renderizar, portanto, definir um deles deixa o painel vazio.

<Callout type="warning" title="Valores não reconhecidos falham silenciosamente">

O campo é uma string de formato livre, não um enum fechado, portanto, um valor fora desta lista passa na validação de configuração. Ele simplesmente não corresponde a nenhum conjunto de parâmetros, e o painel do endpoint é renderizado **sem nenhum parâmetro**, em vez de relatar um erro. Se o seu painel de parâmetros estiver inesperadamente vazio, verifique este valor primeiro.
...
#### Intervalos Sentinel [#sentinel-ranges]

Use `range.positiveMin` quando `range.min` for um valor sentinela especial, mas os valores regulares tiverem um limite inferior mais alto. Por exemplo, um orçamento de pensamento pode usar `-1` para comportamento automático, enquanto aceita orçamentos explícitos apenas de `128` a `32768`:
...
Com `positiveMin`, os únicos valores aceitos são o sentinela `min` ou valores de `positiveMin` até `max`; valores entre eles não são válidos. `positiveMin` não pode exceder `max`, e o padrão deve ser `min` ou pelo menos `positiveMin`.
...
### Replay de raciocínio [#reasoning-replay]
