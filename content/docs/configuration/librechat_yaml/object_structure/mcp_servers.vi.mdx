...
<Callout type="warning" title="Các máy chủ được hỗ trợ bởi tiến trình là do người vận hành sở hữu">
  Các MCP server sử dụng `type: stdio` hoặc các trường quy trình như `command`, `args`, `env`, `cwd`, hoặc `stderr` phải được cấu hình trong `librechat.yaml`. API cấu hình quản trị viên sẽ từ chối các mục dựa trên quy trình (process-backed), và các ghi đè trong cơ sở dữ liệu hoặc theo từng principal không thể thay thế hoặc xóa một server dựa trên quy trình đã tồn tại. Các MCP server từ xa vẫn tiếp tục tuân theo thứ tự ưu tiên ghi đè cấu hình thông thường.
</Callout>
...
<Callout type="warning" title="Sử dụng các tên duy nhất an toàn cho chuẩn hóa">
  Các khóa công cụ dành cho model bảo toàn các chữ cái, số, `_`, `.`, và `-`; các ký tự khác trong tên máy chủ sẽ được thay thế bằng `_`. LibreChat ánh xạ các khóa đã chuẩn hóa đó trở lại tên đã cấu hình và sửa chữa các tham chiếu công cụ Agent cũ đã lưu. Tránh đặt hai tên có cùng giá trị sau khi chuẩn hóa, chẳng hạn như `Sales Force` và `Sales:Force`: các khóa công cụ của chúng bị trùng lặp, vì vậy LibreChat sẽ cảnh báo và loại trừ các công cụ của máy chủ bị che khuất thay vì mạo hiểm định tuyến lệnh gọi đến sai máy chủ.
</Callout>
...
<OptionTable
  options={[
    ...
    [
      'chatMenu',
      'Boolean',
      '(Tùy chọn) Khi đặt là `false`, loại trừ máy chủ MCP khỏi trình chọn trò chuyện thông thường và từ chối nó khỏi các lựa chọn trò chuyện do trình chọn cung cấp. Các Agent đã lưu và các model specs được chỉ định rõ ràng vẫn có thể sử dụng nó. Mặc định là `true`.',
      'chatMenu: false',
    ],
    [
      'serverInstructions',
...
- **Xác thực:** Phải bắt đầu bằng một chữ cái hoặc chữ số Unicode. Các ký tự còn lại có thể bao gồm chữ cái, chữ số, dấu kết hợp, khoảng trắng, dấu gạch nối và dấu nháy đơn thẳng hoặc cong Unicode.
- **Ví dụ:**
  ```yaml
  my-server:
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Dấu thời gian chấp nhận điều khoản                          |

**Lưu ý:** Các trường bị thiếu sẽ được thay thế bằng chuỗi trống.

Các trình giữ chỗ `{{LIBRECHAT_BODY_*}}` có phạm vi theo yêu cầu (request-scoped). LibreChat tạo kết nối MCP cho lượt chạy (run) đang hoạt động, tái sử dụng nó trong các lệnh gọi công cụ (tool calls) trong lượt chạy đó và dọn dẹp kết nối khi yêu cầu kết thúc. Các máy chủ có phạm vi theo yêu cầu bị loại trừ khỏi bộ nhớ đệm công cụ cố định (persistent tool cache) để các tiêu đề (headers) và URL cụ thể của yêu cầu không bị tái sử dụng ngoài lượt chạy đang hoạt động. Agent Builder đính kèm toàn bộ máy chủ thông qua một lựa chọn runtime-tools và phân giải các công cụ của nó sau khi một lượt chạy cung cấp các giá trị body. Việc hoàn tất OAuth sẽ lưu trữ ủy quyền nhưng trì hoãn kết nối cho đến khi ngữ cảnh yêu cầu đó tồn tại. Các trình giữ chỗ `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_OPENID_*}}` và `{{LIBRECHAT_GRAPH_*}}` vẫn làm cho máy chủ có phạm vi theo người dùng (user-scoped), nhưng các phương thức truyền tải HTTP sẽ làm mới các tiêu đề đã phân giải trước mỗi lệnh gọi công cụ mà không tự buộc phải kết nối lại.

`{{LIBRECHAT_BODY_PARENTMESSAGEID}}` nhận tin nhắn gốc của lượt hội thoại trong các cuộc trò chuyện Agent gốc, các yêu cầu API Agent tương thích với OpenAI và các lượt chạy Assistants cũ. Các yêu cầu Open Responses không có mô hình tin nhắn gốc tương thích, vì vậy máy chủ yêu cầu trình giữ chỗ này sẽ thất bại trên giao diện đó; `previous_response_id` không được thay thế cho nó.

OpenID access và ID token hết hạn độc lập với nhau. `{{LIBRECHAT_OPENID_ACCESS_TOKEN}}` và `{{LIBRECHAT_OPENID_TOKEN}}` cũ sử dụng thời hạn của access-token đã lưu, trong khi `{{LIBRECHAT_OPENID_ID_TOKEN}}` yêu cầu một ID token hiện hành. Nếu credential token không khả dụng hoặc đã hết hạn, LibreChat sẽ trả về lỗi xác thực lại `401` thay vì gửi một giá trị bearer trống hoặc theo nghĩa đen; hãy đăng nhập lại trước khi thử lại. Đối với một yêu cầu không có danh tính OpenID, header chứa một trong các trình giữ chỗ credential chưa được giải quyết đó sẽ bị lược bỏ hoàn toàn. Các trình giữ chỗ metadata danh tính vẫn giữ hành vi giá trị trống hiện tại của chúng.

Các nhà vận hành đáng tin cậy có thể chuyển trực tiếp access token OpenID trực tiếp của người dùng đã đăng nhập tới một máy chủ MCP từ xa được định nghĩa trong YAML:
...
Chỉ sử dụng chuyển tiếp trực tiếp (direct passthrough) khi máy chủ MCP được tin tưởng để nhận mã truy cập (access token) ngược dòng của người dùng. Xác thực OAuth MCP rõ ràng, [trao đổi thay mặt](#obo) và khóa API quản trị viên được ưu tiên hơn tiêu đề này. LibreChat giải quyết mã phiên trực tiếp trước khi kết nối và làm mới kết nối một lần sau lỗi `401` hoặc `403`. Nó không tự động phát lại lệnh gọi công cụ đã bị từ chối vì máy chủ có thể đã thực thi lệnh đó; hãy thử lại công cụ một cách có chủ ý sau khi quá trình khôi phục thành công. Nếu mã thông báo không thể làm mới, người dùng phải đăng nhập lại.
...
- **Loại:** Object (Tùy chọn, dành cho các loại `sse` và `streamable-http`)
- **Mô tả:** Các header chỉ được giải quyết cho một yêu cầu chat đang hoạt động. Chúng sử dụng các trình giữ chỗ (placeholder) giống như `headers`, được hợp nhất đè lên `headers`, và sẽ được ưu tiên nếu cả hai đối tượng cùng định nghĩa một tên giống nhau.
- **Hành vi khám phá:** Việc khám phá công cụ bỏ qua hoàn toàn `requestHeaders`, vì vậy các giá trị chỉ dành cho yêu cầu như `{{LIBRECHAT_BODY_*}}` sẽ không ẩn danh mục công cụ của máy chủ. Quá trình chạy đang hoạt động sẽ phân giải và gửi chúng khi kết nối.
...
- **Mô tả:** Khi được đặt thành `false`, sẽ loại trừ máy chủ MCP khỏi trình chọn trò chuyện thông thường và các yêu cầu trò chuyện do trình chọn cung cấp. Máy chủ này vẫn khả dụng đối với các Agent đã lưu và Model Specs được chỉ định rõ ràng cho nó.
- **Giá trị mặc định:** `true` (Máy chủ MCP có thể được chọn trong cuộc trò chuyện thông thường)
...
- **Ghi chú:**
  ...
  - LibreChat lưu trữ văn bản được máy chủ quảng bá tách biệt với khai báo đã cấu hình. Việc kiểm tra không thay thế `true` hoặc một chuỗi do người vận hành tạo, và các mục đăng ký được hỗ trợ bởi Redis từ các bản dựng trước đó sẽ được làm mới tự động trong quá trình khởi tạo.
- **Ví dụ:**
...
#### Điều phối làm mới OAuth [#oauth-refresh-coordination]

Đối với các triển khai đa bản sao (multi-replica), `oauthRefreshCoordination: true` sẽ tuần tự hóa việc làm mới token thông qua instance Redis dùng chung. `oauthRefreshWaitTimeout` kiểm soát thời gian một bản sao chờ kết quả làm mới, và `oauthPersistenceWaitTimeout` kiểm soát thời gian chờ để thông tin xác thực đã làm mới có thể đọc được; cả hai đều có giá trị mặc định là `15000` mili giây.

Chỉ bật tính năng điều phối sau khi mọi bản sao (replica) đã được nâng cấp và kết nối với cùng một triển khai Redis. Sử dụng các giá trị giống hệt nhau trên mọi bản sao, giải phóng các yêu cầu đang hoạt động trước khi thay đổi cài đặt và tránh bật/tắt tính năng này trong quá trình triển khai luân phiên (rolling deployment).
...
- **Các khóa con tùy chọn:**
  ...
  - `token_exchange_method`: String - Ghi đè phương thức trao đổi token tùy chọn. Sử dụng `default_post` để gửi thông tin xác thực của client trong phần thân POST được mã hóa theo biểu mẫu, hoặc `basic_auth_header` để gửi chúng trong tiêu đề xác thực HTTP Basic. Khi bị bỏ qua, LibreChat sẽ tự động phát hiện các phương thức xác thực endpoint token được nhà cung cấp quảng cáo và mặc định sử dụng Basic auth nếu không có phương thức nào được quảng cáo.
...
Khi cả `token_exchange_method` và `token_endpoint_auth_methods_supported` đều bị bỏ qua đối với một client secret được cấu hình sẵn, LibreChat cho phép tối đa năm giây để khám phá siêu dữ liệu của tài nguyên được bảo vệ và máy chủ ủy quyền. Nó chỉ sử dụng các khả năng được quảng bá khi điểm cuối token được khám phá khớp chính xác với `token_url` đã cấu hình; quá trình khám phá không thể chuyển hướng thông tin xác thực khách hàng đã cấu hình sang một điểm cuối khác. Hãy đặt `token_exchange_method` một cách rõ ràng khi nhà cung cấp không công bố siêu dữ liệu có thể sử dụng hoặc cần một phương thức bắt buộc.
