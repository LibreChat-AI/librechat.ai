...
<OptionTable
  options={[
    ...
    ['autoTranscribeAudio', 'Boolean', 'Giữ micrô luôn ở trạng thái lắng nghe thay vì dừng lại ở khoảng lặng đầu tiên. Với một engine bên ngoài, nó cũng sẽ bật tính năng phát hiện khoảng lặng dựa trên `decibelValue`.', ''],
    ['decibelValue', 'Number', 'Ngưỡng decibel để phát hiện khoảng lặng. Phạm vi từ -100 đến -30, mặc định là -45.', ''],
    ['autoSendText', 'Number', 'Số giây chờ sau khi phiên âm trước khi tự động gửi. `0` để gửi ngay lập tức; `-1` để tắt tự động gửi.', ''],
  ]}
/>
