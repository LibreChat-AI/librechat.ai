...
Khi một endpoint tùy chỉnh được viết thông qua API cấu hình quản trị viên, LibreChat sẽ mã hóa `apiKey` theo nghĩa đen ở trạng thái nghỉ và chỉ trả về `apiKeyPreview`. Các tham chiếu `${ENV_VAR}` và `user_provided` vẫn là các giá trị chuyển tiếp có thể đọc được. Đối với các endpoint được định nghĩa trực tiếp trong `librechat.yaml`, hãy ưu tiên sử dụng tham chiếu môi trường như đã hiển thị ở trên.
...
- Trên các endpoint tùy chỉnh tương thích với OpenAI, các model ID chứa `claude` sẽ sử dụng các hạn chế về tài liệu của Claude: các tệp PDF và tài liệu văn bản được chấp nhận, trong khi các loại tài liệu nhị phân không phải PDF sẽ bị bỏ qua trước khi yêu cầu được gửi đi.
...
<OptionTable
  options={[
    [
      'addParams',
      'Object/Dictionary',
      'Thêm các tham số bổ sung vào các yêu cầu. Các giá trị có thể là chuỗi, số, boolean, mảng hoặc các đối tượng lồng nhau. Hỗ trợ các nút chuyển đổi công cụ của nhà cung cấp như `web_search: true` và `url_context: true` của Google.',
...
<OptionTable
  options={[
    ...
    [
      ...
      'Boolean',
      'Phát lại `reasoning_content` của nhà cung cấp trong các lượt gọi công cụ cho các endpoint tùy chỉnh tương thích với OpenAI yêu cầu điều đó.',
      'includeReasoningContent: true',
    ],
    [
      'includeReasoningHistory',
      'Boolean',
      'Tái tạo `reasoning_content` từ lịch sử hội thoại đã lưu qua các lượt. Ngụ ý `includeReasoningContent`.',
...
Tại bước phân giải đầu ra cuối cùng, các trình giữ chỗ `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_BODY_*}}` và `{{LIBRECHAT_OPENID_*}}` được nhận dạng mà không có giá trị sẽ trở thành chuỗi rỗng thay vì được chuyển tiếp dưới dạng văn bản mẫu. Các tên trình giữ chỗ không xác định sẽ không thay đổi để có thể phát hiện các lỗi đánh máy trong cấu hình.
...
Các yêu cầu mô hình Agent cũng hỗ trợ `{{LIBRECHAT_USER_TENANT_ID}}` và `{{LIBRECHAT_USER_TENANTID}}` trong các tiêu đề endpoint tùy chỉnh. Cả hai bí danh này đều sử dụng tenant có phạm vi yêu cầu xác thực thay vì bản ghi người dùng có khả năng đã cũ, áp dụng cho các Agent gốc, Subagent và các lệnh gọi tóm tắt tùy chỉnh, đồng thời sẽ phân giải thành một chuỗi trống khi không có tenant nào hiện diện. Chúng không phải là các trình giữ chỗ trường người dùng chung cho các trình tiêu thụ mẫu không phải Agent.
