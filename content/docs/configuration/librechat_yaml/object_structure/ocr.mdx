...
<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'Validated private host:port entries reserved for exempting OCR targets from SSRF checks. Entries require a port and cannot be URLs, paths, CIDR ranges, bare hosts, or public IP literals.',
      'allowedAddresses: ["ocr.internal:8080"]',
    ],
  ]}
/>
...
<Callout type="warning" title="v0.8.8-rc1 behavior">
  `ocr.allowedAddresses` is validated and preserved by the configuration loader, but OCR requests do not consult it yet. It does not authorize private OCR targets in this release candidate.
</Callout>
