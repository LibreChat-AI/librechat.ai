...
<Callout type="warning" title="Los servidores respaldados por procesos son propiedad del operador">
  Los servidores MCP que utilizan `type: stdio` o campos de proceso como `command`, `args`, `env`, `cwd` o `stderr` deben configurarse en `librechat.yaml`. La API de configuración de administrador rechaza las entradas respaldadas por procesos, y las anulaciones de base de datos o por principal no pueden reemplazar ni eliminar un servidor respaldado por procesos existente. Los servidores MCP remotos continúan siguiendo la precedencia normal de anulación de configuración.
</Callout>
...
<Callout type="warning" title="Utilice nombres únicos seguros para la normalización">
  Las claves de herramientas orientadas al modelo conservan letras, números, `_`, `.` y `-`; otros caracteres en un nombre de servidor se reemplazan por `_`. LibreChat asigna esas claves normalizadas de nuevo al nombre configurado y repara las referencias de herramientas de Agente guardadas anteriormente. Evite dos nombres que se normalicen al mismo valor, como `Sales Force` y `Sales:Force`: sus claves de herramientas son ambiguas, por lo que LibreChat advierte y excluye las herramientas del servidor oculto en lugar de arriesgarse a enrutar una llamada al servidor incorrecto.
</Callout>
...
<OptionTable
  options={[
    ...
    [
      'chatMenu',
      'Boolean',
      '(Opcional) Cuando es `false`, excluye al servidor MCP del selector de chat habitual y lo rechaza de las selecciones de chat proporcionadas por el selector. Los agentes guardados y las Model Specs asignadas explícitamente aún pueden usarlo. El valor predeterminado es `true`.',
      'chatMenu: false',
    ],
    [
      'serverInstructions',
...
- **Validación:** Debe comenzar con una letra o número Unicode. Los caracteres restantes pueden incluir letras, números, marcas de combinación, espacios, guiones y apóstrofes rectos o curvos Unicode.
- **Ejemplo:**
  ```yaml
  my-server:
...
| `{{LIBRECHAT_USER_TERMSACCEPTEDAT}}`  | `termsAcceptedAt`  | Date → String    | Marca de tiempo de aceptación de términos |

**Nota:** Los campos faltantes serán reemplazados por cadenas vacías.

Los marcadores de posición `{{LIBRECHAT_BODY_*}}` tienen un ámbito de solicitud. LibreChat crea la conexión MCP para la ejecución activa, la reutiliza en las llamadas a herramientas durante esa ejecución y la cierra cuando finaliza la solicitud. Los servidores con ámbito de solicitud se excluyen de la caché persistente de herramientas, por lo que los encabezados y las URLs específicos de la solicitud no se reutilizan fuera de la ejecución activa. Agent Builder adjunta el servidor completo a través de una selección de runtime-tools y resuelve sus herramientas después de que una ejecución proporcione los valores del cuerpo. Completar OAuth almacena la autorización, pero pospone la conexión hasta que exista ese contexto de solicitud. Los marcadores de posición `{{LIBRECHAT_USER_*}}`, `{{LIBRECHAT_OPENID_*}}` y `{{LIBRECHAT_GRAPH_*}}` siguen haciendo que el servidor tenga un ámbito de usuario, pero los transportes HTTP actualizan los encabezados resueltos antes de cada llamada a herramienta sin forzar una reconexión por sí mismos.

`{{LIBRECHAT_BODY_PARENTMESSAGEID}}` recibe el mensaje principal de turno raíz en chats de Agentes nativos, solicitudes de la API de Agentes compatibles con OpenAI y ejecuciones de Assistants heredadas. Las solicitudes de Open Responses no tienen un modelo de mensaje principal compatible, por lo que un servidor que requiera este marcador de posición fallará de forma cerrada en esa interfaz; `previous_response_id` no se sustituye por él.

Los tokens de acceso e ID de OpenID caducan de forma independiente. `{{LIBRECHAT_OPENID_ACCESS_TOKEN}}` y el heredado `{{LIBRECHAT_OPENID_TOKEN}}` utilizan la caducidad del token de acceso almacenado, mientras que `{{LIBRECHAT_OPENID_ID_TOKEN}}` requiere un token de ID actual. Si un token de credencial no está disponible o ha caducado, LibreChat devuelve un error de reautenticación `401` procesable en lugar de enviar un valor de portador vacío o literal; inicie sesión de nuevo antes de volver a intentarlo. Para una solicitud sin identidad OpenID, se omite por completo el encabezado que contiene uno de esos marcadores de posición de credenciales no resueltos. Los marcadores de posición de metadatos de identidad mantienen su comportamiento existente de valor vacío.

Los operadores de confianza pueden pasar el token de acceso OpenID en vivo del usuario que ha iniciado sesión directamente a un servidor MCP remoto definido en YAML:
...
Utilice el paso directo (direct passthrough) solo cuando se confíe en que el servidor MCP recibirá el token de acceso ascendente del usuario. La autenticación explícita MCP OAuth, el [intercambio en nombre de](#obo) y la autenticación mediante clave de API de administrador tienen prioridad sobre este encabezado. LibreChat resuelve el token de sesión activo antes de conectarse y actualiza la conexión una vez después de un `401` o `403`. No vuelve a ejecutar automáticamente la llamada a la herramienta rechazada porque es posible que el servidor ya la haya ejecutado; vuelva a intentar la herramienta deliberadamente una vez que la recuperación sea exitosa. Si el token no se puede actualizar, el usuario debe iniciar sesión nuevamente.
...
- **Tipo:** Objeto (Opcional, para los tipos `sse` y `streamable-http`)
- **Descripción:** Los encabezados se resuelven solo para una solicitud de chat activa. Utilizan los mismos marcadores de posición que `headers`, se combinan sobre `headers` y prevalecen cuando ambos objetos definen el mismo nombre.
- **Comportamiento de descubrimiento:** El descubrimiento de herramientas omite `requestHeaders` por completo, por lo que los valores exclusivos de la solicitud, como `{{LIBRECHAT_BODY_*}}`, no ocultan el catálogo de herramientas del servidor. La ejecución activa los resuelve y los envía cuando se conecta.
...
- **Descripción:** Cuando se establece en `false`, excluye al servidor MCP del selector de chat habitual y de las solicitudes de chat proporcionadas por el selector. Permanece disponible para los Agents guardados y los Model Specs que lo asignen explícitamente.
- **Valor predeterminado:** `true` (El servidor MCP puede seleccionarse en el chat normal)
...
- **Notas:**
  ...
  - LibreChat almacena el texto anunciado por el servidor por separado de la declaración configurada. La inspección no reemplaza `true` ni una cadena creada por el operador, y las entradas del registro respaldadas por Redis de versiones anteriores se actualizan automáticamente durante la inicialización.
- **Ejemplo:**
...
#### Coordinación de actualización de OAuth [#oauth-refresh-coordination]

Para implementaciones de múltiples réplicas, `oauthRefreshCoordination: true` serializa la actualización de tokens a través de la instancia de Redis compartida. `oauthRefreshWaitTimeout` controla cuánto tiempo espera una réplica para obtener el resultado de la actualización, y `oauthPersistenceWaitTimeout` controla cuánto tiempo espera para que las credenciales actualizadas sean legibles; ambos tienen un valor predeterminado de `15000` milisegundos.

Habilite la coordinación solo después de que cada réplica se haya actualizado y conectado a la misma implementación de Redis. Utilice valores idénticos en cada réplica, vacíe las solicitudes activas antes de cambiar la configuración y evite alternarla durante una implementación continua.
...
- **Subclaves opcionales:**
  ...
  - `token_exchange_method`: String - Sustitución opcional del intercambio de tokens. Utilice `default_post` para enviar las credenciales del cliente en el cuerpo de una solicitud POST codificada en formulario, o `basic_auth_header` para enviarlas en un encabezado de autorización HTTP Basic. Cuando se omite, LibreChat detecta los métodos de autenticación del endpoint de tokens anunciados por el proveedor y utiliza la autenticación Basic por defecto si no se anuncia ninguno.
...
Cuando `token_exchange_method` y `token_endpoint_auth_methods_supported` se omiten para un secreto de cliente preconfigurado, LibreChat permite hasta cinco segundos para la detección de metadatos del recurso protegido y del servidor de autorización. Solo utiliza las capacidades anunciadas cuando el endpoint de token detectado coincide exactamente con la `token_url` configurada; la detección no puede redirigir las credenciales de cliente configuradas a otro endpoint. Establezca `token_exchange_method` explícitamente cuando un proveedor no publique metadatos utilizables o requiera un método forzado.
