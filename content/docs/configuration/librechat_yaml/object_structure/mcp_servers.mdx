...
mcpServers:
  ...
  per-user-credentials-example:
    ...
    customUserVars:
      ...
      MY_SERVICE_REGION:
        title: 'Region'
        description: 'Pick the region to query.'
        sensitive: false
        values:
          - eu-west-1
          - value: us-east-1
...
<OptionTable
  options={[
    ...
    [
      'customUserVars',
      'Object',
      '(Optional) Defines custom variables that users can set for this MCP server, allowing for per-user credentials or configurations (e.g., API keys). These variables can then be referenced in `headers` or `env` fields. Declare `values` on a variable to restrict it to a predefined list, rendered as a select.',
...
Trusted operators can pass the signed-in user's live OpenID access token directly to a YAML-defined remote MCP server:
...
Use direct passthrough only when the MCP server is trusted to receive the user's upstream access token. Explicit MCP OAuth, [on-behalf-of exchange](#obo), and administrator API-key authentication take precedence over this header. LibreChat resolves the live session token before connecting and refreshes the connection once after a `401` or `403`. It does not automatically replay the rejected tool call because the server may already have executed it; retry the tool deliberately after recovery succeeds. If the token cannot be refreshed, the user must sign in again.
...
- **Type:** Object (Optional, for `sse` and `streamable-http` types)
- **Description:** Headers resolved only for an active chat request. They use the same placeholders as `headers`, are merged over `headers`, and win when both objects define the same name.
- **Discovery behavior:** Tool discovery omits `requestHeaders` entirely, so request-only values such as `{{LIBRECHAT_BODY_*}}` do not hide the server's tool catalog. The active run resolves and sends them when it connects.
...
For multi-replica deployments, `oauthRefreshCoordination: true` serializes token refresh through the shared Redis instance. `oauthRefreshWaitTimeout` controls how long a replica waits for the refresh result, and `oauthPersistenceWaitTimeout` controls how long it waits for refreshed credentials to become readable; both default to `15000` milliseconds.

Enable coordination only after every replica is upgraded and connected to the same Redis deployment. Use identical values on every replica, drain active requests before changing the setting, and avoid toggling it during a rolling deployment.
...
- **Structure:**
  ...
  - Each variable name is an object with the following subkeys:
    ...
    - `values`: Array (Optional) - Predefined choices for the variable. When set, the field renders as a select in the UI instead of a free-text input, and only the listed values are accepted: a value submitted outside the list is rejected before anything is stored. Each entry is either a plain string (used as both the stored value and the displayed label) or an object with `value` and `label` when the label should read differently from the technical value. Entries must be unique and carry no surrounding whitespace.
    - `multiple`: Boolean (Optional) - Allows selecting several of the declared `values`. Requires `values`, and no declared value may contain a comma, because selections are stored as a single comma-joined string (for example `read,write`). Omit it for a single-choice select.
- **Predefined values:**
  - Use `values` when a variable only accepts a known set of inputs, such as a region, an environment, or a set of scopes. Users pick from a list instead of typing, and a typo can no longer produce a server that fails to connect.
  - The constraint is enforced on the server, not only in the UI, so it also scopes what a user can point the server at — for example the directory passed to a `stdio` server, or the host of a remote one.
  - Pair `values` with `sensitive: false` for non-secret selects: the current choice is then preselected when the user reopens the dialog. Values of secret variables are never sent back to the browser, so a select left as `sensitive` (the default) always opens empty.
...
- **Example:**
  ...
  customUserVars:
    ...
    SOME_OTHER_VAR:
      ...
      sensitive: false
  ```
  Predefined values, as a single-choice and a multi-choice select:
  ```yaml
  customUserVars:
    MY_SERVICE_REGION:
      title: 'Region'
    ...
    MY_SERVICE_SCOPES:
      title: 'Scopes'
      description: 'Permissions granted to this integration.'
      sensitive: false
      multiple: true
      values:
        - read
