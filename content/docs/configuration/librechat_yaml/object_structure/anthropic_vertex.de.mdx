...
> **Wichtig:** Claude Opus 4.7 und neuer, Opus 5, Sonnet 5 sowie Fable/Mythos 5 oder 5.1 erfordern `global`, `us` oder `eu`. Spezifische Regionen wie `us-east5` unterstützen Claude Sonnet 4.6 und älter und geben für neuere Modelle eine „model-not-found“-Antwort zurück. Verwenden Sie `global` für die breiteste Verfügbarkeit ohne regionale Preisaufschläge oder `us`/`eu`, wenn Sie eine Datenresidenz in mehreren Regionen benötigen.
...
Wenn `models` weggelassen wird, verwendet LibreChat seinen integrierten Vertex-Katalog und entfernt Modelle, die nicht mit der konfigurierten `region` kompatibel sind. Eine explizite `models`-Liste oder -Map wird niemals gefiltert, daher muss jede Deployment-ID am ausgewählten Standort verfügbar sein.
...
> **Hinweis:** Bei der Verwendung von Umgebungsvariablen ist das Model Mapping nicht verfügbar und LibreChat enthält seinen standardmäßigen Anthropic-Modellkatalog. Setzen Sie `ANTHROPIC_VERTEX_REGION` auf `global`, `us` oder `eu`, bevor Sie ein modernes Modell auswählen, das in einer bestimmten Region nicht verfügbar ist.
...
- Modelle: Der integrierte Vertex-Katalog von LibreChat für den ausgewählten Standort
...
- Verwenden Sie `global`, `us` oder `eu` für Opus 4.7+, Opus 5, Sonnet 5 und Fable/Mythos 5
...
- Wechseln Sie zu `global`, `us` oder `eu` für Modelle, die neuer als Sonnet 4.6 sind
