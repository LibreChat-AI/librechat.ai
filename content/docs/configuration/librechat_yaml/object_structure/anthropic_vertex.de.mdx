---
title: Anthropic Vertex AI Objektstruktur
icon: Anthropic
---

LibreChat unterstützt die Ausführung von Anthropic Claude Modellen über **Google Cloud Vertex AI**. Dies ermöglicht es Ihnen, Claude Modelle mit Ihrer bestehenden Google Cloud Infrastruktur, Abrechnung und Ihren Anmeldedaten zu verwenden.

**[Für eine schnelle Einrichtung mithilfe von Umgebungsvariablen siehe den Anthropic Konfigurationsleitfaden](/docs/configuration/pre_configured_ai/anthropic#vertex-ai)**

## Vorteile [#benefits]

- **Einheitliche Abrechnung:** Verwenden Sie Ihr bestehendes Google Cloud-Abrechnungskonto
- **Enterprise-Funktionen:** Zugriff auf die Sicherheits- und Compliance-Funktionen von Google Cloud für Unternehmen
- **Regionale Compliance:** Bereitstellung in spezifischen Regionen, um Anforderungen an die Datenresidenz zu erfüllen
- **Bestehende Infrastruktur:** Nutzen Sie Ihre aktuellen GCP-Dienstkonten und IAM-Richtlinien

## Voraussetzungen [#prerequisites]

Bevor Sie Anthropic Vertex AI konfigurieren, stellen Sie sicher, dass Sie Folgendes haben:

1. **Google Cloud Project** mit aktivierter Vertex AI API
2. **Dienstkonto** mit der Rolle `Vertex AI User` (`roles/aiplatform.user`)
3. **Claude-Modelle**, die in Ihrem [Vertex AI Model Garden](https://console.cloud.google.com/vertex-ai/model-garden) aktiviert sind
4. **Service Account Key** (JSON-Datei), heruntergeladen und für LibreChat zugänglich

## Beispielkonfiguration [#example-configuration]

```yaml filename="Example Anthropic Vertex AI Configuration"
endpoints:
  anthropic:
    streamRate: 20
    titleModel: "claude-3.5-haiku"  # Use the visible model name (key from models config)

    vertex:
      region: "global"
      # serviceKeyFile: "/path/to/service-account.json"  # Optional, defaults to api/data/auth.json
      # projectId: "${VERTEX_PROJECT_ID}"  # Optional, auto-detected from service key

      # Model mapping: visible name -> Vertex AI deployment name
      models:
        claude-opus-5:
          deploymentName: claude-opus-5
        claude-sonnet-5:
          deploymentName: claude-sonnet-5
        claude-3.7-sonnet:
          deploymentName: claude-3-7-sonnet-20250219
        claude-3.5-sonnet:
          deploymentName: claude-3-5-sonnet-v2@20241022
        claude-3.5-haiku:
          deploymentName: claude-3-5-haiku@20241022
```

> **Hinweis:** Der Anthropic endpoint unterstützt alle [Shared Endpoint Settings](/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings), einschließlich `streamRate`, `titleModel`, `titleMethod`, `titlePrompt`, `titlePromptTemplate` und `titleEndpoint`.

---

## vertex

Das `vertex` Objekt enthält alle Vertex AI-spezifischen Konfigurationsoptionen.

### region

**Schlüssel:**
<OptionTable
  options={[
    ['region', 'String', 'Die Google Cloud-Region, in der Ihr Vertex AI endpoint bereitgestellt wird.', 'Must be a region where Claude models are available on Vertex AI.'],
  ]}
/>

**Standard:** `us-east5`

**Verfügbare Regionen:**

- `global`
- `us`
- `eu`
- `us-east5`
- `us-central1`
- `europe-west1`
- `europe-west4`
- `asia-southeast1`

> **Wichtig:** Claude Opus 4.7 und neuer, Opus 5, Sonnet 5 sowie Fable/Mythos 5 erfordern `global`, `us` oder `eu`. Spezifische Regionen wie `us-east5` unterstützen Claude Sonnet 4.6 und früher und geben bei neueren Modellen eine „model-not-found“-Antwort zurück. Verwenden Sie `global` für die breiteste Verfügbarkeit ohne regionalen Preisaufschlag oder `us`/`eu`, wenn Sie eine standortübergreifende Datenresidenz benötigen.

**Beispiel:**
```yaml filename="endpoints / anthropic / vertex / region"
region: "global"
```

### projectId

**Schlüssel:**
<OptionTable
  options={[
    ['projectId', 'String', 'Die Google Cloud Project ID. Unterstützt Referenzen auf Umgebungsvariablen.', 'Optional. If not specified, auto-detected from the service account key file.'],
  ]}
/>

**Standard:** Automatisch aus der Service-Schlüsseldatei erkannt

**Beispiel:**
```yaml filename="endpoints / anthropic / vertex / projectId"
projectId: "${GOOGLE_PROJECT_ID}"
```

### serviceKeyFile

**Schlüssel:**
<OptionTable
  options={[
    ['serviceKeyFile', 'String', 'Pfad zur JSON-Datei des Google Cloud-Dienstkontoschlüssels.', 'Can be absolute or relative to the LibreChat root directory.'],
  ]}
/>

**Standard:** `api/data/auth.json` (oder Umgebungsvariable `GOOGLE_SERVICE_KEY_FILE`)

**Beispiel:**
```yaml filename="endpoints / anthropic / vertex / serviceKeyFile"
serviceKeyFile: "/etc/secrets/gcp-service-account.json"
```

---

## models

Das Feld `models` definiert die verfügbaren Claude-Modelle und ordnet benutzerfreundliche Namen den Vertex AI-Deployment-IDs zu. Dies funktioniert ähnlich wie das [Azure OpenAI-Modell-Mapping](/docs/configuration/librechat_yaml/object_structure/azure_openai#group-object-structure).

Wenn `models` weggelassen wird, verwendet LibreChat seinen integrierten Vertex-Katalog und entfernt Modelle, die nicht mit der konfigurierten `region` kompatibel sind. Eine explizite `models`-Liste oder -Map wird niemals gefiltert, daher muss jede Deployment-ID am ausgewählten Standort verfügbar sein.

### Formatoptionen [#format-options]

Sie können Modelle auf drei Arten konfigurieren:

#### Option 1: Einfaches Array [#option-1-simple-array]

Verwenden Sie die tatsächlichen Vertex AI Modell-IDs direkt. Diese werden in der Benutzeroberfläche unverändert angezeigt:

```yaml filename="Simple array format"
models:
  - "claude-opus-5"
  - "claude-sonnet-5"
  - "claude-sonnet-4-20250514"
  - "claude-3-7-sonnet-20250219"
  - "claude-3-5-haiku@20241022"
```

#### Option 2: Objekt mit benutzerdefinierten Namen (Empfohlen) [#option-2-object-with-custom-names-recommended]

Ordnen Sie benutzerfreundliche Namen den Vertex AI-Bereitstellungsnamen zu:

```yaml filename="Object format with custom names"
models:
  claude-opus-5:           # Visible in UI
    deploymentName: claude-opus-5  # Actual Vertex AI model ID
  claude-sonnet-5:
    deploymentName: claude-sonnet-5
  claude-3.5-haiku:
    deploymentName: claude-3-5-haiku@20241022
```

#### Option 3: Gemischtes Format mit Standardwert [#option-3-mixed-format-with-default]

Legen Sie einen Standard-Bereitstellungsnamen fest und verwenden Sie boolesche Werte für Modelle, die diesen übernehmen:

```yaml filename="Mixed format"
deploymentName: claude-sonnet-4-20250514  # Default deployment
models:
  claude-sonnet-4: true  # Uses default deploymentName
  claude-3.5-haiku:
    deploymentName: claude-3-5-haiku@20241022  # Override for this model
```

### Eigenschaften des Model-Objekts [#model-object-properties]

<OptionTable
  options={[
    ['deploymentName', 'String', 'Die tatsächliche Vertex AI Modell-ID, die für API-Aufrufe verwendet wird.', 'Required for each model unless using boolean `true` with a group-level default.'],
  ]}
/>

**Beispiel:**
```yaml filename="Model with deploymentName"
models:
  claude-sonnet-4:
    deploymentName: claude-sonnet-4-20250514
```

---

## Alternative für Umgebungsvariablen [#environment-variable-alternative]

Für einfachere Setups können Sie Vertex AI mithilfe von Umgebungsvariablen anstelle von YAML konfigurieren:

```bash filename=".env"
# Enable Vertex AI mode
ANTHROPIC_USE_VERTEX=true

# Vertex AI region (optional, defaults to us-east5)
ANTHROPIC_VERTEX_REGION=global

# Path to service account key (optional, defaults to api/data/auth.json)
GOOGLE_SERVICE_KEY_FILE=/path/to/service-account.json
```

> **Hinweis:** Bei der Verwendung von Umgebungsvariablen ist das Model Mapping nicht verfügbar und LibreChat enthält seinen standardmäßigen Anthropic-Modellkatalog. Setzen Sie `ANTHROPIC_VERTEX_REGION` auf `global`, `us` oder `eu`, bevor Sie ein modernes Modell auswählen, das in einer bestimmten Region nicht verfügbar ist.

---

## Vollständige Beispiele [#complete-examples]

### Grundlegende Einrichtung [#basic-setup]

Minimale Konfiguration unter Verwendung der Standardwerte (Vertex AI wird durch das Vorhandensein des `vertex`-Abschnitts aktiviert):

```yaml filename="Basic Vertex AI Setup"
endpoints:
  anthropic:
    vertex:
      region: global
```

Dies verwendet:

- Region: `global`
- Service-Schlüssel: `api/data/auth.json` (oder `GOOGLE_SERVICE_KEY_FILE` env var)
- Projekt-ID: Automatisch vom Service-Key erkannt
- Modelle: Der integrierte Vertex-Katalog von LibreChat für den ausgewählten Standort

### Produktions-Setup mit Model Mapping [#production-setup-with-model-mapping]

Vollständige Konfiguration mit benutzerdefinierten Modellnamen und Titeln:

```yaml filename="Production Vertex AI Setup"
endpoints:
  anthropic:
    streamRate: 20
    titleModel: "haiku"
    titleMethod: "completion"

    vertex:
      region: "global"
      serviceKeyFile: "${GOOGLE_SERVICE_KEY_FILE}"

      models:
        opus:
          deploymentName: claude-opus-5
        sonnet:
          deploymentName: claude-sonnet-5
        haiku:
          deploymentName: claude-3-5-haiku@20241022
```

### Multi-Region-Setup [#multi-region-setup]

Sie können nur eine Region pro Deployment konfigurieren. Für Anforderungen mit mehreren Regionen sollten Sie separate LibreChat-Instanzen oder benutzerdefinierte endpoints in Betracht ziehen.

---

## Fehlerbehebung [#troubleshooting]

### Häufige Fehler [#common-errors]

**"Die Standard-Anmeldeinformationen konnten nicht geladen werden"**
- Stellen Sie sicher, dass die Dienstkonto-Schlüsseldatei unter dem angegebenen Pfad vorhanden ist
- Überprüfen Sie die Dateiberechtigungen (müssen für den LibreChat-Prozess lesbar sein)
- Überprüfen Sie, ob die JSON-Datei gültig und nicht beschädigt ist

**"Permission denied" oder "403 Forbidden"**
- Stellen Sie sicher, dass das Dienstkonto über die Rolle `Vertex AI User` verfügt.
- Stellen Sie sicher, dass Claude-Modelle in Ihrem Vertex AI Model Garden aktiviert sind.
- Überprüfen Sie, ob das Dienstkonto zum richtigen Projekt gehört

**"Modell nicht gefunden"**
- Überprüfen Sie, ob die Modell-ID in `deploymentName` korrekt ist
- Überprüfen Sie, ob das Modell in der von Ihnen ausgewählten Region verfügbar ist.
- Verwenden Sie `global`, `us` oder `eu` für Opus 4.7+, Opus 5, Sonnet 5 und Fable/Mythos 5
- Stellen Sie sicher, dass das Modell in Ihrem Vertex AI Model Garden aktiviert ist.

### Region-Probleme [#region-issues]

**"Invalid region" oder "Region not supported"**
- Verwenden Sie eine der oben aufgeführten unterstützten Regionen
- Versuchen Sie es mit der Region `global`, die ein automatisches Routing bietet
- Überprüfen Sie die [Dokumentation von Google Cloud](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude#regions) für die aktuelle Liste der Regionen, in denen Claude verfügbar ist.

**"Modell in dieser Region nicht verfügbar"**
- Nicht alle Claude-Modelle sind in allen Regionen verfügbar
- Wechseln Sie zu `global`, `us` oder `eu` für Modelle, die neuer als Sonnet 4.6 sind
- Überprüfen Sie den [Vertex AI Model Garden](https://console.cloud.google.com/vertex-ai/model-garden), um zu sehen, welche Modelle in Ihrer Region verfügbar sind.

**Latenzprobleme**
- Wenn Sie eine hohe Latenz feststellen, versuchen Sie, eine Region zu verwenden, die geografisch näher an Ihren Benutzern liegt.
- Die `global` Region leitet automatisch an die nächstgelegene verfügbare Region weiter
- Für Produktions-Workloads mit strengen Latenzanforderungen sollten Sie verschiedene Regionen testen und diejenige auswählen, die für Ihren Anwendungsfall die beste Leistung bietet.

### Einrichtung überprüfen [#verifying-setup]

1. Stellen Sie sicher, dass Ihr Dienstkontoschlüssel gültig ist:
   ```bash
   gcloud auth activate-service-account --key-file=/path/to/key.json
   gcloud auth list
   ```

2. Testen Sie den Vertex AI-Zugriff:
   ```bash
   gcloud ai models list --region=us-east5
   ```

3. Überprüfen Sie den Zugriff auf das Claude-Modell:
   ```bash
   curl -X POST \
     -H "Authorization: Bearer $(gcloud auth print-access-token)" \
     -H "Content-Type: application/json" \
     "https://us-east5-aiplatform.googleapis.com/v1/projects/YOUR_PROJECT/locations/us-east5/publishers/anthropic/models/claude-3-5-haiku@20241022:rawPredict" \
     -d '{"anthropic_version": "vertex-2023-10-16", "max_tokens": 100, "messages": [{"role": "user", "content": "Hello"}]}'
   ```

---

## Hinweise [#notes]

- Vertex AI und die direkte Anthropic API schließen sich gegenseitig aus. Wenn ein `vertex` Konfigurationsabschnitt vorhanden ist, wird die Umgebungsvariable `ANTHROPIC_API_KEY` ignoriert.
- Die Websuchfunktion wird mit Vertex AI vollständig unterstützt.
- Prompt Caching wird durch automatische Header-Filterung für die Kompatibilität mit Vertex AI unterstützt.
- Function calling und Tool-Nutzung funktionieren genauso wie bei der direkten Anthropic API.
