...
> **Ważne:** Claude Opus 4.7 i nowsze, Opus 5, Sonnet 5 oraz Fable/Mythos 5 wymagają `global`, `us` lub `eu`. Określone regiony, takie jak `us-east5`, obsługują Claude Sonnet 4.6 i starsze, a w przypadku nowszych modeli zwracają odpowiedź model-not-found. Użyj `global`, aby uzyskać najszerszą dostępność i brak dodatkowych opłat regionalnych, lub `us`/`eu`, gdy potrzebujesz rezydencji danych w wielu regionach.
...
Gdy `models` jest pominięte, LibreChat używa swojego wbudowanego katalogu Vertex i usuwa modele niekompatybilne ze skonfigurowanym `region`. Jawna lista lub mapa `models` nigdy nie jest filtrowana, więc każdy identyfikator wdrożenia (deployment ID) musi być dostępny w wybranej lokalizacji.
...
> **Uwaga:** Podczas korzystania ze zmiennych środowiskowych mapowanie modeli nie jest dostępne, a LibreChat zawiera standardowy katalog modeli Anthropic. Ustaw `ANTHROPIC_VERTEX_REGION` na `global`, `us` lub `eu` przed wybraniem nowoczesnego modelu, który jest niedostępny w określonym regionie.
...
- Modele: Wbudowany katalog Vertex w LibreChat dla wybranej lokalizacji
...
- Użyj `global`, `us` lub `eu` dla Opus 4.7+, Opus 5, Sonnet 5 oraz Fable/Mythos 5
...
- Przełącz na `global`, `us` lub `eu` dla modeli nowszych niż Sonnet 4.6
