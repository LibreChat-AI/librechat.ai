...
> **Importante:** Claude Opus 4.7 e versões mais recentes, Opus 5, Sonnet 5 e Fable/Mythos 5 exigem `global`, `us` ou `eu`. Regiões específicas como `us-east5` atendem ao Claude Sonnet 4.6 e anteriores e retornam uma resposta de modelo não encontrado para modelos mais recentes. Use `global` para a maior disponibilidade e sem custo adicional regional, ou `us`/`eu` quando precisar de residência de dados em várias regiões.
