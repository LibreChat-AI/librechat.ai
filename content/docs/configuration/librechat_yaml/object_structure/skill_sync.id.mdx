...
- Memvalidasi dan memublikasikan Skill secara independen, sehingga satu Skill yang tidak valid atau berkonflik tidak akan menyembunyikan Skill yang valid dari sumber yang sama.
- Mempertahankan mirror terakhir yang diketahui baik dari Skill yang dilewati dan membatalkan penulisan yang tidak lengkap untuk Skill tersebut.
- Menandai sebuah run sebagai `partial` ketika ia memublikasikan setidaknya satu Skill tetapi melewatkan yang lain, serta mencatat jumlah yang dilewati secara terbatas dan detail untuk pembacaan status administrator.
- Menggunakan kunci sinkronisasi sehingga hanya satu proses sinkronisasi GitHub yang berjalan dalam satu waktu.

Kunci frontmatter `SKILL.md` yang tidak dikenal akan diterima selama nilainya tetap berada dalam batas ekstensi yang aman dan sesuai JSON. Bidang yang dikenali namun tidak valid, nilai ekstensi yang tidak aman, nama duplikat, dan konflik kepemilikan hanya akan melewatkan Skill yang terdampak. Kegagalan autentikasi, limit laju (rate-limit), kehilangan kunci (lock-loss), akses repositori, dan kegagalan rollback tetap menjadi kegagalan di tingkat sumber.
...
<OptionTable
  options={[
    [
      ...
      'Endpoint',
      'Mengembalikan status aktif, pengaturan jadwal, status sumber, keberadaan kredensial, jumlah sinkronisasi terkini, dan detail Skill yang dilewati untuk eksekusi parsial.',
      '',
    ],
    [
