...
OpenAPI 메타데이터로부터 Action이 생성될 때, LibreChat은 제출된 Action 도메인이 해당 사양의 서버 URL과 일치하는지도 확인합니다. 제출된 도메인에 명시적인 포트가 포함된 경우, 이는 서버 URL의 유효 포트(URL에서 포트가 생략된 경우 HTTPS는 `443`, HTTP는 `80`)와 일치해야 합니다.
