...
- Becerileri bağımsız olarak doğrular ve yayınlar; böylece geçersiz veya çakışan bir Beceri, aynı kaynaktan gelen geçerli Becerileri gizlemez.
- Atlanan bir Skill'in bilinen son iyi durumdaki yansımasını korur ve o Skill için tamamlanmamış yazma işlemlerini geri alır.
- En az bir Skill yayınlayıp diğerini atladığında bir çalışmayı `partial` olarak işaretler ve yönetici durum okumaları için sınırlı bir atlanan sayısı ile ayrıntıları kaydeder.
- Aynı anda yalnızca bir GitHub senkronizasyonunun çalışmasını sağlamak için bir senkronizasyon kilidi kullanır.

Değerleri sınırlı, JSON açısından güvenli uzantı sınırları içinde kaldığı sürece bilinmeyen `SKILL.md` ön madde (frontmatter) anahtarları kabul edilir. Geçersiz tanınan alanlar, güvenli olmayan uzantı değerleri, yinelenen isimler ve sahiplik çakışmaları yalnızca ilgili Skill'i atlar. Kimlik doğrulama, hız sınırlaması, kilit kaybı, depo erişimi ve geri alma hataları kaynak düzeyinde hatalar olarak kalmaya devam eder.
...
<OptionTable
  options={[
    [
      ...
      'Endpoint',
      'Etkin durum, zamanlama ayarları, kaynak durumu, kimlik bilgisi varlığı, son eşitleme sayıları ve kısmi çalıştırmalar için sınırlandırılmış atlanan-Skill ayrıntılarını döndürür.',
      '',
    ],
    [
