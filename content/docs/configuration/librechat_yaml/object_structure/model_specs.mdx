...
Requires the opt-in endpoint-level `run_in_background` capability. LibreChat marks the spec's eligible tools as background-capable; the model can start one, continue working, and poll with `check_background_task`. Background task state is process-local and does not survive a server restart or move across replicas.
