...
<OptionTable
  options={[
    [
      'askUserQuestion',
      'Boolean',
      'Lets the model spec pause to ask the user a question and resume with the answer.',
      'false',
    ],
  ]}
/>
...
<OptionTable
  options={[
    [
      'runInBackground',
      'Boolean',
      'Allows eligible tools attached to the model spec to run as background tasks.',
      'false',
    ],
  ]}
/>

Requires the opt-in endpoint-level `run_in_background` capability. LibreChat marks the spec's eligible MCP tools as background-capable; when `executeCode: true`, Code Interpreter and shell calls are eligible too. The model can start an eligible tool, continue working, and poll with `check_background_task`. Completed code output and generated files are attached to the original code call. Background task state is process-local and does not survive a server restart or move across replicas.
