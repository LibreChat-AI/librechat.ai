...
<Callout type="info" title="Esta configuração controla apenas o armazenamento, não a exibição">

`transactions` decide se os registros de uso são **gravados no banco de dados**. Isso não exibe contagens de tokens em nenhum lugar no aplicativo, portanto, ativá-lo sozinho não fará com que as informações de token apareçam na interface.

Para *ver* o uso de tokens, use `interface.contextUsage`, que exibe o medidor de contexto na conversa e está ativado por padrão. `interface.contextCost` é uma configuração separada, desativada por padrão, que adiciona valores de custo a esse medidor; deixe-a desativada se quiser contagens de tokens sem exibir preços. Veja [Uso de Tokens](/docs/configuration/token_usage#viewing-context-usage-and-cost). Para aplicar limites de crédito por usuário, veja [Saldo](/docs/configuration/librechat_yaml/object_structure/balance).
