...
<OptionTable
  options={[
    ['serverless', 'Boolean', '指示使用用于 Azure OpenAI 聊天补全的无服务器推理 endpoint。当设置为 `true` 时，指定该组被配置为使用无服务器推理 endpoint 作为 Azure “模型即服务”(Models as a Service) 模型。', 'More info [here](../ai_endpoints/azure.mdx#serverless-inference-endpoints)'],
  ]}
/>
