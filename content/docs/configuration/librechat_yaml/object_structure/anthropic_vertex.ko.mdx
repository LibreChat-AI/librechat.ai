...
> **중요:** Claude Opus 4.7 이상, Opus 5, Sonnet 5, 그리고 Fable/Mythos 5는 `global`, `us` 또는 `eu`를 사용해야 합니다. `us-east5`와 같은 특정 리전은 Claude Sonnet 4.6 이하 버전을 지원하며, 최신 모델의 경우 model-not-found 응답을 반환합니다. 가장 폭넓은 가용성과 리전별 추가 요금이 없는 `global`을 사용하거나, 다중 리전 데이터 보존이 필요한 경우 `us`/`eu`를 사용하세요.
