...
> **중요:** Claude Opus 4.7 이상, Opus 5, Sonnet 5, 그리고 Fable/Mythos 5 또는 5.1은 `global`, `us`, 또는 `eu`를 필요로 합니다. `us-east5`와 같은 특정 지역은 Claude Sonnet 4.6 이하 버전을 제공하며, 최신 모델의 경우 model-not-found 응답을 반환합니다. 가장 폭넓은 가용성과 지역별 가격 프리미엄이 없는 `global`을 사용하거나, 다중 지역 데이터 상주가 필요한 경우 `us`/`eu`를 사용하세요.
...
`models`가 생략되면, LibreChat은 내장된 Vertex 카탈로그를 사용하며 구성된 `region`과 호환되지 않는 모델을 제거합니다. 명시적인 `models` 목록이나 맵은 필터링되지 않으므로, 모든 배포 ID는 선택한 위치에서 사용 가능해야 합니다.
...
> **참고:** 환경 변수를 사용할 때는 모델 매핑을 사용할 수 없으며, LibreChat은 표준 Anthropic 모델 카탈로그를 포함합니다. 특정 지역에서 사용할 수 없는 최신 모델을 선택하기 전에 `ANTHROPIC_VERTEX_REGION`을 `global`, `us` 또는 `eu`로 설정하세요.
...
- 모델: 선택한 위치에 대한 LibreChat의 내장 Vertex 카탈로그
...
- Opus 4.7+, Opus 5, Sonnet 5 및 Fable/Mythos 5의 경우 `global`, `us` 또는 `eu`를 사용하세요.
...
- Sonnet 4.6보다 최신 모델의 경우 `global`, `us` 또는 `eu`로 전환하세요.
