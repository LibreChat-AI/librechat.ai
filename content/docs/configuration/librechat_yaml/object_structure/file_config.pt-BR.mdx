...
<Callout type="info" title="Sintaxe de padrão MIME">
  O LibreChat avalia os padrões de `supportedMimeTypes` configurados pelo administrador no servidor com um mecanismo de expressão regular de tempo linear compatível com RE2. Referências retroativas (backreferences) e lookaround não são suportados. Padrões inválidos são registrados em log e ignorados; se todos os padrões em uma lista configurada forem inválidos, essa lista falha de forma segura e rejeita todos os arquivos em vez de se tornar irrestrita.
</Callout>
## skills
