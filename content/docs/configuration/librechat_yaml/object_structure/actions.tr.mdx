...
OpenAPI meta verilerinden bir Action oluşturulduğunda, LibreChat ayrıca gönderilen Action alan adının (domain) spesifikasyonun sunucu URL'si ile eşleştiğini doğrular. Gönderilen alan adı açık bir port içeriyorsa, bu port sunucu URL'sinin geçerli portuyla (URL'de belirtilmediğinde HTTPS için `443` veya HTTP için `80`) eşleşmelidir.
