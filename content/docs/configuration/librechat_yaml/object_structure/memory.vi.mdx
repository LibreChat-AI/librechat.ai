...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Cấu hình tác nhân bộ nhớ tự động tùy chọn. Có thể là tham chiếu đến một tác nhân hiện có theo ID hoặc cấu hình tác nhân hoàn chỉnh.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

Tính năng trích xuất tự động là tùy chọn (opt-in). Hãy đặt `agent.enabled: true` trong một trong hai định dạng cấu hình dưới đây. Việc bỏ qua `enabled`, hoặc đặt thành `false`, sẽ giữ cho bộ nhớ thủ công và các công cụ bộ nhớ Agent khả dụng mà không cần chạy agent bộ nhớ tự động trên các yêu cầu trò chuyện.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Kích hoạt tính năng tự động trích xuất bộ nhớ với memory agent đã được cấu hình.',
      'enabled: true',
    ],
  ]}
/>
...
- `agent.enabled: true` là bắt buộc để tự động trích xuất bộ nhớ
- Cấu hình Agent cho phép tùy chỉnh hành vi xử lý bộ nhớ tự động
