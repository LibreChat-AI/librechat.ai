---
title: 음성 설정
icon: Mic
---

## 개요 [#overview]

`speech` 객체를 사용하면 `librechat.yaml` 구성 파일에서 직접 텍스트 음성 변환(TTS) 및 음성 텍스트 변환(STT) 제공업체를 구성할 수 있습니다. 이를 통해 사용자가 자신의 API 키를 구성할 필요 없이 서버 측 음성 서비스를 활성화할 수 있습니다.

**`speech` 하위 필드:**

- `tts` - Text-to-Speech 공급자 설정
- `stt` - Speech-to-Text 공급자 구성
- `speechTab` - 음성 기능을 위한 기본 UI 설정

**참고:**

- 여러 공급자를 동시에 구성할 수 있습니다.
- 사용자는 사용 가능한 옵션 중에서 선호하는 제공업체를 선택할 수 있습니다.
- config 파일의 API 키는 보안을 위해 환경 변수 참조를 사용해야 합니다.

## 예시 [#example]

```yaml filename="speech"
speech:
  tts:
    allowedAddresses: ["tts.internal:8080"]
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["voice-id-1", "voice-id-2"]
  stt:
    allowedAddresses: ["stt.internal:8080"]
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: true
    advancedMode: false
    speechToText: true
    textToSpeech: true
```

---

## SSRF 보호 [#ssrf-protection]

LibreChat은 연결 시점에 운영자가 제공한 STT 및 TTS URL을 보호합니다. 사설, 루프백, 링크 로컬 및 클라우드 메타데이터 대상은 기본적으로 차단되며, 음성 요청에 대한 리디렉션은 비활성화됩니다.

`speech.tts.allowedAddresses` 및 `speech.stt.allowedAddresses`는 엄격한 허용 목록이 아닌 별도의 예외 목록입니다. 해당 섹션이 신뢰할 수 있는 자체 호스팅 서비스에 연결되어야 하는 경우에만 정확한 사설 `host:port`, `private.ip:port` 또는 `[ipv6]:port`를 추가하십시오. URL, 경로, CIDR 범위, 호스트/IP 단독, 또는 공인 IP 리터럴을 사용하지 마십시오. 기본 포트는 정규화되므로 명시적인 포트가 없는 HTTPS URL은 포트 `443`으로 확인됩니다.

면제된 호스트 이름은 해당 포트에서 확인되는 모든 사설 주소를 신뢰합니다. 가능하면 사설 IP 리터럴을 사용하는 것을 권장하며, 사용자가 DNS를 제어할 수 있고 공격자가 재지정할 수 없는 이름만 나열하십시오.

포워드 프록시가 호스트네임 요청을 전달할 때, 프록시는 대상 DNS 및 이그레스(egress)를 수행하므로 프록시 자체의 SSRF 정책을 적용해야 합니다. 리터럴 사설 대상은 정확한 주소와 포트가 예외 처리되지 않는 한, 프록시 처리 이전에 여전히 차단됩니다.

---

## tts

`tts` 객체는 Text-to-Speech 공급자를 구성합니다. 여러 공급자를 구성할 수 있으며, 사용자는 사용할 공급자를 선택할 수 있습니다.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'TTS 연결 시간 SSRF 검사에 대한 신뢰할 수 있는 비공개 host:port 예외입니다. 공용 대상은 계속 사용할 수 있습니다.',
      'allowedAddresses: ["tts.internal:8080"]',
    ],
  ]}
/>

### openai

`tts-1` 또는 `tts-1-hd`와 같은 모델을 사용하는 OpenAI TTS 구성.

<OptionTable
  options={[
    ['url', 'String', '사용자 지정 API URL (선택 사항). OpenAI 호환 endpoint에 사용하세요.', ''],
    ['apiKey', 'String', 'OpenAI API 키. 환경 변수 참조를 사용하세요.', 'Required'],
    ['model', 'String', '사용할 TTS 모델 (예: "tts-1", "tts-1-hd").', 'Required'],
    ['voices', 'Array of Strings', '사용자가 선택할 수 있는 사용 가능한 음성 옵션입니다.', 'Required'],
  ]}
/>

**예시:**
```yaml filename="speech / tts / openai"
tts:
  openai:
    apiKey: "${TTS_API_KEY}"
    model: "tts-1"
    voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
```

### azureOpenAI

Azure OpenAI TTS 구성.

<OptionTable
  options={[
    ['instanceName', 'String', 'Azure OpenAI 인스턴스 이름.', 'Required'],
    ['apiKey', 'String', 'Azure OpenAI API 키', 'Required'],
    ['deploymentName', 'String', 'TTS 모델의 배포 이름입니다.', 'Required'],
    ['apiVersion', 'String', 'Azure OpenAI API 버전.', 'Required'],
    ['model', 'String', 'TTS 모델 식별자.', 'Required'],
    ['voices', 'Array of Strings', '사용 가능한 음성 옵션.', 'Required'],
  ]}
/>

**예시:**
```yaml filename="speech / tts / azureOpenAI"
tts:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_TTS_API_KEY}"
    deploymentName: "tts-deployment"
    apiVersion: "2024-02-15-preview"
    model: "tts-1"
    voices: ["alloy", "echo", "nova"]
```

### elevenlabs

고품질 음성 합성을 위한 ElevenLabs TTS 구성.

<OptionTable
  options={[
    ['url', 'String', '사용자 지정 API URL (선택 사항).', ''],
    ['websocketUrl', 'String', '스트리밍을 위한 WebSocket URL (선택 사항).', ''],
    ['apiKey', 'String', 'ElevenLabs API 키', 'Required'],
    ['model', 'String', 'ElevenLabs 모델 (예: "eleven_multilingual_v2").', 'Required'],
    ['voices', 'Array of Strings', '선택 가능한 Voice ID.', 'Required'],
    ['voice_settings', 'Object', '음성 사용자 지정 설정 (선택 사항).', ''],
    ['pronunciation_dictionary_locators', 'Array of Strings', '발음 사전 ID (선택 사항).', ''],
  ]}
/>

**voice_settings 하위 키:**
<OptionTable
  options={[
    ['similarity_boost', 'Number', '음성 유사도 향상 (0-1).', ''],
    ['stability', 'Number', '음성 안정성 (0-1).', ''],
    ['style', 'Number', '스타일 과장 정도 (0-1).', ''],
    ['use_speaker_boost', 'Boolean', '스피커 부스트 활성화', ''],
  ]}
/>

**예시:**
```yaml filename="speech / tts / elevenlabs"
tts:
  elevenlabs:
    apiKey: "${ELEVENLABS_API_KEY}"
    model: "eleven_multilingual_v2"
    voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
    voice_settings:
      stability: 0.5
      similarity_boost: 0.75
      use_speaker_boost: true
```

### localai

자체 호스팅 음성 합성을 위한 LocalAI TTS 구성.

<OptionTable
  options={[
    ['url', 'String', 'LocalAI 서버 URL.', 'Required'],
    ['apiKey', 'String', '인증이 활성화된 경우의 API key (선택 사항).', ''],
    ['voices', 'Array of Strings', '사용 가능한 음성 모델.', 'Required'],
    ['backend', 'String', '사용할 TTS 백엔드 (예: "piper").', 'Required'],
  ]}
/>

**예시:**
```yaml filename="speech / tts / localai"
tts:
  localai:
    url: "http://localhost:8080"
    voices: ["en-us-amy-low", "en-us-danny-low"]
    backend: "piper"
```

---

## stt

`stt` 객체는 Speech-to-Text 공급자를 구성합니다.

<OptionTable
  options={[
    [
      'allowedAddresses',
      'Array of Strings',
      'STT 연결 시간 SSRF 검사에 대한 신뢰할 수 있는 비공개 host:port 예외입니다. 공용 대상은 계속 사용할 수 있습니다.',
      'allowedAddresses: ["stt.internal:8080"]',
    ],
  ]}
/>

### openai [#openai-1]

OpenAI Whisper STT 구성.

<OptionTable
  options={[
    ['url', 'String', '사용자 지정 API URL (선택 사항). OpenAI 호환 endpoint에 사용하세요.', ''],
    ['apiKey', 'String', 'OpenAI API 키. 환경 변수 참조를 사용하세요.', 'Required'],
    ['model', 'String', '사용할 STT 모델 (예: "whisper-1").', 'Required'],
  ]}
/>

**예시:**
```yaml filename="speech / stt / openai"
stt:
  openai:
    apiKey: "${STT_API_KEY}"
    model: "whisper-1"
```

### azureOpenAI [#azureopenai-1]

Azure OpenAI Whisper STT 구성.

<OptionTable
  options={[
    ['instanceName', 'String', 'Azure OpenAI 인스턴스 이름.', 'Required'],
    ['apiKey', 'String', 'Azure OpenAI API 키', 'Required'],
    ['deploymentName', 'String', 'Whisper 모델의 배포 이름입니다.', 'Required'],
    ['apiVersion', 'String', 'Azure OpenAI API 버전.', 'Required'],
  ]}
/>

**예시:**
```yaml filename="speech / stt / azureOpenAI"
stt:
  azureOpenAI:
    instanceName: "my-azure-instance"
    apiKey: "${AZURE_STT_API_KEY}"
    deploymentName: "whisper-deployment"
    apiVersion: "2024-02-15-preview"
```

---

## speechTab

`speechTab` 객체는 음성 기능을 위한 기본 UI 설정을 구성합니다. 이 설정들은 사용자가 음성 설정 패널에서 기본적으로 보게 될 항목들을 제어합니다.

<OptionTable
  options={[
    ['conversationMode', 'Boolean', '기본적으로 대화 모드를 활성화합니다.', 'false'],
    ['advancedMode', 'Boolean', '기본적으로 고급 음성 설정 표시', 'false'],
    ['speechToText', 'Boolean or Object', 'STT를 기본값으로 활성화하거나 상세 STT 설정을 구성하세요.', 'false'],
    ['textToSpeech', 'Boolean or Object', 'TTS를 기본으로 활성화하거나 상세 TTS 설정을 구성하세요.', 'false'],
  ]}
/>

### speechToText (객체 형식) [#speechtotext-object-format]

boolean 대신 객체를 사용할 때:

<OptionTable
  options={[
    [
      'engineSTT',
      'String',
      '기본 STT 엔진입니다. `"browser"` 또는 `"external"`을 사용하세요. 레거시 값인 `"openai"`와 `"azureOpenAI"`도 허용되며 `"external"`로 정규화됩니다.',
      'browser',
    ],
    ['languageSTT', 'String', 'STT 기본 언어.', ''],
    ['autoTranscribeAudio', 'Boolean', '첫 번째 일시 중지에서 멈추지 않고 마이크를 계속 활성화합니다. 외부 엔진을 사용하는 경우 `decibelValue`를 기반으로 하는 무음 감지도 켜집니다.', ''],
    ['decibelValue', 'Number', '무음 감지를 위한 데시벨 임계값입니다. 범위는 -100에서 -30까지이며, 기본값은 -45입니다.', ''],
    ['autoSendText', 'Number', '전사 후 자동 전송까지 대기할 시간(초)입니다. `0`은 즉시 전송하며, `-1`은 자동 전송을 비활성화합니다.', ''],
  ]}
/>

### textToSpeech (객체 형식) [#texttospeech-object-format]

boolean 대신 객체를 사용할 때:

<OptionTable
  options={[
    [
      'engineTTS',
      'String',
      '기본 TTS 엔진입니다. `"browser"` 또는 `"external"`을 사용하세요. 레거시 공급자 값도 허용되며 `"external"`로 정규화됩니다.',
      'browser',
    ],
    ['voice', 'String', '기본 음성 선택.', ''],
    ['languageTTS', 'String', 'TTS의 기본 언어입니다.', ''],
    ['automaticPlayback', 'Boolean', 'TTS 응답을 자동으로 재생합니다.', ''],
    ['playbackRate', 'Number', '기본 재생 속도(1.0 = 보통). 범위: 0.25–4.0.', ''],
    ['cacheTTS', 'Boolean', '반복 재생을 위해 TTS 오디오를 캐시합니다.', ''],
  ]}
/>

**예시:**
```yaml filename="speech / speechTab"
speechTab:
  conversationMode: false
  advancedMode: false
  speechToText:
    engineSTT: "external"
    autoTranscribeAudio: true
    decibelValue: -45
  textToSpeech:
    engineTTS: "external"
    voice: "nova"
    automaticPlayback: false
    playbackRate: 1.0
    cacheTTS: true
```

---

## 전체 예시 [#complete-example]

```yaml filename="librechat.yaml"
version: 1.3.15
cache: true

speech:
  tts:
    openai:
      apiKey: "${TTS_API_KEY}"
      model: "tts-1-hd"
      voices: ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
    elevenlabs:
      apiKey: "${ELEVENLABS_API_KEY}"
      model: "eleven_multilingual_v2"
      voices: ["21m00Tcm4TlvDq8ikWAM", "AZnzlk1XvdvUeBnXmlld"]
      voice_settings:
        stability: 0.5
        similarity_boost: 0.75
  stt:
    openai:
      apiKey: "${STT_API_KEY}"
      model: "whisper-1"
  speechTab:
    conversationMode: false
    advancedMode: false
    speechToText: true
    textToSpeech:
      engineTTS: "external"
      voice: "nova"
      automaticPlayback: false
```

---

## 참고 사항 [#notes]

- 구성 파일의 API 키에는 항상 환경 변수 참조(예: `${API_KEY}`)를 사용하세요.
- 여러 TTS 공급자를 구성할 수 있으며, 사용자는 UI에서 선호하는 옵션을 선택할 수 있습니다.
- `speechTab` 설정은 사용자가 개인 설정에서 재정의할 수 있는 기본값을 정의합니다.
- `browser`는 브라우저 내장 음성 지원을 사용하며, `external`은 `speech.stt` 또는 `speech.tts` 아래에 구성된 서버 측 제공업체를 사용합니다.
- 기존의 `openai` 및 `azureOpenAI` STT 기본값과 `openai`, `azureOpenAI`, `elevenlabs`, `localai` TTS 기본값은 `external`로 마이그레이션되며, 사용할 수 없는 외부 엔진은 `browser`로 대체됩니다.
- 자세한 기능 문서는 [Speech to Text & Text to Speech](/docs/configuration/stt_tts)를 참조하세요.
