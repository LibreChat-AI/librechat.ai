...
<OptionTable
  options={[
    [
      'disabled',
      'Boolean',
      'Desactiva la funcionalidad de memoria cuando se establece en true. Cuando está desactivado, el sistema no almacenará ni utilizará la memoria de la conversación.',
      'disabled: false',
    ],
  ]}
/>
...
<OptionTable
  options={[
    [
      'agent',
      'Object | Union',
      'Configura el agente de memoria automático opcional. Puede ser una referencia a un agente existente por ID o una configuración de agente completa.',
      'agent: { enabled: true, provider: "openAI", model: "gpt-4" }',
    ],
  ]}
/>

La extracción automática es opcional. Establezca `agent.enabled: true` en cualquiera de los formatos de configuración a continuación. Omitir `enabled`, o establecerlo en `false`, mantiene disponibles la memoria manual y las herramientas de memoria de Agent sin ejecutar el agente de memoria automático en las solicitudes de chat.
...
<OptionTable
  options={[
    [
      'enabled',
      'Boolean',
      'Habilita la extracción automática de memoria con el agente de memoria configurado.',
      'enabled: true',
    ],
  ]}
/>
...
### Configuración de Agentes Personalizados [#custom-agent-configuration]
...
Al utilizar una configuración de agente personalizada, los siguientes campos están disponibles:
...
<OptionTable
  options={[
    [
      'model_parameters',
      'Object',
      'Parámetros adicionales para pasar al modelo y ajustar su comportamiento. Los valores deben ser cadenas, números o booleanos.',
...
- `agent.enabled: true` es necesario para la extracción automática de memoria
- La configuración de agentes permite personalizar el comportamiento del procesamiento automático de memoria.
