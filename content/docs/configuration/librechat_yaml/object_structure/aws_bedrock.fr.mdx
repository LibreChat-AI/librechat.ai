...
<OptionTable
  options={[
    [
      'streamRate',
      'Number',
      'Définit le taux de traitement de chaque nouveau jeton en millisecondes.',
...
**Par défaut :** Non spécifié (utilise la liste des modèles Bedrock par défaut)
...
<OptionTable
  options={[
    [
      'guardrailIdentifier',
      'String',
      'L\'identifiant unique de la barrière de sécurité à appliquer.',
    ...
    [
      ...
      'String',
      'Contrôle le mode de traitement du flux de garde-fous. Options : "sync" ou "async".',
...
- Définissez `trace` sur `"enabled"` ou `"enabled_full"` pendant le développement pour voir quelles politiques de garde-fous sont déclenchées
- Définissez `streamProcessingMode` sur `"async"` pour diffuser les réponses plus rapidement (au prix d'une éventuelle autorisation de contenu inapproprié par les garde-fous jusqu'à ce que leur analyse soit terminée)
