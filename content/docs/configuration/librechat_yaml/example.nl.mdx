...
## Schoon voorbeeld [#clean-example]

<Callout type="example" title="Voorbeeld" collapsible>

Dit voorbeeldconfiguratiebestand bevat alle gedocumenteerde endpoints (behalve Azure, LiteLLM, MLX en Ollama, die allemaal aanvullende configuraties vereisen)
...
## Voorbeeld met commentaar [#example-with-comments]

Dit voorbeeldconfiguratiebestand stelt LibreChat in met gedetailleerde opties voor verschillende belangrijke gebieden:

- **Caching**: Ingeschakeld om de prestaties te verbeteren.
- **Bestandsafhandeling**:
  - **Bestandsstrategie**: Uitgecommentarieerd, maar wijst op een mogelijke integratie met Firebase voor bestandsopslag.
  - **Bestandsconfiguraties**: Past bestandsuploadlimieten en toegestane MIME-types aan voor verschillende endpoints, inclusief een globale bestandsgrootte-limiet voor de server en een specifieke limiet voor gebruikersavatar-afbeeldingen.
- **Rate Limiting**: Definieert drempelwaarden voor het maximale aantal bestandsuploads dat is toegestaan per IP en gebruiker binnen een opgegeven tijdsvenster, met als doel misbruik te voorkomen.
- **Registratie**:
  - Staat registratie toe via opgegeven social login-providers en e-maildomeinen, wat de beveiliging en het gebruikersbeheer verbetert.
- **Endpoints**:
  - **Assistants**: Configureert het assistants-endpoint met een polling-interval en een time-out voor bewerkingen, en biedt een optie om de builder-interface uit te schakelen.
  - **Custom Endpoints**:
    - Configureert twee externe AI-service-endpoints, Mistral en OpenRouter, inclusief API-sleutels, basis-URL's, modelafhandeling en specifieke functie-toggles zoals gesprektitels, samenvattingen en parameterinstellingen.
    - Voor Mistral maakt dit het dynamisch ophalen van modellen mogelijk, past het aanvullende parameters toe voor veilige prompts en verwijdert het expliciet niet-ondersteunde parameters.
    - Voor OpenRouter stelt het een basisconfiguratie in zonder dynamische model-fetching en specificeert het een model voor gesprektitels.

<Callout type="example" title="Voorbeeld met commentaar" collapsible>
