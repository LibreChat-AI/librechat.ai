...
## Ví dụ sạch [#clean-example]

<Callout type="example" title="Ví dụ" collapsible>

Cấu hình mẫu này bao gồm tất cả các endpoint đã được ghi lại (Ngoại trừ Azure, LiteLLM, MLX và Ollama, tất cả đều yêu cầu các cấu hình bổ sung)
...
## Ví dụ với chú thích [#example-with-comments]

Tệp cấu hình ví dụ này thiết lập LibreChat với các tùy chọn chi tiết trên một số lĩnh vực chính:

- **Caching**: Đã bật để cải thiện hiệu suất.
- **Xử lý tệp**:
  - **Chiến lược tệp (File Strategy)**: Đã được chú thích nhưng gợi ý về khả năng tích hợp với Firebase để lưu trữ tệp.
  - **File Configurations**: Tùy chỉnh các giới hạn tải lên tệp và các loại MIME được phép cho các endpoint khác nhau, bao gồm giới hạn kích thước tệp máy chủ toàn cục và giới hạn cụ thể cho hình ảnh đại diện của người dùng.
- **Giới hạn tốc độ (Rate Limiting)**: Xác định các ngưỡng cho số lượng tệp tải lên tối đa được phép cho mỗi IP và người dùng trong một khoảng thời gian nhất định, nhằm mục đích ngăn chặn hành vi lạm dụng.
- **Đăng ký**:
  - Cho phép đăng ký từ các nhà cung cấp đăng nhập mạng xã hội và tên miền email được chỉ định, giúp tăng cường bảo mật và quản lý người dùng.
- **Endpoints**:
  - **Assistants**: Cấu hình endpoint cho assistants với khoảng thời gian thăm dò (polling interval) và thời gian chờ (timeout) cho các thao tác, đồng thời cung cấp tùy chọn để vô hiệu hóa giao diện trình tạo (builder interface).
  - **Custom Endpoints**:
    - Cấu hình hai endpoint dịch vụ AI bên ngoài là Mistral và OpenRouter, bao gồm các khóa API, URL cơ sở, cách xử lý model và các tùy chọn bật/tắt tính năng cụ thể như tiêu đề cuộc trò chuyện, tóm tắt và điều chỉnh tham số.
    - Đối với Mistral, tính năng này cho phép tìm nạp mô hình động, áp dụng các tham số bổ sung cho các câu lệnh an toàn (safe prompts) và loại bỏ rõ ràng các tham số không được hỗ trợ.
    - Đối với OpenRouter, nó thiết lập một cấu hình cơ bản mà không cần tìm nạp mô hình động và chỉ định một mô hình cho các tiêu đề cuộc trò chuyện.

<Callout type="example" title="Ví dụ có chú thích" collapsible>
