...
## Ejemplo limpio [#clean-example]

<Callout type="example" title="Ejemplo" collapsible>

Esta configuración de ejemplo incluye todos los endpoints documentados (Excepto Azure, LiteLLM, MLX y Ollama, los cuales requieren configuraciones adicionales)
...
## Ejemplo con comentarios [#example-with-comments]

Este archivo de configuración de ejemplo configura LibreChat con opciones detalladas en varias áreas clave:

- **Caching**: Habilitado para mejorar el rendimiento.
- **Manejo de archivos**:
  - **Estrategia de archivos**: Comentado, pero sugiere una posible integración con Firebase para el almacenamiento de archivos.
  - **Configuraciones de archivos**: Personaliza los límites de carga de archivos y los tipos MIME permitidos para diferentes endpoint, incluyendo un límite global de tamaño de archivo para el servidor y un límite específico para las imágenes de avatar de los usuarios.
- **Limitación de tasa (Rate Limiting)**: Define umbrales para el número máximo de subidas de archivos permitidas por IP y por usuario dentro de un intervalo de tiempo especificado, con el objetivo de prevenir abusos.
- **Registro**:
  - Permite el registro desde proveedores de inicio de sesión social y dominios de correo electrónico especificados, mejorando la seguridad y la gestión de usuarios.
- **Endpoints**:
  - **Assistants**: Configura el endpoint de los assistants con un intervalo de sondeo y un tiempo de espera para las operaciones, y proporciona una opción para deshabilitar la interfaz del constructor.
  - **Custom Endpoints**:
    - Configura dos endpoints de servicios de IA externos, Mistral y OpenRouter, incluyendo claves de API, URLs base, manejo de modelos y activadores de funciones específicas como títulos de conversación, resumen y ajustes de parámetros.
    - Para Mistral, permite la obtención dinámica de modelos, aplica parámetros adicionales para prompts seguros y descarta explícitamente los parámetros no compatibles.
    - Para OpenRouter, establece una configuración básica sin obtención dinámica de modelos y especifica un modelo para los títulos de las conversaciones.

<Callout type="example" title="Ejemplo comentado" collapsible>
