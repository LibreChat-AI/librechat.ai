...
## Sauberes Beispiel [#clean-example]

<Callout type="example" title="Beispiel" collapsible>

Dieses Beispiel-Config enthält alle dokumentierten endpoints (außer Azure, LiteLLM, MLX und Ollama, die alle zusätzliche Konfigurationen erfordern)
...
## Beispiel mit Kommentaren [#example-with-comments]

Diese Beispielkonfigurationsdatei richtet LibreChat mit detaillierten Optionen in mehreren Schlüsselbereichen ein:

- **Caching**: Aktiviert, um die Leistung zu verbessern.
- **Dateiverarbeitung**:
  - **Dateistrategie**: Auskommentiert, deutet jedoch auf eine mögliche Integration mit Firebase für die Dateispeicherung hin.
  - **Dateikonfigurationen**: Passt Datei-Upload-Limits und erlaubte MIME-Typen für verschiedene endpoints an, einschließlich eines globalen Server-Dateigrößenlimits und eines spezifischen Limits für Benutzer-Avatar-Bilder.
- **Rate Limiting**: Definiert Schwellenwerte für die maximale Anzahl an Datei-Uploads pro IP und Benutzer innerhalb eines festgelegten Zeitfensters, um Missbrauch zu verhindern.
- **Registrierung**:
  - Ermöglicht die Registrierung über bestimmte Social-Login-Anbieter und E-Mail-Domains, was die Sicherheit und Benutzerverwaltung verbessert.
- **Endpoints**:
  - **Assistants**: Konfiguriert den Assistants-endpoint mit einem Abfrageintervall (Polling-Intervall) und einem Timeout für Vorgänge und bietet eine Option zum Deaktivieren der Builder-Oberfläche.
  - **Benutzerdefinierte Endpoints**:
    - Konfiguriert zwei externe AI-Service-endpoints, Mistral und OpenRouter, einschließlich API-Schlüsseln, Basis-URLs, Modellhandhabung und spezifischen Funktions-Toggles wie Konversationstiteln, Zusammenfassungen und Parameteranpassungen.
    - Für Mistral ermöglicht dies das dynamische Abrufen von Modellen, wendet zusätzliche Parameter für sichere Prompts an und verwirft explizit nicht unterstützte Parameter.
    - Für OpenRouter wird eine grundlegende Konfiguration ohne dynamisches Abrufen von Modellen eingerichtet und ein Modell für Konversationstitel festgelegt.

<Callout type="example" title="Kommentiertes Beispiel" collapsible>
