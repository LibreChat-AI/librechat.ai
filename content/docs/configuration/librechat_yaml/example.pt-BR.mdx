...
## Exemplo Limpo [#clean-example]

<Callout type="example" title="Exemplo" collapsible>

Este exemplo de configuração inclui todos os endpoints documentados (Exceto Azure, LiteLLM, MLX e Ollama, que exigem configurações adicionais)
...
## Exemplo com comentários [#example-with-comments]

Este arquivo de configuração de exemplo configura o LibreChat com opções detalhadas em várias áreas principais:

- **Caching**: Habilitado para melhorar o desempenho.
- **Manipulação de Arquivos**:
  - **Estratégia de Arquivo**: Comentado, mas sugere uma possível integração com o Firebase para armazenamento de arquivos.
  - **Configurações de Arquivo**: Personaliza os limites de upload de arquivos e os tipos MIME permitidos para diferentes endpoint, incluindo um limite global de tamanho de arquivo no servidor e um limite específico para imagens de avatar de usuário.
- **Rate Limiting**: Define limites para o número máximo de uploads de arquivos permitidos por IP e por usuário dentro de um intervalo de tempo especificado, visando prevenir abusos.
- **Registro**:
  - Permite o registro a partir de provedores de login social e domínios de e-mail especificados, aprimorando a segurança e o gerenciamento de usuários.
- **Endpoints**:
  - **Assistants**: Configura o endpoint dos assistants com um intervalo de polling e um tempo limite para operações, e fornece uma opção para desativar a interface do construtor.
  - **Endpoints Personalizados**:
    - Configura dois endpoints de serviço de IA externos, Mistral e OpenRouter, incluindo chaves de API, URLs base, tratamento de modelos e alternadores de recursos específicos como títulos de conversação, sumarização e ajustes de parâmetros.
    - Para o Mistral, ele habilita a busca dinâmica de modelos, aplica parâmetros adicionais para prompts seguros e descarta explicitamente parâmetros não suportados.
    - Para o OpenRouter, ele define uma configuração básica sem busca dinâmica de modelos e especifica um modelo para títulos de conversas.

<Callout type="example" title="Exemplo Comentado" collapsible>
