...
## Contoh Bersih [#clean-example]

<Callout type="example" title="Contoh" collapsible>

Contoh konfigurasi ini mencakup semua endpoint yang didokumentasikan (Kecuali Azure, LiteLLM, MLX, dan Ollama, yang semuanya memerlukan konfigurasi tambahan)
...
## Contoh dengan Komentar [#example-with-comments]

Contoh file konfigurasi ini mengatur LibreChat dengan opsi mendetail di beberapa area utama:

- **Caching**: Diaktifkan untuk meningkatkan performa.
- **Penanganan File**:
  - **Strategi File**: Dikomentari tetapi memberikan petunjuk mengenai kemungkinan integrasi dengan Firebase untuk penyimpanan file.
  - **Konfigurasi File**: Menyesuaikan batas unggahan file dan tipe MIME yang diizinkan untuk berbagai endpoint, termasuk batas ukuran file server global dan batas khusus untuk gambar avatar pengguna.
- **Rate Limiting**: Menentukan ambang batas untuk jumlah maksimum unggahan file yang diizinkan per IP dan pengguna dalam jangka waktu tertentu, yang bertujuan untuk mencegah penyalahgunaan.
- **Pendaftaran**:
  - Mengizinkan pendaftaran dari penyedia login sosial dan domain email tertentu, sehingga meningkatkan keamanan dan manajemen pengguna.
- **Endpoints**:
  - **Assistants**: Mengonfigurasi endpoint assistants dengan interval polling dan batas waktu (timeout) untuk operasi, serta menyediakan opsi untuk menonaktifkan antarmuka pembuat (builder interface).
  - **Custom Endpoints**:
    - Mengonfigurasi dua endpoint layanan AI eksternal, Mistral dan OpenRouter, termasuk kunci API, base URL, penanganan model, dan toggle fitur spesifik seperti judul percakapan, peringkasan, dan penyesuaian parameter.
    - Untuk Mistral, ini memungkinkan pengambilan model secara dinamis, menerapkan parameter tambahan untuk prompt yang aman, dan secara eksplisit menghapus parameter yang tidak didukung.
    - Untuk OpenRouter, ini menyiapkan konfigurasi dasar tanpa pengambilan model dinamis dan menentukan model untuk judul percakapan.

<Callout type="example" title="Contoh Berkomentar" collapsible>
