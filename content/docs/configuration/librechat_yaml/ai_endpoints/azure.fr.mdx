...
<OptionTable
  options={[
    ...
    ['summarize', 'boolean', 'Active la synthèse de conversation pour tous les modèles Azure. Définissez sur `true` pour activer la synthèse. Par défaut : `false`.', 'summarize:false'],
...
<OptionTable
options={[
['deploymentName', 'string', 'Le nom de déploiement au niveau du groupe est facultatif, mais requis si un modèle au sein du groupe est défini sur true.', 'deploymentName: my-deployment'],
...
['addParams', 'object', 'Ajoute ou remplace des paramètres supplémentaires pour les requêtes de l\'API Azure OpenAI. Utile pour spécifier des options propres à l\'API sous forme de paires clé-valeur.', 'addParams: {temperature: 0.7}'],
...
Au sein de chaque groupe, le champ `models` contient un mappage des identifiants de modèle vers leurs configurations :
...
- Pour les identifiants, reposez-vous sur des variables d'environnement personnalisées spécifiées dans chaque configuration de groupe compatible avec les assistants.
...
- Les fichiers que vous téléchargez vers Azure OpenAI, que ce soit au niveau du message ou de l'assistant, ne seront disponibles que dans la région dont fait partie le modèle de l'assistant actuel.
...
L'URL de base pour les requêtes de l'API Azure OpenAI peut être configurée dynamiquement. Ceci est utile pour les services de proxy tels que [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/providers/azureopenai/), ou si vous souhaitez remplacer explicitement la gestion de baseURL de l'application.
...
- Ensuite, vous devez déployer le modèle de génération d'images dans l'une des régions ci-dessus.
    - Lisez le [Azure OpenAI Image Generation Quickstart Guide](https://learn.microsoft.com/en-us/azure/ai-services/openai/dall-e-quickstart) pour obtenir une assistance supplémentaire.
...
- Les modèles Azure AI Foundry provisionnent désormais des endpoints sous `/models/chat/completions?api-version=version` pour l'inférence serverless.
  ...
  - Le paramètre de requête `version` est optionnel et peut être spécifié dans le champ `baseURL`.
...
- Ces endpoint/modèles d'inférence serverless peuvent ou non prendre en charge l'appel de fonction (function calling) selon les spécifications de l'API OpenAI, ce qui permet leur utilisation avec des Agents.
