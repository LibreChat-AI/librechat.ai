...
## Endpoint-Level Konfiguration [#endpoint-level-configuration]
...
### Modell-Bereitstellungen [#model-deployments]

Die Liste der Modelle, die Ihren Benutzern zur Verfügung stehen, wird durch die Modellgruppierungen bestimmt, die in Ihrer [`azureOpenAI` endpoint config](/docs/configuration/librechat_yaml/object_structure/azure_openai) festgelegt sind.
...
1) Setzen Sie das Feld `assistants` **unter** dem `azureOpenAI` endpoint, d. h. auf der [Endpoint-Ebene](#endpoint-level-configuration), auf `true`, wie folgt:
...
Die Basis-URL für Azure OpenAI API-Anfragen kann dynamisch konfiguriert werden. Dies ist nützlich für Proxy-Dienste wie [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/providers/azureopenai/) oder wenn Sie die baseURL-Verarbeitung der App explizit überschreiben möchten.
...
<Callout type="info" title="Unterstützung für das Azure Endpoint Domain-Format">
...
<OptionTable
  options={[
    ['DALLE3_BASEURL', 'string', 'Die Basis-URL für DALL-E 3 API endpoints. Unterstützt sowohl `.openai.azure.com` (legacy) als auch `.cognitiveservices.azure.com` (neu) Domain-Formate.','# DALLE3_BASEURL=https://<AZURE_OPENAI_API_INSTANCE_NAME>.openai.azure.com/openai/deployments/<DALLE3_DEPLOYMENT_NAME>/\n# OR\n# DALLE3_BASEURL=https://<AZURE_OPENAI_API_INSTANCE_NAME>.cognitiveservices.azure.com/openai/deployments/<DALLE3_DEPLOYMENT_NAME>/'],
...
Über die `librechat.yaml` Datei können Sie serverlose Azure AI Studio Inference-Endpunkte konfigurieren, um auf Modelle aus der [Azure AI Foundry](https://ai.azure.com/explore) zuzugreifen. Es werden lediglich eine Modellkennung, `baseURL` und `apiKey` zusammen mit dem Feld `serverless` benötigt, um die spezielle Handhabung anzugeben, die diese Endpunkte erfordern.
