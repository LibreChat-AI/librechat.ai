...
title: Huggingface
icon: HuggingFace
description: Configura Huggingface come endpoint personalizzato in LibreChat.
---
...
Crea un token su [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens). Aggiungilo al tuo file `.env`:
...
L'elenco dei modelli qui sopra è stato aggiornato l'ultima volta il 9 maggio 2024.
...
- Il recupero dell'elenco dei modelli non è supportato, quindi imposta tu stesso l'array `default`.
- `dropParams: ["top_p"]` è richiesto. Senza di esso, le richieste falliscono perché Huggingface rifiuta il parametro `top_p`. Vedi [`dropParams`](/docs/configuration/librechat_yaml/object_structure/custom_endpoint#dropparams).

<Callout type="warning" title="Altri errori del modello" collapsible>
    Ecco un elenco degli altri modelli che sono stati testati insieme ai relativi errori
