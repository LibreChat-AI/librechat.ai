...
title: together.ai
icon: TogetherAI
description: 在 LibreChat 中将 together.ai 配置为自定义 endpoint。
---

together.ai 通过兼容 OpenAI 的 API 提供大量开源模型，您可以将其作为自定义 endpoint 添加到 LibreChat 中。

## 获取 API key [#get-an-api-key]

在 [api.together.xyz/settings/api-keys](https://api.together.xyz/settings/api-keys) 创建一个密钥。将其添加到你的 `.env` 文件中：
...
上述模型列表最后更新于 2024 年 8 月 1 日。

## 注意事项 [#notes]

- 不支持获取模型列表，因此 `fetch` 被设置为 `false`，你需要自行维护 `default` 数组。
- together.ai 的目录更新频繁。请使用下方的脚本来拉取当前的聊天模型，而不是手动编辑列表。

<Callout type="tip" title="获取并排序模型" collapsible>
此 Python 脚本用于获取可用的聊天模型，并将其排序后写入 `models_togetherai.json`，格式便于粘贴到 yaml 配置中。请先设置您的 API key。
