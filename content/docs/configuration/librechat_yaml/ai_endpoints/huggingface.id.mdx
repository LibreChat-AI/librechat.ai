...
title: Huggingface
icon: HuggingFace
description: Konfigurasikan Huggingface sebagai endpoint kustom di LibreChat.
---
...
Buat token di [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens). Tambahkan ke file `.env` Anda:
...
- `dropParams: ["top_p"]` diperlukan. Tanpanya, permintaan akan gagal karena Huggingface menolak parameter `top_p`. Lihat [`dropParams`](/docs/configuration/librechat_yaml/object_structure/custom_endpoint#dropparams).

<Callout type="warning" title="Kesalahan Model Lainnya" collapsible>
    Berikut adalah daftar model lain yang telah diuji beserta kesalahan yang terkait
