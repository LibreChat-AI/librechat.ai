...
## クリーンな例 [#clean-example]

<Callout type="example" title="例" collapsible>

この設定例には、ドキュメント化されているすべての endpoint が含まれています（Azure、LiteLLM、MLX、および Ollama を除く。これらにはすべて追加の設定が必要です）。
...
## コメント付きの例 [#example-with-comments]

この設定ファイルの例では、LibreChatをいくつかの主要な領域にわたる詳細なオプションでセットアップしています。

- **Caching**: パフォーマンス向上のために有効化されています。
- **ファイル処理**:
  - **File Strategy**: コメントアウトされていますが、ファイルストレージに Firebase を統合する可能性を示唆しています。
  - **File Configurations**: さまざまな endpoint のファイルアップロード制限および許可される MIME タイプをカスタマイズします。これには、グローバルなサーバーファイルサイズ制限や、ユーザーアバター画像専用の制限が含まれます。
- **レート制限**: 指定された時間枠内において、IPおよびユーザーごとに許可される最大ファイルアップロード数のしきい値を定義し、不正利用を防止することを目的としています。
- **登録**:
  - 指定されたソーシャルログインプロバイダーおよびメールドメインからの登録を許可し、セキュリティとユーザー管理を強化します。
- **エンドポイント**:
  - **Assistants**: Assistants エンドポイントのポーリング間隔と操作のタイムアウトを設定し、ビルダーインターフェースを無効にするオプションを提供します。
  - **カスタムエンドポイント**:
    - MistralとOpenRouterという2つの外部AIサービスエンドポイントを設定します。これには、APIキー、ベースURL、モデルの処理、および会話タイトル、要約、パラメータ調整といった特定の機能の切り替えが含まれます。
    - Mistralの場合、動的なモデル取得を有効にし、安全なプロンプトのための追加パラメータを適用し、サポートされていないパラメータを明示的に除外します。
    - OpenRouterの場合、動的なモデル取得を行わない基本的な設定を行い、会話タイトルの生成に使用するモデルを指定します。

<Callout type="example" title="コメント付きの例" collapsible>
