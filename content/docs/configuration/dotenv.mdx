...
<OptionTable
  options={[
    ...
    [
      ...
      'string',
      'Dedicated Code Interpreter base URL for highly experimental stateful Agent sessions. The service must advertise the stateful profile. Stateful requests fail closed when this is unset.',
...
[`stateful_code_sessions`](/docs/configuration/librechat_yaml/object_structure/agents#capabilities) is highly experimental and requires a separate Code Interpreter deployment configured with `LIBRECHAT_CODE_BASEURL_STATEFUL`. The endpoint must run the `stateful` profile; LibreChat sends `X-CodeAPI-Expected-Profile: stateful` with stateful requests.

Stateful requests never fall back to `LIBRECHAT_CODE_BASEURL`. If the dedicated URL is missing, LibreChat stops the Agent run with a configuration error. Stateless agents continue using `LIBRECHAT_CODE_BASEURL`, and the two execution profiles do not share a live workspace.
...
Scheduled Chats are experimental and disabled until [`interface.schedules`](/docs/configuration/librechat_yaml/object_structure/interface#schedules) is configured. Multi-replica deployments require `USE_REDIS_STREAMS=true`; a deployment without shared Redis streams fails schedule writes closed unless it explicitly declares a truly single-process topology with `SCHEDULES_SINGLE_PROCESS=true`. See [Scheduled Chats](/docs/features/scheduled_chats) for setup and runtime behavior.
...
<OptionTable
  options={[
    [
      ...
      'string',
      'Base URL used by the trusted Agent trigger execution host to re-enter LibreChat fire, continue, and steer admission. Defaults to the current process bound listener.',
      '# AGENT_TRIGGERS_SELF_URL=http://127.0.0.1:3080',
    ],
    [
      'ENABLE_SUBAGENT_COMPLETION_WAKEUPS',
      'boolean',
      'Automatically continue a saved parent Agent after a detached child settles. Enable only after every API replica has been upgraded. Default: false.',
      'ENABLE_SUBAGENT_COMPLETION_WAKEUPS=false',
    ],
  ]}
/>

`ENABLE_SUBAGENT_COMPLETION_WAKEUPS=true` lets a detached child result start a new turn on its saved parent Agent and exact initiating branch. The result is persisted before delivery, and duplicate completion events are idempotent. Deploy support with this setting disabled, wait until every API replica is upgraded, then enable it in a later rollout. Cancelled children and ephemeral parent Agents do not trigger a wakeup; manual result collection remains available when automatic delivery cannot run.

Leave `AGENT_TRIGGERS_SELF_URL` unset for the normal in-process path. Set it only when trusted internal trigger admission must traverse another HTTP origin, such as a TLS front door. The value must be an HTTP or HTTPS URL without embedded credentials. It does not expose or configure a public trigger endpoint.

Trusted source adapters enqueue through LibreChat's shared durable trigger service. MongoDB-backed ordering lanes and leases support workers across replicas; successful delivery records expire after 90 days, while dead letters remain until an explicit trusted requeue or removal. Source adapters are responsible for authentication, authorization, target selection, and payload sanitization. See [Trusted Agent Trigger Delivery](/docs/features/agents#trusted-agent-trigger-delivery) for delivery semantics and current boundaries.
...
- For Redis-backed generation streams, first deploy the v2-capable release to every replica with `GENERATION_PROTOCOL_VERSION` unset (or `1`). After old active generations drain, set it to `2` in a second rollout. Do not roll back to a pre-v2 image while v2 generations remain active. See [Generation Protocol Rollout](/docs/configuration/redis#generation-protocol-rollout).
