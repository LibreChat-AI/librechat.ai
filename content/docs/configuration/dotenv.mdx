...
<OptionTable
  options={[
    [
      'MONGO_AUTO_INDEX',
      'boolean',
      'Set to false to disable automatic index creation for all models associated with this connection. When omitted, empty, or whitespace-only, uses Mongoose default behavior.',
      '# MONGO_AUTO_INDEX=',
    ],
    [
      'MONGO_AUTO_CREATE',
      'boolean',
      'Set to false to disable Mongoose automatically calling createCollection() on every model created on this connection. When omitted, empty, or whitespace-only, uses Mongoose default behavior.',
      '# MONGO_AUTO_CREATE=',
    ],
  ]}
/>
...
- To report issues, reproduce the error and submit logs from `./api/logs/debug-%DATE%.log` at: **[LibreChat GitHub Issues](https://github.com/LibreChat-AI/LibreChat/issues)**
...
- `CONSOLE_JSON`: When handling console logs in cloud deployments (such as GCP or AWS), enabling this will dump the logs with a UTC timestamp and format them as JSON.
  - See: [feat: Add CONSOLE_JSON](https://github.com/LibreChat-AI/LibreChat/pull/2146)
...
LibreChat can emit backend OpenTelemetry traces for general API, HTTP, MongoDB, Mongoose, Redis, and outbound request visibility, and can export its Winston application logs as correlated OpenTelemetry log records. Traces and logs are independently opt-in; enabling logs alone does not install tracing instrumentation. Use Langfuse for GenAI-specific prompt/model observability.
...
<OptionTable
  options={[
    ...
    [
      'OTEL_LOGS_ENABLED',
      'boolean',
      'Export application logs through OpenTelemetry. Logs can be enabled with or without tracing and remain disabled when OTEL_SDK_DISABLED=true.',
      '# OTEL_LOGS_ENABLED=false',
    ],
    [
      'OTEL_LOGS_LEVEL',
      'string',
      'Minimum Winston level exported as OpenTelemetry logs. Defaults to info; unknown levels warn and fall back to info.',
    ...
    [
      ...
      'string',
      'Default protocol for every OTLP signal: http/protobuf or grpc. Per-signal protocol variables override it.',
    ...
    [
      ...
      'string',
      'Log-specific OTLP endpoint. Overrides the base endpoint for logs when set.',
      '# OTEL_EXPORTER_OTLP_LOGS_ENDPOINT=',
    ],
    [
    ...
    ['OTEL_LOGS_EXPORTER', 'string', 'Log exporter selection.', '# OTEL_LOGS_EXPORTER=otlp'],
    [
      'OTEL_TRACES_SAMPLER',
      'string',
...
<OptionTable
  options={[
    ...
    [
      ...
      'string',
      'Optional server-only Authorization header forwarded by authenticated proxy mode. Provide the complete header value expected by the collector.',
      '# RUM_PROXY_AUTHORIZATION=',
    ],
    [
      'RUM_PROXY_TIMEOUT_MS',
...
<OptionTable
  options={[
    ...
    [
      ...
      'number',
      'Advertises immutable, conversation-owned Code-environment decisions. Enable protocol version 1 only after every API replica supports it.',
...
<OptionTable
  options={[
    ...
    ['SHARE_IP_MAX', 'number', 'Maximum shared-link retrievals per IP during the IP window. Default: 100.', '# SHARE_IP_MAX=100'],
    ['SHARE_IP_WINDOW', 'number', 'Shared-link IP rate-limit window in minutes. Default: 1.', '# SHARE_IP_WINDOW=1'],
    ['SHARE_USER_MAX', 'number', 'Maximum shared-link retrievals per authenticated user during the user window. Default: 60.', '# SHARE_USER_MAX=60'],
    ['SHARE_USER_WINDOW', 'number', 'Shared-link user rate-limit window in minutes. Default: 1.', '# SHARE_USER_WINDOW=1'],
    ['SHARE_VIOLATION_SCORE', 'number', 'Violation score added when a shared-link retrieval limit is exceeded. Default: 0.', '# SHARE_VIOLATION_SCORE=0'],
  ]}
/>
...
Every shared-link read uses the IP bucket because public links are reachable without authentication. Signed-in viewers also use the user bucket. Exceeding either limit returns HTTP 429; a nonzero `SHARE_VIOLATION_SCORE` records the event in the existing violation system.
...
<OptionTable
  options={[
    ...
    [
      'REDIS_PING_TIMEOUT',
      'number',
      'Milliseconds an enabled heartbeat PING may remain unanswered before LibreChat reconnects the socket. Default: 5000.',
      '# REDIS_PING_TIMEOUT=5000',
    ],
    [
      'REDIS_SUBSCRIBER_PING_INTERVAL',
      'number',
      'Heartbeat interval in seconds for dedicated pub/sub subscriber connections. Default: 15; set to 0 to disable.',
      '# REDIS_SUBSCRIBER_PING_INTERVAL=15',
    ],
    [
      'REDIS_KEEP_ALIVE',
      'number',
      'TCP keepalive idle delay in milliseconds for ioredis sockets. Default: 10000; set to 0 for the kernel default.',
...
- `REDIS_PING_INTERVAL` controls the optional heartbeat on the main ioredis client; dedicated pub/sub subscribers use `REDIS_SUBSCRIBER_PING_INTERVAL` so an otherwise idle dead socket is detected. Both use `REDIS_PING_TIMEOUT` as the response deadline and reconnect when it expires.
