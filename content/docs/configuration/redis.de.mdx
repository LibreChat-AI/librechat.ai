...
Die Einstellung ist deaktiviert, wenn sie nicht festgelegt oder auf `0` gesetzt ist. Es werden `25` Millisekunden empfohlen, und Werte über `1000` werden begrenzt. Batching kann zu einer konfigurierten Zustellungsverzögerung führen.

<Callout type="warning" title="Sicherheit bei Rolling Deployments">
  Aktiviere Coalescing erst, nachdem jede LibreChat-Replik Batch-Frames unterstützt. Ältere Abonnenten verwerfen zusammengefasste (coalesced) Frames.
</Callout>
