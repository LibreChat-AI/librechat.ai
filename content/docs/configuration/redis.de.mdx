...
### Einführung des Generation Protocol [#generation-protocol-rollout]

Das v2-Generierungsprotokoll fügt die dauerhafte Warteschlange, Unterbrechung, Pause/Fortsetzen sowie Wiederherstellungsgarantien hinzu, die von aktuellen Agent-Ausführungen verwendet werden. Redis-basierte Bereitstellungen verbleiben absichtlich auf Protokoll v1, wenn `GENERATION_PROTOCOL_VERSION` nicht gesetzt ist, damit Replikate, die unterschiedliche LibreChat-Versionen ausführen, während eines Rolling-Upgrades koexistieren können.

Verwenden Sie eine zweistufige Einführung:

1. Stellen Sie das v2-fähige LibreChat-Image auf **jeder** Replik bereit, bei der `GENERATION_PROTOCOL_VERSION` nicht gesetzt oder auf `1` gesetzt ist.
2. Warten Sie, bis aktive Generierungen, die vom vorherigen Image stammen, abgeschlossen oder beendet sind.
3. Setze `GENERATION_PROTOCOL_VERSION=2` und rolle dasselbe Image erneut aus.
...
<Callout type="warning" title="Sicherheit bei Rolling Deployments">
  Aktivieren Sie das Protokoll v2 erst, wenn jedes Replikat es unterstützt. Nachdem die v2-Generierungen begonnen haben, führen Sie kein Rollback auf ein LibreChat-Image vor v2 durch, bis diese Generierungen abgeschlossen sind; ältere Replikate können nicht sicher mit ihrem Redis-Status arbeiten.
</Callout>

In-Memory-Generierungs-Streams teilen keinen Status zwischen Replikaten und verwenden standardmäßig das Protokoll v2, daher ist diese Umstellung nicht erforderlich.
