...
- [Déploiement du protocole de génération](#generation-protocol-rollout)
...
Le protocole de génération v2 ajoute la file d'attente durable, l'interruption, la pause/reprise et les garanties de récupération utilisées par les exécutions d'Agent actuelles. Les déploiements basés sur Redis restent intentionnellement sur le protocole v1 lorsque `GENERATION_PROTOCOL_VERSION` n'est pas défini, afin que les répliques exécutant différentes versions de LibreChat puissent coexister lors d'une mise à jour progressive.

Utilisez un déploiement en deux étapes :

1. Déployez l'image LibreChat compatible v2 sur **chaque** réplique avec `GENERATION_PROTOCOL_VERSION` non défini ou défini sur `1`.
2. Attendez que les générations actives appartenant à l'image précédente se terminent ou soient vidées.
3. Définissez `GENERATION_PROTOCOL_VERSION=2` et déployez à nouveau la même image.
...
<Callout type="warning" title="Sécurité du déploiement progressif">
  N'activez pas le protocole v2 tant que chaque réplica ne le prend pas en charge. Une fois que les générations v2 ont commencé, ne revenez pas à une image LibreChat antérieure à la v2 tant que ces générations n'ont pas été purgées ; les réplicas plus anciens ne peuvent pas fonctionner en toute sécurité avec leur état Redis.
</Callout>

Les flux de génération en mémoire ne partagent pas d'état entre les réplicas et utilisent le protocole v2 par défaut, ils ne nécessitent donc pas cette transition.
