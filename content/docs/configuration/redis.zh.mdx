...
- [生成协议发布](#generation-protocol-rollout)
...
v2 生成协议增加了当前 Agent 运行所使用的持久队列、中断、暂停/恢复以及恢复保证。当 `GENERATION_PROTOCOL_VERSION` 未设置时，基于 Redis 的部署将有意保留在协议 v1 上，以便在滚动升级期间运行不同 LibreChat 版本的副本可以共存。

使用两阶段发布：

1. 将支持 v2 的 LibreChat 镜像部署到 `GENERATION_PROTOCOL_VERSION` 未设置或设置为 `1` 的**每一个**副本中。
2. 等待由上一个镜像拥有的活跃生成任务完成或排空。
3. 设置 `GENERATION_PROTOCOL_VERSION=2` 并再次部署相同的镜像。
...
<Callout type="warning" title="滚动部署安全性">
  在所有副本都支持协议 v2 之前，请勿启用协议 v2。在 v2 生成开始后，请勿回滚到 v2 之前的 LibreChat 镜像，直到这些生成任务处理完毕；旧版本的副本无法安全地操作其 Redis 状态。
</Callout>

内存中生成流不会在副本之间共享状态，且默认使用协议 v2，因此不需要进行此切换。
