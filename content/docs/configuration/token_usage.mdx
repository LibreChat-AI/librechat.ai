...
title: Token Usage
icon: Coins
description: This covers how to track and control token usage in LibreChat. You will learn how to view context and cost, configure transactions, enable user balances, and add credits to an account.
---
...
- A click-through breakdown for dialogue, retained tool results, Agent instructions, cached input, completions, branch totals, and conversation totals
- Per-tool disclosures and a runway estimate when compatible snapshots can project remaining context pressure
...
Tool traffic, cache, and Agent instructions are subsets of their containing categories and do not increase the total a second time. The runway estimate appears only when consecutive snapshots share the same model, provider, Agent, context budget, and fixed overhead. Usage breakdowns are persisted with messages and conversations, including the final primary call's cache split and completed output. Reopened chats retain branch and total usage/cost details instead of relying only on the active streaming session; Subagent totals are explicitly labeled as covering all branches.
