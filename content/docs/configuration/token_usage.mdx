...
For OpenAI-compatible custom endpoints, define model-specific context windows and per-million-token rates with `endpoints.custom[].tokenConfig`:
...
For custom OpenAI-compatible endpoints, prefer `endpoints.custom[].tokenConfig` in `librechat.yaml` for per-model rates and context windows.
