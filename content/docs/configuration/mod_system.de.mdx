...
Die folgenden Umgebungsvariablen sind alle relevanten Variablen, um das Mod-System zu nutzen und zu konfigurieren. Beachten Sie, dass diese auch in der Datei [/.env.example](https://github.com/danny-avila/LibreChat/blob/main/.env.example) zu finden sind und in Ihrer eigenen `.env` Datei gesetzt werden müssen.
...
<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', 'Ob Benutzer bei Verstößen gesperrt werden sollen (sie werden weiterhin protokolliert).','BAN_VIOLATIONS=true'],
...
<OptionTable
  options={[    
    ...
    ['STT_VIOLATION_SCORE', 'integer', 'Bewertung für Verstöße bei der Spracherkennung.','STT_VIOLATION_SCORE=0'],
...
<OptionTable
  options={[    
    ['LOGIN_MAX', 'number', 'Die maximale Anzahl an erlaubten Anmeldungen pro IP innerhalb eines LOGIN_WINDOW. Standardwert ist `7`.'],
...
#### Import-Konversations-Ratenbegrenzung (pro Benutzer) [#import-conversation-rate-limiting-per-user]
...
#### Ratenbegrenzung für das Forken von Unterhaltungen (pro IP) [#conversation-forking-rate-limiting-per-ip]
...
Beachte, dass dies möglicherweise nicht mit allen Reverse-Proxys funktioniert:
