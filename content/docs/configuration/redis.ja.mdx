...
期限付きのRedisハートビートとTCPキープアライブを設定します：
...
- `REDIS_PING_INTERVAL` は秒単位で指定され、オプションのメイン ioredis ハートビートを制御します。
- 専用のpub/subサブスクライバーは、アイドル状態のデッドソケットが検出されないままになるのを防ぐため、独自のデフォルトオンの間隔を使用します。このハートビートを無効にするには、`REDIS_SUBSCRIBER_PING_INTERVAL=0` を設定してください。
- 有効化されたすべてのハートビートは、応答の期限として `REDIS_PING_TIMEOUT` を使用し、PING が応答されない場合にソケットを再接続します。
- `REDIS_KEEP_ALIVE` は、アプリケーションレベルの PING とは独立して ioredis の TCP ソケットを設定します。
...
- メインクライアントのPINGは `REDIS_PING_INTERVAL` によって制御されており、デフォルトでは無効になっています。
- 専用のサブスクライバーPINGは `REDIS_SUBSCRIBER_PING_INTERVAL` によって制御され、デフォルトは15秒です。
- Heartbeatsは `REDIS_PING_TIMEOUT` を使用して、応答のないデッドソケットを検出し、強制的に再接続を行います。
- TCP keepaliveは、アプリケーションレベルのハートビートとは独立して `REDIS_KEEP_ALIVE` を使用します。
- 切断またはクローズイベントが発生すると、Heartbeatタイマーはクリアされます。
...
設定が未指定の場合、デフォルト値は `25` ミリ秒です。結合（coalescing）を無効にするには `0` を明示的に設定してください。`1000` を超える値は上限値として扱われます。バッチ処理は設定された配信レイテンシのウィンドウ分だけ遅延を追加する可能性があり、フラッシュされる前にパブリッシュプロセスが終了すると、バッファリングされた末尾のデータが失われる可能性があります。個々のチャンクフレームはプロトコルの互換性のために保持されるため、この設定はサブスクライバーのフレーム形式を変更することなく、Redisスクリプトとラウンドトリップの負荷を軽減します。インメモリ生成ストリームには影響しません。
