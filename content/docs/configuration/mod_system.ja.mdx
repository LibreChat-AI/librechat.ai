...
自動モデレーションシステムは、ユーザーの違反を追跡するためにスコアリングメカニズムを使用します。ユーザーが過度なログイン、登録、メッセージ送信などの行為を行うと、違反スコアが蓄積されます。設定されたしきい値に達すると、そのユーザーとIPアドレスは一時的に禁止されます。このシステムは、急速な活動や疑わしい活動を監視およびペナルティを科すことで、プラットフォームのセキュリティを確保します。
...
<OptionTable
  options={[    
    ...
    ['CONCURRENT_MESSAGE_MAX', 'integer', '1リクエストあたりにユーザーが送信できる最大メッセージ数。','CONCURRENT_MESSAGE_MAX=2'],
  ]}
/>
...
<OptionTable
  options={[    
    ['LIMIT_FORK_USER', 'boolean', 'ユーザーが `FORK_USER_WINDOW` ごとに作成できる会話フォークの数を制限するかどうか。','LIMIT_FORK_USER=false'],
