...
本番環境では、サーバーを過剰なリクエストから確実に保護するために、Cloudflareやその他のDDoS保護を導入する必要があります。しかし、これらの変更を行うことで、デプロイされたインスタンスをプロキシとして悪用しようとする単一または少数の悪意ある攻撃者から、大部分を守ることができます。
...
<OptionTable
  options={[    
    ...
    ['IMPORT_VIOLATION_SCORE', 'integer', 'インポートした会話の違反に対するスコア。','IMPORT_VIOLATION_SCORE=1'],
...
<OptionTable
  options={[    
    ['LOGIN_MAX', 'number', 'LOGIN_WINDOW ごとの IP あたりの最大ログイン許可回数。デフォルトは `7` です。'],
    ...
    ['REGISTER_WINDOW', 'number', 'REGISTER_MAXの登録回数を判定する時間枠を分単位で指定します。デフォルトは `60` です。'],
  ]}
/>
...
<OptionTable
  options={[    
    ['LIMIT_MESSAGE_IP', 'boolean', 'IPが `MESSAGE_IP_WINDOW` ごとに送信できるメッセージ数を制限するかどうか。','LIMIT_MESSAGE_IP=true'],
...
#### 会話フォークのレート制限（IPごと） [#conversation-forking-rate-limiting-per-ip]
...
> 注: 不正なモデルリクエストは、サードパーティが自動化されたスクリプトを通じてサーバーにアクセスしようとしていることを意味するため、ほぼ例外なく悪意のあるものです。そのため、5以上の比較的高いスコアを設定することを推奨します。
...
<OptionTable
  options={[    
    ...
    ['OPENAI_MODERATION_API_KEY', 'string', 'あなたのOpenAI APIキー。','OPENAI_MODERATION_API_KEY='],
  ]}
/>
...
<OptionTable
  options={[    
    ['OPENAI_MODERATION_REVERSE_PROXY', 'string', '注: デフォルトではコメントアウトされています。これはすべてのリバースプロキシで動作するわけではありません。','# OPENAI_MODERATION_REVERSE_PROXY='],
  ]}
/>
