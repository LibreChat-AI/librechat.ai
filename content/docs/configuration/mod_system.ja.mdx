...
<OptionTable
  options={[
    ...
    ['VIOLATION_SCORE_TTL', 'integer', '新しい違反が発生せずに違反スコアが保持される期間（ミリ秒単位）。新しい違反が発生するたびにカウントダウンがリセットされるため、スコアは累積し続けるのではなく、一定期間の静穏後に減衰します。0に設定すると、スコアは期限切れになりません（レガシーな動作）。','VIOLATION_SCORE_TTL=1000 * 60 * 60'],
  ]}
/>
