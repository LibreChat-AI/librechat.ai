...
自動モデレーションシステムは、ユーザーの違反を追跡するためにスコアリングメカニズムを使用します。ユーザーが過度なログイン、登録、メッセージ送信などの行為を行うと、違反スコアが蓄積されます。設定されたしきい値に達すると、そのユーザーおよびIPアドレスは一時的にBAN（禁止）されます。このシステムは、急速な活動や疑わしい活動を監視およびペナルティを科すことで、プラットフォームのセキュリティを確保します。
...
<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', '違反に対するユーザーの禁止を有効にするかどうか（禁止されたユーザーもログには記録されます）。','BAN_VIOLATIONS=true'],
...
<OptionTable
  options={[    
    ...
    ['VERIFY_EMAIL_VIOLATION_SCORE', 'integer', 'メール認証違反に対するスコア。','VERIFY_EMAIL_VIOLATION_SCORE=0'],
...
<OptionTable
  options={[    
    ...
    ['IMPORT_USER_WINDOW', 'integer', '`IMPORT_USER_MAX` インポートの対象となる時間枠を分単位で指定します。','IMPORT_USER_WINDOW=1'],
  ]}
/>
