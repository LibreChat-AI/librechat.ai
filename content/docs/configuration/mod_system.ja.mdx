...
自動モデレーションシステムは、ユーザーの違反を追跡するためにスコアリングメカニズムを使用します。ユーザーが過度なログイン、登録、メッセージ送信などの行為を行うと、違反スコアが蓄積されます。設定されたしきい値に達すると、そのユーザーとIPアドレスは一時的にBAN（禁止）されます。このシステムは、急速な活動や疑わしい活動を監視およびペナルティを課すことで、プラットフォームのセキュリティを確保します。
...
<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', '違反に対するユーザーの禁止を有効にするかどうか（禁止した場合でもログには記録されます）。','BAN_VIOLATIONS=true'],
