...
En production, vous devriez disposer de Cloudflare ou d'une autre protection DDoS pour protéger réellement le serveur contre les requêtes excessives, mais ces changements vous protégeront en grande partie contre un ou plusieurs acteurs malveillants ciblant votre instance déployée pour le proxying.
...
<OptionTable
  options={[
    ...
    ['BAN_INTERVAL', 'integer', 'L\'utilisateur sera banni chaque fois que son score atteint ou dépasse le seuil de l\'intervalle.','BAN_INTERVAL=20'],
  ]}
/>
...
<OptionTable
  options={[    
    ...
    ['IMPORT_USER_WINDOW', 'integer', 'Détermine en minutes la fenêtre temporelle pour les importations `IMPORT_USER_MAX`.','IMPORT_USER_WINDOW=1'],
  ]}
/>
...
<OptionTable
  options={[    
    ['LIMIT_FORK_IP', 'boolean', 'Indique s\'il faut limiter le nombre de ramifications de conversation qu\'une IP peut créer par `FORK_IP_WINDOW`.','LIMIT_FORK_IP=true'],
    ...
    ['FORK_IP_WINDOW', 'integer', 'En minutes, détermine la fenêtre de temps pour les forks `FORK_IP_MAX`.','FORK_IP_WINDOW=1'],
  ]}
/>
...
<OptionTable
  options={[    
    ['ILLEGAL_MODEL_REQ_SCORE', 'integer', 'Score pour les requêtes de modèles illégales.','ILLEGAL_MODEL_REQ_SCORE=5'],
  ]}
/>
