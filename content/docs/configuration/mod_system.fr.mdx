...
Le système de modération automatisé utilise un mécanisme de notation pour suivre les violations des utilisateurs. Lorsque les utilisateurs effectuent des actions telles que des connexions, des inscriptions ou des envois de messages excessifs, ils accumulent des scores de violation. Une fois qu'un seuil défini est atteint, l'utilisateur et son adresse IP sont temporairement bannis. Ce système garantit la sécurité de la plateforme en surveillant et en pénalisant les activités rapides ou suspectes.

En production, vous devriez mettre en place Cloudflare ou une autre protection DDoS pour réellement protéger le serveur contre les requêtes excessives, mais ces changements vous protégeront en grande partie contre un ou plusieurs acteurs malveillants ciblant votre instance déployée pour faire du proxying.

**Pour plus de détails, consultez le guide de l'utilisateur fourni ici : [Modération automatisée](/docs/features/mod_system)**
...
**Note :** actuellement, la plupart de ces valeurs sont configurées via le fichier .env, mais elles pourraient bientôt migrer pour être exclusivement configurées à partir du [fichier de configuration `librechat.yaml`](/docs/configuration/librechat_yaml/object_structure/config#ratelimits).

### Violation, Interval, Duration [#violation-interval-duration]

<OptionTable
  options={[
    ['BAN_VIOLATIONS', 'boolean', 'Indique si le bannissement des utilisateurs en cas d\'infraction doit être activé ou non (ils seront toujours enregistrés).','BAN_VIOLATIONS=true'],
    ['BAN_DURATION', 'integer', 'Durée du bannissement de l\'utilisateur et de l\'adresse IP associée (en millisecondes).','BAN_DURATION=1000 * 60 * 60 * 2'],
...
#### Limitation du débit de messages (par IP) [#message-rate-limiting-per-ip]
...
<OptionTable
  options={[    
    ...
    ['MESSAGE_USER_WINDOW', 'integer', 'En minutes, détermine la fenêtre temporelle pour les messages `MESSAGE_USER_MAX`.','MESSAGE_USER_WINDOW=1'],
  ]}
/>

### Limitation du taux d'importation de conversations [#import-conversation-rate-limiting]

Limite la fréquence à laquelle les utilisateurs peuvent importer des conversations afin de prévenir les abus.

> Remarque : Vous pouvez utiliser les deux limiteurs, mais la valeur par défaut est de limiter uniquement par IP.

#### Limitation du taux d'importation de conversations (par IP) [#import-conversation-rate-limiting-per-ip]
...
<OptionTable
  options={[    
    ['OPENAI_MODERATION', 'boolean', 'Indique si la modération OpenAI doit être activée ou non sur les endpoints **OpenAI** et **Plugins**.','OPENAI_MODERATION=false'],
...
<OptionTable
  options={[    
    ['OPENAI_MODERATION_REVERSE_PROXY', 'string', 'Note : Commenté par défaut, cela ne fonctionne pas avec tous les reverse proxys.','# OPENAI_MODERATION_REVERSE_PROXY='],
  ]}
/>
