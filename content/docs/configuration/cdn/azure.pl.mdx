...
### Co z tego korzysta [#what-uses-this]

Ustawienie strategii plików zmienia miejsce, w którym LibreChat przechowuje wszystkie pliki, zamiast włączać konkretną funkcję. Dotyczy to awatarów użytkowników i agentów, plików przesłanych na czacie, obrazów zwracanych przez narzędzia do generowania obrazów oraz plików utworzonych przez Code Interpreter.

Domyślna strategia zapisuje wszystko w systemie plików kontenera API. Standardowe wdrożenie Docker Compose montuje dowiązania (bind-mount) `./images` oraz `./uploads` do hosta, więc w konfiguracji compose z pojedynczą instancją pliki te przetrwają ponowne tworzenie kontenera. Lokalna pamięć masowa nie pozwala jednak na współdzielenie plików między instancjami i traci je przy każdym wdrożeniu, które pozostawia te ścieżki w warstwie zapisu kontenera, takim jak Kubernetes bez trwałego wolumenu (persistent volume). Skonfiguruj Azure Blob Storage (lub innego dostawcę w sekcji [CDN](/docs/configuration/cdn)), gdy uruchamiasz więcej niż jedną instancję lub gdy w tych ścieżkach nie zamontowano żadnej trwałej pamięci.
