...
### Những gì sử dụng tính năng này [#what-uses-this]

Việc thiết lập một file strategy sẽ thay đổi vị trí LibreChat lưu trữ mọi tệp tin, thay vì chỉ kích hoạt một tính năng cụ thể. Điều này bao gồm ảnh đại diện của người dùng và tác nhân (agent), các tệp được tải lên trong cuộc trò chuyện, hình ảnh được trả về bởi các công cụ tạo ảnh và các tệp được tạo ra bởi Code Interpreter.

Chiến lược mặc định ghi tất cả dữ liệu đó vào hệ thống tệp của container API. Việc triển khai Docker Compose tiêu chuẩn sẽ bind-mount `./images` và `./uploads` vào máy chủ, vì vậy trên thiết lập compose đơn instance, các tệp đó đã được giữ lại khi tạo lại container. Điều mà lưu trữ cục bộ không thể làm được là chia sẻ tệp giữa các instance, và nó sẽ làm mất dữ liệu trên bất kỳ quá trình triển khai nào để các đường dẫn đó nằm trên lớp có thể ghi của container, chẳng hạn như Kubernetes mà không có persistent volume. Hãy cấu hình Azure Blob Storage (hoặc một nhà cung cấp khác trong mục [CDN](/docs/configuration/cdn)) khi bạn chạy nhiều hơn một instance, hoặc khi không có bộ nhớ bền vững nào được gắn vào các đường dẫn đó.
