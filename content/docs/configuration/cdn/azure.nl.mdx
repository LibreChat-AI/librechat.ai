...
### Wat gebruikt dit [#what-uses-this]

Het instellen van een bestandsstrategie verandert de locatie waar LibreChat elk bestand opslaat, in plaats van een specifieke functie in te schakelen. Dit omvat gebruikers- en agentavatars, bestanden die in chats zijn geüpload, afbeeldingen die door de tools voor het genereren van afbeeldingen zijn geretourneerd en bestanden die door de Code Interpreter zijn geproduceerd.

De standaardstrategie schrijft dit alles naar het bestandssysteem van de API-container. De standaard Docker Compose-implementatie bind-mount `./images` en `./uploads` naar de host, dus bij een opstelling met één instantie blijven die bestanden al behouden bij het opnieuw aanmaken van de container. Wat lokale opslag niet kan, is bestanden delen tussen instanties, en ze gaan verloren bij elke implementatie die die paden op de beschrijfbare laag van de container laat staan, zoals Kubernetes zonder persistent volume. Configureer Azure Blob Storage (of een andere provider onder [CDN](/docs/configuration/cdn)) wanneer u meer dan één instantie draait, of wanneer er niets persistents is gekoppeld achter die paden.
