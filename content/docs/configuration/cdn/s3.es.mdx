...
- **S3_URL_EXPIRY_SECONDS:** (Opcional) Tiempo de vida de cada URL prefirmada, en segundos. Consulta la nota sobre las URLs prefirmadas a continuación para conocer los límites del lado del proveedor que se aplican.
- **S3_REFRESH_EXPIRY_MS:** (Opcional) Regenera una URL prefirmada una vez que alcanza esta antigüedad, en milisegundos, en lugar de utilizar la lógica de búfer de caducidad predeterminada. Desactivado por defecto. Cualquier valor que no sea un número entero positivo será ignorado y se mostrará una advertencia en los registros.
...
Con estos pasos, su aplicación LibreChat utilizará Amazon S3 para gestionar la carga, descarga y eliminación de archivos. Además, con el soporte de IRSA, su aplicación puede ejecutarse de forma segura en Kubernetes sin necesidad de integrar credenciales de AWS a largo plazo.
