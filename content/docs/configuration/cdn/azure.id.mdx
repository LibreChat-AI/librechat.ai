...
### Apa yang menggunakan ini [#what-uses-this]

Mengatur strategi file akan mengubah lokasi penyimpanan setiap file oleh LibreChat, alih-alih mengaktifkan fitur tertentu. Hal ini mencakup avatar pengguna dan agen, file yang diunggah dalam obrolan, gambar yang dihasilkan oleh alat pembuat gambar, serta file yang dihasilkan oleh Code Interpreter.

Strategi default menulis semua itu ke sistem berkas container API. Deployment Docker Compose standar melakukan bind-mount pada `./images` dan `./uploads` ke host, sehingga pada pengaturan compose instans tunggal, berkas-berkas tersebut sudah bertahan saat container dibuat ulang. Yang tidak dapat dilakukan oleh penyimpanan lokal adalah berbagi berkas antar instans, dan berkas tersebut akan hilang pada deployment apa pun yang membiarkan jalur tersebut berada di lapisan yang dapat ditulis (writable layer) pada container, seperti Kubernetes tanpa persistent volume. Konfigurasikan Azure Blob Storage (atau penyedia lain di bawah [CDN](/docs/configuration/cdn)) saat Anda menjalankan lebih dari satu instans, atau saat tidak ada penyimpanan persisten yang di-mount di balik jalur tersebut.
