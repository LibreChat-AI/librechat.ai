...
- **S3_URL_EXPIRY_SECONDS:** (İsteğe bağlı) Her bir önceden imzalanmış URL'nin saniye cinsinden ömrü. Uygulanan sağlayıcı tarafı sınırları için aşağıdaki önceden imzalanmış URL'ler ile ilgili nota bakın.
- **S3_REFRESH_EXPIRY_MS:** (İsteğe bağlı) Varsayılan süre sonu arabelleği (expiry-buffer) mantığını kullanmak yerine, milisaniye cinsinden bu süreye ulaştığında önceden imzalanmış (presigned) bir URL'yi yeniden oluşturur. Varsayılan olarak ayarlanmamıştır. Pozitif bir tam sayı olmayan değerler, günlüklerde bir uyarı ile göz ardı edilir.
