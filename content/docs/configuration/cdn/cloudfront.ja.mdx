...
- 起動ログ `S3 must be initialized`: 最初に S3 環境変数を設定してください。
- 署名付きクッキーが設定されていません：`imageSigning: "cookies"`、`cookieDomain`、`CLOUDFRONT_KEY_PAIR_ID`、および `CLOUDFRONT_PRIVATE_KEY` を確認してください。
- ブラウザで画像が読み込めない場合：APIとCDNのホスト名が設定された親ドメインを共有していること、および `Secure` と `SameSite=None` でCookieが許可されていることを確認してください。
