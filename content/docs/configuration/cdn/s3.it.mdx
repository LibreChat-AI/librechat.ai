...
- **S3_URL_EXPIRY_SECONDS:** (Opzionale) Durata di ogni URL pre-firmato, in secondi. Consulta la nota sugli URL pre-firmati qui sotto per i limiti lato provider applicabili.
- **S3_REFRESH_EXPIRY_MS:** (Opzionale) Rigenera un URL pre-firmato una volta raggiunta questa età, in millisecondi, invece di utilizzare la logica predefinita di buffer di scadenza. Non impostato per impostazione predefinita. Un valore che non sia un numero intero positivo viene ignorato con un avviso nei log.
