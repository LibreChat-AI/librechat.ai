...
- **S3_URL_EXPIRY_SECONDS:** (Opcional) Tempo de vida de cada URL pré-assinada, em segundos. Consulte a nota sobre URLs pré-assinadas abaixo para os limites do lado do provedor que se aplicam.
- **S3_REFRESH_EXPIRY_MS:** (Opcional) Regenera uma URL pré-assinada assim que ela atinge esta idade, em milissegundos, em vez de usar a lógica de buffer de expiração padrão. Não definido por padrão. Um valor que não seja um número inteiro positivo é ignorado com um aviso nos logs.
