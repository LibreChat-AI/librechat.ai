...
### Was verwendet dies [#what-uses-this]

Das Festlegen einer Dateistrategie ändert den Speicherort aller von LibreChat gespeicherten Dateien, anstatt eine bestimmte Funktion zu aktivieren. Dies umfasst Benutzer- und Agenten-Avatare, in Chats hochgeladene Dateien, von Bildgenerierungstools erstellte Bilder sowie Dateien, die vom Code Interpreter produziert wurden.

Die Standardstrategie schreibt all dies in das Dateisystem des API-Containers. Die standardmäßige Docker Compose-Bereitstellung bindet `./images` und `./uploads` per Bind-Mount an den Host, sodass diese Dateien bei einer Single-Instance-Compose-Einrichtung bereits das Neuerstellen des Containers überdauern. Was lokaler Speicher nicht leisten kann, ist das Teilen von Dateien über Instanzen hinweg; zudem gehen sie bei jeder Bereitstellung verloren, die diese Pfade auf der beschreibbaren Ebene des Containers belässt, wie etwa bei Kubernetes ohne persistentes Volume. Konfigurieren Sie Azure Blob Storage (oder einen anderen Anbieter unter [CDN](/docs/configuration/cdn)), wenn Sie mehr als eine Instanz ausführen oder wenn hinter diesen Pfaden nichts Persistentes eingebunden ist.
