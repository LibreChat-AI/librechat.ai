...
- **S3_URL_EXPIRY_SECONDS:** (Optioneel) Levensduur van elke presigned URL, in seconden. Zie de onderstaande opmerking over presigned URLs voor de limieten aan de kant van de provider die van toepassing zijn.
- **S3_REFRESH_EXPIRY_MS:** (Optioneel) Genereer een presigned URL opnieuw zodra deze deze leeftijd bereikt, in milliseconden, in plaats van de standaard expiry-buffer logica te gebruiken. Standaard niet ingesteld. Een waarde die geen positief geheel getal is, wordt genegeerd met een waarschuwing in de logs.
