...
### Qu'est-ce qui utilise ceci [#what-uses-this]

Définir une stratégie de fichiers modifie l'emplacement où LibreChat stocke chaque fichier, plutôt que d'activer une fonctionnalité particulière. Cela couvre les avatars des utilisateurs et des agents, les fichiers téléchargés dans les conversations, les images renvoyées par les outils de génération d'images et les fichiers produits par le Code Interpreter.

La stratégie par défaut écrit tout cela dans le système de fichiers du conteneur de l'API. Le déploiement Docker Compose standard monte en liaison (bind-mount) `./images` et `./uploads` vers l'hôte ; ainsi, sur une configuration compose à instance unique, ces fichiers survivent déjà à la recréation du conteneur. Ce que le stockage local ne peut pas faire, c'est partager des fichiers entre plusieurs instances, et il les perd lors de tout déploiement qui laisse ces chemins sur la couche inscriptible du conteneur, comme Kubernetes sans volume persistant. Configurez Azure Blob Storage (ou un autre fournisseur sous [CDN](/docs/configuration/cdn)) lorsque vous exécutez plus d'une instance, ou lorsqu'aucun élément persistant n'est monté derrière ces chemins.
