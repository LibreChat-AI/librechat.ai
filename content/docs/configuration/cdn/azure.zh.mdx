...
### 哪些功能会用到它 [#what-uses-this]

设置文件策略会更改 LibreChat 存储所有文件的位置，而不是启用特定功能。这涵盖了用户和智能体头像、聊天中上传的文件、图像生成工具返回的图像，以及代码解释器生成的文件。

默认策略会将所有内容写入 API 容器的文件系统。标准的 Docker Compose 部署会将 `./images` 和 `./uploads` 绑定挂载（bind-mount）到宿主机，因此在单实例 compose 设置中，这些文件在重建容器后依然存在。本地存储无法做到的是跨实例共享文件，并且在任何将这些路径保留在容器可写层（writable layer）的部署中（例如没有持久卷的 Kubernetes），它都会丢失这些文件。当您运行多个实例，或者这些路径后没有挂载任何持久化存储时，请配置 Azure Blob Storage（或 [CDN](/docs/configuration/cdn) 下的其他提供商）。
...
如果您正在使用 VS Code 扩展，可以通过在下拉菜单中选择“Azurite Blob”来在 Output 面板中查看 Azurite 日志。

<Callout type="info" title="注意">
  默认的 Azurite 账户密钥是一个仅用于开发目的的固定值。切勿在生产环境中使用此密钥。请务必确保您的连接字符串保持安全，且永远不要将其提交到公共仓库中。
</Callout>
