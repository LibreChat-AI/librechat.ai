...
- `CLOUDFRONT_KEY_PAIR_ID` y `CLOUDFRONT_PRIVATE_KEY` al utilizar cookies firmadas o URLs de descarga firmadas
...
`CLOUDFRONT_PRIVATE_KEY` debe contener la clave privada PEM completa. En `.env`, póngala entre comillas y conserve los saltos de línea, o inyéctela desde el gestor de secretos de su plataforma.
...
Las cookies firmadas son el modo seguro para imágenes y avatares en línea privados. Permiten que LibreChat mantenga URLs de CloudFront estables en mensajes y registros, mientras autoriza el acceso con cookies de corta duración.
...
- `/a/...` activos de avatar, con alcance al inquilino cuando `tenantId` está presente
...
Para las anulaciones de nombre de archivo y tipo de contenido en descargas directas, configure la política de caché/solicitud de origen de CloudFront para reenviar estas cadenas de consulta a S3:
...
De forma predeterminada, LibreChat elimina el objeto S3 y no crea una invalidación de CloudFront. Habilite la invalidación cuando los archivos eliminados deban desaparecer de la caché perimetral de inmediato:
...
Esto solo afecta a las claves generadas recientemente. Los archivos existentes no se mueven. LibreChat no configura orígenes de CloudFront, Route 53 ni enrutamiento regional por usted.
...
<OptionTable
  options={[
    ...
    [
      'storageRegion',
      'string',
      'Etiqueta de región opcional para rutas de objetos que reconocen regiones.',
      'storageRegion: "us-east-2"',
    ],
    [
      'includeRegionInPath',
...
Utilice permisos IAM de privilegios mínimos para el bucket de S3. Añada el permiso de invalidación de CloudFront solo si `invalidateOnDelete` está habilitado.
...
- El navegador aún no puede cargar las imágenes: confirme que los nombres de host de la API y de la CDN compartan el dominio principal configurado y que las cookies estén permitidas con `Secure` y `SameSite=None`.
