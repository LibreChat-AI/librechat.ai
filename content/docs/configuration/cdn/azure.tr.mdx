...
### Bunu ne kullanır [#what-uses-this]

Bir dosya stratejisi belirlemek, belirli bir özelliği etkinleştirmek yerine LibreChat'in depoladığı her dosyayı nereye koyacağını değiştirir. Bu; kullanıcı ve aracı avatarlarını, sohbete yüklenen dosyaları, görsel oluşturma araçları tarafından döndürülen görselleri ve Code Interpreter tarafından üretilen dosyaları kapsar.

Varsayılan strateji, tüm bunları API container'ının dosya sistemine yazar. Standart Docker Compose dağıtımı, `./images` ve `./uploads` dizinlerini host'a bind-mount eder; bu nedenle tek örnekli (single-instance) bir compose kurulumunda bu dosyalar, container yeniden oluşturulduğunda zaten kaybolmaz. Yerel depolamanın yapamadığı şey, dosyaları örnekler arasında paylaşmaktır ve bu yolları container'ın yazılabilir katmanında bırakan herhangi bir dağıtımda (kalıcı bir volume olmayan Kubernetes gibi) dosyalar kaybolur. Birden fazla örnek çalıştırdığınızda veya bu yolların arkasında kalıcı bir mount bulunmadığında Azure Blob Storage'ı (veya [CDN](/docs/configuration/cdn) altındaki başka bir sağlayıcıyı) yapılandırın.
