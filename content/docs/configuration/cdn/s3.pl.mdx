...
- **S3_URL_EXPIRY_SECONDS:** (Opcjonalne) Czas życia każdego wstępnie podpisanego adresu URL (presigned URL), w sekundach. Zobacz poniższą uwagę dotyczącą wstępnie podpisanych adresów URL, aby zapoznać się z limitami po stronie dostawcy.
- **S3_REFRESH_EXPIRY_MS:** (Opcjonalne) Regeneruje podpisany adres URL (presigned URL) po osiągnięciu tego wieku, w milisekundach, zamiast używać domyślnej logiki bufora wygaśnięcia. Domyślnie nieustawione. Wartość, która nie jest liczbą całkowitą dodatnią, jest ignorowana z ostrzeżeniem w logach.
