...
### 무엇이 이것을 사용하는가 [#what-uses-this]

파일 전략을 설정하면 특정 기능을 활성화하는 것이 아니라 LibreChat이 저장하는 모든 파일의 위치가 변경됩니다. 여기에는 사용자 및 에이전트 아바타, 채팅에 업로드된 파일, 이미지 생성 도구로 반환된 이미지, Code Interpreter로 생성된 파일이 포함됩니다.

기본 전략은 모든 데이터를 API 컨테이너의 파일 시스템에 기록합니다. 표준 Docker Compose 배포는 `./images` 및 `./uploads`를 호스트에 바인드 마운트하므로, 단일 인스턴스 Compose 설정에서는 컨테이너를 다시 생성해도 해당 파일들이 유지됩니다. 로컬 스토리지가 할 수 없는 것은 인스턴스 간에 파일을 공유하는 것이며, Kubernetes에서 영구 볼륨(persistent volume) 없이 사용하는 경우처럼 해당 경로를 컨테이너의 쓰기 가능 계층에 남겨두는 배포 방식에서는 파일이 유실됩니다. 둘 이상의 인스턴스를 실행하거나 해당 경로에 영구적인 마운트가 설정되어 있지 않은 경우, Azure Blob Storage(또는 [CDN](/docs/configuration/cdn) 하위의 다른 제공업체)를 구성하십시오.
