...
### Cosa utilizza questo [#what-uses-this]

L'impostazione di una file strategy modifica la posizione in cui LibreChat archivia ogni file, anziché abilitare una funzionalità specifica. Ciò riguarda gli avatar degli utenti e degli agenti, i file caricati nelle chat, le immagini restituite dagli strumenti di generazione di immagini e i file prodotti dal Code Interpreter.

La strategia predefinita scrive tutto ciò nel filesystem del container dell'API. La distribuzione standard Docker Compose esegue il bind-mount di `./images` e `./uploads` sull'host, quindi in una configurazione compose a istanza singola quei file sopravvivono già alla ricreazione del container. Ciò che l'archiviazione locale non può fare è condividere i file tra le istanze, e li perde in qualsiasi distribuzione che lasci quei percorsi sul layer scrivibile del container, come Kubernetes senza un volume persistente. Configura Azure Blob Storage (o un altro provider sotto [CDN](/docs/configuration/cdn)) quando esegui più di un'istanza, o quando non c'è nulla di persistente montato dietro quei percorsi.
