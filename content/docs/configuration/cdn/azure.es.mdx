...
### Qué utiliza esto [#what-uses-this]

Configurar una estrategia de archivos cambia dónde LibreChat coloca cada archivo que almacena, en lugar de habilitar una función en particular. Esto abarca los avatares de usuarios y agentes, los archivos subidos en el chat, las imágenes devueltas por las herramientas de generación de imágenes y los archivos producidos por el Code Interpreter.

La estrategia predeterminada escribe todo eso en el sistema de archivos del contenedor de la API. La implementación estándar de Docker Compose realiza un montaje de enlace (bind-mount) de `./images` y `./uploads` al host, por lo que en una configuración de una sola instancia con compose, esos archivos ya sobreviven a la recreación del contenedor. Lo que el almacenamiento local no puede hacer es compartir archivos entre instancias, y sí los pierde en cualquier implementación que deje esas rutas en la capa grabable del contenedor, como Kubernetes sin un volumen persistente. Configure Azure Blob Storage (u otro proveedor bajo [CDN](/docs/configuration/cdn)) cuando ejecute más de una instancia, o cuando no haya nada persistente montado detrás de esas rutas.
