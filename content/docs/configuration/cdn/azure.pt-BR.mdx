...
### O que utiliza isso [#what-uses-this]

Definir uma file strategy altera onde o LibreChat coloca cada arquivo que ele armazena, em vez de habilitar um recurso específico. Isso abrange avatares de usuários e agentes, arquivos enviados no chat, imagens retornadas pelas ferramentas de geração de imagem e arquivos produzidos pelo Code Interpreter.

A estratégia padrão grava tudo isso no sistema de arquivos do contêiner da API. A implantação padrão do Docker Compose faz o bind-mount de `./images` e `./uploads` para o host, portanto, em uma configuração de compose de instância única, esses arquivos já sobrevivem à recriação do contêiner. O que o armazenamento local não pode fazer é compartilhar arquivos entre instâncias, e ele os perde em qualquer implantação que deixe esses caminhos na camada gravável do contêiner, como o Kubernetes sem um volume persistente. Configure o Azure Blob Storage (ou outro provedor em [CDN](/docs/configuration/cdn)) quando você executar mais de uma instância, ou quando nada persistente estiver montado atrás desses caminhos.
