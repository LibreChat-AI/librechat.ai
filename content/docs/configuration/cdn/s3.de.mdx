...
- **S3_URL_EXPIRY_SECONDS:** (Optional) Lebensdauer jeder vorsignierten URL in Sekunden. Siehe den Hinweis zu vorsignierten URLs weiter unten bezüglich der anbieterseitigen Obergrenzen, die hierbei gelten.
- **S3_REFRESH_EXPIRY_MS:** (Optional) Generiert eine vorab signierte URL neu, sobald sie dieses Alter in Millisekunden erreicht, anstatt die standardmäßige Expiry-Buffer-Logik zu verwenden. Standardmäßig nicht gesetzt. Ein Wert, der keine positive Ganzzahl ist, wird ignoriert und führt zu einer Warnung in den Protokollen.
