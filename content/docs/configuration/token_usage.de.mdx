...
- Ein farbcodiertes Messgerät, das die Token-Kategorien trennt, die derzeit das Kontextfenster nutzen
- Eine Klick-Aufschlüsselung für Dialoge, beibehaltene Tool-Ergebnisse, Agent-Anweisungen, zwischengespeicherte Eingaben, Vervollständigungen, Zweigsummen und Konversationssummen
- Offenlegungen pro Tool und eine Laufzeitschätzung, wenn kompatible Snapshots den verbleibenden Kontextdruck projizieren können
- Für autorisierte Administratoren ein direkter Langfuse-Sitzungslink, wenn die Konversation eine stichprobenartige Ablaufverfolgung (Sampled Trace) in der aktiven In-App-Verbindung aufweist

Die detaillierte Aufschlüsselung ist standardmäßig eingeklappt, damit der kategorisierte Zähler kompakt bleibt. Benutzer können ihn bei Bedarf erweitern, um die Summen einzusehen, und LibreChat merkt sich diese Auswahl für spätere Unterhaltungen. Zurückgestellte Tool-Definitionen verwenden dieselbe Kategoriefarbe mit einer schraffierten Darstellung, während Schätzungen ohne bekannte Kategorienzusammensetzung als einfache Füllung beibehalten werden.

Tool-Traffic, Cache und Agent-Anweisungen sind Teilmengen ihrer übergeordneten Kategorien und erhöhen die Gesamtsumme nicht ein zweites Mal. Die Laufzeitschätzung erscheint nur, wenn aufeinanderfolgende Snapshots dasselbe Modell, denselben Anbieter, denselben Agenten, dasselbe Kontext-Budget und denselben festen Overhead teilen. Nutzungsaufschlüsselungen werden zusammen mit Nachrichten und Unterhaltungen gespeichert, einschließlich des Cache-Splits des letzten primären Aufrufs und der vollständigen Ausgabe. Wieder geöffnete Chats behalten die Details zu Verzweigungen und der Gesamtnutzung/-kosten bei, anstatt sich nur auf die aktive Streaming-Sitzung zu verlassen; Subagent-Summen werden explizit als alle Verzweigungen umfassend gekennzeichnet.
