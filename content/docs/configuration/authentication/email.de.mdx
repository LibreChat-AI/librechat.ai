...
## Einrichtung mit benutzerdefiniertem Mailserver [#setup-with-custom-mail-server]

Um einen benutzerdefinierten Mailserver einzurichten, folgen Sie diesen Schritten:

1. Sammeln Sie Ihre SMTP-Anmeldedaten von Ihrem Anbieter. Die Schritte sind für jeden Anbieter unterschiedlich, aber in der Regel werden dort Werte für alle Variablen aufgelistet.
2. Ändern Sie in der `.env` Datei die Variablen wie folgt, unter der Annahme einiger sinnvoller Beispielwerte:
...
3. **Probleme mit der EU-Region**: Wenn sich Ihr Mailgun-Konto in der EU-Region befindet, stellen Sie sicher, dass `MAILGUN_HOST=https://api.eu.mailgun.net` gesetzt ist.
4. **Fallback auf SMTP**: Wenn nur einer der Werte `MAILGUN_API_KEY` oder `MAILGUN_DOMAIN` festgelegt ist, greift das System auf die SMTP-Konfiguration zurück.
