...
## Stabiele Identiteitsbinding [#stable-identity-binding]

LibreChat koppelt SAML-accounts aan de stabiele NameID van de assertion, niet aan een veranderlijk e-mailadres. Een eerste aanmelding kan een bestaand SAML-account matchen op basis van e-mailadres en de bijbehorende NameID vastleggen, maar latere aanmeldingen moeten dezelfde NameID presenteren. Een bestaand account weigert een andere NameID, zelfs wanneer de e-mailclaim overeenkomt.

Vraag een stabiel formaat aan van de IdP en pin optioneel de entity ID:
...
`SAML_NAME_ID_FORMAT` bepaalt het formaat dat wordt aangevraagd in het authenticatieverzoek. Persistent NameIDs worden aanbevolen. LibreChat weigert een transient formaat in de configuratie en weigert assertions die hun NameID als transient identificeren. `SAML_IDP_ISSUER` vereist, indien ingesteld, dat de issuer van de assertion exact overeenkomt; een ontbrekende of afwijkende issuer wordt geweigerd.

E-mail blijft vereist voor profiel- en domeinbeleidscontroles. Het toegestane e-maildomeinbeleid van de deployment en tenant is nog steeds van toepassing bij elke aanmelding; deze vervangen de stabiele NameID-binding niet.
