...
## Stable Identity Binding [#stable-identity-binding]

LibreChat wiąże konta SAML ze stałym identyfikatorem NameID zawartym w asercji, a nie ze zmiennym adresem e-mail. Pierwsze logowanie może dopasować istniejące konto SAML na podstawie adresu e-mail i zapisać jego NameID, ale późniejsze logowania muszą przedstawić ten sam NameID. Istniejące konto odrzuci inny NameID, nawet jeśli oświadczenie (claim) dotyczące adresu e-mail będzie się zgadzać.

Zażądaj stabilnego formatu od IdP i opcjonalnie przypnij jego identyfikator jednostki (entity ID):
...
`SAML_NAME_ID_FORMAT` kontroluje format żądany w żądaniu uwierzytelnienia. Zalecane są trwałe (persistent) NameID. LibreChat odrzuca format tymczasowy (transient) w konfiguracji oraz odrzuca asercje, które identyfikują swój NameID jako tymczasowy. `SAML_IDP_ISSUER`, jeśli jest ustawiony, wymaga, aby wystawca asercji był dokładnie zgodny; brakujący lub inny wystawca jest odrzucany.

Adres e-mail pozostaje wymagany do weryfikacji profilu oraz polityki domenowej. Zasady dotyczące dozwolonych domen e-mail dla wdrożenia i dzierżawy (tenant) nadal mają zastosowanie przy każdym logowaniu; nie zastępują one stałego powiązania NameID.
