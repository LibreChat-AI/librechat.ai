...
LibreChat associa gli account SAML al NameID stabile dell'asserzione, non a un indirizzo email modificabile. Un primo accesso può corrispondere a un account SAML esistente tramite email e registrarne il NameID, ma gli accessi successivi devono presentare lo stesso NameID. Un account esistente rifiuta un NameID diverso anche quando il claim dell'email corrisponde.

Richiedi un formato stabile dall'IdP e, facoltativamente, blocca il suo entity ID:
...
`SAML_NAME_ID_FORMAT` controlla il formato richiesto nella richiesta di autenticazione. Sono raccomandati i NameID persistenti. LibreChat rifiuta un formato transitorio nella configurazione e rifiuta le asserzioni che identificano il loro NameID come transitorio. `SAML_IDP_ISSUER`, quando impostato, richiede che l'emittente dell'asserzione corrisponda esattamente; un emittente mancante o diverso viene rifiutato.

L'email rimane necessaria per i controlli del profilo e delle policy di dominio. Le policy sui domini email consentiti del deployment e del tenant si applicano ancora a ogni accesso; non sostituiscono il binding NameID stabile.
