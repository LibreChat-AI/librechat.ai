...
## 稳定身份绑定 [#stable-identity-binding]

LibreChat 将 SAML 账户绑定到断言中稳定的 NameID，而不是可变的电子邮件地址。首次登录时，可以通过电子邮件匹配现有的 SAML 账户并记录其 NameID，但后续登录必须提供相同的 NameID。即使电子邮件声明匹配，现有账户也会拒绝不同的 NameID。

从 IdP 请求稳定格式，并可选择固定其 entity ID：
...
`SAML_NAME_ID_FORMAT` 控制身份验证请求中要求的格式。建议使用 Persistent NameID。LibreChat 会在配置中拒绝 transient 格式，并拒绝将 NameID 标识为 transient 的断言。`SAML_IDP_ISSUER` 一旦设置，则要求断言颁发者必须完全匹配；如果颁发者缺失或不匹配，则会被拒绝。

电子邮件对于个人资料和域策略检查仍然是必需的。部署和租户的允许电子邮件域策略在每次登录时仍然适用；它们不会取代稳定的 NameID 绑定。
...
`SAML_CERT` 环境变量用于指定身份提供商 (IdP) 的签名证书，以验证 SAML 响应。此证书必须以 **PEM 格式**提供，并可以通过以下任一方式指定：
...
当在 .env 文件中配置证书并同时保留完整的 PEM 结构时，此格式非常有用。

### 证书格式要求 [#certificate-format-requirements]
- 该证书**必须始终为 PEM 格式**（Base64 编码的 X.509 证书）。
- 如果作为文件提供，它必须是有效的 **RFC7468 严格文本消息 PEM 格式**。
