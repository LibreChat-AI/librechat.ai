...
LibreChatは、SAMLアカウントを可変のメールアドレスではなく、アサーションの不変なNameIDに紐付けます。初回ログイン時は、既存のSAMLアカウントをメールアドレスで照合してそのNameIDを記録できますが、その後のログインでは同じNameIDを提示する必要があります。メールクレームが一致する場合であっても、既存のアカウントは異なるNameIDでのログインを拒否します。

IdPから安定した形式を要求し、必要に応じてそのエンティティIDを固定します：
...
`SAML_NAME_ID_FORMAT` は、認証リクエストで要求される形式を制御します。Persistent NameID が推奨されます。LibreChat は設定内の transient 形式を拒否し、NameID が transient であると識別されるアサーションを拒否します。`SAML_IDP_ISSUER` が設定されている場合、アサーションの発行者は完全に一致する必要があります。発行者が欠落しているか異なる場合は拒否されます。

プロファイルおよびドメインポリシーのチェックには、引き続きメールアドレスが必要です。デプロイメントおよびテナントで許可されたメールドメインポリシーは、ログインのたびに適用されます。これらは、安定したNameIDバインディングを置き換えるものではありません。
