...
## Liaison d'identité stable [#stable-identity-binding]

LibreChat lie les comptes SAML au NameID stable de l'assertion, et non à une adresse e-mail mutable. Une première connexion peut faire correspondre un compte SAML existant par e-mail et enregistrer son NameID, mais les connexions ultérieures doivent présenter le même NameID. Un compte existant refusera un NameID différent, même si la revendication d'e-mail correspond.

Demander un format stable à l'IdP et, en option, épingler son ID d'entité :
...
`SAML_NAME_ID_FORMAT` contrôle le format demandé dans la requête d'authentification. Les NameID persistants sont recommandés. LibreChat rejette un format transitoire dans la configuration et rejette les assertions qui identifient leur NameID comme transitoire. `SAML_IDP_ISSUER`, lorsqu'il est défini, exige que l'émetteur de l'assertion corresponde exactement ; un émetteur manquant ou différent est rejeté.

L'e-mail reste requis pour les vérifications de profil et de politique de domaine. Les politiques de domaine d'e-mail autorisées du déploiement et du locataire s'appliquent toujours à chaque connexion ; elles ne remplacent pas la liaison stable NameID.
