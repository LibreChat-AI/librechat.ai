---
title: Auth0
icon: Auth0
description: >-
  Aprenda a configurar o LibreChat para usar o Auth0 para autenticação de
  usuários.
---

## Passo 1: Criar um aplicativo SAML no Auth0 [#step-1-create-a-saml-application-in-auth0]

1. Faça login no seu Auth0 Dashboard.
2. Navegue até `Applications > Applications`.
3. Clique em `Create Application`.
4. Insira um Nome de Aplicativo (por exemplo, `LibreChat`) e selecione `Regular Web Application`.
5. Clique em `Create`.

![auth0-1](/images/docs/configuration/authentication/SAML/auth0/1.png)


## Passo 2: Configurar o Add-On SAML [#step-2-configure-the-saml-add-on]

1. Abra o aplicativo recém-criado no Auth0.
2. Vá para a aba `Addons`.
3. Clique no controle deslizante para ativar o `SAML2 Web App`.
4. Clique no painel `SAML2 Web App`.
5. Configure as seguintes definições:
   - **Application Callback URL**: Defina isso como a sua URL de callback SAML do LibreChat:
   `https://YOUR_DOMAIN/oauth/saml/callback`
   - **Configurações (Formato JSON)**: Use a seguinte configuração:
        ```json
        {
            "audience": "https://your-librechat-domain.com",
            "mappings": {
                "email": "email",
                "name": "username"
            }
        }
        ```
        <Callout type="warning" title="Audience deve corresponder a SAML_ISSUER">
        Defina `audience` com a mesma string que você usará para `SAML_ISSUER` no Passo 4.
        Se você omiti-la, o Auth0 definirá seu próprio audience padrão, o que não corresponderá
        ao que o LibreChat envia, e o login falhará com um erro de incompatibilidade de audience.
        </Callout>
        <Callout type="note" title="nota">
        Se a sua aplicação exigir atributos adicionais como `given_name`, `family_name`, `username` ou `picture`, certifique-se de que esses mapeamentos estejam configurados corretamente nas configurações SAML do Auth0.
        </Callout>
6. Clique em `Save`.


![auth0-2](/images/docs/configuration/authentication/SAML/auth0/2.png)
![auth0-3](/images/docs/configuration/authentication/SAML/auth0/3.png)

## Passo 3: Obtenha os metadados SAML do Auth0 [#step-3-obtain-the-auth0-saml-metadata]

1. Assim que o SAML estiver ativado, volte para as configurações do `SAML2 Web App`.
2. Vá para a aba `Usage`.
3. Clique em `Identity Provider Certificate: Download Auth0 certificate`.
4. Use a `Identity Provider Login URL` para `SAML_ENTRY_POINT`.
5. Use o `Issuer` para `SAML_IDP_ISSUER` (opcional, veja o aviso abaixo).
6. Copie o arquivo de certificado baixado para a pasta do seu projeto.

<Callout type="warning" title="SAML_ISSUER é seu, não do Auth0">

`SAML_ISSUER` é o ID da entidade que o **LibreChat envia sobre si mesmo** em suas solicitações de autenticação, não um valor que você copia do Auth0. Você o escolhe, e é a mesma string que você insere no lado do Auth0 como **Audience**. Sua URL base do LibreChat é a escolha convencional:

```bash filename=".env"
SAML_ISSUER=https://your-librechat-domain.com
```

O valor `Issuer` do próprio Auth0 vai na variável separada `SAML_IDP_ISSUER`, que o LibreChat usa para verificar se uma asserção realmente veio do seu provedor de identidade.

O LibreChat não publica um documento de metadados SAML, portanto, não há uma URL de metadados para fornecer ao Auth0. Configure o Auth0 manualmente com o Audience acima e a URL de callback abaixo.

</Callout>

![auth0-4](/images/docs/configuration/authentication/SAML/auth0/4a.png)

## Passo 4: Configure o LibreChat com SAML [#step-4-configure-librechat-with-saml]

Abra o arquivo `.env` na pasta do seu projeto e adicione as seguintes variáveis:

  ```bash filename=".env"
  SAML_ENTRY_POINT=https://dev-xxxxx.us.auth0.com/samlp/aaaaaa

  # Your own entity ID, sent to Auth0. Must match the Audience you set in Auth0.
  SAML_ISSUER=https://your-librechat-domain.com
  # Auth0's Issuer, used to verify incoming assertions (optional)
  SAML_IDP_ISSUER=urn:dev-xxxxx.us.auth0.com

  SAML_CERT=dev-xxxxx.pem
  SAML_CALLBACK_URL=/oauth/saml/callback
  SAML_SESSION_SECRET=[JustGenerateARandomSessionSecret]

  # Attribute mappings (optional)
  SAML_EMAIL_CLAIM=
  SAML_USERNAME_CLAIM=
  SAML_GIVEN_NAME_CLAIM=
  SAML_FAMILY_NAME_CLAIM=
  SAML_PICTURE_CLAIM=
  SAML_NAME_CLAIM=

  # Login button settings (optional)
  SAML_BUTTON_LABEL=
  SAML_IMAGE_URL=

  # Whether the SAML Response should be signed.
  # - If "true", the entire `SAML Response` will be signed.
  # - If "false" or unset, only the `SAML Assertion` will be signed (default behavior).
  # SAML_USE_AUTHN_RESPONSE_SIGNED=
  ```
