...
O LibreChat vincula contas SAML ao NameID estável da asserção, e não a um endereço de e-mail mutável. Um primeiro login pode corresponder a uma conta SAML existente pelo e-mail e registrar seu NameID, mas logins posteriores devem apresentar o mesmo NameID. Uma conta existente recusa um NameID diferente, mesmo quando a declaração de e-mail coincide.

Solicite um formato estável do IdP e, opcionalmente, fixe seu entity ID:
...
`SAML_NAME_ID_FORMAT` controla o formato solicitado na requisição de autenticação. NameIDs persistentes são recomendados. O LibreChat rejeita um formato transitório na configuração e rejeita asserções que identifiquem seu NameID como transitório. `SAML_IDP_ISSUER`, quando definido, exige que o emissor da asserção corresponda exatamente; um emissor ausente ou diferente é rejeitado.

O e-mail continua sendo necessário para verificações de perfil e de política de domínio. As políticas de domínio de e-mail permitidas da implantação e do tenant ainda se aplicam a cada login; elas não substituem a vinculação estável de NameID.
