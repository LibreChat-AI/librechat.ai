...
LibreChat mengikat akun SAML ke NameID yang stabil dari assertion, bukan ke alamat email yang dapat berubah. Login pertama dapat mencocokkan akun SAML yang sudah ada berdasarkan email dan mencatat NameID-nya, namun login berikutnya harus menyajikan NameID yang sama. Akun yang sudah ada akan menolak NameID yang berbeda meskipun klaim email cocok.

Minta format stabil dari IdP dan secara opsional sematkan (pin) ID entitasnya:
...
`SAML_NAME_ID_FORMAT` mengontrol format yang diminta dalam permintaan autentikasi. NameID persisten sangat disarankan. LibreChat menolak format transien dalam konfigurasi dan menolak asersi yang mengidentifikasi NameID mereka sebagai transien. `SAML_IDP_ISSUER`, jika diatur, mengharuskan penerbit asersi untuk cocok secara tepat; penerbit yang hilang atau berbeda akan ditolak.

Email tetap diperlukan untuk pemeriksaan profil dan kebijakan domain. Kebijakan domain email yang diizinkan pada deployment dan tenant tetap berlaku pada setiap login; kebijakan tersebut tidak menggantikan pengikatan NameID yang stabil.
