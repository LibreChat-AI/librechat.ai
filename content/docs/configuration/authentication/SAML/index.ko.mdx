...
LibreChat은 SAML 계정을 변경 가능한 이메일 주소가 아닌, 어설션(assertion)의 고유한 NameID에 바인딩합니다. 첫 로그인 시 이메일을 통해 기존 SAML 계정을 매칭하고 해당 NameID를 기록할 수 있지만, 이후 로그인 시에는 반드시 동일한 NameID를 제시해야 합니다. 기존 계정은 이메일 클레임이 일치하더라도 다른 NameID를 제시할 경우 이를 거부합니다.

IdP로부터 안정적인 형식을 요청하고 선택적으로 해당 엔티티 ID를 고정하세요:
...
`SAML_NAME_ID_FORMAT`은 인증 요청 시 요청할 형식을 제어합니다. Persistent NameID 사용을 권장합니다. LibreChat은 설정에서 transient 형식을 거부하며, NameID가 transient로 식별되는 어설션(assertion)도 거부합니다. `SAML_IDP_ISSUER`가 설정된 경우, 어설션 발급자(issuer)가 정확히 일치해야 합니다. 발급자가 누락되었거나 다른 경우 거부됩니다.

프로필 및 도메인 정책 확인을 위해 이메일은 여전히 필요합니다. 배포 및 테넌트의 허용된 이메일 도메인 정책은 모든 로그인 시 여전히 적용되며, 안정적인 NameID 바인딩을 대체하지 않습니다.
