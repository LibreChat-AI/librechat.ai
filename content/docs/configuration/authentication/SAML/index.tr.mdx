...
## Kararlı Kimlik Bağlama (Stable Identity Binding) [#stable-identity-binding]

LibreChat, SAML hesaplarını değiştirilebilir bir e-posta adresine değil, onaylamanın (assertion) sabit NameID değerine bağlar. İlk giriş, mevcut bir SAML hesabıyla e-posta üzerinden eşleşebilir ve NameID değerini kaydedebilir, ancak sonraki girişlerde aynı NameID değerinin sunulması gerekir. Mevcut bir hesap, e-posta talebi eşleşse bile farklı bir NameID değerini reddeder.

IdP'den kararlı bir format talep edin ve isteğe bağlı olarak entity ID'sini sabitleyin:
...
`SAML_NAME_ID_FORMAT`, kimlik doğrulama isteğinde talep edilen formatı kontrol eder. Persistent NameID'ler önerilir. LibreChat, yapılandırmada transient formatı reddeder ve NameID'lerini transient olarak tanımlayan assertion'ları kabul etmez. `SAML_IDP_ISSUER` ayarlandığında, assertion yayıncısının tam olarak eşleşmesi gerekir; eksik veya farklı bir yayıncı reddedilir.

Profil ve alan adı ilkesi denetimleri için e-posta gerekliliği devam etmektedir. Dağıtımın ve kiracının izin verilen e-posta alanı ilkeleri her girişte geçerliliğini korur; bunlar kararlı NameID bağlamasının yerini almazlar.
