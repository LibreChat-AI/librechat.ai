...
## Ràng buộc định danh ổn định (Stable Identity Binding) [#stable-identity-binding]

LibreChat liên kết các tài khoản SAML với NameID cố định của xác nhận (assertion), thay vì địa chỉ email có thể thay đổi. Lần đăng nhập đầu tiên có thể khớp với một tài khoản SAML hiện có thông qua email và ghi lại NameID của tài khoản đó, nhưng các lần đăng nhập sau đó phải cung cấp cùng một NameID. Một tài khoản hiện có sẽ từ chối NameID khác ngay cả khi xác nhận email khớp.

Yêu cầu định dạng ổn định từ IdP và tùy chọn ghim entity ID của nó:
...
`SAML_NAME_ID_FORMAT` kiểm soát định dạng được yêu cầu trong yêu cầu xác thực. Khuyến nghị nên sử dụng Persistent NameID. LibreChat từ chối định dạng transient trong cấu hình và từ chối các xác nhận (assertions) xác định NameID của chúng là transient. `SAML_IDP_ISSUER`, khi được thiết lập, yêu cầu nhà phát hành xác nhận phải khớp chính xác; nếu thiếu hoặc khác nhà phát hành, yêu cầu sẽ bị từ chối.

Email vẫn là bắt buộc để kiểm tra hồ sơ và chính sách tên miền. Các chính sách về tên miền email được phép của quá trình triển khai và đối tượng thuê (tenant) vẫn áp dụng cho mỗi lần đăng nhập; chúng không thay thế liên kết NameID ổn định.
