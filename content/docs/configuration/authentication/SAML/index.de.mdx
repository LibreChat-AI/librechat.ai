...
## Stabile Identitätsbindung [#stable-identity-binding]

LibreChat bindet SAML-Konten an die stabile NameID der Assertion, nicht an eine veränderbare E-Mail-Adresse. Eine erste Anmeldung kann ein bestehendes SAML-Konto über die E-Mail-Adresse abgleichen und dessen NameID speichern, aber spätere Anmeldungen müssen dieselbe NameID vorweisen. Ein bestehendes Konto lehnt eine andere NameID ab, selbst wenn der E-Mail-Claim übereinstimmt.

Fordern Sie ein stabiles Format vom IdP an und fixieren Sie optional dessen Entity ID:
...
`SAML_NAME_ID_FORMAT` steuert das im Authentifizierungs-Request angeforderte Format. Persistente NameIDs werden empfohlen. LibreChat lehnt ein transientes Format in der Konfiguration ab und weist Assertionen zurück, die ihre NameID als transient identifizieren. `SAML_IDP_ISSUER` erfordert, sofern gesetzt, dass der Assertion-Issuer exakt übereinstimmt; ein fehlender oder abweichender Issuer wird abgelehnt.

E-Mail bleibt für Profil- und Domain-Richtlinienprüfungen erforderlich. Die Richtlinien für zulässige E-Mail-Domains der Bereitstellung und des Mandanten gelten weiterhin bei jeder Anmeldung; sie ersetzen nicht die stabile NameID-Bindung.
...
- Falls als Datei bereitgestellt, muss es ein gültiges **RFC7468 strict textual message PEM format** sein.
...
2. Wenn sowohl `SAML_GIVEN_NAME_CLAIM` als auch `SAML_FAMILY_NAME_CLAIM` angegeben sind, werden ihre entsprechenden Werte verkettet, um den Benutzernamen zu bilden.
