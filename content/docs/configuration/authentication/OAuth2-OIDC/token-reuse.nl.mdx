...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Kort herstelvenster voor een geroteerd refresh token terwijl LibreChat de vernieuwde sessie publiceert (standaard: 60000 ms / 1 minuut). Het accepteert rekenkundige expressies. Verhoog dit alleen wanneer trage sessiepersistentie of publicatie tussen replica's meer tijd nodig heeft.
...
- `GRAPH_API_SCOPES`: Door spaties gescheiden Microsoft Graph-scopes die worden aangevraagd bij het oplossen van `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` in een via YAML gedefinieerde MCP-server. De standaardwaarde is `https://graph.microsoft.com/.default`; dit staat los van de `OPENID_GRAPH_SCOPES` die worden gebruikt voor het zoeken naar personen en groepen.
...
- LibreChat weigert ID-tokens wanneer een toegangstoken vereist is voor hergebruik van bearer-tokens; configureer de provider om een echt toegangstoken uit te geven voor de aangevraagde audience en scopes
- Token-responses met ontbrekende, ongeldige of buitensporige `expires_in`-waarden worden beperkt tot de veilige levensduur van het credential in plaats van een gecachet token daarbuiten te verlengen.
- LibreChat lost OpenID-referenties op voor MCP en Microsoft Graph vanuit de actieve sessie, coördineert verversingen tussen replica's en schermt uitlogacties af, zodat een verlate verversing geen uitgelogde sessie opnieuw kan aanmaken.
