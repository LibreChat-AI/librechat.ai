...
icon: Authelia
description: >-
  Pelajari cara mengonfigurasi LibreChat untuk menggunakan Authelia bagi
  autentikasi pengguna.
---

- Hasilkan client secret menggunakan:
  ```
  docker run --rm authelia/authelia:latest authelia crypto hash generate pbkdf2 --variant sha512 --random --random.length 72 --random.charset rfc3986
  ```
- Kemudian di dalam `configuration.yml` Anda, tambahkan kode berikut di bagian oidc:
