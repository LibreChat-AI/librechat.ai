...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Jendela pemulihan singkat untuk refresh token yang dirotasi saat LibreChat memublikasikan sesi yang diperbarui (default: 60000 ms / 1 menit). Nilai ini menerima ekspresi aritmatika. Tingkatkan nilainya hanya jika persistensi sesi yang lambat atau publikasi lintas-replika memerlukan waktu lebih lama.
...
- `GRAPH_API_SCOPES`: Scope Microsoft Graph yang dipisahkan oleh spasi yang diminta saat menyelesaikan `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` di server MCP yang didefinisikan dalam YAML. Default-nya adalah `https://graph.microsoft.com/.default`; ini terpisah dari `OPENID_GRAPH_SCOPES` yang digunakan untuk pencarian orang dan grup.
...
- LibreChat menolak token ID ketika token akses diperlukan untuk penggunaan ulang bearer-token; konfigurasikan penyedia untuk menerbitkan token akses yang sebenarnya untuk audiens dan cakupan yang diminta
- Respons token dengan nilai `expires_in` yang hilang, tidak valid, atau berlebihan akan dibatasi pada masa pakai kredensial yang aman, alih-alih memperpanjang token yang di-cache melebihi masa pakainya.
- LibreChat menyelesaikan kredensial OpenID untuk MCP dan Microsoft Graph dari sesi langsung, mengoordinasikan penyegaran di seluruh replika, dan membatasi logout sehingga penyegaran yang terlambat tidak dapat membuat ulang sesi yang telah keluar.
