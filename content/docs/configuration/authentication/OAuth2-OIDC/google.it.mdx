---
title: Google
icon: Google
description: >-
  Scopri come configurare LibreChat per utilizzare Google per l'autenticazione
  degli utenti.
---

## Creare un'applicazione Google [#create-a-google-application]

- Visita: **[Google Cloud Console](https://cloud.google.com)** e apri la `Console`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/a7d290ea-6031-43b3-b367-36ce00e46f20)

- Crea un nuovo progetto e dagli un nome

![image](https://github.com/danny-avila/LibreChat/assets/138638445/ce71c9ca-7ddd-4021-9133-a872c64c20c4)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/8abbd41e-8332-4851-898d-9cddb373c527)

## Configurazione dell'applicazione Google [#google-application-configuration]

- Seleziona il progetto che hai appena creato e vai su `APIs and Services`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/c6265582-2cf6-430f-ae51-1edbdd9f2c48)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/006e16ba-56b8-452d-b324-5f2d202637ab)

- Seleziona `Credentials` e fai clic su `CONFIGURE CONSENT SCREEN`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/e4285cbb-833f-4366-820d-addf04a2ad77)

- Seleziona `External` quindi clicca su `CREATE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/232d46c0-dd00-4637-b538-3ba3bdbdc0b2)

- Inserisci le informazioni della tua App

> Nota: Puoi ottenere un logo dalla tua cartella LibreChat qui: `docs\assets\favicon_package\android-chrome-192x192.png`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/e6c4c8ec-2f02-4af5-9458-c72394d0b7c5)

- Configura il tuo `App domain` e aggiungi le tue `Developer contact information`, quindi fai clic su `SAVE AND CONTINUE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/6c2aa557-9b9b-412d-bc2b-76a0dc11f394)

- Configure the `Sopes`
    - Aggiungi `email`, `profile` e `openid`
    - Fai clic su `UPDATE` e `SAVE AND CONTINUE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/46af2fb9-8cfd-41c5-a763-814b308e45c3)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/4e832970-d392-4c67-bb38-908a5c51660a)

- Fai clic su `SAVE AND CONTINUE`
- Controlla la tua app e torna alla dashboard

- Torna alla scheda `Credentials`, clicca su `+ CREATE CREDENTIALS` e seleziona `OAuth client ID`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/beef1982-55a3-4837-8e8c-20bad8d846ba)

- Seleziona `Web application` e assegnale un nome

![image](https://github.com/danny-avila/LibreChat/assets/138638445/badde864-f6b5-468f-a72f-bac93326ffa5)

- Configura le `Authorized JavaScript origins`, puoi aggiungere sia il tuo dominio che localhost se lo desideri
    - Esempio per localhost: `http://localhost:3080`
    - Esempio per un dominio: `https://example.com`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/f7e3763a-5f74-4850-8638-44f81693b9ac)

- Aggiungi un `Authorized redirect URIs` valido
    - Esempio per localhost: `http://localhost:3080/oauth/google/callback`
    - Esempio per un dominio: `https://example.com/oauth/google/callback`
    - Se si utilizza il Pannello di Amministrazione: `https://example.com/api/admin/oauth/google/callback`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/0db34b19-d780-4651-9c2f-d33e24a74d55)

## Configurazione di .env [#env-configuration]

- Fai clic su `CREATE` e copia il tuo `Client ID` e il tuo `Client secret`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/fa8572bf-f482-457a-a285-aec7d41af76b)

- Aggiungili al tuo file `.env`:

```bash filename=".env"
DOMAIN_CLIENT=https://your-domain.com # use http://localhost:3080 if not using a custom domain
DOMAIN_SERVER=https://your-domain.com # use http://localhost:3080 if not using a custom domain

GOOGLE_CLIENT_ID=your_client_id
GOOGLE_CLIENT_SECRET=your_client_secret
GOOGLE_CALLBACK_URL=/oauth/google/callback
```

- Salva il file `.env`

> Nota: Se si utilizza Docker, eseguire `docker compose up -d` per applicare le modifiche alla configurazione nel file .env

## Sessioni del Pannello di Amministrazione [#admin-panel-sessions]

Il [Admin Panel](/docs/features/admin_panel) utilizza gli stessi `GOOGLE_CLIENT_ID` e `GOOGLE_CLIENT_SECRET`, ma il suo callback è `${DOMAIN_SERVER}/api/admin/oauth/google/callback`. Il suo flusso Google richiede l'accesso offline e il consenso, in modo che un token di amministrazione di LibreChat scaduto possa essere aggiornato. LibreChat ricontrolla l'identità dell'utente, lo stato di ban, il dominio consentito, il tenant e l'attuale accesso amministrativo a ogni aggiornamento.
