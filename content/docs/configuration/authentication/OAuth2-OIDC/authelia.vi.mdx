...
icon: Authelia
description: >-
  Tìm hiểu cách cấu hình LibreChat để sử dụng Authelia cho việc xác thực người
  dùng.
---

- Tạo một client secret bằng cách sử dụng:
  ```
  docker run --rm authelia/authelia:latest authelia crypto hash generate pbkdf2 --variant sha512 --random --random.length 72 --random.charset rfc3986
  ```
- Sau đó, trong `configuration.yml` của bạn, hãy thêm nội dung sau vào phần oidc:
