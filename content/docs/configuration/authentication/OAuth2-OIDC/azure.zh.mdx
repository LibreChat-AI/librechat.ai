...
## 进阶：Token 复用 [#advanced-token-reuse]
...
**重要提示：** 您必须启用 OpenID 令牌重用（token reuse）才能使此功能正常工作：
```bash filename=".env"
...
- 要使此功能正常工作，**必须启用令牌重用 (`OPENID_REUSE_TOKENS=true`)**
...
<Callout type="warning" title="安全说明">
