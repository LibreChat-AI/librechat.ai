...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: LibreChat이 갱신된 세션을 게시하는 동안 교체된 새로 고침 토큰에 대한 짧은 복구 창(기본값: 60000 ms / 1분). 산술 표현식을 허용합니다. 세션 지속성이 느리거나 복제본 간 게시 작업에 더 많은 시간이 필요할 때만 이 값을 늘리십시오.
...
- `GRAPH_API_SCOPES`: YAML로 정의된 MCP 서버에서 `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}`을 확인할 때 요청되는 Microsoft Graph 스코프(공백으로 구분). 기본값은 `https://graph.microsoft.com/.default`이며, 이는 사람 및 그룹 검색에 사용되는 `OPENID_GRAPH_SCOPES`와는 별개입니다.
...
- LibreChat은 베어러 토큰 재사용을 위해 액세스 토큰이 필요할 때 ID 토큰을 거부합니다. 요청된 대상(audience) 및 범위(scopes)에 대해 실제 액세스 토큰을 발급하도록 공급자(provider)를 구성하세요.
- `expires_in` 값이 누락되었거나, 유효하지 않거나, 과도한 토큰 응답은 캐시된 토큰을 자격 증명의 안전한 수명 이상으로 연장하는 대신 해당 수명 내로 제한됩니다.
- LibreChat은 라이브 세션에서 MCP 및 Microsoft Graph를 위한 OpenID 자격 증명을 확인하고, 복제본 간의 새로 고침을 조정하며, 로그아웃을 차단하여 지연된 새로 고침으로 인해 로그아웃된 세션이 다시 생성되지 않도록 합니다.
