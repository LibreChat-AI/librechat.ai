...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: LibreChat yenilenen oturumu yayınlarken döndürülen bir yenileme belirteci (refresh token) için kısa kurtarma penceresi (varsayılan: 60000 ms / 1 dakika). Aritmetik ifadeleri kabul eder. Yalnızca yavaş oturum kalıcılığı veya replikalar arası yayınlama daha fazla zaman gerektirdiğinde artırın.
...
- `GRAPH_API_SCOPES`: YAML ile tanımlanmış bir MCP sunucusunda `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` çözümlenirken istenen, boşlukla ayrılmış Microsoft Graph kapsamları. Varsayılan değer `https://graph.microsoft.com/.default` şeklindedir; bu, kişi ve grup araması için kullanılan `OPENID_GRAPH_SCOPES` değerinden ayrıdır.
...
- LibreChat, taşıyıcı belirteç (bearer-token) yeniden kullanımı için bir erişim belirteci gerektiğinde kimlik (ID) belirteçlerini reddeder; sağlayıcıyı, istenen hedef kitle (audience) ve kapsamlar (scopes) için gerçek bir erişim belirteci verecek şekilde yapılandırın.
- Eksik, geçersiz veya aşırı `expires_in` değerlerine sahip belirteç yanıtları, önbelleğe alınmış bir belirteci güvenli ömrünün ötesine uzatmak yerine, kimlik bilgisinin güvenli ömrü ile sınırlandırılır.
- LibreChat, MCP ve Microsoft Graph için OpenID kimlik bilgilerini canlı oturumdan çözer, kopyalar arasında yenilemeleri koordine eder ve geç bir yenilemenin oturumu kapatılmış bir oturumu yeniden oluşturmasını engellemek için çıkış işlemini korumaya alır.
