...
icon: Authelia
description: >-
  Scopri come configurare LibreChat per utilizzare Authelia per l'autenticazione
  degli utenti.
---

- Genera un client secret utilizzando:
  ```
  docker run --rm authelia/authelia:latest authelia crypto hash generate pbkdf2 --variant sha512 --random --random.length 72 --random.charset rfc3986
  ```
- Quindi nel tuo `configuration.yml` aggiungi quanto segue nella sezione oidc:
