...
1. Créer une API dans Auth0 (requise pour les jetons d'accès JWT) :
...
Pour une configuration détaillée d'Auth0, consultez : [Auth0 OpenID Connect Configuration](/docs/configuration/authentication/OAuth2-OIDC/auth0)
...
2. Configurez votre fournisseur OpenID (en utilisant Azure Entra ID comme exemple) :
   ...
   - Saisissez votre URI d'API (par ex. "api://librechat") et enregistrez
...
<Callout type="info" title="Intégration de l'API Microsoft Graph">
  Lorsque vous utilisez Azure Entra ID avec la réutilisation de jetons, vous pouvez également activer l'intégration de l'API Microsoft Graph pour des capacités améliorées de recherche de personnes et de groupes. Voir [Intégration de l'API Microsoft Graph](/docs/configuration/authentication/OAuth2-OIDC/azure#advanced-microsoft-graph-api-integration) pour plus de détails.
</Callout>
...
- `OPENID_REUSE_MAX_SESSION_AGE_MS` : Âge maximal auquel un jeton de session OpenID réutilisé est servi avant que LibreChat ne force un rafraîchissement auprès de l'IdP (par défaut : 900000 ms / 15 minutes). Accepte des expressions arithmétiques comme `60 * 60 * 24 * 1000`. Augmentez cette valeur vers la durée de vie du jeton d'accès de votre IdP si votre fournisseur révoque le jeton d'accès précédent à chaque rafraîchissement.
- `OPENID_REFRESH_BRIDGE_GRACE_MS` : Courte fenêtre de récupération pour un jeton d'actualisation ayant subi une rotation pendant que LibreChat publie la session actualisée (par défaut : 60000 ms / 1 minute). Il accepte les expressions arithmétiques. Ne l'augmentez que si la persistance lente de la session ou la publication inter-répliques nécessite plus de temps.
...
- `GRAPH_API_SCOPES` : étendues Microsoft Graph séparées par des espaces demandées lors de la résolution de `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` dans un serveur MCP défini par YAML. La valeur par défaut est `https://graph.microsoft.com/.default` ; ceci est distinct des `OPENID_GRAPH_SCOPES` utilisés pour la recherche de personnes et de groupes.
- `OPENID_USE_END_SESSION_ENDPOINT` : Active l'utilisation de l'endpoint de fin de session pour la déconnexion
- `OPENID_MAX_LOGOUT_URL_LENGTH` : Longueur maximale de l'URL avant d'utiliser `logout_hint` au lieu de `id_token_hint` pour éviter les erreurs de type URI trop longue (par défaut : 2000)
...
- Renouvelez régulièrement les secrets client
- Utilisez des paramètres de cookie sécurisés pour le stockage des jetons
- LibreChat rejette les jetons d'ID lorsqu'un jeton d'accès est requis pour la réutilisation de jetons porteurs (bearer-token) ; configurez le fournisseur pour émettre un véritable jeton d'accès pour l'audience et les portées demandées.
- Les réponses de jetons contenant des valeurs `expires_in` manquantes, invalides ou excessives sont limitées à la durée de vie sécurisée de l'identifiant au lieu d'étendre un jeton mis en cache au-delà de celle-ci.
- LibreChat résout les identifiants OpenID pour MCP et Microsoft Graph à partir de la session active, coordonne les rafraîchissements entre les réplicas et verrouille la déconnexion afin qu'un rafraîchissement tardif ne puisse pas recréer une session déconnectée.
