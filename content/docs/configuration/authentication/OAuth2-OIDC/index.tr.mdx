...
### OpenID JWT Kullanıcı Önbelleği [#openid-jwt-user-cache]

Yüksek istek oranına sahip dağıtımlar, OpenID JWT kimlik doğrulaması için çözümlenen kullanıcı belgesini beş saniyeliğine önbelleğe almak üzere `AUTH_USER_CACHE_MODE=on` ayarını kullanabilir. Bu, kimlik doğrulama belirteci ömrünü uzatmadan kısa süreli istek patlamaları sırasında tekrarlanan veritabanı okumalarını azaltır.

Önbellek varsayılan olarak kapalıdır ve Redis tarafından desteklenen `AUTH_USER_DOC` ad alanı ile `USE_REDIS=true` ayarını gerektirir. LibreChat, bu gereksinimler karşılanmadığında modu devre dışı bırakır ve bir uyarı günlüğe kaydeder. Önbelleğe alınan belgeler depolamadan önce temizlenir ve kullanıcı güncellendiğinde geçersiz kılınır.
