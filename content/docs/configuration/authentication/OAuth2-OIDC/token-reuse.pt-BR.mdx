...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Janela de recuperação curta para um refresh token rotacionado enquanto o LibreChat publica a sessão atualizada (padrão: 60000 ms / 1 minuto). Aceita expressões aritméticas. Aumente apenas quando a persistência lenta da sessão ou a publicação entre réplicas precisar de mais tempo.
...
- `GRAPH_API_SCOPES`: Escopos do Microsoft Graph separados por espaço solicitados ao resolver `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` em um servidor MCP definido em YAML. O padrão é `https://graph.microsoft.com/.default`; isso é separado dos `OPENID_GRAPH_SCOPES` usados para pesquisa de pessoas e grupos.
...
- O LibreChat rejeita tokens de ID quando um token de acesso é necessário para a reutilização de bearer-token; configure o provedor para emitir um token de acesso real para o público (audience) e escopos solicitados.
- Respostas de token com valores `expires_in` ausentes, inválidos ou excessivos são limitadas ao tempo de vida seguro da credencial, em vez de estender um token em cache além dele.
- O LibreChat resolve credenciais OpenID para MCP e Microsoft Graph a partir da sessão ativa, coordena atualizações entre réplicas e protege o logout para que uma atualização tardia não possa recriar uma sessão encerrada.
