...
1. Votre enregistrement d'application Azure doit disposer des autorisations Microsoft Graph API appropriées.
...
```
Voir [Configuration de la réutilisation des jetons](#advanced-token-reuse) ci-dessus pour plus de détails.
</Callout>
...
3. Votre enregistrement d'application Azure doit exposer et accorder une étendue d'API LibreChat, telle que `api://<client-id>/access_as_user`
...
Avant d'activer les variables SharePoint, assurez-vous que la configuration de réutilisation du jeton OpenID demande la portée (scope) de l'API de l'application LibreChat. Cela donne à Azure un jeton d'accès avec une audience d'application qui peut être utilisé comme assertion « on-behalf-of » pour l'échange de jetons SharePoint et Graph.
