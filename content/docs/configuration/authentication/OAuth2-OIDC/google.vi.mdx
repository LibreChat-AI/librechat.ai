---
title: Google
icon: Google
description: >-
  Tìm hiểu cách cấu hình LibreChat để sử dụng Google cho việc xác thực người
  dùng.
---

## Tạo một Google Application [#create-a-google-application]

- Truy cập: **[Google Cloud Console](https://cloud.google.com)** và mở `Console`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/a7d290ea-6031-43b3-b367-36ce00e46f20)

- Tạo một Dự án mới và đặt tên cho nó

![image](https://github.com/danny-avila/LibreChat/assets/138638445/ce71c9ca-7ddd-4021-9133-a872c64c20c4)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/8abbd41e-8332-4851-898d-9cddb373c527)

## Cấu hình ứng dụng Google [#google-application-configuration]

- Chọn dự án bạn vừa tạo và đi tới `APIs and Services`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/c6265582-2cf6-430f-ae51-1edbdd9f2c48)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/006e16ba-56b8-452d-b324-5f2d202637ab)

- Chọn `Credentials` và nhấp vào `CONFIGURE CONSENT SCREEN`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/e4285cbb-833f-4366-820d-addf04a2ad77)

- Chọn `External` sau đó nhấp vào `CREATE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/232d46c0-dd00-4637-b538-3ba3bdbdc0b2)

- Điền thông tin Ứng dụng của bạn

> Lưu ý: Bạn có thể lấy logo từ thư mục LibreChat của mình tại đây: `docs\assets\favicon_package\android-chrome-192x192.png`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/e6c4c8ec-2f02-4af5-9458-c72394d0b7c5)

- Cấu hình `App domain` và thêm `Developer contact information` của bạn, sau đó nhấp vào `SAVE AND CONTINUE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/6c2aa557-9b9b-412d-bc2b-76a0dc11f394)

- Configure the `Sopes`
    - Thêm `email`, `profile` và `openid`
    - Nhấp vào `UPDATE` và `SAVE AND CONTINUE`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/46af2fb9-8cfd-41c5-a763-814b308e45c3)

![image](https://github.com/danny-avila/LibreChat/assets/138638445/4e832970-d392-4c67-bb38-908a5c51660a)

- Nhấp vào `SAVE AND CONTINUE`
- Xem lại ứng dụng của bạn và quay lại bảng điều khiển

- Quay lại tab `Credentials`, nhấp vào `+ CREATE CREDENTIALS` và chọn `OAuth client ID`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/beef1982-55a3-4837-8e8c-20bad8d846ba)

- Chọn `Web application` và đặt tên cho nó

![image](https://github.com/danny-avila/LibreChat/assets/138638445/badde864-f6b5-468f-a72f-bac93326ffa5)

- Cấu hình `Authorized JavaScript origins`, bạn có thể thêm cả tên miền của mình và localhost nếu muốn.
    - Ví dụ cho localhost: `http://localhost:3080`
    - Ví dụ cho một tên miền: `https://example.com`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/f7e3763a-5f74-4850-8638-44f81693b9ac)

- Thêm một `Authorized redirect URIs` hợp lệ
    - Ví dụ cho localhost: `http://localhost:3080/oauth/google/callback`
    - Ví dụ cho một tên miền: `https://example.com/oauth/google/callback`
    - Nếu sử dụng Admin Panel: `https://example.com/api/admin/oauth/google/callback`

![image](https://github.com/danny-avila/LibreChat/assets/138638445/0db34b19-d780-4651-9c2f-d33e24a74d55)

## Cấu hình .env [#env-configuration]

- Nhấp vào `CREATE` và sao chép `Client ID` cùng `Client secret` của bạn

![image](https://github.com/danny-avila/LibreChat/assets/138638445/fa8572bf-f482-457a-a285-aec7d41af76b)

- Thêm chúng vào tệp `.env` của bạn:

```bash filename=".env"
DOMAIN_CLIENT=https://your-domain.com # use http://localhost:3080 if not using a custom domain
DOMAIN_SERVER=https://your-domain.com # use http://localhost:3080 if not using a custom domain

GOOGLE_CLIENT_ID=your_client_id
GOOGLE_CLIENT_SECRET=your_client_secret
GOOGLE_CALLBACK_URL=/oauth/google/callback
```

- Lưu tệp `.env`

> Lưu ý: Nếu sử dụng Docker, hãy chạy `docker compose up -d` để áp dụng các thay đổi cấu hình trong .env

## Các phiên Admin Panel [#admin-panel-sessions]

[Admin Panel](/docs/features/admin_panel) sử dụng cùng `GOOGLE_CLIENT_ID` và `GOOGLE_CLIENT_SECRET`, nhưng callback của nó là `${DOMAIN_SERVER}/api/admin/oauth/google/callback`. Luồng Google của nó yêu cầu quyền truy cập ngoại tuyến và sự đồng ý để token quản trị viên LibreChat đã hết hạn có thể được làm mới. LibreChat kiểm tra lại danh tính, trạng thái bị cấm, tên miền được phép, tenant và quyền truy cập quản trị viên hiện tại của người dùng trong mỗi lần làm mới.
