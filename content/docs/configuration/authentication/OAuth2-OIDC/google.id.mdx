...
- Tambahkan `Authorized redirect URIs` yang valid
    ...
    - Jika menggunakan Admin Panel: `https://example.com/api/admin/oauth/google/callback`
...
## Sesi Panel Admin [#admin-panel-sessions]

[Admin Panel](/docs/features/admin_panel) menggunakan `GOOGLE_CLIENT_ID` dan `GOOGLE_CLIENT_SECRET` yang sama, namun callback-nya adalah `${DOMAIN_SERVER}/api/admin/oauth/google/callback`. Alur Google-nya meminta akses offline dan persetujuan agar token admin LibreChat yang kedaluwarsa dapat diperbarui. LibreChat memeriksa kembali identitas pengguna, status banned, domain yang diizinkan, tenant, dan akses admin saat ini pada setiap pembaruan.
