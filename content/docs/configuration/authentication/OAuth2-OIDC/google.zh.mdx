...
- 添加一个有效的 `Authorized redirect URIs`
    ...
    - 如果使用 Admin Panel：`https://example.com/api/admin/oauth/google/callback`
...
## 管理面板会话 [#admin-panel-sessions]

[Admin Panel](/docs/features/admin_panel) 使用相同的 `GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_SECRET`，但其回调地址为 `${DOMAIN_SERVER}/api/admin/oauth/google/callback`。其 Google 流程会请求离线访问权限和许可，以便在 LibreChat 管理员令牌过期时进行刷新。LibreChat 会在每次刷新时重新检查用户的身份、封禁状态、允许的域名、租户以及当前的管理员访问权限。
