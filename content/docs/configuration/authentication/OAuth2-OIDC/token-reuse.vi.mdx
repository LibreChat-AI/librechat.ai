...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Cửa sổ khôi phục ngắn cho refresh token đã được xoay vòng trong khi LibreChat xuất bản phiên đã làm mới (mặc định: 60000 ms / 1 phút). Nó chấp nhận các biểu thức số học. Chỉ tăng giá trị này khi việc duy trì phiên chậm hoặc việc xuất bản trên nhiều bản sao (cross-replica) cần thêm thời gian.
...
- `GRAPH_API_SCOPES`: Các phạm vi Microsoft Graph được phân tách bằng dấu cách, được yêu cầu khi giải quyết `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` trong một máy chủ MCP được định nghĩa bằng YAML. Mặc định là `https://graph.microsoft.com/.default`; giá trị này tách biệt với `OPENID_GRAPH_SCOPES` được sử dụng cho việc tìm kiếm người dùng và nhóm.
...
- LibreChat từ chối các ID token khi cần có access token để tái sử dụng bearer-token; hãy cấu hình nhà cung cấp để cấp một access token thực cho audience và các scope được yêu cầu.
- Các phản hồi token có giá trị `expires_in` bị thiếu, không hợp lệ hoặc vượt quá mức sẽ bị giới hạn trong thời gian tồn tại an toàn của thông tin xác thực thay vì mở rộng token đã lưu trong bộ nhớ đệm vượt quá thời hạn đó.
- LibreChat giải quyết các thông tin xác thực OpenID cho MCP và Microsoft Graph từ phiên hoạt động, điều phối việc làm mới trên các bản sao và chặn đăng xuất để đảm bảo việc làm mới muộn không thể tạo lại phiên đã đăng xuất.
