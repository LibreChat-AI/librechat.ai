...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Breve ventana de recuperación para un token de actualización rotado mientras LibreChat publica la sesión actualizada (predeterminado: 60000 ms / 1 minuto). Acepta expresiones aritméticas. Auméntelo solo cuando la persistencia lenta de la sesión o la publicación entre réplicas necesiten más tiempo.
...
- `GRAPH_API_SCOPES`: Scopes de Microsoft Graph separados por espacios solicitados al resolver `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` en un servidor MCP definido en YAML. El valor predeterminado es `https://graph.microsoft.com/.default`; esto es independiente de los `OPENID_GRAPH_SCOPES` utilizados para la búsqueda de personas y grupos.
...
- LibreChat rechaza los tokens de ID cuando se requiere un token de acceso para la reutilización de tokens de portador (bearer-token); configure el proveedor para emitir un token de acceso real para la audiencia y los alcances solicitados.
- Las respuestas de tokens con valores `expires_in` faltantes, inválidos o excesivos se limitan a la vida útil segura de la credencial en lugar de extender un token en caché más allá de ella.
- LibreChat resuelve las credenciales OpenID para MCP y Microsoft Graph desde la sesión activa, coordina las actualizaciones entre réplicas y protege el cierre de sesión para que una actualización tardía no pueda recrear una sesión cerrada.
