...
- `Webhook` セクションにある `Active` というラベルのチェックボックスのチェックを外します。
...
- `.env` ファイル内の `Client ID` と `Client Secret` をコピーします。
...
> **注:** Docker を使用している場合は、`docker compose up -d` を実行して .env の設定変更を適用してください。
