...
title: Auth0
icon: Auth0
description: Konfiguriere Auth0 als OpenID Connect-Anbieter für LibreChat
---
...
Auth0 erlaubt keine `http://localhost` URLs in Produktionsanwendungen. Für die lokale Entwicklung/das Testen müssen Sie HTTPS verwenden. Sie können Dienste nutzen wie:
...
1. Auf dem **Einstellungen**-Tab Ihrer Anwendung:
2. Legen Sie **Allowed Callback URLs** fest:
   ```bash
...
3. **API konfigurieren:**
   ...
   - **Identifier**: `https://api.librechat.ai` (oder Ihr bevorzugter Identifier)
     - **Hinweis**: Dies ist lediglich eine eindeutige Kennung, keine tatsächliche URL. Sie muss nicht aufrufbar sein.
...
### Schritt 5: Konfigurationswerte zusammenstellen [#step-5-gather-configuration-values]

Im Bereich **Basic Information** Ihrer Auth0-Anwendung finden Sie:
...
Die in Auth0 angezeigte **Domain** enthält nicht das `https://`-Präfix. Sie müssen es bei der Konfiguration von `OPENID_ISSUER` hinzufügen.
...
## OPENID_AUDIENCE verstehen [#understanding-openid_audience]
...
<OptionTable
  options={[
    [
      'OPENID_AUDIENCE',
      'string',
      'Der Bezeichner Ihrer Auth0 API. Erforderlich bei Verwendung von OPENID_REUSE_TOKENS=true mit Auth0, um Probleme mit opaken Tokens zu vermeiden. Kommagetrennte Werte werden für die JWT-Validierung akzeptiert; Autorisierungsanfragen verwenden den ersten nicht leeren Wert.',
      'OPENID_AUDIENCE=https://api.librechat.ai',
    ],
  ]}
/>
...
1. Stellen Sie sicher, dass `OPENID_AUDIENCE` auf Ihren Auth0 API-Identifier gesetzt ist.
...
1. Debug-Logging aktivieren: `DEBUG_OPENID_REQUESTS=true`
