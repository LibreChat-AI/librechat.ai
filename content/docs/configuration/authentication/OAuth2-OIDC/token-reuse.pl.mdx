...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Krótkie okno odzyskiwania dla obróconego tokena odświeżania, podczas gdy LibreChat publikuje odświeżoną sesję (domyślnie: 60000 ms / 1 minuta). Akceptuje wyrażenia arytmetyczne. Zwiększ tę wartość tylko wtedy, gdy wolna trwałość sesji lub publikacja między replikami wymaga więcej czasu.
...
- `GRAPH_API_SCOPES`: Rozdzielone spacjami zakresy Microsoft Graph wymagane podczas rozwiązywania `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` w serwerze MCP zdefiniowanym w YAML. Wartością domyślną jest `https://graph.microsoft.com/.default`; jest to oddzielne od `OPENID_GRAPH_SCOPES` używanych do wyszukiwania osób i grup.
...
- LibreChat odrzuca tokeny ID, gdy token dostępu jest wymagany do ponownego użycia tokena bearer; skonfiguruj dostawcę tak, aby wydawał rzeczywisty token dostępu dla żądanej grupy odbiorców (audience) i zakresów (scopes)
- Odpowiedzi tokenów z brakującymi, nieprawidłowymi lub nadmiernymi wartościami `expires_in` są ograniczane do bezpiecznego czasu życia poświadczenia, zamiast przedłużać buforowany token poza ten czas.
- LibreChat rozpoznaje poświadczenia OpenID dla MCP oraz Microsoft Graph z aktywnej sesji, koordynuje odświeżanie między replikami i zabezpiecza wylogowanie, aby spóźnione odświeżenie nie mogło odtworzyć wylogowanej sesji.
