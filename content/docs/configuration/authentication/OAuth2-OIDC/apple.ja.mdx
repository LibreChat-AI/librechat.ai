...
### 3. 新しい App ID を作成する [#3-create-a-new-app-id]
...
### 3. "Sign in with Apple" を設定する [#3-configure-sign-in-with-apple]
...
4. **Next** をクリックし、**Register** をクリックします。
...
### 3. プライベートキーのダウンロード [#3-download-the-private-key]
...
> - `YOUR_TEAM_ID` および `YOUR_KEY_ID` を、Apple Developerアカウントのそれぞれの値に置き換えてください。
...
`.env` ファイルを更新した後、変更を適用するために LibreChat を再起動してください。
...
- **ドメインの検証に失敗しました:**
    ...
    - 設定時に入力したドメイン名にタイプミスがないか確認してください。

- **Docker 設定の問題:**
    - Dockerを使用している場合は、`.p8` ファイルが正しくマウントされていること、および `APPLE_PRIVATE_KEY_PATH` のパスがコンテナ内からアクセス可能であることを確認してください。
