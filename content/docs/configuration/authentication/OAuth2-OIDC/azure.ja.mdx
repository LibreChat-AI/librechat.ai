...
2. 特定の Graph API スコープ（`GroupMember.Read.All` など）には、管理者の同意が必要な場合があります。
...
## 上級編: SharePoint 統合 [#advanced-sharepoint-integration]
...
### SharePointの権限を追加する [#adding-sharepoint-permissions]

1. Azureアプリの登録で、**API permissions** に移動します。
2. **Add a permission** をクリックします

#### SharePoint アクセス用: [#for-sharepoint-access]
3. **SharePoint** を選択してください（Microsoft Graph ではありません）
