...
- Voeg een geldige `Authorized redirect URIs` toe
    ...
    - Indien je het Admin Panel gebruikt: `https://example.com/api/admin/oauth/google/callback`
...
## Admin Panel Sessies [#admin-panel-sessions]

Het [Admin Panel](/docs/features/admin_panel) gebruikt dezelfde `GOOGLE_CLIENT_ID` en `GOOGLE_CLIENT_SECRET`, maar de callback is `${DOMAIN_SERVER}/api/admin/oauth/google/callback`. De Google-flow hiervan vraagt om offline toegang en toestemming, zodat een verlopen LibreChat admin-token kan worden vernieuwd. LibreChat controleert bij elke vernieuwing opnieuw de identiteit van de gebruiker, de ban-status, het toegestane domein, de tenant en de huidige admin-toegang.
