...
icon: Authelia
description: >-
  LibreChat'i kullanıcı kimlik doğrulaması için Authelia kullanacak şekilde
  nasıl yapılandıracağınızı öğrenin.
---

- Şu komutu kullanarak bir client secret oluşturun:
  ```
  docker run --rm authelia/authelia:latest authelia crypto hash generate pbkdf2 --variant sha512 --random --random.length 72 --random.charset rfc3986
  ```
- Ardından `configuration.yml` dosyanızda oidc bölümüne aşağıdakileri ekleyin:
  ...
  ```
- Ardından Authelia'yı yeniden başlatın
