...
- `OPENID_REFRESH_BRIDGE_GRACE_MS`: Breve finestra di ripristino per un refresh token ruotato mentre LibreChat pubblica la sessione aggiornata (predefinito: 60000 ms / 1 minuto). Accetta espressioni aritmetiche. Aumentalo solo quando la persistenza lenta della sessione o la pubblicazione tra repliche richiedono più tempo.
...
- `GRAPH_API_SCOPES`: Ambiti Microsoft Graph separati da spazi richiesti durante la risoluzione di `{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` in un server MCP definito tramite YAML. Il valore predefinito è `https://graph.microsoft.com/.default`; questo è distinto da `OPENID_GRAPH_SCOPES` utilizzato per la ricerca di persone e gruppi.
...
- LibreChat rifiuta i token ID quando è richiesto un access token per il riutilizzo del bearer-token; configura il provider affinché rilasci un vero access token per l'audience e gli scope richiesti
- Le risposte dei token con valori `expires_in` mancanti, non validi o eccessivi vengono limitate alla durata sicura della credenziale invece di estendere un token memorizzato nella cache oltre tale limite.
- LibreChat risolve le credenziali OpenID per MCP e Microsoft Graph dalla sessione live, coordina gli aggiornamenti tra le repliche e protegge il logout in modo che un aggiornamento tardivo non possa ricreare una sessione disconnessa.
