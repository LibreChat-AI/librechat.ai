...
<OptionTable
  options={[
    ...
    ['VIOLATION_SCORE_TTL', 'integer', 'Quanto tempo uma pontuação de violação permanece ativa sem novas violações (em milissegundos). Cada nova violação reinicia a contagem regressiva, portanto, as pontuações diminuem após um período de inatividade em vez de se acumularem para sempre. Defina como 0 para nunca expirar as pontuações (comportamento legado).','VIOLATION_SCORE_TTL=1000 * 60 * 60'],
  ]}
/>
