...
### Generation Protocol Kullanıma Sunumu [#generation-protocol-rollout]

v2 nesil protokolü, mevcut Agent çalıştırmaları tarafından kullanılan dayanıklı kuyruk, kesinti, duraklatma/devam ettirme ve kurtarma garantilerini ekler. `GENERATION_PROTOCOL_VERSION` ayarlanmadığında Redis destekli dağıtımlar kasıtlı olarak v1 protokolünde kalır; böylece farklı LibreChat sürümlerini çalıştıran replikalar, kademeli bir yükseltme sırasında bir arada bulunabilir.

İki aşamalı bir dağıtım kullanın:

1. v2 uyumlu LibreChat imajını, `GENERATION_PROTOCOL_VERSION` değeri ayarlanmamış veya `1` olarak ayarlanmış **her** replikaya dağıtın.
2. Önceki görsele ait aktif nesillerin tamamlanmasını veya boşaltılmasını bekleyin.
3. `GENERATION_PROTOCOL_VERSION=2` değerini ayarlayın ve aynı imajı tekrar dağıtın.
...
<Callout type="warning" title="Kademeli dağıtım güvenliği">
  Her replika destekleyene kadar protokol v2'yi etkinleştirmeyin. v2 nesilleri başladıktan sonra, bu nesiller tükenene kadar v2 öncesi bir LibreChat imajına geri dönmeyin; eski replikalar Redis durumları üzerinde güvenli bir şekilde çalışamaz.
</Callout>

Bellek içi (in-memory) üretim akışları, kopyalar (replicas) arasında durum paylaşmaz ve varsayılan olarak v2 protokolünü kullanır, bu nedenle bu geçişi gerektirmezler.
