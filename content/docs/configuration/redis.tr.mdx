...
Ayar belirtilmediğinde veya `0` olarak ayarlandığında kapalıdır; `25` milisaniye önerilir ve `1000` üzerindeki değerler sınırlandırılır. Gruplama (batching), yapılandırılmış bir teslimat gecikmesi penceresine kadar ekleme yapabilir.

<Callout type="warning" title="Kademeli dağıtım güvenliği">
  Birleştirmeyi (coalescing) yalnızca her LibreChat kopyası toplu çerçeveleri (batch frames) destekledikten sonra etkinleştirin. Daha eski aboneler birleştirilmiş çerçeveleri düşürür.
</Callout>
