...
- [Protocolo de Geração Rollout](#generation-protocol-rollout)
...
### Implementação do Protocolo de Geração [#generation-protocol-rollout]

O protocolo de geração v2 adiciona a fila durável, interrupção, pausa/retomada e garantias de recuperação usadas pelas execuções atuais de Agentes. Implantações baseadas em Redis permanecem intencionalmente no protocolo v1 quando `GENERATION_PROTOCOL_VERSION` não está definido, para que réplicas executando diferentes versões do LibreChat possam coexistir durante uma atualização gradual (rolling upgrade).

Use um lançamento em duas etapas:

1. Implante a imagem do LibreChat compatível com v2 em **todas** as réplicas com `GENERATION_PROTOCOL_VERSION` não definido ou definido como `1`.
2. Aguarde a conclusão ou o esvaziamento das gerações ativas pertencentes à imagem anterior.
3. Defina `GENERATION_PROTOCOL_VERSION=2` e implemente a mesma imagem novamente.
...
<Callout type="warning" title="Segurança de implantação contínua">
  Não habilite o protocolo v2 até que todas as réplicas o suportem. Após o início das gerações v2, não reverta para uma imagem do LibreChat anterior à v2 até que essas gerações tenham sido drenadas; réplicas mais antigas não podem operar com segurança no seu estado Redis.
</Callout>

Fluxos de geração em memória não compartilham estado entre réplicas e usam o protocolo v2 por padrão, portanto, não requerem essa transição.
