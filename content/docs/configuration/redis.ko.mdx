...
### Generation Protocol 배포 [#generation-protocol-rollout]

v2 생성 프로토콜은 현재 Agent 실행에서 사용되는 내구성 있는 큐(durable queue), 중단(interrupt), 일시 중지/재개(pause/resume), 복구 보장 기능을 추가합니다. Redis 기반 배포는 `GENERATION_PROTOCOL_VERSION`이 설정되지 않은 경우 의도적으로 프로토콜 v1을 유지하므로, 롤링 업그레이드 중에 서로 다른 LibreChat 버전을 실행하는 복제본들이 공존할 수 있습니다.

2단계 롤아웃을 사용하세요:

1. `GENERATION_PROTOCOL_VERSION`이 설정되지 않았거나 `1`로 설정된 **모든** 복제본(replica)에 v2 지원 LibreChat 이미지를 배포하세요.
2. 이전 이미지에서 소유한 활성 생성 작업이 완료되거나 비워질 때까지 기다리십시오.
3. `GENERATION_PROTOCOL_VERSION=2`를 설정하고 동일한 이미지를 다시 배포하세요.
...
<Callout type="warning" title="롤링 배포 안전성">
  모든 복제본(replica)이 지원할 때까지 프로토콜 v2를 활성화하지 마십시오. v2 세대가 시작된 후에는 해당 세대가 모두 소진될 때까지 v2 이전의 LibreChat 이미지로 롤백하지 마십시오. 이전 버전의 복제본은 Redis 상태에서 안전하게 작동할 수 없습니다.
</Callout>

인메모리 생성 스트림은 복제본 간에 상태를 공유하지 않으며 기본적으로 프로토콜 v2를 사용하므로, 이 전환이 필요하지 않습니다.
