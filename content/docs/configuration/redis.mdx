...
title: Redis
icon: Redis
description: Setting up Redis for caching, session storage, and horizontal scaling in LibreChat
---
...
Configure deadline-bounded Redis heartbeats and TCP keepalive:
...
- `REDIS_PING_INTERVAL` is specified in seconds and controls the optional main ioredis heartbeat
- Dedicated pub/sub subscribers use their own default-on interval because an idle dead socket may otherwise remain undetected; set `REDIS_SUBSCRIBER_PING_INTERVAL=0` to disable that heartbeat
- Every enabled heartbeat uses `REDIS_PING_TIMEOUT` as its response deadline and reconnects the socket when a PING remains unanswered
- `REDIS_KEEP_ALIVE` configures the ioredis TCP socket independently from application-level PINGs
...
- Main-client PINGs are controlled by `REDIS_PING_INTERVAL` and disabled by default
- Dedicated subscriber PINGs are controlled by `REDIS_SUBSCRIBER_PING_INTERVAL` and default to 15 seconds
- Heartbeats use `REDIS_PING_TIMEOUT` to detect silently dead sockets and force reconnection
- TCP keepalive uses `REDIS_KEEP_ALIVE`, independently of application-level heartbeats
- Heartbeat timers are cleared on disconnect or close events
...
The setting defaults to `25` milliseconds when unset. Set it explicitly to `0` to disable coalescing; values above `1000` are capped. Batching can add up to one configured window of delivery latency and may lose the buffered tail if the publishing process exits before it flushes. Individual chunk frames are retained for protocol compatibility, so this setting reduces Redis script and round-trip work without changing the subscriber frame format. In-memory generation streams are unchanged.
