...
icon: SharePoint
description: >-
  Konfigurieren der SharePoint Online- und OneDrive for Business-Integration mit
  LibreChat
---
...
4. **Admin-Zugriff** auf Ihren Azure-Mandanten für App-Berechtigungen
...
2. Gehen Sie im linken Menü zu **API permissions**
...
Der „On-Behalf-Of“-Flow benötigt das anfängliche OpenID-Zugriffstoken, um auf Ihre LibreChat-App-API abzuzielen, nicht auf Microsoft Graph. Geben Sie einen API-Scope frei, damit Azure ein Token mit LibreChat als Zielgruppe (Audience) ausstellen kann.
...
Verwenden Sie den vollständigen `api://<client-id>/access_as_user` Scope-Wert in `OPENID_SCOPE` später in dieser Anleitung.
...
1. Nachdem Sie die Berechtigungen hinzugefügt haben, werden diese in einer Liste angezeigt
...
<Callout type="warning" title="OpenID-Bereichs-Zielgruppe">
...
2. Der SharePoint Online-Dateiauswähler öffnet sich in einem eingebetteten iframe
...
2. Die Fortschrittsanzeige zeigt die aktuelle Datei und den prozentualen Abschluss an
...
3. Der SharePoint-Dateiauswähler wird sich öffnen
...
- **Fehlerbehandlung**: Klare Meldungen bei Problemen
...
- Verwendet einen sicheren On-Behalf-Of-Flow für den Token-Austausch
...
4. Stellen Sie sicher, dass `SHAREPOINT_PICKER_SHAREPOINT_SCOPE` die vollständige Mandanten-URL verwendet, wie zum Beispiel `https://contoso.sharepoint.com/AllSites.Read`
...
**Lösungen**:
1. Überprüfen Sie, ob die Berechtigung `Files.Read.All` erteilt wurde
...
- Optimiert sowohl auf Geschwindigkeit als auch auf Stabilität

### Überlegungen zur Dateigröße [#file-size-considerations]

- Das Herunterladen großer Dateien kann einige Zeit in Anspruch nehmen
- Der Fortschrittsindikator hilft dabei, die Erwartungen der Benutzer zu steuern.
...
4. **Gründlich testen**: Überprüfen Sie die Funktionalität nach allen Änderungen an Azure AD
...
Für Organisationen mit spezifischen Anforderungen können Sie Scopes anpassen:
...
- Die SharePoint-Integration respektiert alle Richtlinien zu Informationsbarrieren.
...
- [Konfiguration des Datei-Uploads](/docs/configuration/librechat_yaml/object_structure/file_config)
