...
- 現在コンテキストウィンドウを使用しているトークンカテゴリを分類する、色分けされたメーター
- 対話、保持されたツール結果、Agentの指示、キャッシュされた入力、補完、ブランチの合計、および会話の合計に関するクリックスルーの内訳
- ツールごとの開示情報、および互換性のあるスナップショットで残りのコンテキスト負荷を予測できる場合のランウェイ（実行可能期間）の見積もり
- 権限を持つ管理者の場合、会話にサンプリングされたトレースが含まれている際に、アプリ内のアクティブな接続からLangfuseセッションへの直接リンクが表示されます。

詳細な内訳はデフォルトで折りたたまれているため、分類されたメーターはコンパクトに保たれます。ユーザーは合計が必要なときに展開することができ、LibreChatはその選択を後の会話のために記憶します。遅延ツール定義（Deferred tool definitions）は同じカテゴリ色を使用し、ハッチング処理が施されます。一方、カテゴリ構成が不明な見積もりは、単一の塗りつぶしのままとなります。

Tool traffic、キャッシュ、およびAgentの指示は、それらが含まれるカテゴリのサブセットであり、合計が二重に加算されることはありません。ランウェイ（推定残高）の予測は、連続するスナップショットが同じモデル、プロバイダー、Agent、コンテキスト予算、および固定オーバーヘッドを共有している場合にのみ表示されます。使用状況の内訳は、最終的なプライマリコールのキャッシュ分割や完了した出力を含め、メッセージや会話とともに永続化されます。再開されたチャットは、アクティブなストリーミングセッションのみに依存するのではなく、ブランチおよび合計の使用量/コストの詳細を保持します。Subagentの合計は、すべてのブランチを網羅していることが明示的にラベル付けされます。
