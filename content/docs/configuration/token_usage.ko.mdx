...
- 현재 컨텍스트 창을 사용 중인 토큰 범주를 구분하는 색상 코드 미터
- 대화, 유지된 도구 결과, 에이전트 지침, 캐시된 입력, 완료, 브랜치 합계 및 대화 합계에 대한 클릭형 분석
- 호환되는 스냅샷이 남은 컨텍스트 압력을 예측할 수 있을 때의 도구별 공개 및 런웨이(runway) 추정치
- 인증된 관리자의 경우, 활성 인앱 연결 내에 샘플링된 추적(trace)이 포함된 대화에 대해 Langfuse 세션으로 직접 연결되는 링크가 제공됩니다.

상세 내역은 기본적으로 접혀 있어 분류된 미터가 간결하게 유지됩니다. 사용자는 합계가 필요할 때 이를 펼칠 수 있으며, LibreChat은 이후 대화에서도 해당 선택을 기억합니다. 지연된 도구 정의(Deferred tool definitions)는 동일한 카테고리 색상에 빗금 처리가 적용되며, 알려진 카테고리 구성이 없는 추정치는 단일 색상으로 채워집니다.

Tool traffic, cache, 및 Agent instructions는 해당 범주에 포함되는 하위 집합이며, 전체 합계에 중복으로 계산되지 않습니다. Runway 추정치는 연속된 스냅샷이 동일한 모델, 제공자(provider), Agent, 컨텍스트 예산, 고정 오버헤드를 공유할 때만 나타납니다. 사용량 분석은 최종 기본 호출의 캐시 분할 및 완료된 출력을 포함하여 메시지 및 대화와 함께 유지됩니다. 다시 열린 채팅은 활성 스트리밍 세션에만 의존하는 대신 브랜치 및 전체 사용량/비용 세부 정보를 유지합니다. Subagent 합계는 모든 브랜치를 포함하는 것으로 명시적으로 표시됩니다.
