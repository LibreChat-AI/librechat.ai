...
`userIdField` accepts `id`, `email`, `username`, `name`, `openidId`, `samlId`, `ldapId`, `googleId`, `githubId`, `discordId`, `appleId`, or `facebookId`. If the selected field is missing for a user, the internal ID remains the trace `userId`.
...
See the [Langfuse fanout deployment reference](https://github.com/LibreChat-AI/LibreChat/blob/dev/otel/langfuse-fanout/README.md) for every gateway variable, custom destination setup, metrics, scaling, and Redis behavior.
...
By default, record input, output, metadata, and tool names remain hidden. Set `interface.traceViewer.showInputOutput: true` only when users may view system prompts, Agent instructions, and tool definitions that can appear in those fields. Set `interface.traceViewer.showToolNames: true` to show tool names without exposing tool input or output; this requires additional Langfuse observation reads, and only the names are retained for the response. `maxRecords`, `maxContentLength`, `requestsPerMinute`, and `requestTimeoutMs` bound each request; see [Config v1.3.16](/changelog/config_v1.3.16) for their defaults and ranges.
