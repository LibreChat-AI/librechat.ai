...
- Un indicatore a colori che separa le categorie di token che stanno attualmente utilizzando la finestra di contesto
- Una suddivisione cliccabile per dialogo, risultati degli strumenti conservati, istruzioni dell'Agent, input memorizzato nella cache, completamenti, totali dei rami e totali della conversazione
- Divulgazioni per singolo strumento e una stima dell'autonomia quando gli snapshot compatibili possono prevedere la pressione residua sul contesto
- Per gli amministratori autorizzati, un link diretto alla sessione Langfuse quando la conversazione presenta una traccia campionata nella connessione attiva in-app

Il dettaglio analitico è compresso per impostazione predefinita, in modo che il contatore categorizzato rimanga compatto. Gli utenti possono espanderlo quando necessitano dei totali e LibreChat memorizza tale scelta per le conversazioni successive. Le definizioni degli strumenti differite utilizzano lo stesso colore di categoria con un trattamento tratteggiato, mentre le stime senza una composizione di categoria nota rimangono a riempimento singolo.

Il traffico dei tool, la cache e le istruzioni dell'Agent sono sottoinsiemi delle loro categorie di appartenenza e non aumentano il totale una seconda volta. La stima dell'autonomia (runway) appare solo quando snapshot consecutivi condividono lo stesso modello, provider, Agent, budget di contesto e overhead fisso. I dettagli dell'utilizzo vengono salvati con i messaggi e le conversazioni, inclusi la suddivisione della cache della chiamata primaria finale e l'output completato. Le chat riaperte conservano i dettagli di utilizzo/costo totali e dei branch invece di basarsi solo sulla sessione di streaming attiva; i totali dei Subagent sono etichettati esplicitamente come comprensivi di tutti i branch.
