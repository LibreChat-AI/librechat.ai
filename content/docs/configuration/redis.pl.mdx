...
- [Wdrożenie protokołu generowania](#generation-protocol-rollout)
...
Protokół generowania v2 dodaje trwałą kolejkę, przerywanie, wstrzymywanie/wznawianie oraz gwarancje odzyskiwania używane przez obecne uruchomienia Agentów. Wdrożenia oparte na Redis celowo pozostają przy protokole v1, gdy `GENERATION_PROTOCOL_VERSION` nie jest ustawione, dzięki czemu repliki z różnymi wersjami LibreChat mogą współistnieć podczas aktualizacji kroczącej.

Zastosuj dwuetapowe wdrażanie:

1. Wdróż obraz LibreChat obsługujący v2 do **każdej** repliki z `GENERATION_PROTOCOL_VERSION` ustawionym na `1` lub nieustawionym.
2. Poczekaj, aż aktywne generacje należące do poprzedniego obrazu zostaną zakończone lub opróżnione.
3. Ustaw `GENERATION_PROTOCOL_VERSION=2` i ponownie wdróż ten sam obraz.
...
<Callout type="warning" title="Bezpieczeństwo wdrożenia kroczącego">
  Nie włączaj protokołu v2, dopóki każda replika go nie obsługuje. Po rozpoczęciu generacji v2 nie przywracaj starszej wersji obrazu LibreChat sprzed v2, dopóki te generacje nie zostaną wyczerpane; starsze repliki nie mogą bezpiecznie operować na swoim stanie w Redis.
</Callout>

Strumienie generowania w pamięci (in-memory) nie współdzielą stanu między replikami i domyślnie korzystają z protokołu v2, więc nie wymagają tego przejścia.
