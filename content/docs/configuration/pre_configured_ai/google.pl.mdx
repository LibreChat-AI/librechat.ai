...
- Gemini 3.7 Flash, Gemini 3.6 Flash, Gemini 3.5 Flash oraz Gemini 3.5 Flash-Lite są obsługiwane zarówno przez Google AI Studio, jak i Vertex AI. LibreChat przypisuje każdemu z nich okno kontekstowe o wielkości 1 048 576 tokenów.
- LibreChat domyślnie ustawia dla Gemini 3.7 Flash, Gemini 3.6 Flash oraz Gemini 3.5 Flash poziom myślenia `MEDIUM`, a dla Gemini 3.5 Flash-Lite poziom `MINIMAL`. Gemini 3.7 Flash odrzuca `MINIMAL`, więc LibreChat zastępuje go poziomem `LOW`, gdy wymagany jest ten poziom; inne obsługiwane jawne wartości `thinkingLevel` są zachowywane.
...
- Gemini 3.7 Flash nie obsługuje prefillu asystenta. LibreChat zapobiega ponownemu przesyłaniu edytowanych odpowiedzi asystenta jako nieobsługiwanej końcowej tury roli modelu.
- Wbudowane szacunki kosztów wykorzystują stawki wprowadzające Gemini 3.6/3.7 Flash obowiązujące do 31 grudnia 2026 r.: 0,75 USD za wejście, 3,75 USD za wyjście oraz 0,075 USD za wejście w pamięci podręcznej na milion tokenów.
