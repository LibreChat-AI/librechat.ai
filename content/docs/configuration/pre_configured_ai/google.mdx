...
- Use a multimodal Gemini model such as `gemini-2.5-flash` or `gemini-2.5-pro` if you want to support image or PDF inputs.
...
- Some older PaLM/Codey models may fail when `maxOutputTokens` is set to a high value. If you encounter this issue, try reducing the value through the conversation parameters.
