...
- Gemini 3.7 Flash, Gemini 3.6 Flash, Gemini 3.5 Flash ve Gemini 3.5 Flash-Lite, hem Google AI Studio hem de Vertex AI aracılığıyla desteklenmektedir. LibreChat, her birine 1.048.576 token'lık bir bağlam penceresi atar.
- LibreChat, Gemini 3.7 Flash, Gemini 3.6 Flash ve Gemini 3.5 Flash modelleri için varsayılan olarak `MEDIUM` düşünme seviyesini, Gemini 3.5 Flash-Lite için ise `MINIMAL` seviyesini kullanır. Gemini 3.7 Flash `MINIMAL` değerini reddettiği için, bu seviye talep edildiğinde LibreChat bunun yerine `LOW` değerini kullanır; desteklenen diğer açık `thinkingLevel` değerleri ise olduğu gibi korunur.
...
- Gemini 3.7 Flash, assistant prefill özelliğini desteklemez. LibreChat, düzenlenen asistan yanıtlarının desteklenmeyen bir sondaki model-rolü dönüşü olarak yeniden gönderilmesini engeller.
- Yerleşik maliyet tahminleri, 31 Aralık 2026'ya kadar Gemini 3.6/3.7 Flash giriş fiyatlarını kullanır: milyon token başına 0,75 $ giriş, 3,75 $ çıkış ve 0,075 $ önbelleğe alınmış giriş.
