...
- Internal OpenAI defaults include `gpt-6-astra` for the canonical OpenAI and Agents endpoints. LibreChat routes every Astra request through the Responses API because its tool calls are Responses-only, removes unsupported sampling parameters, and applies a 1,050,000-token context window with a 128,000-token maximum output. Astra is intentionally excluded from the Assistants, Azure Assistants, and Azure OpenAI default catalogs.
