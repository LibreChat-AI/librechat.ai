...
如果省略，LibreChat 会自动包含其已知支持的模型。Claude 4 及更新版本的默认设置使用 Bedrock 跨区域推理配置文件 ID，因为它们原始的 `anthropic.` 基础模型 ID 不支持按需 Converse 请求。LibreChat 在可用时使用 `global.` 配置文件，并在 Claude Opus 4.1 上使用 `us.` 配置文件。请使用 `BEDROCK_AWS_MODELS` 来选择您账户中可用的其他配置文件。

- Bedrock 上的 Claude Fable/Mythos 类模型仅支持推理配置（inference-profile）。请使用诸如 `global.anthropic.claude-fable-5` 的配置 ID，并在调用它们之前在 Bedrock 控制台或数据保留 API（Data Retention API）中启用 Anthropic 数据共享。

- Claude Opus 5 拥有 1M token 的上下文窗口和 128K token 的最大输出。除非明确禁用，否则默认启用自适应思考（Adaptive thinking）。当禁用思考功能时，LibreChat 会将 `xhigh` 或 `max` 努力程度限制为 `high`，以确保 Bedrock 能够接受该请求。

- Claude Sonnet 5 拥有 1M token 的上下文窗口和 128K token 的最大输出。LibreChat 将自适应思维（adaptive thinking）应用于基础模型、跨区域推理配置（例如 `us.anthropic.claude-sonnet-5`）以及裸应用推理配置 ID。关闭思维功能会发送一个明确的禁用设置，以便在重新加载对话时该选择能够保持。
...
- 默认最大文件大小为 **4.5 MB**。
- 当 `fileConfig` 未设置更小的限制时，Claude 4+ 的 PDF 文件以及 Amazon Nova 的 PDF 或 DOCX 文件最大可达 **32 MB**。
