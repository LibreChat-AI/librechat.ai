...
- Bedrock 上的 Claude Fable/Mythos 类模型仅支持推理配置文件（inference-profile）。请使用类似 `global.anthropic.claude-fable-5-1` 的配置文件 ID，并在调用它们之前在 Bedrock 控制台或数据保留 API（Data Retention API）中启用 Anthropic 数据共享。Fable 5.1 具有 1M token 的上下文窗口、128K token 的最大输出、自适应思维（adaptive thinking）以及提示词缓存（prompt-cache）支持。
