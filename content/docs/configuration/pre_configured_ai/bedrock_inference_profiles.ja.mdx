...
4. Bedrock endpointが設定された **LibreChat**（[AWS Bedrock Setup](/docs/configuration/pre_configured_ai/bedrock) を参照）
...
#### Step 2: カスタム推論プロファイルを作成する [#step-2-create-a-custom-inference-profile]
...
### 1. CloudWatch ロググループを作成する [#1-create-cloudwatch-log-group]
...
## 設定の確認 [#verifying-your-configuration]
...
| プロファイルが見つからない | リージョンが誤っている | プロファイルが同じリージョンで作成・使用されていることを確認してください |
...
- `agreementAvailability.status` を `AVAILABLE` に設定します
