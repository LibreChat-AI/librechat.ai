...
Jeśli zostanie pominięte, LibreChat automatycznie uwzględni znane obsługiwane modele. Domyślne ustawienia dla Claude 4 i nowszych wersji wykorzystują identyfikatory profili wnioskowania międzyregionalnego (cross-region inference) Bedrock, ponieważ ich podstawowe identyfikatory modeli bazowych `anthropic.` nie obsługują żądań Converse na żądanie. LibreChat używa profili `global.` tam, gdzie są dostępne, oraz `us.` dla Claude Opus 4.1. Użyj `BEDROCK_AWS_MODELS`, aby wybrać inny profil dostępny dla Twojego konta.

- Modele klasy Claude Fable/Mythos w Bedrock są dostępne wyłącznie poprzez profile wnioskowania (inference-profile). Użyj identyfikatora profilu, takiego jak `global.anthropic.claude-fable-5`, i włącz udostępnianie danych Anthropic w konsoli Bedrock lub za pomocą API Data Retention przed ich wywołaniem.

- Claude Opus 5 posiada okno kontekstowe o wielkości 1 mln tokenów oraz maksymalną długość wyjściową 128 tys. tokenów. Funkcja adaptive thinking jest włączona, chyba że zostanie wyraźnie wyłączona. Gdy funkcja thinking jest wyłączona, LibreChat ogranicza poziom `xhigh` lub `max` effort do `high`, aby Bedrock zaakceptował żądanie.

- Claude Sonnet 5 posiada okno kontekstowe o wielkości 1 mln tokenów oraz maksymalną długość wyjściową 128 tys. tokenów. LibreChat stosuje adaptacyjne myślenie (adaptive thinking) do modeli podstawowych, międzyregionalnych profili wnioskowania (na przykład `us.anthropic.claude-sonnet-5`) oraz identyfikatorów profili wnioskowania dla czystych aplikacji. Wyłączenie myślenia wysyła jawne ustawienie wyłączenia, dzięki czemu wybór ten zostaje zachowany po ponownym załadowaniu konwersacji.
...
- Domyślny maksymalny rozmiar pliku wynosi **4,5 MB**.
