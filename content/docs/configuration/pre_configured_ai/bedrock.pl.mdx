...
- Modele klasy Claude Fable/Mythos w Bedrock są dostępne wyłącznie poprzez profile wnioskowania (inference-profile). Użyj identyfikatora profilu, takiego jak `global.anthropic.claude-fable-5-1`, i włącz udostępnianie danych Anthropic w konsoli Bedrock lub za pomocą API Data Retention przed ich wywołaniem. Fable 5.1 posiada okno kontekstowe o wielkości 1 mln tokenów, maksymalną długość wyjściową 128 tys. tokenów, adaptacyjne myślenie (adaptive thinking) oraz obsługę pamięci podręcznej promptów (prompt-cache).
