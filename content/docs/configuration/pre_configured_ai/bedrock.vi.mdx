...
- Các mô hình thuộc lớp Claude Fable/Mythos trên Bedrock chỉ dành cho inference-profile. Hãy sử dụng ID hồ sơ như `global.anthropic.claude-fable-5-1` và bật tính năng chia sẻ dữ liệu Anthropic trong bảng điều khiển Bedrock hoặc Data Retention API trước khi gọi chúng. Fable 5.1 có cửa sổ ngữ cảnh 1M-token, đầu ra tối đa 128K-token, khả năng tư duy thích ứng (adaptive thinking) và hỗ trợ prompt-cache.
