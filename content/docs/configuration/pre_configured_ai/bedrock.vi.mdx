...
Nếu bỏ qua, LibreChat sẽ tự động bao gồm các model được hỗ trợ đã biết. Các mặc định của Claude 4 và mới hơn sử dụng ID hồ sơ suy luận đa vùng (cross-region inference profile IDs) của Bedrock vì các ID model nền tảng `anthropic.` thuần túy của chúng không hỗ trợ các yêu cầu Converse theo yêu cầu. LibreChat sử dụng các hồ sơ `global.` ở những nơi khả dụng và `us.` cho Claude Opus 4.1. Sử dụng `BEDROCK_AWS_MODELS` để chọn một hồ sơ khác khả dụng cho tài khoản của bạn.

- Các mô hình Claude Fable/Mythos-class trên Bedrock chỉ dành cho inference-profile. Hãy sử dụng profile ID như `global.anthropic.claude-fable-5` và bật tính năng chia sẻ dữ liệu Anthropic trong bảng điều khiển Bedrock hoặc Data Retention API trước khi gọi chúng.

- Claude Opus 5 có cửa sổ ngữ cảnh 1M-token và đầu ra tối đa 128K-token. Adaptive thinking được bật trừ khi bị vô hiệu hóa rõ ràng. Khi thinking bị vô hiệu hóa, LibreChat giới hạn nỗ lực `xhigh` hoặc `max` xuống `high` để Bedrock chấp nhận yêu cầu.

- Claude Sonnet 5 có cửa sổ ngữ cảnh 1M-token và đầu ra tối đa 128K-token. LibreChat áp dụng tư duy thích ứng (adaptive thinking) cho các ID profile suy luận nền tảng, profile suy luận liên vùng (ví dụ: `us.anthropic.claude-sonnet-5`) và profile suy luận ứng dụng thuần túy. Việc tắt tính năng tư duy sẽ gửi một thiết lập vô hiệu hóa rõ ràng để lựa chọn đó được duy trì khi cuộc trò chuyện được tải lại.
...
- Kích thước tệp tối đa mặc định là **4.5 MB**.
- Khi `fileConfig` không đặt giới hạn nhỏ hơn, các tệp PDF Claude 4+ và các tệp PDF hoặc DOCX của Amazon Nova có thể có dung lượng lên đến **32 MB**.
