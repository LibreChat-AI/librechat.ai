...
- Gemini 3.8 Flash、Gemini 3.7 Flash、Gemini 3.6 Flash、Gemini 3.5 Flash、および Gemini 3.5 Flash-Lite は、Google AI Studio と Google Cloud Gemini Enterprise Agent Platform の両方を通じてサポートされています。LibreChat はそれぞれに 1,048,576 トークンのコンテキストウィンドウを割り当てます。Gemini 3.8 Flash は最大 65,536 出力トークンをサポートします。
- LibreChatは、Gemini 3.8 Flash、Gemini 3.7 Flash、Gemini 3.6 Flash、およびGemini 3.5 Flashのデフォルトのthinkingを`MEDIUM`に設定し、Gemini 3.5 Flash-Liteを`MINIMAL`に設定します。Gemini 3.8および3.7 Flashは`MINIMAL`を拒否するため、そのレベルが要求された場合、LibreChatは`LOW`に置き換えます。その他のサポートされている明示的な`thinkingLevel`の値はそのまま保持されます。
...
- Gemini Flashモデルのバージョン3.6以降では、アシスタントのプレフィル（prefill）はサポートされていません。LibreChatでは、編集されたアシスタントの返信が、サポートされていない末尾のモデルロールのターンとして再送信されることを防ぎます。
- 組み込みのコスト見積もりでは、2026年12月31日までGemini 3.6/3.7/3.8 Flashの導入料金（入力100万トークンあたり$0.75、出力$3.75、キャッシュ入力$0.075）を使用します。2027年1月1日以降の料金は、それぞれ$1.50、$7.50、$0.15となります。
...
`vertexai` プロバイダーを使用して保存されたエージェントも、Agent Builder および実行時にこの共有 Google モデルカタログを使用します。デプロイメントで明示的な `vertexai` モデルカタログが提供されている場合、そのプロバイダー固有のリストが優先されます。それ以外の場合は、Vertex AI の選択と検証に `GOOGLE_MODELS` が使用されます。
