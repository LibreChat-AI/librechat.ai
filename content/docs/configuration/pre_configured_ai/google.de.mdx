...
Für den Google endpoint können Sie entweder die **Gemini API über Google AI Studio** oder die **Vertex AI API** verwenden.
...
Zum Beispiel können die folgenden aktuellen Modelle für die Gemini API konfiguriert werden:
...
- Gemini 3.8 Flash, Gemini 3.7 Flash, Gemini 3.6 Flash, Gemini 3.5 Flash und Gemini 3.5 Flash-Lite werden sowohl über Google AI Studio als auch über die Google Cloud Gemini Enterprise Agent Platform unterstützt. LibreChat weist jedem ein Kontextfenster von 1.048.576 Token zu; Gemini 3.8 Flash unterstützt bis zu 65.536 Ausgabe-Token.
- LibreChat setzt für Gemini 3.8 Flash, Gemini 3.7 Flash, Gemini 3.6 Flash und Gemini 3.5 Flash standardmäßig `MEDIUM` thinking ein, und für Gemini 3.5 Flash-Lite `MINIMAL`. Gemini 3.8 und 3.7 Flash lehnen `MINIMAL` ab, daher ersetzt LibreChat dies durch `LOW`, wenn diese Stufe angefordert wird; andere unterstützte explizite `thinkingLevel`-Werte bleiben erhalten.
- Für diese Modelle entfernt LibreChat nicht unterstützte Sampling- und Penalty-Parameter (`temperature`, `topP`, `topK`, `presencePenalty` und `frequencyPenalty`), ignoriert das ältere numerische `thinkingBudget` und verwendet stattdessen `thinkingLevel`.
- Gemini Flash-Modelle ab Version 3.6 unterstützen kein Assistant-Prefill. LibreChat verhindert, dass bearbeitete Assistant-Antworten als nicht unterstützter nachfolgender Model-Role-Turn erneut übermittelt werden.
- Die integrierten Kostenschätzungen verwenden die Einführungspreise für Gemini 3.6/3.7/3.8 Flash bis zum 31. Dezember 2026: 0,75 $ für Eingaben, 3,75 $ für Ausgaben und 0,075 $ für zwischengespeicherte Eingaben pro Million Token. Ab dem 1. Januar 2027 betragen die Preise 1,50 $, 7,50 $ bzw. 0,15 $.
...
Für Vertex AI verwenden Sie die Vertex-Modell-IDs, falls diese von der Gemini API abweichen:
...
Gespeicherte Agents, die den `vertexai` Provider verwenden, nutzen ebenfalls diesen gemeinsamen Google-Modellkatalog im Agent Builder und zur Laufzeit. Wenn die Bereitstellung einen expliziten `vertexai` Modellkatalog bereitstellt, hat diese providerspezifische Liste Vorrang; andernfalls wird `GOOGLE_MODELS` für die Auswahl und Validierung von Vertex AI verwendet.
