...
Falls weggelassen, fügt LibreChat seine bekannten unterstützten Modelle automatisch hinzu. Claude 4 und neuere Standardeinstellungen verwenden Bedrock Cross-Region Inference Profile-IDs, da ihre reinen `anthropic.` Foundation-Model-IDs keine On-Demand-Converse-Anfragen unterstützen. LibreChat verwendet `global.` Profile, sofern verfügbar, und `us.` für Claude Opus 4.1. Verwenden Sie `BEDROCK_AWS_MODELS`, um ein anderes für Ihr Konto verfügbares Profil auszuwählen.

- Claude Fable/Mythos-Klasse-Modelle auf Bedrock sind nur für Inference-Profile verfügbar. Verwenden Sie eine Profil-ID wie `global.anthropic.claude-fable-5` und aktivieren Sie die Anthropic-Datenfreigabe in der Bedrock-Konsole oder über die Data Retention API, bevor Sie diese aufrufen.

- Claude Opus 5 verfügt über ein Kontextfenster von 1 Mio. Token und eine maximale Ausgabe von 128.000 Token. Adaptives Denken ist standardmäßig aktiviert, sofern es nicht explizit deaktiviert wird. Wenn das Denken deaktiviert ist, begrenzt LibreChat den Aufwand `xhigh` oder `max` auf `high`, damit Bedrock die Anfrage akzeptiert.

- Claude Sonnet 5 verfügt über ein Kontextfenster von 1 Mio. Token und eine maximale Ausgabe von 128.000 Token. LibreChat wendet adaptives Denken auf Foundation-, regionsübergreifende Inference-Profile (zum Beispiel `us.anthropic.claude-sonnet-5`) sowie auf Bare-Application-Inference-Profile-IDs an. Das Deaktivieren der Denkfunktion sendet eine explizite Einstellung zur Deaktivierung, sodass diese Auswahl beim Neuladen einer Konversation beibehalten wird.
...
- Die standardmäßige maximale Dateigröße beträgt **4,5 MB**.
- Wenn `fileConfig` kein kleineres Limit festlegt, können Claude 4+ PDFs sowie Amazon Nova PDFs oder DOCX-Dateien bis zu **32 MB** groß sein.
