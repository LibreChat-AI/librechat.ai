...
Si omis, LibreChat inclut automatiquement ses modèles pris en charge connus. Les valeurs par défaut de Claude 4 et des versions ultérieures utilisent les ID de profil d'inférence inter-région de Bedrock, car leurs ID de modèle de base `anthropic.` ne prennent pas en charge les requêtes Converse à la demande. LibreChat utilise les profils `global.` lorsqu'ils sont disponibles et `us.` pour Claude Opus 4.1. Utilisez `BEDROCK_AWS_MODELS` pour sélectionner un profil différent disponible sur votre compte.

- Les modèles de classe Claude Fable/Mythos sur Bedrock sont uniquement destinés aux profils d'inférence. Utilisez un ID de profil tel que `global.anthropic.claude-fable-5` et activez le partage de données Anthropic dans la console Bedrock ou via l'API Data Retention avant de les invoquer.

- Claude Opus 5 dispose d'une fenêtre de contexte de 1M de jetons et d'une sortie maximale de 128K jetons. La réflexion adaptative (adaptive thinking) est activée sauf si elle est explicitement désactivée. Lorsque la réflexion est désactivée, LibreChat limite l'effort `xhigh` ou `max` à `high` afin que Bedrock accepte la requête.

- Claude Sonnet 5 dispose d'une fenêtre de contexte de 1M de jetons et d'une sortie maximale de 128K jetons. LibreChat applique une réflexion adaptative aux profils d'inférence de base, inter-régions (par exemple, `us.anthropic.claude-sonnet-5`) et aux profils d'inférence d'application nus. Désactiver la réflexion envoie un paramètre explicite de désactivation afin que ce choix persiste lors du rechargement d'une conversation.
