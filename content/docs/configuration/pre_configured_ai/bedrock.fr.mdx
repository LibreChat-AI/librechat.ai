...
- Les modèles Claude Fable/Mythos sur Bedrock sont uniquement destinés aux profils d'inférence. Utilisez un ID de profil tel que `global.anthropic.claude-fable-5-1` et activez le partage de données Anthropic dans la console Bedrock ou via l'API de rétention des données avant de les invoquer. Fable 5.1 dispose d'une fenêtre de contexte de 1M de jetons, d'une sortie maximale de 128K jetons, d'une réflexion adaptative et d'une prise en charge du cache de prompts.
