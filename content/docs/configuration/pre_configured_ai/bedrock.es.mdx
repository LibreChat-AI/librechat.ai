...
Si se omite, LibreChat incluye automáticamente sus modelos compatibles conocidos. Los valores predeterminados de Claude 4 y versiones posteriores utilizan IDs de perfil de inferencia entre regiones de Bedrock, ya que sus IDs de modelo base `anthropic.` no admiten solicitudes Converse bajo demanda. LibreChat utiliza perfiles `global.` cuando están disponibles y `us.` para Claude Opus 4.1. Utilice `BEDROCK_AWS_MODELS` para seleccionar un perfil diferente disponible en su cuenta.

- Los modelos Claude Fable/Mythos en Bedrock son solo para perfiles de inferencia. Utilice un ID de perfil como `global.anthropic.claude-fable-5` y habilite el intercambio de datos de Anthropic en la consola de Bedrock o en la Data Retention API antes de invocarlos.

- Claude Opus 5 tiene una ventana de contexto de 1M de tokens y una salida máxima de 128K tokens. El pensamiento adaptativo (adaptive thinking) está habilitado a menos que se deshabilite explícitamente. Cuando el pensamiento está deshabilitado, LibreChat limita el esfuerzo `xhigh` o `max` a `high` para que Bedrock acepte la solicitud.

- Claude Sonnet 5 tiene una ventana de contexto de 1M de tokens y una salida máxima de 128K tokens. LibreChat aplica pensamiento adaptativo a foundation, cross-region inference-profile (por ejemplo, `us.anthropic.claude-sonnet-5`) y bare application inference-profile IDs. Desactivar el pensamiento envía un ajuste explícito de desactivación para que esa elección persista cuando se recarga una conversación.
...
- El tamaño máximo predeterminado de archivo es **4.5 MB**.
- Cuando `fileConfig` no establece un límite menor, los archivos PDF de Claude 4+ y los archivos PDF o DOCX de Amazon Nova pueden tener un tamaño de hasta **32 MB**.
