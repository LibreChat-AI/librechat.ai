...
要验证您的推理配置文件是否被正确使用，请启用 AWS Bedrock 模型调用日志记录。
...
| 环境变量未解析 | 拼写错误或未设置 | 检查 `.env` 文件并确保变量名称与 `${VAR_NAME}` 匹配 |
| 拒绝访问 | 缺少 IAM 权限 | 为推理配置 ARN 添加 `bedrock:InvokeModel*` 权限 |
| 模型访问被拒绝 | 缺少模型协议或协议生效延迟 | 接受 Bedrock 模型协议并等待可用性生效 |
| 未找到配置 | 区域错误 | 确保在同一区域创建/使用配置 |
...
在调试 LibreChat 映射之前，请先检查底层模型的可用性：
...
如果缺少协议，请在 Bedrock 控制台中接受模型协议，或使用具有管理 Bedrock 模型协议和 Marketplace 订阅权限的 AWS 主体进行操作。待其状态变为 `AVAILABLE` 后，请等待几分钟，然后重试调用应用程序推理配置文件。
