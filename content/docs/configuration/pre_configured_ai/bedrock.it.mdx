...
- I modelli Claude Fable/Mythos su Bedrock sono esclusivamente basati su inference-profile. Utilizza un ID profilo come `global.anthropic.claude-fable-5-1` e abilita la condivisione dei dati Anthropic nella console Bedrock o tramite la Data Retention API prima di invocarli. Fable 5.1 dispone di una finestra di contesto da 1M di token, un output massimo di 128K token, adaptive thinking e supporto per il prompt-cache.
