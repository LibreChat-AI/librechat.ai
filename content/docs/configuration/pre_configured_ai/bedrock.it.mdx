...
Se omesso, LibreChat include automaticamente i suoi modelli supportati noti. I valori predefiniti di Claude 4 e versioni successive utilizzano gli ID dei profili di inferenza cross-region di Bedrock, poiché i loro ID di modello di base `anthropic.` non supportano le richieste Converse on-demand. LibreChat utilizza i profili `global.` dove disponibili e `us.` per Claude Opus 4.1. Utilizza `BEDROCK_AWS_MODELS` per selezionare un profilo diverso disponibile per il tuo account.

- I modelli Claude Fable/Mythos-class su Bedrock sono esclusivamente per inference-profile. Utilizza un ID profilo come `global.anthropic.claude-fable-5` e abilita la condivisione dei dati Anthropic nella console Bedrock o tramite la Data Retention API prima di invocarli.

- Claude Opus 5 ha una finestra di contesto da 1M di token e un output massimo di 128K token. L'Adaptive thinking è abilitato a meno che non venga esplicitamente disabilitato. Quando il thinking è disabilitato, LibreChat limita `xhigh` o `max` effort a `high` affinché Bedrock accetti la richiesta.

- Claude Sonnet 5 ha una finestra di contesto da 1 milione di token e un output massimo di 128.000 token. LibreChat applica il pensiero adattivo (adaptive thinking) ai foundation, ai cross-region inference-profile (ad esempio, `us.anthropic.claude-sonnet-5`) e ai bare application inference-profile ID. Disattivare il pensiero invia un'impostazione esplicita di disabilitazione, in modo che la scelta persista quando una conversazione viene ricaricata.
...
- La dimensione massima predefinita del file è **4.5 MB**.
- Quando `fileConfig` non imposta un limite inferiore, i PDF per Claude 4+ e i file PDF o DOCX per Amazon Nova possono pesare fino a **32 MB**.
