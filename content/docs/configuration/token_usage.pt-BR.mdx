...
- Um medidor codificado por cores que separa as categorias de tokens que estão utilizando a janela de contexto no momento
- Uma análise detalhada clicável para diálogo, resultados de ferramentas retidos, instruções do Agent, entrada em cache, conclusões, totais de ramificações e totais de conversas
- Divulgações por ferramenta e uma estimativa de execução quando snapshots compatíveis puderem projetar a pressão de contexto restante
- Para administradores autorizados, um link direto para a sessão do Langfuse quando a conversa tiver um rastreamento amostrado na conexão ativa no aplicativo

O detalhamento está recolhido por padrão para que o medidor categorizado permaneça compacto. Os usuários podem expandi-lo quando precisarem dos totais e o LibreChat memoriza essa escolha para conversas futuras. Definições de ferramentas adiadas usam a mesma cor de categoria com um tratamento hachurado, enquanto estimativas sem uma composição de categoria conhecida permanecem com um preenchimento único.

O tráfego de ferramentas, o cache e as instruções do Agent são subconjuntos de suas categorias contidas e não aumentam o total uma segunda vez. A estimativa de execução aparece apenas quando snapshots consecutivos compartilham o mesmo modelo, provedor, Agent, orçamento de contexto e overhead fixo. Os detalhamentos de uso são persistidos com mensagens e conversas, incluindo a divisão de cache da chamada primária final e a saída concluída. Chats reabertos mantêm detalhes de ramificação e uso/custo total em vez de depender apenas da sessão de streaming ativa; os totais de Subagent são explicitamente rotulados como abrangendo todas as ramificações.
