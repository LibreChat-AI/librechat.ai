...
<OptionTable
  options={[
    ...
    ['VIOLATION_SCORE_TTL', 'integer', '새로운 위반 사항 없이 위반 점수가 유지되는 시간(밀리초 단위)입니다. 새로운 위반이 발생할 때마다 카운트다운이 재설정되므로, 점수는 무한히 누적되지 않고 일정 기간 활동이 없으면 감소합니다. 점수가 만료되지 않게 하려면 0으로 설정하십시오(레거시 동작).','VIOLATION_SCORE_TTL=1000 * 60 * 60'],
  ]}
/>
