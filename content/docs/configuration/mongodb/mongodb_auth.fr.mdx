...
icon: MongoDB
description: >-
  Configurez l'authentification sur votre mongodb Docker avec le fichier
...
- Bien que les étapes décrites puissent également s'appliquer aux environnements Docker Desktop, ou à des configurations sans Docker, avec MongoDB local ou d'autres conteneurs, les détails spécifiques à ces scénarios ne sont pas fournis.
...
> Remarque : Le flag `-d` détache l'instance de terminal actuelle pendant que le conteneur s'exécute en arrière-plan. Si vous souhaitez voir les sorties de log de mongodb, omettez-le et continuez dans un terminal séparé.
...
Optionnel : Pendant que nous y sommes, nous pouvons désactiver la télémétrie pour mongodb si souhaité, ce qui correspond à des données d'utilisation anonymes collectées et envoyées périodiquement à MongoDB :
...
:warning: **Important :** si vous utilisez `mongo-express` pour gérer votre base de données, vous avez besoin des autorisations supplémentaires pour que le service `mongo-express` s'exécute correctement :
