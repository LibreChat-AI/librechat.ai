...
### 创建 MongoDB Atlas 账户 [#create-a-mongodb-atlas-account]
...
### 设置数据库凭据 [#set-up-database-credentials]
...
### 获取您的连接字符串 [#get-your-connection-string]
...
5. 复制提供的连接字符串。
6. 将连接字符串中的 `<db_username>` 和 `<db_password>` 替换为您在凭据步骤中设置的用户名和密码。请移除密码周围的 `<>` 字符。
