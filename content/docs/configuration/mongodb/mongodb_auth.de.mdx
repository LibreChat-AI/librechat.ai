...
### Erstellen eines Admin-Benutzers [#creating-an-admin-user]
...
Sobald es läuft, rufen wir das Terminal des Containers auf und führen `mongosh` aus:
...
Optional: Während wir hier sind, können wir bei Bedarf die Telemetrie für MongoDB deaktivieren, bei der es sich um anonyme Nutzungsdaten handelt, die regelmäßig gesammelt und an MongoDB gesendet werden:
...
:warning: **Wichtig:** Wenn Sie `mongo-express` zur Verwaltung Ihrer Datenbank verwenden, benötigen Sie die zusätzlichen Berechtigungen, damit der `mongo-express`-Dienst korrekt ausgeführt werden kann:
...
Und beenden Sie den laufenden Container:
...
### Aktivieren der Authentifizierung und Erstellen eines Benutzers mit `readWrite`-Zugriff [#enabling-authentication-and-creating-a-user-with-readwrite-access]
...
Ich hatte ein Problem, bei dem der neu erstellte Benutzer nach der Erstellung nicht gespeichert wurde. Um dies zu beheben, habe ich die Schritte einfach wiederholt, um sicherzustellen, dass er erstellt wurde. Hier sind sie der Einfachheit halber für dich:
...
### Aktualisieren Sie die `MONGO_URI`, um die neuen Anmeldedaten zu verwenden [#update-the-mongo_uri-to-use-the-new-credentials]
...
Falls bei dir Authentifizierungsfehler auftreten, führe den letzten Teil von Schritt 2 erneut aus. Ich bin mir nicht sicher, warum es manchmal hakt, aber nach ein paar Versuchen wird es funktionieren.
