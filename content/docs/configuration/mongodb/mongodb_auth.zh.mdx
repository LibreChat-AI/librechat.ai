...
## 第 1 步：创建管理员用户 [#step-1-creating-an-admin-user]
...
:warning: **重要：** 如果您正在使用 `mongo-express` 来[管理您的数据库（指南在此）](../../features/manage_your_database.md)，您需要额外的权限才能使 `mongo-express` 服务正常运行：
...
## 步骤 2：启用身份验证并创建具有 `readWrite` 权限的用户 [#step-2-enabling-authentication-and-creating-a-user-with-readwrite-access]
...
## 步骤 3：更新 `MONGO_URI` 以使用新凭据 [#step-3-update-the-mongo_uri-to-use-the-new-credentials]
