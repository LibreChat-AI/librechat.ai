...
### 启用身份验证并创建具有 `readWrite` 访问权限的用户 [#enabling-authentication-and-creating-a-user-with-readwrite-access]

我们现在必须创建/编辑 `docker-compose.override.yml` 文件，以便为我们的 mongodb 容器启用身份验证。你可以使用此配置来启动或参考：
...
现在我们将创建用于 MongoDB 连接字符串的实际凭据，该凭据将仅限于对 "LibreChat" 数据库的读/写访问权限。与之前一样，请将示例替换为您所需的凭据：
...
再次使用 `exit` 退出 Mongosh/容器终端，并关闭容器：
...
最后，我们将新创建的凭据及其连接字符串添加到 `docker-compose.override.yml` 文件中 `api` 服务下：
