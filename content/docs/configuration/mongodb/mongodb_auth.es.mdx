...
## Paso 1: Crear un usuario administrador [#step-1-creating-an-admin-user]
...
:warning: **Importante:** si está utilizando `mongo-express` para [gestionar su base de datos (guía aquí)](../../features/manage_your_database.md), necesita permisos adicionales para que el servicio `mongo-express` se ejecute correctamente:
...
## Paso 2: Habilitar la autenticación y crear un usuario con acceso `readWrite` [#step-2-enabling-authentication-and-creating-a-user-with-readwrite-access]
...
## Paso 3: Actualizar la `MONGO_URI` para usar las nuevas credenciales [#step-3-update-the-mongo_uri-to-use-the-new-credentials]
...
Si tienes errores de Authentication, ejecuta la última parte del Paso 2 de nuevo. No estoy seguro de por qué es caprichoso, pero funcionará después de algunos intentos.
