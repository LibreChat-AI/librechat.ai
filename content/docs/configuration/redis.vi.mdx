...
Cài đặt này sẽ bị tắt khi không được thiết lập hoặc được đặt thành `0`, khuyến nghị nên để là `25` mili giây và các giá trị trên `1000` sẽ bị giới hạn. Việc xử lý theo lô (batching) có thể làm tăng thêm độ trễ phân phối lên tới một khoảng thời gian đã cấu hình.

<Callout type="warning" title="An toàn triển khai cuốn chiếu">
  Chỉ kích hoạt tính năng gộp (coalescing) sau khi mọi bản sao LibreChat đều hỗ trợ batch frames. Các subscriber cũ hơn sẽ loại bỏ các frame đã được gộp.
</Callout>
