...
- [Triển khai Giao thức Tạo (Generation Protocol Rollout)](#generation-protocol-rollout)
...
Giao thức thế hệ v2 bổ sung hàng đợi bền vững (durable queue), ngắt, tạm dừng/tiếp tục và các đảm bảo khôi phục được sử dụng bởi các lần chạy Agent hiện tại. Các triển khai dựa trên Redis cố ý giữ nguyên giao thức v1 khi `GENERATION_PROTOCOL_VERSION` không được thiết lập để các bản sao chạy các phiên bản LibreChat khác nhau có thể cùng tồn tại trong quá trình nâng cấp luân phiên (rolling upgrade).

Sử dụng quy trình triển khai hai giai đoạn:

1. Triển khai image LibreChat hỗ trợ v2 cho **mọi** bản sao (replica) có `GENERATION_PROTOCOL_VERSION` chưa được thiết lập hoặc được đặt thành `1`.
2. Đợi các thế hệ đang hoạt động thuộc về hình ảnh trước đó hoàn tất hoặc giải phóng.
3. Đặt `GENERATION_PROTOCOL_VERSION=2` và triển khai lại cùng một image đó.
...
<Callout type="warning" title="An toàn triển khai cuốn chiếu">
  Không bật protocol v2 cho đến khi mọi bản sao (replica) đều hỗ trợ nó. Sau khi các thế hệ v2 bắt đầu, không được quay lại phiên bản LibreChat tiền-v2 cho đến khi các thế hệ đó đã được giải phóng; các bản sao cũ hơn không thể vận hành an toàn trên trạng thái Redis của chúng.
</Callout>

Các luồng tạo nội dung trong bộ nhớ (in-memory generation streams) không chia sẻ trạng thái giữa các bản sao và sử dụng giao thức v2 theo mặc định, vì vậy chúng không yêu cầu quá trình chuyển đổi này.
