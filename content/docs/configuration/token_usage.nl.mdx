...
- Een kleurgecodeerde meter die de tokencategorieën scheidt die momenteel het contextvenster gebruiken
- Een doorklik-overzicht voor dialoog, behouden toolresultaten, Agent-instructies, gecachte input, voltooiingen, vertakkingstotalen en gesprekstotalen
- Openbaringen per tool en een schatting van de resterende ruimte wanneer compatibele snapshots de resterende contextdruk kunnen projecteren
- Voor geautoriseerde beheerders, een directe Langfuse-sessielink wanneer het gesprek een gesamplede trace heeft in de actieve in-app verbinding

De gedetailleerde uitsplitsing is standaard ingeklapt, zodat de gecategoriseerde meter compact blijft. Gebruikers kunnen deze uitvouwen wanneer ze de totalen nodig hebben en LibreChat onthoudt die keuze voor latere gesprekken. Uitgestelde tooldefinities gebruiken dezelfde categoriekleur met een gearceerde weergave, terwijl schattingen zonder bekende categoriesamenstelling als een enkel vlak worden weergegeven.

Tool traffic, cache en Agent-instructies zijn subsets van hun overkoepelende categorieën en verhogen het totaal niet een tweede keer. De runway-schatting verschijnt alleen wanneer opeenvolgende snapshots hetzelfde model, dezelfde provider, Agent, contextbudget en vaste overhead delen. Gebruiksoverzichten worden opgeslagen bij berichten en gesprekken, inclusief de cache-split van de uiteindelijke primaire aanroep en de voltooide output. Heropende chats behouden details over vertakkingen en totaalgebruik/kosten in plaats van alleen te vertrouwen op de actieve streamingsessie; totalen van Subagents worden expliciet gelabeld als zijnde inclusief alle vertakkingen.
