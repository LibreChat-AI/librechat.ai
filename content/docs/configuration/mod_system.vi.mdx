...
<OptionTable
  options={[
    ...
    ['BAN_INTERVAL', 'integer', 'Người dùng sẽ bị cấm mỗi khi điểm số của họ đạt/vượt qua ngưỡng khoảng.','BAN_INTERVAL=20'],
  ]}
/>
...
<OptionTable
  options={[    
    ['LOGIN_VIOLATION_SCORE', 'integer', 'Điểm số cho các vi phạm đăng nhập.','LOGIN_VIOLATION_SCORE=1'],
...
<OptionTable
  options={[    
    ...
    ['CONCURRENT_MESSAGE_MAX', 'integer', 'Số lượng tin nhắn tối đa mà người dùng có thể gửi trong mỗi yêu cầu.','CONCURRENT_MESSAGE_MAX=2'],
  ]}
/>
...
<OptionTable
  options={[    
    ...
    ['IMPORT_IP_MAX', 'integer', 'Số lượng nhập hội thoại tối đa mà một IP có thể thực hiện trong mỗi `IMPORT_IP_WINDOW`.','IMPORT_IP_MAX=100'],
...
<OptionTable
  options={[    
    ['LIMIT_IMPORT_USER', 'boolean', 'Liệu có giới hạn số lượng nhập hội thoại mà một người dùng có thể thực hiện trong mỗi `IMPORT_USER_WINDOW` hay không.','LIMIT_IMPORT_USER=false'],
...
<OptionTable
  options={[    
    ['ILLEGAL_MODEL_REQ_SCORE', 'integer', 'Điểm số cho các yêu cầu mô hình bất hợp pháp.','ILLEGAL_MODEL_REQ_SCORE=5'],
  ]}
/>
