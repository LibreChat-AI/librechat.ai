...
<OptionTable
  options={[
    ...
    ['VIOLATION_SCORE_TTL', 'integer', 'Thời gian tồn tại của một điểm vi phạm mà không có vi phạm mới (tính bằng mili giây). Mỗi vi phạm mới sẽ khởi động lại bộ đếm ngược, vì vậy điểm số sẽ giảm dần sau một khoảng thời gian yên tĩnh thay vì tích lũy mãi mãi. Đặt thành 0 để điểm số không bao giờ hết hạn (hành vi cũ).','VIOLATION_SCORE_TTL=1000 * 60 * 60'],
  ]}
/>
