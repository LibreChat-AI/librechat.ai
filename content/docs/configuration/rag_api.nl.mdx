...
Bij de standaard Docker-installatie wordt het `.env` bestand gedeeld tussen LibreChat en de RAG API. Definieer de RAG-variabelen in datzelfde bestand. De volledige lijst staat in de [RAG API README](https://github.com/LibreChat-AI/rag-api/blob/main/README.md).
</Callout>
...
Docker gebruikt standaard de "lite" image van de RAG API (`registry.librechat.ai/librechat-ai/librechat-rag-api-dev-lite:latest`), die alleen remote embeddings ondersteunt van OpenAI of een remote HuggingFace/Ollama-service die je hebt geconfigureerd.

Voor lokale embeddings, wijzig de image in het compose-bestand naar de volledige build, `registry.librechat.ai/librechat-ai/librechat-rag-api-dev:latest`. Breng deze wijziging aan in je [Docker Compose Override File](/docs/configuration/docker_override). Zie `docker-compose.override.yml.example` in de root van het project voor een voorbeeld.
...
Een niet-container installatie is meer hands-on. Volg de instructies in de [RAG API repo](https://github.com/LibreChat-AI/rag-api/).
...
Voor de volledige lijst met variabelen en hun beschrijvingen, zie de [RAG API repo](https://github.com/LibreChat-AI/rag-api/).
