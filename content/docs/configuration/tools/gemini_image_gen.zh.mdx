...
2. 在你的 `.env` 文件中设置 `GEMINI_API_KEY` 环境变量：
...
配置凭据后，请重启 LibreChat 并将 **Gemini Image Tools** 添加到智能体的 **Tools** 列表中。
...
| `gemini-3-pro-image-preview` | 更高质量，生成更详细 |

## 功能特性 [#features]

### 核心功能 [#core-capabilities]

- **文本转图像生成**：根据详细的文本描述创建图像
- **图像上下文支持**：使用现有图像作为新生成的上下文/灵感
- **图像编辑**：基于对现有图像的修改来生成新图像
- **安全过滤**：内置内容安全机制，并提供用户友好的错误提示
...
当你有一张现有的图片并希望以此为灵感创作时：

1. 在 `image_ids` 参数中引用图像 ID
2. 描述您的需求：“为此风景场景创建一个冬季版本，包含覆盖着积雪的树木和结冰的湖泊”
...
| "No image was generated" | 尝试不同的提示词或简化您的请求 |
...
- **Gemini API**: 查看 [Google AI Studio](https://aistudio.google.com/) 以获取当前限制
