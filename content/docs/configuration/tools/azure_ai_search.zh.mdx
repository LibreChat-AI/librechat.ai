...
向智能体提问，该问题应由您的 Azure AI Search 索引来回答。如果工具返回的内容过多，请调整 `AZURE_AI_SEARCH_SEARCH_OPTION_TOP` 和 `AZURE_AI_SEARCH_SEARCH_OPTION_SELECT`。

![image](/images/azure-ai-search/chat_with_azure_ai_search.png)

## 可选 [#optional]

以下是搜索时非必需但可以作为参数指定的配置值。

如果担心搜索结果数据过大并超过提示词（prompt）大小限制，请考虑通过使用 AZURE_AI_SEARCH_SEARCH_OPTION_TOP 和 AZURE_AI_SEARCH_SEARCH_OPTION_SELECT 来减小搜索结果数据的大小。
