...
调试日志对于开发工作和修复问题至关重要。如果您在运行 LibreChat 时遇到任何问题，请尽可能复现该问题，并附上您在 `./api/logs/debug-%DATE%.log` 中找到的日志，**[报告该问题](https://github.com/LibreChat-AI/LibreChat/issues)**。
...
默认情况下，JSON 字符串长度会被截断为 255 个字符。您可以通过以下环境变量进行配置：
