...
- Pengukur berkode warna yang memisahkan kategori token yang saat ini menggunakan jendela konteks
- Rincian klik untuk dialog, hasil alat yang dipertahankan, instruksi Agent, input yang di-cache, penyelesaian, total cabang, dan total percakapan
- Pengungkapan per-alat dan estimasi durasi saat snapshot yang kompatibel dapat memproyeksikan tekanan konteks yang tersisa
- Bagi administrator yang berwenang, tautan sesi Langfuse langsung tersedia saat percakapan memiliki jejak (trace) yang diambil sampelnya dalam koneksi dalam aplikasi yang aktif.

Rincian mendalam diciutkan secara default agar pengukur yang dikategorikan tetap ringkas. Pengguna dapat meluaskannya saat mereka membutuhkan totalnya dan LibreChat akan mengingat pilihan tersebut untuk percakapan selanjutnya. Definisi alat yang ditangguhkan menggunakan warna kategori yang sama dengan perlakuan berarsir, sementara estimasi tanpa komposisi kategori yang diketahui tetap berupa isian tunggal.

Lalu lintas alat (tool traffic), cache, dan instruksi Agen merupakan bagian dari kategori induknya dan tidak menambah total untuk kedua kalinya. Estimasi durasi (runway) hanya muncul ketika snapshot berturut-turut menggunakan model, penyedia, Agen, anggaran konteks, dan overhead tetap yang sama. Rincian penggunaan disimpan bersama pesan dan percakapan, termasuk pembagian cache panggilan utama terakhir dan output yang telah selesai. Objek chat yang dibuka kembali tetap menyimpan detail penggunaan/biaya cabang dan total alih-alih hanya mengandalkan sesi streaming yang aktif; total Subagent secara eksplisit diberi label sebagai cakupan untuk semua cabang.
