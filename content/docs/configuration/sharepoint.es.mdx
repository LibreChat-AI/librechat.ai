...
4. **Acceso de administrador** a su tenant de Azure para los permisos de la aplicación
...
### Paso 2: Exponer y otorgar un alcance de API de LibreChat [#step-2-expose-and-grant-a-librechat-api-scope]

El flujo "on-behalf-of" necesita el token de acceso inicial de OpenID para dirigirse a la API de su aplicación LibreChat, no a Microsoft Graph. Exponga un ámbito (scope) de API para que Azure pueda emitir un token con LibreChat como audiencia.
...
4. Nombre el scope `access_as_user`
...
<Callout type="info" title="Intercambio de token de Userinfo">
...
3. El usuario navega y selecciona archivos utilizando la interfaz familiar de SharePoint
...
2. Seleccione "From SharePoint" en el menú
3. El selector de archivos de SharePoint se abrirá
4. Explora y selecciona archivos según sea necesario
5. Haz clic en "Select" para comenzar la descarga

### Funciones disponibles [#features-available]

- **Selección múltiple de archivos**: Selecciona hasta 10 archivos a la vez
...
- Solo los archivos a los que el usuario tiene permiso para acceder en SharePoint están disponibles
...
3. Comprobar que el usuario esté autenticado mediante Azure Entra ID
4. Reinicie LibreChat después de realizar cambios en la configuración
...
2. Asegúrese de que se haya otorgado el consentimiento del administrador
3. Comprueba que `SHAREPOINT_BASE_URL` coincida exactamente con tu tenant
4. Confirma que `SHAREPOINT_PICKER_SHAREPOINT_SCOPE` utiliza la URL completa del inquilino, como `https://contoso.sharepoint.com/AllSites.Read`
5. Confirmar que HTTPS está habilitado en producción
...
**Soluciones**:
1. Confirme que el registro de su aplicación Azure tenga un ámbito de API expuesto, como `api://<client-id>/access_as_user`
2. Agregue ese alcance completo a `OPENID_SCOPE`
...
- Optimiza tanto para la velocidad como para la estabilidad
...
2. **Tamaños de archivo**: Tenga en cuenta que los archivos grandes pueden ralentizar las conversaciones
