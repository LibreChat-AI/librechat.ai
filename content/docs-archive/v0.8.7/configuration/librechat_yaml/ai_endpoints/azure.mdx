...
This example follows the [Azure OpenAI Endpoint Configuration Docs](/v0.8.7/docs/configuration/librechat_yaml/object_structure/azure_openai).
...
The fields configurable in the Custom Config (`librechat.yaml`) file. For more information on each field, see the [Azure OpenAI section in the Custom Config Docs](/v0.8.7/docs/configuration/librechat_yaml/object_structure/azure_openai).
...
The list of models available to your users are determined by the model groupings specified in your [`azureOpenAI` endpoint config.](/v0.8.7/docs/configuration/librechat_yaml/object_structure/azure_openai)
...
This works the same as it does with the [OpenAI endpoint](/v0.8.7/docs/configuration/pre_configured_ai/openai): there is no need to select the vision model, as it will be switched behind the scenes.
