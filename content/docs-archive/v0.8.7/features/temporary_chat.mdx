...
**Availability** is governed by the `TEMPORARY_CHAT` role permission. Manage it for each role from the [Admin Panel](/v0.8.7/docs/features/admin_panel). The `interface.temporaryChat` option in `librechat.yaml` only seeds this permission for the default `USER` role at startup and is deprecated for permission management.
...
For the full set of options, including `retentionMode` and `retainAgentFiles`, see the [interface reference](/v0.8.7/docs/configuration/librechat_yaml/object_structure/interface#temporarychatretention).
