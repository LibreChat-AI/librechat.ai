...
Docker Desktop is recommended for most users. For remote server installations, see the [Ubuntu Docker Deployment Guide](/v0.8.4/docs/remote/docker_linux).
...
The default `.env` file works out of the box for a basic setup. For in-depth configuration, see the [.env reference](/v0.8.4/docs/configuration/dotenv).
...
For full setup instructions including creating the file from scratch, see the [librechat.yaml guide](/v0.8.4/docs/configuration/librechat_yaml). For more override options, see the [Docker override guide](/v0.8.4/docs/configuration/docker_override).
...
See the [.env reference](/v0.8.4/docs/configuration/dotenv) for all available variables and their defaults.

## Next Steps

<Cards num={3}>
  <Cards.Card title="Custom Endpoints" href="/v0.8.4/docs/quick_start/custom_endpoints" arrow>
    Add OpenRouter, Ollama, and other AI providers
  </Cards.Card>
  <Cards.Card title="Configuration Overview" href="/v0.8.4/docs/configuration" arrow>
    Understand how LibreChat's config files work together
  </Cards.Card>
  <Cards.Card title="Authentication Setup" href="/v0.8.4/docs/configuration/authentication" arrow>
