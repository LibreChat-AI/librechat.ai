...
This page applies to the [`agents`](/v0.8.4/docs/features/agents) endpoint.
...
For more information about agent steps, see [Max Agent Steps](/v0.8.4/docs/features/agents#max-agent-steps).
...
**Note:** Must be one of the following, or a custom endpoint name as defined in your [configuration](/v0.8.4/docs/configuration/librechat_yaml/object_structure/custom_endpoint#name):
...
- It's not recommended to disable the builder interface unless you are using [modelSpecs](/v0.8.4/docs/configuration/librechat_yaml/object_structure/model_specs) to define a list of agents to choose from.
...
- The `ocr` capability requires an OCR service to be configured (see [OCR Configuration](/v0.8.4/docs/configuration/librechat_yaml/object_structure/ocr)).
