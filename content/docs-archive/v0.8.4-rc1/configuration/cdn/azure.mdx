...
<Callout type="warning" title="Azure Blob Storage is object storage, not a CDN">
  Azure Blob Storage stores and serves files directly from origin — it is not a CDN. Images and avatars are best served through a CDN for optimal performance and global delivery. Currently, [Firebase](/v0.8.4-rc1/docs/configuration/cdn/firebase) is the only CDN-backed storage option.
