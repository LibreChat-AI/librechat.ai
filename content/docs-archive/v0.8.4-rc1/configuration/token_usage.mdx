...
**Important:** When `balance.enabled` is set to `true`, transaction recording is automatically enabled regardless of the `transactions.enabled` setting. This ensures that balance tracking functions correctly by maintaining a complete record of all token usage.
    ...
    Check out the [Transactions Configuration](/v0.8.4-rc1/docs/configuration/librechat_yaml/object_structure/transactions) page for more details.
...
Check out the [Balance Configuration](/v0.8.4-rc1/docs/configuration/librechat_yaml/object_structure/balance) page for more details.
