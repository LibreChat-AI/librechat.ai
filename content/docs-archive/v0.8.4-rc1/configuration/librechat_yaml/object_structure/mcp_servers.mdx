...
- **Required Subkeys:**
  ...
  - `redirect_uri`: String - [OAuth redirect URI](/v0.8.4-rc1/docs/features/mcp#oauth-callback-url) (eg. `http://localhost:3080/api/mcp/${serverName}/oauth/callback`)
...
- **Internal/Local Addresses:**
  - **Important**: MCP servers using internal IP addresses (e.g., `172.24.1.165`, `192.168.1.100`), or local domains (e.g., `mcp-server`, `host.docker.internal`) **must** be explicitly allowed in [`mcpSettings.allowedDomains`](/v0.8.4-rc1/docs/configuration/librechat_yaml/object_structure/mcp_settings).
  - See [MCP Settings](/v0.8.4-rc1/docs/configuration/librechat_yaml/object_structure/mcp_settings) for configuration details.
 ...
 **NOTE** See [MCP Server Initialization](/v0.8.4-rc1/docs/features/mcp#server-initialization) for more information about UI based server initialization.
