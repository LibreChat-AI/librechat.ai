...
> **Note:** All capabilities can be toggled via the `librechat.yaml` configuration file. See [docs/configuration/librechat_yaml/object_structure/agents#capabilities](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#capabilities) for more information.
...
- [More info about the Code Interpreter API](/v0.8.3-rc2/docs/features/code_interpreter)
...
- [More info about OCR configuration](/v0.8.3-rc2/docs/features/ocr)

**Note:** File Context includes extracted text in the agent's system instructions. For temporary document questions in individual conversations, use [Upload as Text](/v0.8.3-rc2/docs/features/upload_as_text) from the chat instead.
...
For more information, see documentation on [MCP](/v0.8.3-rc2/docs/features/mcp).
...
Learn More:
- [Configuring MCP Servers](/v0.8.3-rc2/docs/features/mcp)
...
**Note:** The `deferred_tools` capability is enabled by default. It can be toggled via the [`librechat.yaml` agents configuration](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#capabilities).
...
**Note:** The `programmatic_tools` capability is **disabled by default** and requires the latest [Code Interpreter API](/v0.8.3-rc2/docs/features/code_interpreter). It can be enabled via the [`librechat.yaml` agents configuration](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#capabilities).
*/}
...
- [More info about Artifacts](/v0.8.3-rc2/docs/features/artifacts)
...
- **[OpenAI Image Tools](/v0.8.3-rc2/docs/features/image_gen#1--openai-image-tools-recommended)**: Image generation & editing using **[GPT-Image-1](https://platform.openai.com/docs/models/gpt-image-1)**
- **[DALL-E-3](/v0.8.3-rc2/docs/features/image_gen#2--dalle-legacy)**: Image generation from text descriptions
- **[Stable Diffusion](/v0.8.3-rc2/docs/features/image_gen#3--stable-diffusion-local)** / **[Flux](/v0.8.3-rc2/docs/features/image_gen#4--flux)**: Text-to-image generation
- **[Wolfram](/v0.8.3-rc2/docs/configuration/tools/wolfram)**: Computational and mathematical capabilities
- **[OpenWeather](/v0.8.3-rc2/docs/configuration/tools/openweather)**: Weather data retrieval
- **[Google Search](/v0.8.3-rc2/docs/configuration/tools/google_search)**: Access to web search functionality
...
- Tools can be disabled using the [`librechat.yaml`](/v0.8.3-rc2/docs/configuration/librechat_yaml) configuration file:
  - [More info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#capabilities)
...
- Actions can be disabled using the [`librechat.yaml`](/v0.8.3-rc2/docs/configuration/librechat_yaml) configuration file:
  - [More info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#capabilities)
- Individual domains can be whitelisted for agent actions:
  - [More info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/actions#alloweddomains)
...
If left unconfigured, the default is 25 steps, but you can adjust this to suit your needs. For admins, you can set a global default as well as a global maximum in the [`librechat.yaml`](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents#recursionlimit) file.
...
2. **Upload as Text** (requires `context` capability)
   ...
   - See [Upload as Text](/v0.8.3-rc2/docs/features/upload_as_text)
...
4. **Upload for Code Interpreter** (requires `execute_code` capability, toggled ON)
   ...
   - More info about [Code Interpreter](/v0.8.3-rc2/docs/features/code_interpreter)
...
The first account created for your instance is an administrator. If you need to add an additional administrator, you may [access MongoDB](/v0.8.3-rc2/docs/configuration/mongodb/mongodb_auth) and update the user's profile:
...
The use of agents for all users can also be disabled via config, [more info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/interface).
...
LibreChat allows admins to configure the use of agents via the [`librechat.yaml`](/v0.8.3-rc2/docs/configuration/librechat_yaml) file:

- Disable Agents for all users (including admins): [more info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/interface)
- Customize agent capabilities using: [more info](/v0.8.3-rc2/docs/configuration/librechat_yaml/object_structure/agents)
...
#### 2. For the `deploy-compose.yml` (if you followed the [Ubuntu Docker Guide](/v0.8.3-rc2/docs/remote/docker_linux)):
...
See the [Agents API documentation](/v0.8.3-rc2/docs/features/agents_api) for setup and usage details.
