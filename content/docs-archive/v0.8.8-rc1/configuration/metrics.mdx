...
When [backend OpenTelemetry tracing](/v0.8.8-rc1/docs/configuration/dotenv#opentelemetry-tracing) is enabled, the `librechat.agent.startup` span reports the same milestones as events. It includes total startup duration, milestone count, terminal result, and the generation stream ID when one is assigned.
...
When [backend OpenTelemetry tracing](/v0.8.8-rc1/docs/configuration/dotenv#opentelemetry-tracing) is enabled, each HTTP span also summarizes its Redis work with:
