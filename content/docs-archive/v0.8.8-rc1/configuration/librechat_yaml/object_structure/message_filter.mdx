...
The `messageFilter` object configures server-side message filters. The supported filter type is `messageFilter.pii`.
...
<OptionTable
  options={[
    ...
    [
      'regex',
      'String',
      'RE2 regular expression. LibreChat validates it at config load time and applies it globally.',
      "regex: 'sk-ant-[A-Za-z0-9_-]{20,}'",
    ],
  ]}
/>

Custom patterns use the linear-time RE2 engine. Backreferences and lookaround are not supported, and some escapes differ from JavaScript regular expressions. LibreChat rejects patterns that RE2 cannot compile when loading the configuration.
