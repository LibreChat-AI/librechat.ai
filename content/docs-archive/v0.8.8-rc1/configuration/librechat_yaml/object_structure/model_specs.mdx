...
Requires configured [User Memory](/v0.8.8-rc1/docs/features/memory), the endpoint-level `memory` capability, and user memory permissions. When enabled, the model can save or delete structured memories when the user explicitly requests it.
...
The model can start an eligible tool, continue working, and poll with `check_background_task`. Completed code output and generated files are attached to the original code call. Background task state is process-local and does not survive a server restart or move across replicas.
...
**Note:** If you are using a custom endpoint, the `endpoint` value must match the defined [custom endpoint name](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/custom_endpoint#name) exactly.
...
For more information about the ACL permissions system, see the [Agents documentation](/v0.8.8-rc1/docs/features/agents#migration-required-v080-rc3).
