...
- **Validation:**
  ...
  - Users need the `MCP_SERVERS.CONFIGURE_OBO` role permission to configure this field. Seed it with [`interface.mcpServers.configureObo`](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/interface#mcpservers) or manage it from the admin panel.
- **Prerequisites:**
...
See [OpenID Connect Token Reuse](/v0.8.8-rc1/docs/configuration/authentication/OAuth2-OIDC/token-reuse) and [SharePoint Integration](/v0.8.8-rc1/docs/configuration/sharepoint) for related token-reuse and delegated-token setup.
...
- **Required Subkeys:**
  ...
  - `redirect_uri`: String - [OAuth redirect URI](/v0.8.8-rc1/docs/features/mcp#oauth-callback-url) (eg. `http://localhost:3080/api/mcp/${serverName}/oauth/callback`)
...
- **Internal/Local Addresses:**
  - **Important**: MCP servers using internal IP addresses (e.g., `172.24.1.165`, `192.168.1.100`), or local domains (e.g., `mcp-server`, `host.docker.internal`) **must** be explicitly allowed. Use [`mcpSettings.allowedAddresses`](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/mcp_settings#allowedaddresses) for exact private host:port services when you want public destinations to remain reachable, or [`mcpSettings.allowedDomains`](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/mcp_settings#alloweddomains) when you want a strict whitelist.
  - See [MCP Settings](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/mcp_settings) for configuration details.
...
**NOTE** See [MCP Server Initialization](/v0.8.8-rc1/docs/features/mcp#server-initialization) for more information about UI based server initialization.
