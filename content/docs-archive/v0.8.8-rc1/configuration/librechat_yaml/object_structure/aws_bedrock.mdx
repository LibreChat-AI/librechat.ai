...
- For a complete guide on creating and managing inference profiles, see [AWS Bedrock Inference Profiles](/v0.8.8-rc1/docs/configuration/pre_configured_ai/bedrock_inference_profiles)
...
- AWS Bedrock authentication is configured through environment variables. You can use `BEDROCK_AWS_PROFILE`, the AWS SDK default credential provider chain, `BEDROCK_AWS_BEARER_TOKEN` for Bedrock API keys, or Bedrock-specific static credentials. See the [AWS Bedrock setup guide](/v0.8.8-rc1/docs/configuration/pre_configured_ai/bedrock#authentication) for details.
