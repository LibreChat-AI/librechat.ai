...
Redis enables [Resumable Streams](/v0.8.8-rc1/docs/features/resumable_streams) for horizontally scaled deployments. When enabled, AI responses can seamlessly reconnect and resume across server instances.
...
The v2 generation protocol adds the durable queue, interrupt, pause/resume, and recovery guarantees used by current Agent runs. Redis-backed deployments intentionally remain on protocol v1 when `GENERATION_PROTOCOL_VERSION` is unset so replicas running different LibreChat versions can coexist during a rolling upgrade.

Use a two-stage rollout:

1. Deploy the v2-capable LibreChat image to **every** replica with `GENERATION_PROTOCOL_VERSION` unset or set to `1`.
2. Wait for active generations owned by the previous image to finish or drain.
3. Set `GENERATION_PROTOCOL_VERSION=2` and roll out the same image again.
...
<Callout type="warning" title="Rolling deployment safety">
  Do not enable protocol v2 until every replica supports it. After v2 generations begin, do not roll back to a pre-v2 LibreChat image until those generations have drained; older replicas cannot safely operate on their Redis state.
</Callout>

In-memory generation streams do not share state across replicas and use protocol v2 by default, so they do not require this cutover.
...
See [Resumable Streams](/v0.8.8-rc1/docs/features/resumable_streams) for more details on this feature.
...
Redis Cluster deployments use slot-safe catalog keys and update operations; no separate MCP Redis setting is required. See [MCP Server Management](/v0.8.8-rc1/docs/features/mcp#mcp-server-management) for dynamic catalog and OAuth readiness behavior.
