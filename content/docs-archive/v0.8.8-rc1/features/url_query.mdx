...
For a [custom endpoint](/v0.8.8-rc1/docs/quick_start/custom_endpoints), use its configured name as the value (case-insensitive):
...
Select a [model spec](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/model_specs) by name:
...
| `spec` | Select a [model spec](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/model_specs) by exact name. When set, other model parameters are ignored in favor of the spec. If specs are configured with `enforce: true`, this parameter may be required for URL query params to work. |
...
Most of these parameters are shared with [Model Spec Preset Fields](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/model_specs); refer there for the full reference.
