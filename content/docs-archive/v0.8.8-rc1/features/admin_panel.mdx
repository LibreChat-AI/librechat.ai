...
The **LibreChat Admin Panel** is a standalone browser-based management interface for LibreChat. It connects to the same database as LibreChat itself and provides a GUI for the administrative tasks that power [granular access control](/v0.8.8-rc1/docs/features/access_control): user and group administration, role management, configuration overrides scoped to roles or groups, and system-level capability grants.
...
For the underlying permission model (principals, resource ACLs, capabilities, and how the layers compose), see the [Access Control](/v0.8.8-rc1/docs/features/access_control) page.
...
2. Hold the `access:admin` system grant (assigned to another principal via the admin panel itself; see [System Grants](/v0.8.8-rc1/docs/features/access_control#layer-3-system-grants-admin-capabilities)).
...
- [Access Control](/v0.8.8-rc1/docs/features/access_control): the permission model the admin panel is built on
- [Interface Configuration](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/interface): the feature flags the panel edits
- [Authentication](/v0.8.8-rc1/docs/features/authentication): user authentication on LibreChat
