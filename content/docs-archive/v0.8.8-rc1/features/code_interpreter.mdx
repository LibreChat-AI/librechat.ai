...
1. **[AI Agents](/v0.8.8-rc1/docs/features/agents#code-interpreter)**: Enable Code Interpreter in your agent's configuration to allow it to execute code and process files automatically.

2. **Manual Execution**: Use the "Run Code" button in code blocks within the chat interface, as shown here:

3. **[Programmatic Tool Calling](/v0.8.8-rc1/docs/features/agents#programmatic-tool-calling)**: Enable the `programmatic_tools` capability and mark selected MCP tools as Programmatic so agents can orchestrate those tools from sandboxed code.

4. **[Background Tool Calls](/v0.8.8-rc1/docs/features/agents#background-tool-calls)**: Let an agent dispatch eligible code in the background and continue while it runs.

5. **[Stateful Code Sessions](/v0.8.8-rc1/docs/features/agents#stateful-code-sessions)**: Experimentally reuse a per-conversation workspace across an agent's code executions.
...
Generated artifact previews are intentionally size-bounded. Set [`FILE_PREVIEW_MAX_EXTRACT_BYTES`](/v0.8.8-rc1/docs/configuration/dotenv#inline-file-previews) in LibreChat's `.env` to change the source-file size limit for inline preview extraction; larger files remain available for download.
