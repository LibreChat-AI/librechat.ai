...
<Callout type="info" title="Admin Panel">
  A dedicated [**LibreChat Admin Panel**](/v0.8.8-rc1/docs/features/admin_panel) is the upcoming UI for
...
Admins can be promoted manually by updating the user document in MongoDB, see [Administrator Controls](/v0.8.8-rc1/docs/features/agents#administrator-controls).
...
The recommended way to manage feature permissions is the [**LibreChat Admin Panel**](/v0.8.8-rc1/docs/features/admin_panel), which edits the permission matrix directly on each role (including any custom roles you create). Changes take effect without redeploying LibreChat and are scoped to the exact role you want to modify, rather than the global `USER` default.

<Callout type="warning" title="Legacy: `librechat.yaml` interface block">
  The [`interface` block](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/interface) in
...
- **Remote Agents** (`remoteAgent`), for the [Agents API](/v0.8.8-rc1/docs/features/agents_api)
...
- **Entra ID (Azure AD) groups**: synced from Microsoft Graph when a user logs in via Azure OIDC with [token reuse](/v0.8.8-rc1/docs/configuration/authentication/OAuth2-OIDC/token-reuse) enabled. Each synced group stores its Entra Object ID as `idOnTheSource`, which keeps LibreChat in lockstep with tenant membership.
...
Any system or custom role can be used as a principal. Sharing an agent with a role (e.g. `SupportEngineers`) gives every user currently holding that role access, without needing to enumerate individuals. Roles can be hidden from the people picker via [`interface.peoplePicker.roles`](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/interface#peoplepicker) for environments where role-based sharing is an admin-only concern.
...
See the [agents migration guide](/v0.8.8-rc1/docs/features/agents#migration-required-v080-rc3) for Docker variants and batch-size options.

## Related Documentation

- [Agents: Sharing and Permissions](/v0.8.8-rc1/docs/features/agents#sharing-and-permissions)
- [Interface Configuration (feature permissions)](/v0.8.8-rc1/docs/configuration/librechat_yaml/object_structure/interface)
- [Authentication](/v0.8.8-rc1/docs/features/authentication)
- [OpenID Connect Token Reuse (required for Entra ID group sync)](/v0.8.8-rc1/docs/configuration/authentication/OAuth2-OIDC/token-reuse)
- [Azure / Entra ID OAuth2](/v0.8.8-rc1/docs/configuration/authentication/OAuth2-OIDC/azure)
- [SharePoint Integration](/v0.8.8-rc1/docs/configuration/sharepoint)
- [Agents API (Remote Agents)](/v0.8.8-rc1/docs/features/agents_api)
- [LibreChat Admin Panel](/v0.8.8-rc1/docs/features/admin_panel)
