---
title: File Storage & CDN
icon: BookOpen
description: File storage and CDN setup instructions
---

<Callout type="info" title="Note">
  LibreChat supports local storage, object storage backends, and CDN-backed delivery. Use S3 for
  durable object storage, then add CloudFront when you need stable media links, edge caching, signed
  cookies, or signed download URLs.
</Callout>

## File Storage

### Amazon S3

- [Amazon S3](/v0.8.6-rc1/docs/configuration/cdn/s3)

### Azure Blob Storage

- [Azure Blob Storage](/v0.8.6-rc1/docs/configuration/cdn/azure)

## CDN

### CloudFront

- [CloudFront with S3](/v0.8.6-rc1/docs/configuration/cdn/cloudfront)

### Firebase

- [Firebase CDN](/v0.8.6-rc1/docs/configuration/cdn/firebase)
