...
For ongoing management, use the [**LibreChat Admin Panel**](/v0.8.6-rc1/docs/features/admin_panel), which edits the permission matrix directly on each role (including custom roles). These YAML fields remain supported for bootstrapping a fresh instance or fully file-driven deployments, but should no longer be used as the primary way to manage feature permissions.

See [Access Control](/v0.8.6-rc1/docs/features/access_control) for the full permission model.
</Callout>
...
> **Deprecated for permission management.** The `use`, `create`, `share`, and `public` sub-keys seed role permissions at startup. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel) for managing MCP server permissions per role/group/user. The `placeholder` and `trustCheckbox` sub-keys are unaffected.
...
**Notes:**
- This is required to be `true` if using [`modelSpecs.addedEndpoints`](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/model_specs#addedendpoints).
...
> **Deprecated for permission management.** Seeds the `PROMPTS` role permissions at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel) for managing prompt permissions per role/group/user.
...
> **Deprecated for permission management.** Seeds the `BOOKMARKS` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
> **Deprecated for permission management.** Seeds the `MEMORIES` role permissions at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel). Note this toggle is separate from the [`memory`](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/memory) behavior configuration.
...
**Note:** This controls the UI visibility of the memories feature. For detailed memory behavior configuration (token limits, personalization, agent settings), see the [Memory Configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/memory).
...
> **Deprecated for permission management.** Seeds the `MULTI_CONVO` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
More info on [Agents](/v0.8.6-rc1/docs/features/agents)

> **Deprecated for permission management.** Seeds the `AGENTS` role permissions at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel) for managing agent permissions per role/group/user.
...
> **Deprecated for permission management.** Seeds the `REMOTE_AGENTS` role permissions at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
> **Deprecated for permission management.** Seeds the `TEMPORARY_CHAT` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel). `temporaryChatRetention` below is not a permission and remains the recommended way to configure retention.
...
Enables/disables the "Run Code" button for Markdown Code Blocks. More info on the [LibreChat Code Interpreter API](/v0.8.6-rc1/docs/features/code_interpreter)

**Note:** This setting does not disable the [Agents Code Interpreter Capability](/v0.8.6-rc1/docs/features/agents#code-interpreter). To disable the Agents Capability, see the [Agents Endpoint configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/agents) instead.

> **Deprecated for permission management.** Seeds the `RUN_CODE` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
Enables/disables the web search button in the chat interface. More info on [Web Search Configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/web_search)

**Note:** This setting does not disable the [Agents Web Search Capability](/v0.8.6-rc1/docs/features/agents#web-search). To disable the Agents Capability, see the [Agents Endpoint configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/agents) instead.

> **Deprecated for permission management.** Seeds the `WEB_SEARCH` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
**Note:** This setting does not disable the [Agents File Search Capability](/v0.8.6-rc1/docs/features/agents#file-search). To disable the Agents Capability, see the [Agents Endpoint configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/agents) instead.

> **Deprecated for permission management.** Seeds the `FILE_SEARCH` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
> **Deprecated for permission management.** Seeds/globally gates the `FILE_CITATIONS` role permission at startup. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel) for managing citations permissions per role/group/user.
...
- When using agents with file search capability, citation behavior (quantity and quality) can be configured through the [Agents endpoint configuration](/v0.8.6-rc1/docs/configuration/librechat_yaml/object_structure/agents#file-citation-configuration-examples).
...
> **Deprecated for permission management.** Seeds the `PEOPLE_PICKER` role permissions at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
...
> **Deprecated for permission management.** Seeds the `MARKETPLACE` role permission at startup for the default `USER` role only. Prefer the [Admin Panel](/v0.8.6-rc1/docs/features/admin_panel).
