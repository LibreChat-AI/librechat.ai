...
- API may be strict for some models, and may not allow fields like `stop` and `frequency_penalty` may cause an error when set to 0, in which case, you should use [`dropParams`.](/v0.8.6/docs/configuration/librechat_yaml/object_structure/custom_endpoint#dropparams)
