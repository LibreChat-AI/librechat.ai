...
This page applies to the [`agents`](/v0.8.6/docs/features/agents) endpoint.
...
For more information about agent steps, see [Max Agent Steps](/v0.8.6/docs/features/agents#max-agent-steps).
...
**Note:** Must be one of the following, or a custom endpoint name as defined in your [configuration](/v0.8.6/docs/configuration/librechat_yaml/object_structure/custom_endpoint#name): - `openAI, azureOpenAI, google, anthropic, assistants, azureAssistants, bedrock`
...
- **subagents**: Enables isolated-context child agent runs. See [Subagents](/v0.8.6/docs/features/subagents).
...
- **skills**: Enables Skills in the side panel, manual `$` invocation, model-invoked skills, and agent skill allowlists. See [Skills](/v0.8.6/docs/features/skills).
...
For user-facing behavior and limits, see [Subagents](/v0.8.6/docs/features/subagents).

## Notes

- It's not recommended to disable the builder interface unless you are using [modelSpecs](/v0.8.6/docs/configuration/librechat_yaml/object_structure/model_specs) to define a list of agents to choose from.
...
- The `ocr` capability requires an OCR service to be configured (see [OCR Configuration](/v0.8.6/docs/configuration/librechat_yaml/object_structure/ocr)).
