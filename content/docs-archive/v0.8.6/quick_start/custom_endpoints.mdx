...
For a full overview of how these files work together, see the [Configuration Overview](/v0.8.6/docs/configuration).

</Callout>

<Callout type="warning" title="Before You Start">

This guide assumes you have LibreChat installed and running. If not, complete the [Docker setup](/v0.8.6/docs/local/docker) first.
...
Learn more: [Docker Override Guide](/v0.8.6/docs/configuration/docker_override)

## Step 2. Configure librechat.yaml

Create a `librechat.yaml` file in the project root (if it does not exist) and add your endpoint configuration. See the [librechat.yaml guide](/v0.8.6/docs/configuration/librechat_yaml) for detailed setup instructions.
...
Browse all compatible providers in the [AI Endpoints](/v0.8.6/docs/configuration/librechat_yaml/ai_endpoints) section. For the full field reference, see [Custom Endpoint Object Structure](/v0.8.6/docs/configuration/librechat_yaml/object_structure/custom_endpoint).
...
<Cards num={2}>
  <Cards.Card title="AI Endpoints" href="/v0.8.6/docs/configuration/librechat_yaml/ai_endpoints" arrow>
    Browse all compatible AI providers with example configurations
  </Cards.Card>
  <Cards.Card title="librechat.yaml Guide" href="/v0.8.6/docs/configuration/librechat_yaml" arrow>
