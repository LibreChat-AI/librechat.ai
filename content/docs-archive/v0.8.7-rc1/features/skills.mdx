...
Use a GitHub fine-grained personal access token with read-only Contents and Metadata permissions for the selected repository. See [Skill Sync Object Structure](/v0.8.7-rc1/docs/configuration/librechat_yaml/object_structure/skill_sync) for all fields, credential options, tenant scoping, and admin sync endpoints.
