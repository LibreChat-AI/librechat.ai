...
- Tavily Search is an agent tool. LibreChat can also use Tavily as a [Web Search](/v0.8.7-rc1/docs/features/web_search) provider or scraper, which is configured separately.
...
- If the tool does not appear in the Agent Builder, confirm `TAVILY_API_KEY` is set and check [`includedTools`](/v0.8.7-rc1/docs/configuration/librechat_yaml/object_structure/config#includedtools) or [`filteredTools`](/v0.8.7-rc1/docs/configuration/librechat_yaml/object_structure/config#filteredtools).
