...
**For further details about RAG, refer to the user guide provided here: [RAG API Presentation](/v0.8.5/docs/features/rag_api)**

---

**Currently, this feature is available through [Agents](/v0.8.5/docs/features/agents), as well as through Custom Endpoints, OpenAI, Azure OpenAI, Anthropic, and Google.**
...
**Still confused about RAG?** [Read the RAG API Presentation](/v0.8.5/docs/features/rag_api#what-is-rag) explaining the general concept in more detail with a link to a helpful video.
...
For Docker, the setup is configured for you in both the default `docker-compose.yml` and `deploy-compose.yml` files, and you will just need to make sure you are using the latest docker image and compose files. Make sure to read the [Updating LibreChat guide for Docker](/v0.8.5/docs/local/docker#update-librechat) if you are unsure how to update your Docker instance.
...
As always, make these changes in your update URLs [Docker Compose Override File](/v0.8.5/docs/configuration/docker_override). You can find an example for exactly how to change the image in `docker-compose.override.yml.example` at the root of the project.
