...
You can further configure the Bedrock endpoint in your [`librechat.yaml`](/v0.8.5/docs/configuration/librechat_yaml/) file:
...
For the full guide on creating profiles, configuring LibreChat, setting up logging, and troubleshooting, see **[Bedrock Inference Profiles](/v0.8.5/docs/configuration/pre_configured_ai/bedrock_inference_profiles)**.

For the YAML field reference, see **[AWS Bedrock Object Structure](/v0.8.5/docs/configuration/librechat_yaml/object_structure/aws_bedrock#inferenceprofiles)**.
...
For more information on file upload options, see the [OCR for Documents](/v0.8.5/docs/features/ocr#5-upload-files-to-provider-direct) documentation.
...
- AWS Bedrock endpoint supports all [Shared Endpoint Settings](/v0.8.5/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings) via the `librechat.yaml` configuration file, including `streamRate`, `titleModel`, `titleMethod`, `titlePrompt`, `titlePromptTemplate`, and `titleEndpoint`