...
`use` and `create` seed the `SCHEDULES` permissions for the built-in `USER` role at startup. Administrators can manage those permissions per role in the [Admin Panel](/v0.8.8-rc2/docs/features/admin_panel). The remaining values are runtime limits:
...
Time controls follow the browser's [Clock Format preference](/v0.8.8-rc2/docs/features/settings#general), and weekly day controls and summaries follow its **Week Starts On** preference. These preferences change presentation only; the stored IANA zone and selected weekdays determine execution.
...
See [Access Control](/v0.8.8-rc2/docs/features/access_control) for role permissions and resource ACLs.
...
For an immediate deployment-wide stop, set `SCHEDULES_DISABLED=true`. This blocks automatic occurrences and **Run now** without deleting schedule definitions. See [Scheduled Chats environment variables](/v0.8.8-rc2/docs/configuration/dotenv#scheduled-chats).
