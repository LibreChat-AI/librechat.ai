...
Use the [Admin Panel](/v0.8.8-rc2/docs/features/admin_panel) to assign system capabilities. Every Insights query derives tenant scope from the authenticated administrator. Tenant administrators only see records for their own tenant; an administrator without a tenant sees only unscoped records.
...
See [`ENABLE_INSIGHTS`](/v0.8.8-rc2/docs/configuration/dotenv#admin-insights) for the deployment switch.
