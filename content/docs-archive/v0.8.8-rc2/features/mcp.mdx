...
To keep an MCP server out of regular chat selection, set [`chatMenu: false`](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/mcp_servers#chatmenu) in your configuration:
...
- [MCP Server Guides](/v0.8.8-rc2/docs/mcp_servers)
- [MCP Servers Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/mcp_servers)
- [Agent Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/agents)
- [Advanced Agent Features](/v0.8.8-rc2/docs/features/agents#model-context-protocol-mcp)
- [Agent Plugins](/v0.8.8-rc2/docs/features/agent_plugins) for experimentally bundling deployment MCP servers with Skills
...
MCP servers participate in LibreChat's [granular access control](/v0.8.8-rc2/docs/features/access_control) system. In addition to servers defined in `librechat.yaml` (which are managed by admins and governed by [`interface.mcpServers`](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/interface#mcpservers) feature permissions), user-created MCP servers have their own ACL and can be shared with specific **users**, **groups**, **roles**, or **publicly**, at Viewer, Editor, or Owner level.

The `USE`, `CREATE`, `SHARE`, and `SHARE_PUBLIC` feature flags under `interface.mcpServers` control who is allowed to create and share MCP servers at all. See [Access Control](/v0.8.8-rc2/docs/features/access_control) for how the permission layers compose.
...
- And many more (see [MCP Servers Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers) for complete list)
...
`{{LIBRECHAT_GRAPH_ACCESS_TOKEN}}` exchanges the user's reusable OpenID access token through the Microsoft on-behalf-of flow. [`GRAPH_API_SCOPES`](/v0.8.8-rc2/docs/configuration/dotenv#microsoft-graph-api--entra-id-integration) sets the requested space-separated scopes and defaults to `https://graph.microsoft.com/.default`. This is independent of `OPENID_GRAPH_SCOPES`, which configures people and group search.

The `{{LIBRECHAT_BODY_PARENTMESSAGEID}}` placeholder is populated for native Agent chats, OpenAI-compatible Agents API calls, and legacy Assistants runs. Open Responses has no equivalent parent-message identity and fails closed when an MCP server requires that value. See the [MCP server placeholder reference](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/mcp_servers#headers).
