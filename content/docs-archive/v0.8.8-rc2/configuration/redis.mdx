...
Redis enables [Resumable Streams](/v0.8.8-rc2/docs/features/resumable_streams) for horizontally scaled deployments. When enabled, AI responses can seamlessly reconnect and resume across server instances.
...
See [Resumable Streams](/v0.8.8-rc2/docs/features/resumable_streams) for more details on this feature.
...
Redis Cluster deployments use slot-safe catalog keys and update operations; no separate MCP Redis setting is required. See [MCP Server Management](/v0.8.8-rc2/docs/features/mcp#mcp-server-management) for dynamic catalog and OAuth readiness behavior.
