...
For a quick overview, refer to the user guide provided here: [Authentication](/v0.8.8-rc2/docs/features/authentication)
...
When either value is blank, LibreChat can generate and reuse a value from its temporary credentials file. The default Docker Compose stacks persist that file, but this is intended only as a bootstrap convenience. If the file is lost or cannot be written, sessions can become invalid after restart. See [Credentials Configuration](/v0.8.8-rc2/docs/configuration/dotenv#credentials-configuration) for precedence, persistence, and production guidance.
...
To set up the mod system, review [the setup guide](/v0.8.8-rc2/docs/configuration/mod_system).
...
2. For the `deploy-compose.yml` (if you followed the [Ubuntu Docker Guide](/v0.8.8-rc2/docs/remote/docker_linux)):
   ```bash
