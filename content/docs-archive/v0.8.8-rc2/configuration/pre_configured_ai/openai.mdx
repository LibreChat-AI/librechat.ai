...
- GPT-5.6 supports the Responses API-only `reasoning_mode` values `standard` and `pro`, plus `reasoning_context` values `auto`, `current_turn`, and `all_turns`. These can be configured in [model specs](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/model_specs#reasoning_mode).
...
- OpenAI endpoint supports all [Shared Endpoint Settings](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings) via the `librechat.yaml` configuration file, including `streamRate`, `headers`, `titleModel`, `titleMethod`, `titlePrompt`, `titlePromptTemplate`, and `titleEndpoint`
