...
- Anthropic endpoint supports all [Shared Endpoint Settings](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings) via the `librechat.yaml` configuration file, including `streamRate`, `headers`, `titleModel`, `titleMethod`, `titlePrompt`, `titlePromptTemplate`, and `titleEndpoint`
- To configure Anthropic or an Anthropic-compatible gateway as a separate custom endpoint, use [`provider: "anthropic"`](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/custom_endpoint#provider).
...
For detailed YAML configuration options, see [Anthropic Vertex AI Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/anthropic_vertex).
