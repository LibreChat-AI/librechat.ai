...
- The API is strict for some models. Fields like `stop` and `frequency_penalty` can cause an error when set to 0, so drop them with [`dropParams`](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/custom_endpoint#dropparams).
