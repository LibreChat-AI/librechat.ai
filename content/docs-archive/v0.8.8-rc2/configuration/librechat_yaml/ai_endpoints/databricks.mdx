...
- Databricks exposes a full completions endpoint ending in `invocations` for `serving-endpoints`, so [directEndpoint](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/custom_endpoint#directendpoint) is required.
- Set [titleMessageRole](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/custom_endpoint#titlemessagerole) to `user` for title generation. A standalone `system` message is not supported.
