...
- Our quick guide:
  - **[Docker Override](/v0.8.8-rc2/docs/configuration/docker_override)**
...
See [HTTP Security Headers](/v0.8.8-rc2/docs/configuration/security_headers) for every baseline and CSP variable, accepted values, defaults, source-list overrides, and rollout guidance.
...
DocumentDB 4.0 can run with reduced partial-unique-index enforcement. See [Amazon DocumentDB Compatibility](/v0.8.8-rc2/docs/user_guides/mongodb#amazon-documentdb-compatibility) for the supported target and caveats.

See also:

- [MongoDB Atlas](/v0.8.8-rc2/docs/configuration/mongodb/mongodb_atlas) for instructions on how to create an online MongoDB Atlas database (useful for use without Docker)
- [MongoDB Community Server](/v0.8.8-rc2/docs/configuration/mongodb/mongodb_community) for instructions on how to create a local MongoDB database (without Docker)
- [MongoDB Authentication](/v0.8.8-rc2/docs/configuration/mongodb/mongodb_auth) To enable explicit authentication for MongoDB in Docker.
...
LibreChat has built-in central logging, see [Logging System](/v0.8.8-rc2/docs/configuration/logging) for more info.
...
See [Agent Plugins](/v0.8.8-rc2/docs/features/agent_plugins) for package layout, schema versions, supported components, precedence, and current limitations.
...
- see also: [Custom Endpoints & Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml)
...
For detailed configuration and customization options, see: [Web Search Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/web_search)
...
**Note**: Most variable names can be customized in your `librechat.yaml` configuration file. For example, you could use `CUSTOM_SERPER_KEY` instead of `SERPER_API_KEY` by configuring it in the web search settings. `KEENABLE_FETCH_URL` is environment-only. See the [Web Search Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/web_search) documentation for details.

### Anthropic

see: [Anthropic Endpoint](/v0.8.8-rc2/docs/configuration/pre_configured_ai/anthropic)
...
- `ANTHROPIC_TITLE_MODEL` is now deprecated and will be removed in future versions. Use the [`titleModel` Endpoint Setting](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings#titlemodel) instead in the `librechat.yaml` config instead.
...
You can also use Anthropic Claude models through Google Cloud Vertex AI. For detailed YAML configuration options, see: [Anthropic Vertex AI Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/anthropic_vertex)
...
See: [AWS Bedrock Setup](/v0.8.8-rc2/docs/configuration/pre_configured_ai/bedrock)
...
> **Note:** You can omit the access keys to use the default AWS credentials chain (environment variables, SSO credentials, shared credentials files, or EC2/ECS Instance Metadata Service). See [AWS Bedrock Setup](/v0.8.8-rc2/docs/configuration/pre_configured_ai/bedrock) for more details.
...
Follow these instructions to setup the [Google Endpoint](/v0.8.8-rc2/docs/configuration/pre_configured_ai/google)
...
- `GOOGLE_TITLE_MODEL` is now deprecated and will be removed in future versions. Use the [`titleModel` Endpoint Setting](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings#titlemodel) instead in the `librechat.yaml` config instead.
...
Gemini Image Generation is a tool for Agents that supports both the Gemini API and Vertex AI. See: [Gemini Image Generation](/v0.8.8-rc2/docs/configuration/tools/gemini_image_gen)
...
See: [OpenAI Setup](/v0.8.8-rc2/docs/configuration/pre_configured_ai/openai)
...
- `OPENAI_TITLE_MODEL` is now deprecated and will be removed in future versions. Use the [`titleModel` Endpoint Setting](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings#titlemodel) instead in the `librechat.yaml` config instead.
- `OPENAI_REVERSE_PROXY` is now deprecated and will be removed in future versions. Use a [custom endpoint](/v0.8.8-rc2/docs/quick_start/custom_endpoints) instead.

### Assistants

See: [Assistants Setup](/v0.8.8-rc2/docs/configuration/pre_configured_ai/assistants)
...
See detailed instructions here: **[Wolfram Alpha](/v0.8.8-rc2/docs/configuration/tools/wolfram)**
...
See detailed instructions here: **[OpenWeather](/v0.8.8-rc2/docs/configuration/tools/openweather)**
...
Each Agent can scope its stateful workspace to the signed-in user, the user and Agent together, or the user and conversation. See [Stateful Code Sessions](/v0.8.8-rc2/docs/features/code_interpreter#stateful-code-sessions) for setup and persistence limitations.
...
Configure Retrieval-Augmented Generation for document indexing and context-aware responses. See: **[RAG API Configuration](/v0.8.8-rc2/docs/configuration/rag_api)**
...
> **Note:** When using the default Docker setup, the `.env` file is shared between LibreChat and the RAG API. For complete configuration options, see the [RAG API documentation](/v0.8.8-rc2/docs/configuration/rag_api).

## Speech to Text & Text to Speech

Configure Speech-to-Text (STT) and Text-to-Speech (TTS) services. See: **[Speech Settings](/v0.8.8-rc2/docs/configuration/stt_tts)**
...
> **Note:** STT and TTS are primarily configured through the `speech:` section in `librechat.yaml`. These environment variables are referenced in that configuration. See [Speech Settings](/v0.8.8-rc2/docs/configuration/stt_tts) for full YAML configuration options.
...
The in-memory generation store supports process-local Event Actor detached completion while the process remains alive. Redis generation streams add durable restart recovery and replica handoff. See [Generation Protocol Compatibility](/v0.8.8-rc2/docs/configuration/redis#generation-protocol-compatibility) before a mixed-version deployment.
...
When enabled, Insights remains restricted to accounts with the `ADMIN` role plus `access:admin` and `read:insights`. Queries are tenant-scoped and expose persisted activity metrics and recent conversation metadata. See [Admin Insights](/v0.8.8-rc2/docs/features/insights) for metric definitions and access details.
...
see: **[Automated Moderation](/v0.8.8-rc2/docs/configuration/mod_system)**
...
see: **[Token Usage](/v0.8.8-rc2/docs/configuration/token_usage)**
...
see: **[Authentication System](/v0.8.8-rc2/docs/configuration/authentication)**
...
For more details: [OAuth2-OIDC](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC)

#### Apple Authentication

For more information: **[Apple Authentication](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/apple)**
...
For more information: **[Discord](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/discord)**
...
For more information: **[Facebook Authentication](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/facebook)**
...
For more information: **[GitHub Authentication](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/github)**
...
For more information: **[Google Authentication](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/google)**
...
- [Auth0](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/auth0)
- [AWS Cognito](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/aws)
- [Azure Entra/AD](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/azure)
- [Keycloak](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/keycloak)
...
<Callout type="note" title="Note">
  ...
  Session](/v0.8.8-rc2/docs/configuration/authentication/OAuth2-OIDC/token-reuse).
</Callout>
...
For detailed SharePoint configuration instructions, see: [SharePoint Integration Guide](/v0.8.8-rc2/docs/configuration/sharepoint)

#### SAML

For more information:

- [Auth0](/v0.8.8-rc2/docs/configuration/authentication/SAML/auth0)
...
For more information: **[LDAP/AD Authentication](/v0.8.8-rc2/docs/configuration/authentication/ldap)**
...
Email is used for account verification and password reset. LibreChat supports both Mailgun API and traditional SMTP services. See: **[Email setup](/v0.8.8-rc2/docs/configuration/authentication/email)**
...
See: **[Firebase CDN Configuration](/v0.8.8-rc2/docs/configuration/cdn/firebase)**
...
<Callout type="warning" title="Important">
  ...
  Configuration](/v0.8.8-rc2/docs/configuration/librechat_yaml)
</Callout>
...
See: **[Amazon S3 Configuration](/v0.8.8-rc2/docs/configuration/cdn/s3)** and **[CloudFront with S3](/v0.8.8-rc2/docs/configuration/cdn/cloudfront)**
...
See: **[Azure Blob Storage CDN Configuration](/v0.8.8-rc2/docs/configuration/cdn/azure)**
...
For detailed configuration and examples, see: **[Redis Configuration Guide](/v0.8.8-rc2/docs/configuration/redis)**
...
- Current LibreChat clients negotiate generation protocol v2 automatically. The retired `GENERATION_PROTOCOL_VERSION` variable is no longer read. See [Generation Protocol Compatibility](/v0.8.8-rc2/docs/configuration/redis#generation-protocol-compatibility) before mixing release versions or rolling back.
- Enable `STREAM_DELTA_COALESCE_MS` only after every replica supports batch frames. Older subscribers drop coalesced frames. See [Stream Delta Coalescing](/v0.8.8-rc2/docs/configuration/redis#stream-delta-coalescing).
