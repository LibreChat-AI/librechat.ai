...
1. A running LibreChat instance (see [Quick Start](/v0.8.8-rc2/docs/quick_start))
...
Feedback scores are only produced when the feedback buttons are available. Setting [`interface.feedback`](/v0.8.8-rc2/docs/configuration/librechat_yaml/object_structure/interface#feedback) to `false` hides the buttons and rejects feedback writes, so no scores reach Langfuse.
