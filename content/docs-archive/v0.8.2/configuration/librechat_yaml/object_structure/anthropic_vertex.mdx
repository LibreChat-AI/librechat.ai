...
**[For quick setup using environment variables, see the Anthropic configuration guide](/v0.8.2/docs/configuration/pre_configured_ai/anthropic#vertex-ai)**
...
> **Note:** Anthropic endpoint supports all [Shared Endpoint Settings](/v0.8.2/docs/configuration/librechat_yaml/object_structure/shared_endpoint_settings), including `streamRate`, `titleModel`, `titleMethod`, `titlePrompt`, `titlePromptTemplate`, and `titleEndpoint`.
...
The `models` field defines the available Claude models and maps user-friendly names to Vertex AI deployment IDs. This works similarly to [Azure OpenAI model mapping](/v0.8.2/docs/configuration/librechat_yaml/object_structure/azure_openai#group-object-structure).
