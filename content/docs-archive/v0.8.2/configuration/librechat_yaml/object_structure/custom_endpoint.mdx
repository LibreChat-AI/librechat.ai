...
```
**Notes:**
- The `addParams` field allows you to include additional parameters that are not part of the default payload(see the ["Default Parameters"](/v0.8.2/docs/configuration/librechat_yaml/object_structure/default_params) section). This is particularly useful for API-specific options.
...
```
**Note:** 
- The `dropParams` field allows you to remove ["Default Parameters"](/v0.8.2/docs/configuration/librechat_yaml/object_structure/default_params) that are sent with every request. This is helpful when working with APIs that do not accept or recognize certain parameters.
