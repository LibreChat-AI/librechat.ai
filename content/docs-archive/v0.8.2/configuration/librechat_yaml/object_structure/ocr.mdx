...
- `mistral_ocr`: Uses Mistral's OCR capabilities via the standard [Mistral API](/v0.8.2/docs/features/ocr#1-mistral-ocr-default).
- `azure_mistral_ocr`: Uses Mistral OCR models deployed on [Azure AI Foundry](/v0.8.2/docs/features/ocr#2-azure-mistral-ocr).
- `vertexai_mistral_ocr`: Uses Mistral OCR models deployed on [Google Cloud Vertex AI](/v0.8.2/docs/features/ocr#3-google-vertex-ai-mistral-ocr).
- `custom_ocr`: Uses a custom OCR service specified by the `baseURL` [(not yet implemented)](/v0.8.2/docs/features/ocr#future-enhancements).
